...
template <class Unboxed> decltype(auto) Box(JNIEnv &env, Unboxed &&unboxed)
{
    return Boxer<typename std::decay<Unboxed>::type>().Box(
        env, std::forward<Unboxed>(unboxed));
}

template <class T> decltype(auto) Unbox(JNIEnv &env, const T &boxed)
{
...
template <class Tag, class Unboxed> struct PrimitiveTypeBoxer
{
    Local<Object<Tag>> Box(JNIEnv &env, Unboxed unboxed) const
    {
        static auto &klass = Class<Tag>::Singleton(env);
        static auto box = klass.template GetStaticMethod<Object<Tag>(Unboxed)>(
            env, Tag::BoxStaticMethodName());
...
template <class Tag, class Unboxed> struct PrimitiveTypeUnboxer
{
    Unboxed Unbox(JNIEnv &env, const Object<Tag> &boxed) const
    {
        static auto &klass = Class<Tag>::Singleton(env);
        static auto unbox =
            klass.template GetMethod<Unboxed()>(env, Tag::UnboxMethodName());
        return boxed.Call(env, unbox);
    }
};

template <>
struct Unboxer<BooleanTag> : PrimitiveTypeUnboxer<BooleanTag, jboolean>
{
};
...
template <>
struct Unboxer<CharacterTag> : PrimitiveTypeUnboxer<CharacterTag, jchar>
{
};
...
template <class Tag> struct Boxer<jni::Object<Tag>>
{
...
template <class Tag> struct Unboxer
{

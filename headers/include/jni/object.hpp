...
class ObjectBase
{
  protected:
    ...
    explicit ObjectBase(jobject *p) : ptr(p) {}
...
class Object : public TagTraits<TheTag>::SuperType
{
  public:
    ...
    explicit Object(UntaggedType *p) : SuperType(p) {}
  ...
  public:
    ...
    {
        return reinterpret_cast<UntaggedType *>(this->ptr);
    }
    ...
    {
        return Local<T>(env, reinterpret_cast<typename T::UntaggedType *>(
                                 GetField<jobject *>(env, get(), field)));
    }
    ...
    auto Set(JNIEnv &env, const Field<TagType, Expected> &field,
             const Actual &value) const
        -> std::enable_if_t<
            !IsPrimitive<Expected>::value &&
    ...
    auto Call(JNIEnv &env, const Method<TagType, R(ExpectedArgs...)> &method,
              const ActualArgs &...args) const
        -> std::enable_if_t<
            IsPrimitive<R>::value &&
                Conjunction<std::is_convertible<
                    const ActualArgs &, const ExpectedArgs &>...>::value,
            R>
    {
    ...
    auto Call(JNIEnv &env, const Method<TagType, R(ExpectedArgs...)> &method,
              const ActualArgs &...args) const
        -> std::enable_if_t<
            !IsPrimitive<R>::value && !std::is_void<R>::value &&
                Conjunction<std::is_convertible<
                    const ActualArgs &, const ExpectedArgs &>...>::value,
            Local<R>>
    {
        return Local<R>(env, reinterpret_cast<typename R::UntaggedType *>(
                                 CallMethod<jobject *>(env, get(), method,
                                                       Untag(args)...)));
    }

    template <class... ExpectedArgs, class... ActualArgs>
    auto Call(JNIEnv &env, const Method<TagType, void(ExpectedArgs...)> &method,
              const ActualArgs &...args) const
        -> std::enable_if_t<Conjunction<std::is_convertible<
            const ActualArgs &, const ExpectedArgs &>...>::value>
    {
    ...
    template <class R, class... ExpectedArgs, class... ActualArgs>
    auto CallNonvirtual(JNIEnv &env, const Class<TagType> &clazz,
        ...
        -> std::enable_if_t<
            ...
            R>
    {
        return CallNonvirtualMethod<R>(env, get(), clazz, method,
                                       Untag(args)...);
    }
    ...
    {
        return Local<R>(
            env, reinterpret_cast<typename R::UntaggedType *>(
                     CallNonvirtualMethod<jobject *>(env, get(), clazz, method,
                                                     Untag(args)...)));
    }
...
Local<Object<OutTagType>> Cast(JNIEnv &env, const Class<OutTagType> &clazz,
                               const T &object)
{
    if (!object.IsInstanceOf(env, clazz)) {
        ThrowNew(env, FindClass(env, "java/lang/ClassCastException"));
    }
    return Local<Object<OutTagType>>(
        env, reinterpret_cast<typename Object<OutTagType>::UntaggedType *>(
                 NewLocal(env, object).release()));
}
} // namespace jni

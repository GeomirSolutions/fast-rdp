...
namespace jni
{
template <class M, class Enable = void> struct NativeMethodTraits;

template <class R, class... Args> struct NativeMethodTraits<R(Args...)>
{
    using Type = R(Args...);
    using ResultType = R;
};
...
struct NativeMethodTraits<R (*)(Args...)> : NativeMethodTraits<R(Args...)>
{
};
...
template <class T, class R, class... Args>
struct NativeMethodTraits<R (T::*)(Args...)> : NativeMethodTraits<R(Args...)>
{
};
...
{
    template <class M> auto operator()(const char *name, const M &m)
    {
        static M method(m);

        auto wrapper = [](JNIEnv *env, UntaggedType<Subject> subject,
                          UntaggedType<Args>... args) {
            return ReleaseUnique(method(
                *env, AsLvalue(Tag<std::decay_t<Subject>>(*env, *subject)),
                AsLvalue(Tag<std::decay_t<Args>>(*env, args))...));
        };

        return MakeNativeMethod(
            name, TypeSignature<RemoveUniqueType<R>(std::decay_t<Args>...)>()(),
            wrapper);
    }
};
...
struct NativeMethodMaker<void (T::*)(JNIEnv &, Subject, Args...) const>
{
    template <class M> auto operator()(const char *name, const M &m)
    {
        ...
        auto wrapper = [](JNIEnv *env, UntaggedType<Subject> subject,
                          UntaggedType<Args>... args) {
            method(*env, AsLvalue(Tag<std::decay_t<Subject>>(*env, *subject)),
                   AsLvalue(Tag<std::decay_t<Args>>(*env, args))...);
        };

        return MakeNativeMethod(
            name, TypeSignature<void(std::decay_t<Args>...)>()(), wrapper);
    }
};

template <class M> auto MakeNativeMethod(const char *name, const M &m)
{
...
template <class R, class Subject, class... Args,
          R (*method)(JNIEnv &, Subject, Args...)>
...
template <class Subject, class... Args,
          void (*method)(JNIEnv &, Subject, Args...)>
...
template <class M, M method> auto MakeNativeMethod(const char *name)
{
    using FunctionType = typename NativeMethodTraits<M>::Type;
...
template <class L, class> class NativePeerLambdaMethod;
...
class NativePeerLambdaMethod<L, R (L::*)(JNIEnv &, P &, Args...) const>
{
  ...
  public:
    NativePeerLambdaMethod(const char *n, const L &l) : name(n), lambda(l) {}

    template <class Peer, class TagType,
              class = std::enable_if_t<std::is_same<P, Peer>::value>>
    auto operator()(const Field<TagType, jlong> &field)
    {
        auto wrapper = [field, lambda = lambda](
                           JNIEnv &env, Object<TagType> &obj, Args... args) {
            return lambda(env, *reinterpret_cast<P *>(obj.Get(env, field)),
                          args...);
        };
...
template <class M, M *> class NativePeerFunctionPointerMethod;
...
class NativePeerFunctionPointerMethod<R(JNIEnv &, P &, Args...), method>
{
  ...
  public:
    NativePeerFunctionPointerMethod(const char *n) : name(n) {}

    template <class Peer, class TagType,
              class = std::enable_if_t<std::is_same<P, Peer>::value>>
    auto operator()(const Field<TagType, jlong> &field)
    {
        auto wrapper = [field](JNIEnv &env, Object<TagType> &obj,
                               Args... args) {
            auto ptr = reinterpret_cast<P *>(obj.Get(env, field));
            if (ptr)
                return method(env, *ptr, args...);
            ThrowNew(env,
...
template <class M, M> class NativePeerMemberFunctionMethod;
...
class NativePeerMemberFunctionMethod<R (P::*)(JNIEnv &, Args...), method>
{
  ...
  public:
    NativePeerMemberFunctionMethod(const char *n) : name(n) {}
...
/**
 * A registration function for native methods on a "native peer": a long-lived
 * native object corresponding to a Java object, usually created when the Java
 * object is created and destroyed when the Java object's finalizer runs.
 *
 * It assumes that the Java object has a field, named by `fieldName`, of Java
 * type `long`, which is used to hold a pointer to the native peer.
 *
 * `Methods` must be a sequence of `NativePeerMethod` instances, instantiated
 * with pointer to member functions of the native peer class. For each method in
 * `methods`, a native method is bound with a signature corresponding to that of
 * the member function. The wrapper for that native method obtains the native
 * peer instance from the Java field and calls the native peer method on it,
 * passing along any arguments.
 *
 * An overload is provided that accepts a Callable object with a unique_ptr
 * result type and the names for native creation and finalization methods,
 * allowing creation and disposal of the native peer from Java.
 *
...
struct NativePeerHelper<Peer, TagType, std::unique_ptr<Peer>(JNIEnv &, Args...)>
{
    ...
    auto MakeInitializer(const Field<TagType, jlong> &field, const char *name,
                         Initializer *initializer) const
    {
        auto wrapper = [field, initializer](JNIEnv &e, Object<TagType> &obj,
...
void RegisterNativePeer(JNIEnv &env, const Class<TagType> &clazz,
                        const char *fieldName, Initializer initialize,
                        const char *initializeMethodName,
                        const char *finalizeMethodName, Methods &&...methods)
{
    static Field<TagType, jlong> field{env, clazz, fieldName};

    using InitializerMethodType =
        typename NativeMethodTraits<Initializer>::Type;
...
// Like std::make_unique, but with non-universal reference arguments, so it can
// be explicitly specialized (jni::MakePeer<Peer, jni::jboolean, ...>).

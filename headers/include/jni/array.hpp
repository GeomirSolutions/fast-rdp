...
namespace jni
{
template <class E, class Enable> class Array;
    ...
    : public Object<ArrayTag<E>>
{
  ...
  protected:
    ...
    explicit Array(UntaggedType *p) : SuperType(p) {}
  ...
  public:
    ...
    {
        SetArrayRegion(env, SafeDereference(env, this->get()), index, 1,
                       &value);
    }
  ...
  public:
    ...
    Local<ElementType> Get(JNIEnv &env, jsize index) const
    {
        return Local<ElementType>(
            env, reinterpret_cast<UntaggedElementType *>(GetObjectArrayElement(
                     env, SafeDereference(env, this->get()), index)));
    }

    void Set(JNIEnv &env, jsize index, const ElementType &value)
    {
        SetObjectArrayElement(env, SafeDereference(env, this->get()), index,
                              Untag(value));
    }

    static Local<Array<Object<TheTag>>>
    New(JNIEnv &env, jsize length,
        const Object<TheTag> *initialElement = nullptr)
    {
        return Local<Array<Object<TheTag>>>(
            env,
            &NewObjectArray(env, length, *Class<TheTag>::Singleton(env),
                            initialElement ? initialElement->get() : nullptr));
    }
};

template <class T>
std::vector<T> MakeAnything(ThingToMake<std::vector<T>>, JNIEnv &env,
                            const Array<T> &array)
{
...
template <class T>
Local<Array<T>> MakeAnything(ThingToMake<Array<T>>, JNIEnv &env,
                             const std::vector<T> &array)
{
    Local<Array<T>> result =
        Local<Array<T>>(env, &NewArray<T>(env, array.size()));
    SetArrayRegion(env, *result, 0, array);
    return result;
}

inline std::string MakeAnything(ThingToMake<std::string>, JNIEnv &env,
                                const Array<jbyte> &array)
{
    NullCheck(env, array.get());
    std::string result;
    result.resize(GetArrayLength(env, *array));
    GetArrayRegion(env, *array, 0, result.size(),
                   reinterpret_cast<jbyte *>(&result[0]));
    return result;
}

inline Local<Array<jbyte>> MakeAnything(ThingToMake<Array<jbyte>>, JNIEnv &env,
                                        const std::string &string)
{
    Local<Array<jbyte>> result(env, &NewArray<jbyte>(env, string.size()));
    SetArrayRegion(env, *result, 0, string.size(),
                   reinterpret_cast<const jbyte *>(&string[0]));
    return result;
}
} // namespace jni

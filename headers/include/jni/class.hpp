...
template <class TheTag> class Class : public Object<ClassTag>
{
  public:
  ...
  protected:
    ...
    explicit Class(UntaggedType *p) : SuperType(p) {}
  ...
  public:
    ...
    auto New(JNIEnv &env, const Constructor<TagType, ExpectedArgs...> &method,
             const ActualArgs &...args) const
        -> std::enable_if_t<
            Conjunction<std::is_convertible<const ActualArgs &,
                                            const ExpectedArgs &>...>::value,
            Local<Object<TagType>>>
    {
        return Local<Object<TagType>>(
            env, &NewObject(env, *this->get(), method, Untag(args)...));
    }
        ...
        -> std::enable_if_t<!IsPrimitive<T>::value, Local<T>>
    {
        return Local<T>(
            env, reinterpret_cast<typename T::UntaggedType *>(
                     jni::GetStaticField<jobject *>(env, *this->get(), field)));
    }
    ...
    auto Set(JNIEnv &env, const StaticField<TagType, Expected> &field,
            ...
            Local<R>>
    {
        return Local<R>(
            env, reinterpret_cast<typename R::UntaggedType *>(
                     CallStaticMethod<jobject *>(env, *this->get(), method,
                                                 Untag(args)...)));
    }
    ...
    {
        static Global<Class, EnvIgnoringDeleter> singleton =
            NewGlobal<EnvIgnoringDeleter>(env, Find(env));
        return singleton;
    }
    ...
    template <class T>
    StaticMethod<TagType, T> GetStaticMethod(JNIEnv &env,

...
template <class Tag, class = int> struct SuperTag
{
...
template <class Tag> struct TagTraits<ArrayTag<Object<Tag>>>
{
    using SuperType = Object<ObjectTag>;
...
template <class T> using UntaggedType = decltype(Untag(std::declval<T>()));
} // namespace jni

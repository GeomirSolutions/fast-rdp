...
{
    if (err != JNI_OK)
        throw std::system_error(err, ErrorCategory());
}
...
template <class R> R CheckJavaException(JNIEnv &env, R &&r)
{
...
// A function to be called from within a try / catch wrapper for a native
// method:
//
...
//      }
//
// `PendingJavaException` is caught and ignored, other exceptions are converted
// to a pending Java exception of class `java.lang.Error`.

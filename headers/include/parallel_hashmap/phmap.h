...
template <class Policy, class Hash, class Eq, class Alloc> class raw_hash_set
{
  ...
  public:
    ...
    template <class T, RequiresInsertable<T> = 0,
              typename std::enable_if<IsDecomposable<const T &>::value,
                                      int>::type = 0>
...
class raw_hash_map : public raw_hash_set<Policy, Hash, Eq, Alloc>
{
  ...
  private:
    ...
    std::pair<iterator, bool> try_emplace_impl(K &&k, Args &&...args)
    {
        ...
        if (res.second) {
            this->emplace_at(
                res.first, std::piecewise_construct,
...
class parallel_hash_set
{
  ...
  public:
    ...
    class iterator
    {
      ...
      private:
        iterator(Inner *inner, Inner *inner_end, const EmbeddedIterator &it)
            : inner_(inner), inner_end_(inner_end), it_(it)
        { // for begin() and end()
            if (inner)
    ...
    parallel_hash_set(std::initializer_list<T> init, size_t bucket_cnt = 0,
                      ...
                      const key_equal &eq = key_equal(),
                      const allocator_type &alloc = allocator_type())
        : parallel_hash_set(init.begin(), init.end(), bucket_cnt, hash_param,
                            eq, alloc)
    {
    }
                                    ...
                                    F &&f)
    {
        ...
        auto &set = inner.set_;
        typename Lockable::UniqueLock m(inner);
        return make_iterator(&inner, set.lazy_emplace_with_hash(
                                         key, hashval, std::forward<F>(f)));
    }
                        ...
                        FEmplace &&fEmplace)
    {
        ...
        if (std::get<2>(res)) {
            ...
            std::forward<FExists>(fExists)(const_cast<value_type &>(
                *it)); // in case of the set, non "key" part of value_type can
                       // be changed
        }
    ...
    friend void swap(parallel_hash_set &a,
                     parallel_hash_set<N, RefSet, Mtx2_, Policy, Hash, Eq,
                                       Alloc> &b) noexcept(noexcept(a.swap(b)))
    {
        a.swap(b);
    }
    ...
    typename std::enable_if<has_member_type_raw_hash_set<Set>::value>::type>
{
    ...
    static size_t AllocatedByteSize(const Set &c)
    {
        ...
        size_t per_slot =
            Traits::space_used(static_cast<const Slot *>(nullptr));
        if (per_slot != ~size_t{}) {
            m += per_slot * c.size();
        } else {
...
template <typename Set>
struct HashtableDebugAccess<
    Set, typename std::enable_if<has_member_type_EmbeddedSet<Set>::value>::type>
{
    using Traits = typename Set::PolicyTraits;
    using Slot = typename Traits::slot_type;

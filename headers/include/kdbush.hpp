...
template <std::uint8_t I, typename T> struct nth
{
...
template <typename TPoint, typename TIndex = std::size_t> class KDBush
{

  public:
    using TNumber = decltype(nth<0, TPoint>::get(std::declval<TPoint>()));
    static_assert(std::is_same<TNumber, decltype(nth<1, TPoint>::get(
                                            std::declval<TPoint>()))>::value,
    ...
    KDBush(const std::vector<TPoint> &points_,
    ...
    KDBush(const TPointIter &points_begin, const TPointIter &points_end,
           const std::uint8_t nodeSize_ = defaultNodeSize)
        : nodeSize(nodeSize_)
    {
    ...
    void fill(const TPointIter &points_begin, const TPointIter &points_end)
    {
        assert(points.empty());
        const TIndex size =
            static_cast<TIndex>(std::distance(points_begin, points_end));
        ...
        for (auto p = points_begin; p != points_end; p++) {
            points.emplace_back(nth<0, TPoint>::get(*p),
                                nth<1, TPoint>::get(*p));
            ids.push_back(i++);
        }
    ...
    void range(const TNumber minX, const TNumber minY, const TNumber maxX,
               const TNumber maxY, const TVisitor &visitor) const
    {
        range(minX, minY, maxX, maxY, visitor, 0,
              static_cast<TIndex>(ids.size() - 1), 0);
    }

    template <typename TVisitor>
    void within(const TNumber qx, const TNumber qy, const TNumber r,
  ...
  private:
    ...
    template <typename TVisitor>
    void range(const TNumber minX, const TNumber minY, const TNumber maxX,
               const TNumber maxY, const TVisitor &visitor, const TIndex left,
               const TIndex right, const std::uint8_t axis) const
    {
        ...
        if (axis == 0 ? maxX >= x : maxY >= y)
            range(minX, minY, maxX, maxY, visitor, m + 1, right,
                  (axis + 1) % 2);
    }

    template <typename TVisitor>
    void within(const TNumber qx, const TNumber qy, const TNumber r,
                const TVisitor &visitor, const TIndex left, const TIndex right,
    ...
    void select(const TIndex k, TIndex left, TIndex right)
    {
        ...
        while (right > left) {
            if (right - left > 600) {
                ...
                const double r =
                    k - m * s / n +
                    0.5 * std::sqrt(z * s * (1 - s / n)) * (2 * m < n ? -1 : 1);
                select<I>(k, std::max(left, TIndex(r)),
                          std::min(right, TIndex(r + s)));
            }
    ...
    {
        std::iter_swap(ids.begin() + static_cast<std::int32_t>(i),
                       ids.begin() + static_cast<std::int32_t>(j));
        std::iter_swap(points.begin() + static_cast<std::int32_t>(i),
                       points.begin() + static_cast<std::int32_t>(j));
    }

    TNumber sqDist(const TNumber ax, const TNumber ay, const TNumber bx,
                   const TNumber by) const
    {
        auto dx = ax - bx;
        auto dy = ay - by;
        return dx * dx + dy * dy;

...
IGL_INLINE void
igl::opengl::ViewerCore::align_camera_center(const Eigen::MatrixXd &V,
                                             const Eigen::MatrixXi &F)
{
    if (V.rows() == 0)
        return;

    get_scale_and_shift_to_fit_mesh(V, F, camera_base_zoom,
...
igl::opengl::ViewerCore::align_camera_center(const Eigen::MatrixXd &V)
{
    ...
    get_scale_and_shift_to_fit_mesh(V, camera_base_zoom,
                                    camera_base_translation);
    // Rather than crash on empty mesh...
    if (V.size() > 0) {
        object_scale = (V.colwise().maxCoeff() - V.colwise().minCoeff()).norm();
                                              ...
                                              bool update_matrices)
{
    ...
    if (update_matrices) {
        ...
        view =
            ...
            (trackball_angle * Eigen::Scaling(camera_zoom * camera_base_zoom) *
             Eigen::Translation3f(camera_translation + camera_base_translation))
                .matrix();
    ...
    GLint normi =
        glGetUniformLocation(data.meshgl.shader_mesh, "normal_matrix");
    glUniformMatrix4fv(viewi, 1, GL_FALSE, view.data());
    glUniformMatrix4fv(proji, 1, GL_FALSE, proj.data());
    glUniformMatrix4fv(normi, 1, GL_FALSE, norm.data());

    // Light parameters
    GLint specular_exponenti =
        glGetUniformLocation(data.meshgl.shader_mesh, "specular_exponent");
    GLint light_position_eyei =
        glGetUniformLocation(data.meshgl.shader_mesh, "light_position_eye");
    GLint lighting_factori =
        glGetUniformLocation(data.meshgl.shader_mesh, "lighting_factor");
    GLint fixed_colori =
        glGetUniformLocation(data.meshgl.shader_mesh, "fixed_color");
    GLint texture_factori =
        glGetUniformLocation(data.meshgl.shader_mesh, "texture_factor");
    GLint matcap_factori =
        glGetUniformLocation(data.meshgl.shader_mesh, "matcap_factor");
    GLint double_sidedi =
        glGetUniformLocation(data.meshgl.shader_mesh, "double_sided");

    const bool eff_is_directional_light =
        is_directional_light || is_shadow_mapping;
    ...
    if (is_shadow_mapping) {
        glUniformMatrix4fv(
            glGetUniformLocation(data.meshgl.shader_mesh, "shadow_view"), 1,
            GL_FALSE, shadow_view.data());
        glUniformMatrix4fv(
            glGetUniformLocation(data.meshgl.shader_mesh, "shadow_proj"), 1,
            GL_FALSE, shadow_proj.data());
        glActiveTexture(GL_TEXTURE0 + 1);
        glBindTexture(GL_TEXTURE_2D, shadow_depth_tex);
        {
            glUniform1i(
                glGetUniformLocation(data.meshgl.shader_mesh, "shadow_tex"), 1);
        }
    }
    ...
    glUniform1i(
        glGetUniformLocation(data.meshgl.shader_mesh, "is_directional_light"),
        eff_is_directional_light);
    glUniform1i(
        glGetUniformLocation(data.meshgl.shader_mesh, "is_shadow_mapping"),
        is_shadow_mapping);
    glUniform1i(glGetUniformLocation(data.meshgl.shader_mesh, "shadow_pass"),
                false);
    ...
    if (data.V.rows() > 0) {
        ...
        if (is_set(data.show_faces)) {
            // Texture
            glUniform1f(texture_factori,
                        is_set(data.show_texture) ? 1.0f : 0.0f);
        ...
        if (is_set(data.show_lines)) {
            ...
            glUniform4f(fixed_colori, data.line_color[0], data.line_color[1],
                        data.line_color[2], data.line_color[3]);
    ...
    if (is_set(data.show_overlay)) {
        ...
        if (data.lines.rows() > 0) {
            ...
            viewi =
                glGetUniformLocation(data.meshgl.shader_overlay_lines, "view");
            proji =
                glGetUniformLocation(data.meshgl.shader_overlay_lines, "proj");
        ...
        if (data.points.rows() > 0) {
            ...
            viewi =
                glGetUniformLocation(data.meshgl.shader_overlay_points, "view");
            proji =
                glGetUniformLocation(data.meshgl.shader_overlay_points, "proj");
    ...
    if (is_set(data.show_vertex_labels) &&
        data.vertex_labels_positions.rows() > 0)
...
IGL_INLINE void igl::opengl::ViewerCore::initialize_shadow_pass()
{
    ...
    float length = (camera_eye - camera_center).norm();
    float h = tan(camera_view_angle / 360.0 * igl::PI) * (length);
    igl::ortho(-h * shadow_width / shadow_height,
               h * shadow_width / shadow_height, -h, h, camera_dnear,
...
IGL_INLINE void
igl::opengl::ViewerCore::draw_shadow_pass(ViewerData &data,
                                          bool /*update_matrices*/)
{
    if (data.dirty) {
        data.updateGL(data, data.invert_normals, data.meshgl);
...
IGL_INLINE void igl::opengl::ViewerCore::draw_buffer(
    ViewerData &data, bool update_matrices,
...
IGL_INLINE void
igl::opengl::ViewerCore::draw_labels(ViewerData &data,
                                     const igl::opengl::MeshGL::TextGL &labels)
{
    ...
    glUniform1f(
        glGetUniformLocation(data.meshgl.shader_text, "TextShiftFactor"),
        text_shift_scale_factor);
    glUniform3f(glGetUniformLocation(data.meshgl.shader_text, "TextColor"),
                data.label_color(0), data.label_color(1), data.label_color(2));
    glUniform2f(glGetUniformLocation(data.meshgl.shader_text, "CellSize"),
                1.0f / 16, (300.0f / 384) / 6);
    glUniform2f(glGetUniformLocation(data.meshgl.shader_text, "CellOffset"),
                0.5 / 256.0, 0.5 / 256.0);
    ...
    glUniform2f(glGetUniformLocation(data.meshgl.shader_text, "RenderOrigin"),
                -2, 2);
...
IGL_INLINE void igl::opengl::ViewerCore::set(unsigned int &property_mask,
                                             bool value) const
{
    if (!value)
        unset(property_mask);
...
IGL_INLINE void
igl::opengl::ViewerCore::unset(unsigned int &property_mask) const
{
    property_mask &= ~id;
}

IGL_INLINE void
igl::opengl::ViewerCore::toggle(unsigned int &property_mask) const
{
    property_mask ^= id;
}

IGL_INLINE bool
igl::opengl::ViewerCore::is_set(unsigned int property_mask) const
{
...
IGL_INLINE void igl::opengl::ViewerCore::shut() { delete_shadow_buffers(); }
...
IGL_INLINE void igl::opengl::ViewerCore::generate_shadow_buffers()
{
    ...
    glBindRenderbuffer(GL_RENDERBUFFER, shadow_color_rbo);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8, shadow_width,
                          shadow_height);
    // Attach color buffer

...
IGL_INLINE igl::opengl::ViewerData::ViewerData()
    : dirty(MeshGL::DIRTY_ALL), face_based(false), double_sided(false),
      invert_normals(false), is_visible(~unsigned(0)), show_custom_labels(0),
      show_face_labels(0), show_faces(~unsigned(0)), show_lines(~unsigned(0)),
      show_overlay(~unsigned(0)), show_overlay_depth(~unsigned(0)),
      show_texture(false), show_vertex_labels(0), use_matcap(false),
      point_size(30), line_width(0.5f), label_size(1), line_color(0, 0, 0, 1),
      label_color(0, 0, 0.04, 1), shininess(35.0f), id(-1)
{
    clear();
};
...
IGL_INLINE void igl::opengl::ViewerData::set_mesh(const Eigen::MatrixXd &_V,
                                                  const Eigen::MatrixXi &_F)
{
    ...
    if (V.rows() == 0 && F.rows() == 0) {
        ...
        uniform_colors(
            ...
            Eigen::Vector3d(GOLD_SPECULAR[0], GOLD_SPECULAR[1],
                            GOLD_SPECULAR[2]));
...
IGL_INLINE void igl::opengl::ViewerData::set_normals(const Eigen::MatrixXd &N)
{
    ...
    if (N.rows() == V.rows()) {
        ...
        cerr << "ERROR (set_normals): Please provide a normal per face, per "
                "corner or per vertex."
             << endl;
    dirty |= MeshGL::DIRTY_NORMAL;
}

IGL_INLINE void
igl::opengl::ViewerData::set_visible(bool value, unsigned int core_id /*= 1*/)
{
    if (value)
        is_visible |= core_id;
    else
...
IGL_INLINE void igl::opengl::ViewerData::copy_options(const ViewerCore &from,
                                                      const ViewerCore &to)
{
...
IGL_INLINE void igl::opengl::ViewerData::set_colors(const Eigen::MatrixXd &C)
{
    ...
    if (C.rows() == 1) {
        ...
        cerr << "ERROR (set_colors): Please provide a single color, or a color "
                "per face or per vertex."
             << endl;
    dirty |=
        MeshGL::DIRTY_DIFFUSE | MeshGL::DIRTY_SPECULAR | MeshGL::DIRTY_AMBIENT;
}
    ...
    const Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic> &B)
{
    ...
    texture_B = B;
    texture_A =
        Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(
            R.rows(), R.cols(), 255);
                                                  ...
                                                  int num_steps)
{
    if (!show_texture) {
        Eigen::MatrixXd CM;
        igl::colormap(cmap, Eigen::VectorXd::LinSpaced(num_steps, 0, 1).eval(),
                      0, 1, CM);
        set_colormap(CM);
    }
    Eigen::MatrixXd UV =
        ((D.array() - caxis_min) / (caxis_max - caxis_min)).replicate(1, 2);
    if (D.size() == V.rows()) {
        set_uv(UV);
    } else {
...
IGL_INLINE void igl::opengl::ViewerData::clear_points() { points.resize(0, 6); }
...
IGL_INLINE void
igl::opengl::ViewerData::set_edges_from_vector_field(const Eigen::MatrixXd &P,
...
IGL_INLINE void igl::opengl::ViewerData::clear_edges() { lines.resize(0, 9); }
...
IGL_INLINE void
igl::opengl::ViewerData::set_labels(const Eigen::MatrixXd &P,
...
IGL_INLINE void
igl::opengl::ViewerData::uniform_colors(const Eigen::Vector3d &ambient,
...
IGL_INLINE void
igl::opengl::ViewerData::uniform_colors(const Eigen::Vector4d &ambient,
                                        ...
                                        const Eigen::Vector4d &specular)
{
    ...
    dirty |=
        MeshGL::DIRTY_SPECULAR | MeshGL::DIRTY_DIFFUSE | MeshGL::DIRTY_AMBIENT;
}
...
IGL_INLINE void igl::opengl::ViewerData::normal_matcap()
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (int j = 0; j < size; j++) {
            ...
            Eigen::Vector3d C =
                Eigen::Vector3d(x * 0.5 + 0.5, y * 0.5 + 0.5, z);
...
IGL_INLINE void igl::opengl::ViewerData::grid_texture()
{
    ...
    texture_A =
        Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(
            texture_R.rows(), texture_R.cols(), 255);
    dirty |= MeshGL::DIRTY_TEXTURE;
}

// Populate VBOs of a particular label stype (Vert, Face, Custom)
IGL_INLINE void
igl::opengl::ViewerData::update_labels(igl::opengl::MeshGL::TextGL &GL_labels,
    ...
    if (positions.rows() > 0) {
        ...
        for (size_t s = 0; s < strings.size(); s++) {
            ...
            for (size_t c = 0; c < label.length(); c++) {
                GL_labels.label_pos_vbo.row(idx) =
                    positions.row(s).cast<float>();
...
IGL_INLINE void
igl::opengl::ViewerData::updateGL(const igl::opengl::ViewerData &data,
                                  ...
                                  igl::opengl::MeshGL &meshgl)
{
    ...
    const auto per_face = [&data](const Eigen::MatrixXd &X,
    ...
    // Output:
    //   X_vbo  #F*3 by dim scattering per corner
    const auto per_corner = [&data](const Eigen::MatrixXd &X,
                                    MeshGL::RowMatrixXf &X_vbo) {
                ...
                for (unsigned i = 0; i < data.F.rows(); ++i)
                    for (unsigned j = 0; j < 3; ++j)
                        meshgl.V_uv_vbo.row(i * 3 + j) =
                            data.V_uv
                                .row(per_corner_uv ? data.F_uv(i, j)
                                                   : data.F(i, j))
                                .cast<float>();
            }
        }
        ...
        if ((meshgl.dirty & MeshGL::DIRTY_UV) && data.V_uv.rows() > 0) {
            ...
            for (unsigned i = 0; i < data.F.rows(); ++i)
                for (unsigned j = 0; j < 3; ++j)
                    meshgl.V_uv_vbo.row(i * 3 + j) =
                        data.V_uv
                            .row(per_corner_uv ? data.F_uv(i, j) : data.F(i, j))
                            .cast<float>();
        }
    }
    ...
    if (meshgl.dirty & MeshGL::DIRTY_OVERLAY_LINES) {
        ...
        for (unsigned i = 0; i < data.lines.rows(); ++i) {
            meshgl.lines_V_vbo.row(2 * i + 0) =
                data.lines.block<1, 3>(i, 0).cast<float>();
            meshgl.lines_V_vbo.row(2 * i + 1) =
                data.lines.block<1, 3>(i, 3).cast<float>();
            meshgl.lines_V_colors_vbo.row(2 * i + 0) =
                data.lines.block<1, 3>(i, 6).cast<float>();
            meshgl.lines_V_colors_vbo.row(2 * i + 1) =
                data.lines.block<1, 3>(i, 6).cast<float>();
    ...
    if (meshgl.dirty & MeshGL::DIRTY_OVERLAY_POINTS) {
        ...
        for (unsigned i = 0; i < data.points.rows(); ++i) {
            meshgl.points_V_vbo.row(i) =
                data.points.block<1, 3>(i, 0).cast<float>();
            meshgl.points_V_colors_vbo.row(i) =
                data.points.block<1, 3>(i, 3).cast<float>();
    ...
    if (meshgl.dirty & MeshGL::DIRTY_FACE_LABELS) {
        if (face_labels_positions.rows() == 0) {
            ...
            for (int f = 0; f < F.rows(); ++f) {
                ...
                face_labels_positions.row(f) =
                    (faceNormals * 0.05).row(f) + face_labels_positions.row(f);
                face_labels_strings.push_back(faceName);
            }
        }
        update_labels(meshgl.face_labels, face_labels_positions,
                      face_labels_strings);
    }
    ...
    if (meshgl.dirty & MeshGL::DIRTY_VERTEX_LABELS) {
        if (vertex_labels_positions.rows() == 0) {
            ...
            for (int v = 0; v < V.rows(); ++v) {
                ...
                vertex_labels_positions.row(v) =
                    (normalized * 0.1).row(v) + V.row(v);
                vertex_labels_strings.push_back(vertName);
            }
        }
        update_labels(meshgl.vertex_labels, vertex_labels_positions,
                      vertex_labels_strings);
    }

    if (meshgl.dirty & MeshGL::DIRTY_CUSTOM_LABELS) {
        update_labels(meshgl.custom_labels, labels_positions, labels_strings);
    }
}

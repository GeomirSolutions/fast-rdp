...
class ViewerData
{
  public:
    ...
    IGL_INLINE void set_mesh(const Eigen::MatrixXd &V,
                             const Eigen::MatrixXi &F);
    /// Set just the vertices. Call this if _just_ the vertices are changing.
    /// You may need to call compute_normals to update the lighting correctly.
    ///
    /// @param[in] V  #V by 3 list of mesh vertex positions
    ...
    /// 0.1*C and the specular color will be set to 0.3+0.1*(C-0.3).
    ///
    ...
    IGL_INLINE void
    set_data(const Eigen::VectorXd &D, double caxis_min, double caxis_max,
    ...
    IGL_INLINE void
    set_data(const Eigen::VectorXd &D,
             igl::ColorMapType cmap = igl::COLOR_MAP_TYPE_VIRIDIS,
             int num_steps = 21);

    /// Not to be confused with set_colors, this creates a _texture_ that will
    /// be referenced to pseudocolor according to the scalar field passed to
    /// set_data.
    ///
    ...
    IGL_INLINE void set_edges(const Eigen::MatrixXd &P,
    ...
    /// @param[in] C  #P|1 by 3 color(s)
    /// \note This is very confusing. Why does add_edges have a different API
    /// from set_edges?
    ...
    /// @param[in] C  #P|1 by 3 color(s)
    IGL_INLINE void set_edges_from_vector_field(const Eigen::MatrixXd &P,
    ...
    IGL_INLINE void set_labels(const Eigen::MatrixXd &P,
    ...
    IGL_INLINE void uniform_colors(const Eigen::Vector3d &ambient,
                                   const Eigen::Vector3d &diffuse,
                                   const Eigen::Vector3d &specular);
    /// \overload
    IGL_INLINE void uniform_colors(const Eigen::Vector4d &ambient,
    ...
    /// (Every row contains 9 doubles in the following format S_x, S_y, S_z,
    /// T_x, T_y, T_z, C_r, C_g, C_b),
    // with S and T the coordinates of the two vertices of the line in global
    // coordinates, and C the color in floating point rgb format
    Eigen::MatrixXd lines;

    /// Points plotted over the scene
    /// (Every row contains 6 doubles in the following format P_x, P_y, P_z,
    /// C_r, C_g, C_b), with P the position in global coordinates of the center
    /// of the point, and C the color in floating point rgb format
    ...
    /// Each option is a binary mask specifying on which viewport each option is
    /// set. When using a single viewport, standard boolean can still be used
    /// for simplicity.
    ...
    IGL_INLINE void update_labels(igl::opengl::MeshGL::TextGL &GL_labels,
    ...
    IGL_INLINE void updateGL(const igl::opengl::ViewerData &data,
...
{
inline void serialization(bool s, igl::opengl::ViewerData &obj,
...
template <>
inline void serialize(const igl::opengl::ViewerData &obj,
                      std::vector<char> &buffer)
{
    serialization(true, const_cast<igl::opengl::ViewerData &>(obj), buffer);
}
template <>
inline void deserialize(igl::opengl::ViewerData &obj,

...
IGL_INLINE void SelectionWidget::draw()
{
    ...
    for (int pass = 0; pass < 2; pass++) {
        ...
        const bool closed = !(mode == LASSO || mode == POLYGONAL_LASSO) ||
                            !(is_down || is_drawing);
        if (pass == 0) {
...
IGL_INLINE bool SelectionWidget::key_pressed(unsigned int key,
                                             int /*modifiers*/)
{
    Mode old = mode;
...
IGL_INLINE void SelectionWidget::circle(const Eigen::Matrix<float, 2, 2> &M,
...
IGL_INLINE void SelectionWidget::rect(const Eigen::Matrix<float, 2, 2> &M,
                                      std::vector<Eigen::RowVector2f> &L)
{
    L.resize(4);

...
IGL_INLINE bool
igl::opengl::glfw::map_texture(const Eigen::MatrixBase<DerivedV> &_V,
                               const Eigen::MatrixBase<DerivedF> &_F,
                               const Eigen::MatrixBase<DerivedU> &_U,
                               const unsigned char *in_data, const int w,
                               const int h, const int nc,
                               std::vector<unsigned char> &out_data)
{
    int out_w = w;
    int out_h = h;
    int out_nc = nc;
    return map_texture(_V, _F, _U, in_data, w, h, nc, out_data, out_w, out_h,
                       out_nc);
}
...
IGL_INLINE bool igl::opengl::glfw::map_texture(
    ...
    const Eigen::MatrixBase<DerivedU> &U, const unsigned char *in_data,
    const int in_w, const int in_h, const int in_nc,
    std::vector<unsigned char> &out_data, int &out_w, int &out_h, int &out_nc)
{
    GLenum format = -1;
    switch (in_nc) {
    case 1:
...
void main()
{
  color = texture(tex,tex_coord_f).rgb;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, format, out_w, out_h, 0, format, GL_FLOAT,
                 0);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, format, in_w, in_h, 0, format,
                 GL_UNSIGNED_BYTE, in_data);
    glUniform1i(glGetUniformLocation(prog_id, "tex"), 0);

    igl::opengl::bind_vertex_attrib_array(prog_id, "position", vbo, U_vbo,
                                          true);
    igl::opengl::bind_vertex_attrib_array(prog_id, "tex_coord_v", tbo, V_vbo,
                                          true);
...
template bool
igl::opengl::glfw::map_texture<Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                               Eigen::Matrix<int, -1, 3, 1, -1, 3>,
                               Eigen::Matrix<double, -1, -1, 1, -1, -1>>(
    Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 1, -1, -1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, -1, 3, 1, -1, 3>> const &,
    Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 1, -1, -1>> const &,
    unsigned char const *, int, int, int,
    std::vector<unsigned char, std::allocator<unsigned char>> &);
template bool
igl::opengl::glfw::map_texture<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    unsigned char const *, int, int, int,
    std::vector<unsigned char, std::allocator<unsigned char>> &, int &, int &,
    int &);
#endif

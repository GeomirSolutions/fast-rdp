...
///     2  sort each row
/// @param[in] ascending  sort ascending (true, matlab default) or descending
/// (false)
/// @param[out] Y  m by n matrix whose entries are sorted
/// @param[out] IX  m by n matrix of indices so that if dim = 1, then in matlab
/// notation
///     for j = 1:n, Y(:,j) = X(I(:,j),j); end
template <typename DerivedX, typename DerivedY, typename DerivedIX>
IGL_INLINE void sort(const Eigen::DenseBase<DerivedX> &X, const int dim,
...
template <typename DerivedX, typename DerivedY, typename DerivedIX>
IGL_INLINE void sort_new(const Eigen::DenseBase<DerivedX> &X, const int dim,
...
template <typename DerivedX, typename DerivedY, typename DerivedIX>
IGL_INLINE void sort2(const Eigen::DenseBase<DerivedX> &X, const int dim,
                      const bool ascending, Eigen::PlainObjectBase<DerivedY> &Y,
                      Eigen::PlainObjectBase<DerivedIX> &IX);
/// \overload
/// \brief Special case if size(X,dim) == 3
template <typename DerivedX, typename DerivedY, typename DerivedIX>
IGL_INLINE void sort3(const Eigen::DenseBase<DerivedX> &X, const int dim,
                      const bool ascending, Eigen::PlainObjectBase<DerivedY> &Y,
                      Eigen::PlainObjectBase<DerivedIX> &IX);
...
/// @param[out] index_map  an index map such that sorted[i] =
/// unsorted[index_map[i]]
template <class T>

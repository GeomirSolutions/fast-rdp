...
template <typename Derivedx, typename Derivedx0, typename Derivedf,
          typename DerivedF, typename Scalar>
    ...
    -> Eigen::Matrix<Scalar, 3, 1>
{
    ...
    case igl::BrushType::SCALE: {
        ...
        kelvinlet = [&r, &F, &r_norm_sq](const Scalar &epsilon) {
            static constexpr auto b_compressible =
                a / 4; // assumes poisson ratio 0
            const auto r_epsilon = sqrt(r_norm_sq + epsilon * epsilon);
            const auto r_epsilon_3 = r_epsilon * r_epsilon * r_epsilon;
            auto coeff = (2 * b_compressible - a) *
                         (1 / (r_epsilon_3) +
                          3 * (epsilon * epsilon) /
                              (2 * r_epsilon_3 * r_epsilon * r_epsilon));
            return coeff * F * r;
...
//          approximations
template <typename Scalar, typename Derivedx, typename Derivedx0,
          typename Derivedf, typename DerivedF>
IGL_INLINE void integrate(const Scalar t, const Scalar dt,
                          const Eigen::MatrixBase<Derivedx> &x,
                          const Eigen::MatrixBase<Derivedx0> &x0,
                          const Eigen::MatrixBase<Derivedf> &f,
                          const Eigen::MatrixBase<DerivedF> &F,
                          const igl::KelvinletParams<Scalar> &kp,
                          Eigen::MatrixBase<Derivedx> &result, Scalar &error)
{
    ...
    auto k3 = dt * kelvinlet_evaluator(t + dt * a3, x + k1 * b31 + k2 * b32, x0,
                                       f, F, kp);
    auto k4 = dt * kelvinlet_evaluator(t + dt * a4,
                                       x + k1 * b41 + k2 * b42 + k3 * b43, x0,
                                       f, F, kp);
...
template <typename DerivedV, typename Derivedx0, typename Derivedf,
          typename DerivedF, typename DerivedU>
IGL_INLINE void
kelvinlets(const Eigen::MatrixBase<DerivedV> &V,
           const Eigen::MatrixBase<Derivedx0> &x0,
           const Eigen::MatrixBase<Derivedf> &f,
           const Eigen::MatrixBase<DerivedF> &F,
           ...
           Eigen::PlainObjectBase<DerivedU> &U)
{
    ...
    const auto calc_displacement = [&](const int index) {
        ...
        decltype(x) result;
        Scalar error;
        // taking smaller steps seems to prevents weird inside-out artifacts in
        // the final result. This implementation used an adaptive time step
        // solver to numerically integrate the ODEs
        while (t < 1) {
            dt = std::min(dt, 1 - t);
            ...
            if (error <= max_error || dt <= 0.001) {
                ...
                dt = std::max(abs(new_dt - dt) < 0.001 ? dt / 2.f : new_dt,
                              0.001);
            }
        }

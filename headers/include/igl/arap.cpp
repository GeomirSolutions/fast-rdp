...
IGL_INLINE bool
igl::arap_precomputation(const Eigen::MatrixBase<DerivedV> &V,
                         const Eigen::MatrixBase<DerivedF> &F, const int dim,
                         const Eigen::MatrixBase<Derivedb> &b, ARAPData &data)
{
    using namespace std;
...
IGL_INLINE bool igl::arap_solve(const Eigen::MatrixBase<Derivedbc> &bc,
                                ARAPData &data, Eigen::MatrixBase<DerivedU> &U)
{
    using namespace Eigen;
    using namespace std;
    ...
    while (iter < data.max_iter) {
        ...
        // THIS NORMALIZATION IS IMPORTANT TO GET SINGLE PRECISION SVD CODE TO
        // WORK CORRECTLY.
        ...
        if (R.rows() == 2) {
            ...
            //#ifdef __SSE__ // fit_rotations_SSE will convert to float if
            // necessary
        ...
        if (data.with_dynamics) {
            assert(data.M.rows() == n &&
                   "No mass matrix. Call arap_precomputation if changing "
                   "with_dynamics");
            const double h = data.h;
            assert(h != 0);
            ...
            const double dw = (1. / data.ym) * (h * h);
            Dl = dw *
                 (1. / (h * h) * data.M * (-U0 - h * data.vel) - data.f_ext);
        }

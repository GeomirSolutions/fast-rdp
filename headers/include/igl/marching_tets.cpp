...
template <typename DerivedTV, typename DerivedTT, typename DerivedS,
          typename DerivedSV, typename DerivedSF, typename DerivedJ,
          typename BCType>
                        ...
                        Eigen::SparseMatrix<BCType> &BC)
{
    ...
    // 32 bits of a key are the indices of vertices in the mesh. The implication
    // is that you can only have 2^32 vertices which I have deemed sufficient
    ...
    const int mt_cell_lookup[16][4] = {
        {-1, -1, -1, -1}, {0, 2, 1, -1}, {0, 3, 4, -1}, {2, 1, 3, 4},
        {5, 3, 1, -1},    {0, 2, 5, 3},  {0, 1, 5, 4},  {2, 5, 4, -1},
        {4, 5, 2, -1},    {0, 4, 5, 1},  {0, 3, 5, 2},  {1, 3, 5, -1},
        {4, 3, 1, 2},     {0, 4, 3, -1}, {0, 1, 2, -1}, {-1, -1, -1, -1},
    };
    ...
    for (int i = 0; i < TT.rows(); i++) {
        ...
        for (int e = 0; e < 4 && mt_cell_lookup[key][e] != -1; e++) {
            const int tv1_idx =
                TT(i, mt_edge_lookup[mt_cell_lookup[key][e]][0]);
            const int tv2_idx =
                TT(i, mt_edge_lookup[mt_cell_lookup[key][e]][1]);
            const int vertex_id = edge_table.size();
            edge_table.push_back(make_pair(std::min(tv1_idx, tv2_idx),
                                           std::max(tv1_idx, tv2_idx)));
            v_ids[e] = vertex_id;
        }
    ...
    for (int f = 0; f < faces.size(); f++) {
        ...
        for (int v = 0; v < 3; v++) {
            ...
            if (it == emap.end()) // New unique vertex, insert it
            {
                ...
                typedef Eigen::Matrix<typename DerivedTV::Scalar, 1, 3,
                                      Eigen::RowMajor, 1, 3>
                    RowVector;
                using Scalar = typename DerivedS::Scalar;
                const RowVector v1 = TV.row(edge.first).template cast<Scalar>();
                const RowVector v2 =
                    TV.row(edge.second).template cast<Scalar>();
                const Scalar a = abs(isovals(edge.first, 0) - isovalue);
                const Scalar b = abs(isovals(edge.second, 0) - isovalue);
                const Scalar w = a / (a + b);

                // Create a casted copy in case BCType is a float and we need to
                // downcast
                const BCType bc_w = static_cast<BCType>(w);
                bc_triplets.push_back(
                    Eigen::Triplet<BCType>(num_unique, edge.first, 1 - bc_w));
                bc_triplets.push_back(
                    Eigen::Triplet<BCType>(num_unique, edge.second, bc_w));

                // Create a casted copy in case DerivedTV::Scalar is a float and
                // we need to downcast
                const typename DerivedTV::Scalar v_w =
                    static_cast<typename DerivedTV::Scalar>(w);
...
template void igl::marching_tets<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

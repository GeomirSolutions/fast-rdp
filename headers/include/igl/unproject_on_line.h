...
/// @param[out] t  line parameter so that closest poin on line to viewer ray
/// through UV
///     lies at origin+t*dir
template <typename DerivedUV, typename DerivedM, typename DerivedVP,
          typename Derivedorigin, typename Deriveddir>
void unproject_on_line(const Eigen::MatrixBase<DerivedUV> &UV,
...
/// \overload
/// @param[out] Z  3d position of closest point on line to viewing ray through
/// UV
template <typename DerivedUV, typename DerivedM, typename DerivedVP,
          typename Derivedorigin, typename Deriveddir, typename DerivedZ>
void unproject_on_line(const Eigen::MatrixBase<DerivedUV> &UV,
                       const Eigen::MatrixBase<DerivedM> &M,
                       const Eigen::MatrixBase<DerivedVP> &VP,
                       const Eigen::MatrixBase<Derivedorigin> &origin,

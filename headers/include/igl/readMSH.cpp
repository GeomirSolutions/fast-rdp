...
IGL_INLINE bool igl::readMSH(
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, EigenMatrixOptions>
        &X,
    ...
    Eigen::VectorXi &TriTag, Eigen::VectorXi &TetTag,
    std::vector<std::string> &XFields,
    std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic,
                              EigenMatrixOptions>> &XF,
    std::vector<std::string> &EFields,
    std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic,
                              EigenMatrixOptions>> &TriF,
    std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic,
                              EigenMatrixOptions>> &TetF)
{
    try {
...
#ifndef NDEBUG
        std::cout << "readMSH:Total number of nodes:"
                  << _loader.get_nodes().size() << std::endl;
        std::cout << "readMSH:Total number of elements:"
                  << _loader.get_elements().size() << std::endl;

        std::cout << "readMSH:Node fields:" << std::endl;
        for (auto i = std::begin(_loader.get_node_fields_names());
             i != std::end(_loader.get_node_fields_names()); i++) {
            std::cout
                << i->c_str() << ":"
                << _loader
                       .get_node_fields()
                           [i - std::begin(_loader.get_node_fields_names())]
                       .size()
                << std::endl;
        }

        std::cout << "readMSH:Element fields:" << std::endl;
        for (auto i = std::begin(_loader.get_element_fields_names());
             i != std::end(_loader.get_element_fields_names()); i++) {
            std::cout
                << i->c_str() << ":"
                << _loader
                       .get_element_fields()
                           [i - std::begin(_loader.get_element_fields_names())]
                       .size()
                << std::endl;
        }
        ...
        Eigen::Map<const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic,
                                       Eigen::RowMajor>>
            node_map(_loader.get_nodes().data(), _loader.get_nodes().size() / 3,
                     3);
        ...
        for (size_t i = 0; i < _loader.get_node_fields().size(); ++i) {
            Eigen::Map<const Eigen::Matrix<double, Eigen::Dynamic,
                ...
                field_map(_loader.get_node_fields()[i].data(),
                          _loader.get_node_fields()[i].size() /
        ...
        for (size_t i = 0; i < _loader.get_element_fields().size(); ++i) {
            TriF[i].resize(n_tri_el,
                           _loader.get_element_fields_components()[i]);
            TetF[i].resize(n_tet_el,
                           _loader.get_element_fields_components()[i]);
        }
        ...
        for (size_t i = 0; i < _loader.get_elements_lengths().size(); ++i) {
            if (_loader.get_elements_types()[i] == MshLoader::ELEMENT_TRI) {
                ...
                for (size_t j = 0; j < _loader.get_element_fields().size(); ++j)
                    for (size_t k = 0;
                         k < _loader.get_element_fields_components()[j]; ++k)
                        TriF[j](i_tri, k) =
                            _loader.get_element_fields()
                                [j][_loader.get_element_fields_components()[j] *
                                        i +
                                    k];

                ++i_tri;
            } else if (_loader.get_elements_types()[i] ==
                       MshLoader::ELEMENT_TET) {
                ...
                std::cerr << "readMSH: unsupported element type: "
                          << _loader.get_elements_types()[i]
                          << ", length: " << _loader.get_elements_lengths()[i]
                          << std::endl;
            }
    ...
    Eigen::VectorXi &TriTag, Eigen::VectorXi &TetTag)
{
    ...
    std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic,
                              EigenMatrixOptions>>
        TetF;
    return igl::readMSH(msh, X, Tri, Tet, TriTag, TetTag, XFields, XF, EFields,
                        TriF, TetF);
}
...
template bool igl::readMSH<1>(
    ...
    std::vector<
        ...
        std::allocator<std::basic_string<char, std::char_traits<char>,
                                         std::allocator<char>>>> &,
    std::vector<Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                std::allocator<Eigen::Matrix<double, -1, -1, 1, -1, -1>>> &,
    std::vector<
        std::basic_string<char, std::char_traits<char>, std::allocator<char>>,
        std::allocator<std::basic_string<char, std::char_traits<char>,
                                         std::allocator<char>>>> &,
...
template bool igl::readMSH<0>(
    ...
    std::vector<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                std::allocator<Eigen::Matrix<double, -1, -1, 0, -1, -1>>> &);
template bool igl::readMSH<1>(std::basic_string<char, std::char_traits<char>,
                              ...
                              Eigen::Matrix<int, -1, 1, 0, -1, 1> &,
                              Eigen::Matrix<int, -1, 1, 0, -1, 1> &);
template bool igl::readMSH<0>(std::basic_string<char, std::char_traits<char>,
                                                std::allocator<char>> const &,
                              Eigen::Matrix<double, -1, -1, 0, -1, -1> &,
                              Eigen::Matrix<int, -1, -1, 0, -1, -1> &,
                              Eigen::Matrix<int, -1, -1, 0, -1, -1> &,

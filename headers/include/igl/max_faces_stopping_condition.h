...
///
/// @param[in]  m  reference to working variable initially should be set to
/// current
///    number of faces.
/// @param[in] orig_m  number (size) of original face list _**not**_ including
/// any
...
IGL_INLINE void max_faces_stopping_condition(
    int &m, const int orig_m, const int max_m,
    decimate_stopping_condition_callback &stopping_condition);
/// \overload
IGL_INLINE decimate_stopping_condition_callback
max_faces_stopping_condition(int &m, const int orign_m, const int max_m);
} // namespace igl

...
///
/// @tparam T  should be a eigen sparse matrix primitive type like `int` or
/// `double`
/// @param[in] F  #F by dim list of mesh simplices
/// @param[out] A  max(F)+1 by max(F)+1 adjacency matrix, each row i
...
IGL_INLINE void adjacency_matrix(const Eigen::MatrixBase<DerivedF> &F,
...
/// @param[out] A  max(I)+1 by max(I)+1 adjacency matrix, each row i
/// corresponding to V(i,:)
///
template <typename DerivedI, typename DerivedC, typename T>
IGL_INLINE void adjacency_matrix(const Eigen::MatrixBase<DerivedI> &I,

...
template <typename EigsScalar, typename DerivedU, typename DerivedS,
          typename Solver>
IGL_INLINE bool igl::spectra::eigs(const Eigen::SparseMatrix<EigsScalar> &A,
                                   const Eigen::SparseMatrix<EigsScalar> &B,
                                   const int k, const igl::EigsType type,
                                   ...
                                   Eigen::PlainObjectBase<DerivedS> &S)
{
    ...
    // This seems like a hack. For the "eigs: grid" test this is necessary to
    // get at least 1e-4 error for the first 5 eigen values. It's annoying that
    // this means that the zero modes become O(sigma) and this is now rather
    // large.
    //
    // I wonder if this is an issue with SparseLU and if UMFPACK would be
    // better.
    //
...
IGL_INLINE bool igl::spectra::eigs(const Eigen::SparseMatrix<EigsScalar> &A,
                                   const Eigen::SparseMatrix<EigsScalar> &B,
                                   const int k, const EigsScalar sigma,
                                   Eigen::PlainObjectBase<DerivedU> &U,
                                   Eigen::PlainObjectBase<DerivedS> &S)
{
    ...
    class ShiftInvert
    {
      ...
      public:
        ...
        ShiftInvert(const Eigen::SparseMatrix<Scalar> &A,
                    const Eigen::SparseMatrix<Scalar> &B, const Scalar sigma)
            : m_A(A), m_B(B)
        {
            IGL_ASSERT(m_A.rows() == m_A.cols() && "A must be square");
            IGL_ASSERT(m_B.rows() == m_B.cols() && "B must be square");
            IGL_ASSERT(m_A.rows() == m_B.cols() &&
        ...
        void set_shift(const Scalar &sigma, const bool force = false)
        {
            ...
            m_solver.compute(C);
            m_solver_is_successfully_factorized =
    ...
    Spectra::SymGEigsShiftSolver<ShiftInvert, SparseMatProd,
                                 Spectra::GEigsMode::ShiftInvert>
        geigs(op, Bop, k, 2 * k, sigma);
...
template bool
igl::spectra::eigs<double, Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                   Eigen::Matrix<double, -1, 1, 0, -1, 1>,
                   Eigen::SparseLU<Eigen::SparseMatrix<double, 0, int>,
                                   Eigen::COLAMDOrdering<int>>>(
    Eigen::SparseMatrix<double, 0, int> const &,
    Eigen::SparseMatrix<double, 0, int> const &, int, igl::EigsType,

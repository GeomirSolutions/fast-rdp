...
template <typename DerivedX, typename DerivedP, typename DerivedN,
          typename DerivedR, typename Derivedt>
IGL_INLINE void igl::rigid_alignment(const Eigen::MatrixBase<DerivedX> &_X,
                                     ...
                                     Eigen::PlainObjectBase<Derivedt> &t)
{
    ...
    for (int iters = 0; iters < max_iters; iters++) {
        MatrixXS A(k * 3, 6);
        A << Z, X.col(2), -X.col(1), I, Z, Z, -X.col(2), Z, X.col(0), Z, I, Z,
            X.col(1), -X.col(0), Z, Z, Z, I;
        VectorXS B(k * 3, 1);
        B << P.col(0) - X.col(0), P.col(1) - X.col(1), P.col(2) - X.col(2);
        ...
        // This could be a lot faster but isn't rank revealing and may produce
        // wonky results when P and N are all the same point and vector.
        ...
        Matrix3S W;
        W << 0, u(2), -u(1), -u(2), 0, u(0), u(1), -u(0), 0;
        ...
        if (x == 0) {
            ...
            Ri = DerivedR::Identity(3, 3) + sin(x) / x * W +
                 (1.0 - cos(x)) / (x * x) * W * W;
        }
...
template void igl::rigid_alignment<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

...
template <typename DerivedV, typename DerivedTC, typename DerivedF,
          typename DerivedFTC, typename Derivedseams,
          typename Derivedboundaries, typename Derivedfoldovers>
IGL_INLINE void
igl::seam_edges(const Eigen::PlainObjectBase<DerivedV> &V,
                ...
                Eigen::PlainObjectBase<Derivedfoldovers> &foldovers)
{
    ...
    const auto &Orientation = [](const Vector2S &a, const Vector2S &b,
                                 const Vector2S &c) ->
        typename DerivedTC::Scalar {
    ...
    const auto &edge_hasher = [numV](directed_edge const &e) {
        return e.first * numV + e.second;
    };
    // When we pass a hash function object, we also need to specify the number
    // of buckets. The Euler characteristic says that the number of undirected
    // edges is numV + numF -2*genus.
    std::unordered_map<directed_edge, std::pair<int, int>,
                       decltype(edge_hasher)>
        directed_position_edge2face_position_index(2 * (numV + numF),
                                                   edge_hasher);
    for (int fi = 0; fi < F.rows(); ++fi) {
        for (int i = 0; i < 3; ++i) {
            const int j = (i + 1) % 3;
            directed_position_edge2face_position_index[std::make_pair(
                F(fi, i), F(fi, j))] = std::make_pair(fi, i);
        }
    }
    ...
    for (const auto &el : directed_position_edge2face_position_index) {
        ...
        undirected_position_edges.insert(
            std::make_pair(std::min(el.first.first, el.first.second),
    ...
    for (const auto &vp_edge : undirected_position_edges) {
        ...
        const auto vp_edge_reverse =
            std::make_pair(vp_edge.second, vp_edge.first);
        ...
        if (directed_position_edge2face_position_index.count(vp_edge) &&
            ...
            if (std::make_pair(
                ...
                std::make_pair(FTC(backwards.first, (backwards.second + 1) % 3),
                ...
                // If the opposite vertices' texture coordinates fall on the
                // same side of the edge, we have a UV-space foldover.
                const auto orientation_forwards = Orientation(a, b, c_forwards);
                const auto orientation_backwards =
                    Orientation(a, b, c_backwards);
        ...
        // edges. One of them should be.
            ...
            num_boundaries += 1;
        } else if (directed_position_edge2face_position_index.count(
                       vp_edge_reverse)) {
            const auto backwards =
                directed_position_edge2face_position_index[vp_edge_reverse];
            ...
            assert(directed_position_edge2face_position_index.count(vp_edge) ||
                   directed_position_edge2face_position_index.count(
                       vp_edge_reverse));
        }
    }
...
template void igl::seam_edges<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

    ...
    for (int f = 0; f < F.rows(); f++) {
        ...
        for (int c = 0; c < 3; c++) {
            if (std::isinf(V(F(f, c), 0)) || std::isinf(V(F(f, c), 1)) ||
                std::isinf(V(F(f, c), 2))) {
                assert(infinite_corner == -1 &&
                       "Should only be one infinite corner");
                infinite_corner = c;
            }
        }
        // Inputs:
        ...
        //     vectors of the subspace to which we're measuring distance
        //     (usually a plane, m=2)
        ...
        const auto subspace_quadric = [&I](const Eigen::RowVectorXd &p,
            ...
            // gcc <=4.8 can't handle initializer lists correctly: needs
            // explicit cast
        ...
        if (infinite_corner == -1) {
            ...
            S << ev, N;
            Quadric boundary_edge_quadric =
                subspace_quadric(p, S, length * length);
            for (int c = 0; c < 3; c++) {
                if (c != infinite_corner) {
                    quadrics[F(f, c)] =
                        quadrics[F(f, c)] + boundary_edge_quadric;
                }
            }
        }
    }

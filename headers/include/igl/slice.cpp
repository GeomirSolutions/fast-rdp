                           ...
                           Eigen::SparseMatrix<TY> &Y)
{
    ...
    for (int k = 0; k < X.outerSize(); ++k) {
        ...
        for (typename Eigen::SparseMatrix<TX>::InnerIterator it(X, k); it;
             ++it) {
            for (auto rit = RI[it.row()].begin(); rit != RI[it.row()].end();
                 rit++) {
                for (auto cit = CI[it.col()].begin(); cit != CI[it.col()].end();
                     cit++) {
...
template <class T>
IGL_INLINE void igl::slice(const std::vector<T> &unordered,
...
template void igl::slice<Eigen::SparseMatrix<double, 0, int>,
    ...
    Eigen::SparseMatrix<double, 0, int> &);
#ifdef WIN32
template void igl::slice<unsigned int>(
    class std::vector<unsigned int, class std::allocator<unsigned int>> const &,
...
template void
igl::slice<float>(std::vector<float, std::allocator<float>> const &,
                  std::vector<size_t, std::allocator<size_t>> const &,
                  std::vector<float, std::allocator<float>> &);
template void
igl::slice<double>(std::vector<double, std::allocator<double>> const &,
                   std::vector<size_t, std::allocator<size_t>> const &,
                   std::vector<double, std::allocator<double>> &);
template void
igl::slice<int>(std::vector<int, std::allocator<int>> const &,
                std::vector<size_t, std::allocator<size_t>> const &,
                std::vector<int, std::allocator<int>> &);
template void
igl::slice<long>(std::vector<long, std::allocator<long>> const &,
                 std::vector<size_t, std::allocator<size_t>> const &,
                 std::vector<long, std::allocator<long>> &);
#include "SortableRow.h"
template void igl::slice<igl::SortableRow<Eigen::Matrix<int, -1, 1, 0, -1, 1>>>(
    ...
    std::vector<
        ...
        &);
template void
igl::slice<igl::SortableRow<Eigen::Matrix<double, -1, 1, 0, -1, 1>>>(

...
// Returns index value
//
inline BlueNoiseKeyType
blue_noise_key(const BlueNoiseKeyType
                   w, // pass by copy --> int64_t so that multiplication is OK
               const BlueNoiseKeyType
                   x, // pass by copy --> int64_t so that multiplication is OK
               const BlueNoiseKeyType
                   y, // pass by copy --> int64_t so that multiplication is OK
               const BlueNoiseKeyType
                   z) // pass by copy --> int64_t so that multiplication is OK
{
    return x + w * (y + w * z);
}
...
template <typename DerivedX, typename DerivedXs>
inline bool
blue_noise_far_enough(const Eigen::MatrixBase<DerivedX> &X,
                      const Eigen::MatrixBase<DerivedXs> &Xs,
                      const std::unordered_map<BlueNoiseKeyType, int> &S,
                      const double &rr, const int &w, const int i)
{
    const int xi = Xs(i, 0);
    const int yi = Xs(i, 1);
    const int zi = Xs(i, 2);
    ...
    for (int x = std::max(xi - g, 0); x <= std::min(xi + g, w - 1); x++)
        for (int y = std::max(yi - g, 0); y <= std::min(yi + g, w - 1); y++)
            for (int z = std::max(zi - g, 0); z <= std::min(zi + g, w - 1);
                 z++) {
                if (x != xi || y != yi || z != zi) {
...
template <typename DerivedX, typename DerivedXs>
inline bool activate(const Eigen::MatrixBase<DerivedX> &X,
                     const Eigen::MatrixBase<DerivedXs> &Xs, const double &rr,
                     const int &i, const int &w, const BlueNoiseKeyType &nk,
    ...
    while (miter != Mvec.end()) {
        ...
        if (i >= 0 && (X.row(i) - X.row(mi)).squaredNorm() > 4. * rr) {
            ...
            *miter = Mvec.back();
            ...
            Mvec.pop_back();
            if (was_last) {
                // popping from the vector can invalidate the iterator, if it
                // was pointing to the last element that was popped.
                // Alternatively, one could use indices directly...
                miter = Mvec.end();
            }
        }
    }
...
template <typename DerivedX, typename DerivedXs, typename URBG>
inline bool step(const Eigen::MatrixBase<DerivedX> &X,
                 const Eigen::MatrixBase<DerivedXs> &Xs, const double &rr,
                 const int &w, URBG &&urbg,
                 std::unordered_map<BlueNoiseKeyType, std::vector<int>> &M,
                 std::unordered_map<BlueNoiseKeyType, int> &S,
                 std::vector<int> &active, std::vector<int> &collected)
{
    // considered.clear();
    if (active.size() == 0)
        return false;
...
template <typename DerivedV, typename DerivedF, typename DerivedB,
          typename DerivedFI, typename DerivedP, typename URBG>
IGL_INLINE void igl::blue_noise(const Eigen::MatrixBase<DerivedV> &V,
                                ...
                                URBG &&urbg)
{
    ...
    //
    // One thing to try would be to store a list in S (rather than a single
    // point) or equivalently a mask over M and just use M as a generic spatial
    // hash (with arbitrary size) and then tune its size (being careful to make
    // g a function of r and s; and removing the `if S=-1 checks`)
    const Scalar s = r / sqrt(3.0);

    const double area = [&]() {
        Eigen::VectorXd A;
        igl::doublearea(V, F, A);
        return A.array().sum() / 2;
    }();
    ...
    const auto initialize = [&]() -> bool {
        while (true) {
            ...
            if (S[k] < 0) {
                if (activate(X, Xs, rr, -1, w, k, M, S, active))
                    return true;
            }
            Mouter++;
        }
...
template void
igl::blue_noise<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

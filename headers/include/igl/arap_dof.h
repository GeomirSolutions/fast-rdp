...
///       Pi_2_left T_full +                  Lpart3 = M_fullsolve(right) *
///       B_eq_fix Pi_2_right P_pos                    Lpart2 =
///       M_fullsolve(left) * B_eq
...
///     handles)
/// @param[in] G  #V list of group indices (1 to k) for each vertex, such that
/// vertex i
///     is assigned to group G(i)
/// @param[out] data  structure containing all necessary precomputation for
/// calling
///     arap_dof_update
...
///
/// @param[in]  fixed_dim  list of transformation element indices for fixed (or
/// partailly
///   fixed) handles: not necessarily the complement of 'free'
///    NOTE: the constraints for fixed transformations still need to be
///    present in A_eq
/// @param[in] A_eq  dim*#constraint_points by m*dim*(dim+1)  matrix of linear
/// equality
...
/// precomputed system.
///
/// @param[in] data  precomputation data struct output from
/// arap_dof_precomputation
/// @param[in] Beq  dim*#constraint_points constraint values.
/// @param[in] L0  #handles * dim * dim+1 list of initial guess transformation
/// entries,
///     also holds fixed transformation entries for fixed handles
/// @param[in] max_iters  maximum number of iterations
/// @param[in] tol  stopping criteria parameter. If variables (linear
/// transformation
///     matrix entries) change by less than 'tol' the optimization terminates,
///       0.75 (weak tolerance)
///       0.0 (extreme tolerance)
/// @param[out] L  #handles * dim * dim+1 list of final optimized transformation
/// entries,
...
template <typename LbsMatrixType, typename SSCALAR> struct ArapDOFData
{
    ...
    /// Full solve matrix that contains also conversion from rotations to the
    /// right hand side, i.e., solves Poisson transformations just from
    /// rotations and positional constraints

...
/// @param[out] I  vector the same size as Y containing the indices along dim of
/// minimum
///     entries
///
template <typename AType, typename DerivedB, typename DerivedI>
IGL_INLINE void max(const Eigen::SparseMatrix<AType> &A, const int dim,
                    Eigen::PlainObjectBase<DerivedB> &B,
                    Eigen::PlainObjectBase<DerivedI> &I);
/// \overload
template <typename DerivedX, typename DerivedY, typename DerivedI>
IGL_INLINE void max(const Eigen::DenseBase<DerivedX> &X, const int dim,

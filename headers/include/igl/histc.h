...
/// @param[out] N  n-long vector where N(k) reveals how many values in X fall
/// between
///     E(k) <= X < E(k+1)
/// @param[out] B  m-long vector of bin ids so that B(j) = k if E(k) <= X(j) <
/// E(k+1).
///     B(j) = -1 if X(j) is outside of E.
///
template <typename DerivedX, typename DerivedE, typename DerivedN,
          typename DerivedB>
IGL_INLINE void histc(const Eigen::MatrixBase<DerivedX> &X,

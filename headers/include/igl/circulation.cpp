...
IGL_INLINE std::vector<int> igl::circulation(const int e, const bool ccw,
...
IGL_INLINE void igl::circulation(const int e, const bool ccw,
                                 const Eigen::VectorXi &EMAP,
                                 const Eigen::MatrixXi &EF,
                                 ...
                                 std::vector<int> &Nv, std::vector<int> &Nf)
{
  ...
  //   a---b-e-f
    ...
    const int m = EMAP.size() / 3;
    assert(m * 3 == EMAP.size());
    const auto &step = [&](const int e, const int ff, int &ne,
                           // int & re,
                           int &rv, int &nf) {
        assert((EF(e, 1) == ff || EF(e, 0) == ff) && "e should touch ff");
        // const int fside = EF(e,1)==ff?1:0;

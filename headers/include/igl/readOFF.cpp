                             ...
                             std::vector<std::vector<Scalar>> &C)
{
    ...
    if (fscanf(off_file, "%s\n", header) != 1 ||
        ...
        printf("Error: readOFF() first line should be OFF or NOFF or COFF, not "
               "%s...",
               header);
        fclose(off_file);
        return false;
    ...
    for (int i = 0; i < number_of_vertices;) {
        ...
        if (sscanf(line, "%lg %lg %lg %lg %lg %lg", &x, &y, &z, &nx, &ny,
                   &nz) >= 3) {
                             ...
                             Eigen::PlainObjectBase<DerivedV> &N)
{
    ...
    if (vN.size()) {
        ...
        if (!N_rect) {
            // igl::list_to_matrix(vN,N) already printed error message to std
            // err
            return false;
        }
    }
    ...
    if (vC.size()) {
        ...
        if (!C_rect) {
            // igl::list_to_matrix(vC,N) already printed error message to std
            // err
            return false;
        }
    }
...
template bool igl::readOFF<double, int>(

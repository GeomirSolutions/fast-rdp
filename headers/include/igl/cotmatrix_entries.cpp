                                       ...
                                       Eigen::PlainObjectBase<DerivedC> &C)
{
    ...
    case 3: {
        ...
        C.resize(m, 3);
        for (int i = 0; i < m; i++) {
            // Alec: I'm doubtful that using l2 here is actually improving
            // numerics.
    ...
    case 4: {
        ...
        sin_theta.col(0) =
            vol.array() / ((2. / (3. * l.col(0).array())).array() *
                           s.col(1).array() * s.col(2).array());
        sin_theta.col(1) =
            vol.array() / ((2. / (3. * l.col(1).array())).array() *
                           s.col(2).array() * s.col(0).array());
        sin_theta.col(2) =
            vol.array() / ((2. / (3. * l.col(2).array())).array() *
                           s.col(0).array() * s.col(1).array());
        sin_theta.col(3) =
            vol.array() / ((2. / (3. * l.col(3).array())).array() *
                           s.col(3).array() * s.col(0).array());
        sin_theta.col(4) =
            vol.array() / ((2. / (3. * l.col(4).array())).array() *
                           s.col(3).array() * s.col(1).array());
        sin_theta.col(5) =
            vol.array() / ((2. / (3. * l.col(5).array())).array() *
...
IGL_INLINE void igl::cotmatrix_entries(const Eigen::MatrixBase<Derivedl> &l,
                                       Eigen::PlainObjectBase<DerivedC> &C)
{
    using namespace Eigen;
    const int m = l.rows();
    ...
    // the square root and calling this. We can't have a
    // cotmatrix_entries(l,l2,C) overload because it will confuse Eigen with the
    // cotmatrix_entries(V,F,C) overload. In the end, I'd like to be convinced
    // that using l2 directly above is actually better numerically (or
    // significantly faster) than just calling edge_lengths and this
...
// generated by autoexplicit.sh
template void igl::cotmatrix_entries<Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                                     Eigen::Matrix<double, -1, -1, 0, -1, -1>>(

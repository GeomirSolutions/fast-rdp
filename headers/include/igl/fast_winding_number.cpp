...
template <typename DerivedP, typename DerivedA, typename DerivedN,
          typename Index, typename DerivedCH, typename DerivedCM,
          typename DerivedR, typename DerivedEC>
IGL_INLINE void igl::fast_winding_number(
    const Eigen::MatrixBase<DerivedP> &P, const Eigen::MatrixBase<DerivedN> &N,
    const Eigen::MatrixBase<DerivedA> &A,
    const std::vector<std::vector<Index>> &point_indices,
    const Eigen::MatrixBase<DerivedCH> &CH, const int expansion_order,
    Eigen::PlainObjectBase<DerivedCM> &CM, Eigen::PlainObjectBase<DerivedR> &R,
    Eigen::PlainObjectBase<DerivedEC> &EC)
{
    ...
    assert(expansion_order < 3 && expansion_order >= 0 &&
    ...
    helper = [&helper, &P, &N, &A, &point_indices, &CH, &EC, &R,
              &CM](const int index) -> void {
        ...
        for (int i = 0; i < point_indices[index].size(); i++) {
            ...
            if (EC.cols() == (3 + 9 + 27)) {
                for (int k = 0; k < 3; k++) {
                    TempCoeffs = 0.5 * point(k) *
                                 (A(curr_point_index) * point.transpose() *
                                  N.row(curr_point_index));
                    EC.block(index, 12 + 9 * k, 1, 9) +=
...
template <typename DerivedP, typename DerivedA, typename DerivedN,
          typename Index, typename DerivedCH, typename DerivedCM,
          typename DerivedR, typename DerivedEC, typename DerivedQ,
...
IGL_INLINE void igl::fast_winding_number(
    ...
    const Eigen::MatrixBase<DerivedQ> &Q, const BetaType beta,
    Eigen::PlainObjectBase<DerivedWN> &WN)
{
    ...
    auto expansion_eval = [&direct_eval, &EC,
                           &PI_4](const RowVec &loc,
        ...
        if (EC.row(child_index).size() > 3) {
            ...
            wn += Eigen::Map<Eigen::Matrix<real_ec, 1, 9>>(
                      SecondDerivative.data(), SecondDerivative.size())
                      .dot(EC.row(child_index).template segment<9>(3));
        }
        if (EC.row(child_index).size() > 3 + 9) {
            PI_4_r7 = PI_4_r5 * r * r;
            const Eigen::Matrix<real_ec, 3, 3> locTloc =
                loc.transpose() * (loc / (PI_4_r7));
            for (int i = 0; i < 3; i++) {
                ...
                Eigen::Matrix<real_ec, 3, 3> ThirdDerivative =
                    15.0 * loc(i) * locTloc +
                    (-3.0 / (PI_4_r5)) * (RowCol_Diagonal);

                wn += Eigen::Map<Eigen::Matrix<real_ec, 1, 9>>(
                          ThirdDerivative.data(), ThirdDerivative.size())
                          .dot(EC.row(child_index)
                                   .template segment<9>(12 + i * 9));
            }
        }
        return wn;
    };
    ...
    helper =
        [&helper, &P, &N, &A, &point_indices, &CH, &CM, &R, &beta, &direct_eval,
         &expansion_eval](const RowVec &query,
                          const std::vector<int> &near_indices) -> real_wn {
        real_wn wn = 0;
                    ...
                    if (point_indices[child_index].size() > 0) {
                        ...
                        if (CMciq.norm() > beta * R(child_index)) {
                            if (CH(child_index, 0) == -1) {
                                for (int j = 0;
                                     j < point_indices[child_index].size();
                                     j++) {
                                    int curr_row =
                                        point_indices[child_index][j];
                                    wn += direct_eval(P.row(curr_row) - query,
                                                      N.row(curr_row) *
                                                          A(curr_row));
                                }
...
template <typename DerivedV, typename DerivedF, typename DerivedQ,
          typename DerivedW>
IGL_INLINE void igl::fast_winding_number(const Eigen::MatrixBase<DerivedV> &V,
                                         ...
                                         FastWindingNumberBVH &fwn_bvh)
{
    ...
    fwn_bvh.ut_solid_angle.init(fwn_bvh.F.size() / 3, &fwn_bvh.F[0],
                                fwn_bvh.U.size(), &fwn_bvh.U[0], order);
}

template <typename DerivedQ, typename DerivedW>
IGL_INLINE void igl::fast_winding_number(const FastWindingNumberBVH &fwn_bvh,
                                         ...
                                         Eigen::PlainObjectBase<DerivedW> &W)
{
    ...
    igl::parallel_for(
        ...
        [&](int p) {
            ...
            Qp[2] = Q(p, 2);
            W(p) =
                fwn_bvh.ut_solid_angle.computeSolidAngle(Qp, accuracy_scale) /
                (4.0 * igl::PI);
        },
        1000);
}
...
IGL_INLINE typename Derivedp::Scalar
igl::fast_winding_number(const FastWindingNumberBVH &fwn_bvh,
                         const float accuracy_scale,
...
template void igl::fast_winding_number<Eigen::Matrix<double, 1, 3, 1, 1, 3>,
                                       Eigen::Matrix<double, -1, 1, 0, -1, 1>>(
    igl::FastWindingNumberBVH const &, float,
    Eigen::MatrixBase<Eigen::Matrix<double, 1, 3, 1, 1, 3>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> &);
template Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar
igl::fast_winding_number<Eigen::Matrix<float, -1, -1, 0, -1, -1>>(
    igl::FastWindingNumberBVH const &, float,
    Eigen::MatrixBase<Eigen::Matrix<float, -1, -1, 0, -1, -1>> const &);
// generated by autoexplicit.sh
template void igl::fast_winding_number<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> &);
template void
igl::fast_winding_number<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                         Eigen::Matrix<double, -1, 1, 0, -1, 1>,
...
template void igl::fast_winding_number<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> const &, int,
    igl::FastWindingNumberBVH &);
template void igl::fast_winding_number<Eigen::Matrix<float, -1, -1, 0, -1, -1>,
...
template Eigen::Matrix<float, 1, 3, 1, 1, 3>::Scalar
igl::fast_winding_number<Eigen::Matrix<float, 1, 3, 1, 1, 3>>(
    igl::FastWindingNumberBVH const &, float,
    Eigen::MatrixBase<Eigen::Matrix<float, 1, 3, 1, 1, 3>> const &);
template void igl::fast_winding_number<Eigen::Matrix<float, -1, 3, 0, -1, 3>,
                                       Eigen::Matrix<int, -1, 3, 0, -1, 3>>(
    Eigen::MatrixBase<Eigen::Matrix<float, -1, 3, 0, -1, 3>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, -1, 3, 0, -1, 3>> const &, int,
    igl::FastWindingNumberBVH &);
template void igl::fast_winding_number<Eigen::Matrix<float, -1, 3, 1, -1, 3>,
                                       Eigen::Matrix<int, -1, 3, 1, -1, 3>>(
    Eigen::MatrixBase<Eigen::Matrix<float, -1, 3, 1, -1, 3>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, -1, 3, 1, -1, 3>> const &, int,
    igl::FastWindingNumberBVH &);
template Eigen::CwiseUnaryOp<Eigen::internal::scalar_cast_op<double, float>,
                             Eigen::Matrix<double, 1, 3, 1, 1, 3> const>::Scalar
igl::fast_winding_number<
    Eigen::CwiseUnaryOp<Eigen::internal::scalar_cast_op<double, float>,
                        Eigen::Matrix<double, 1, 3, 1, 1, 3> const>>(
    igl::FastWindingNumberBVH const &, float,
    Eigen::MatrixBase<
        Eigen::CwiseUnaryOp<Eigen::internal::scalar_cast_op<double, float>,
                            Eigen::Matrix<double, 1, 3, 1, 1, 3> const>> const
        &);
#endif

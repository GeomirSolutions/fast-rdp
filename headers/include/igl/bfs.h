...
/// @param[out] D  #V list of indices into rows of A in the order in which graph
/// nodes
///     are discovered.
...
IGL_INLINE void bfs(const AType &A, const size_t s,
...
IGL_INLINE void bfs(const std::vector<std::vector<AType>> &A, const size_t s,
                    std::vector<DType> &D, std::vector<PType> &P);
/// \overload
template <typename AType, typename DType, typename PType>
IGL_INLINE void bfs(const Eigen::SparseCompressedBase<AType> &A, const size_t s,
                    std::vector<DType> &D, std::vector<PType> &P);
} // namespace igl

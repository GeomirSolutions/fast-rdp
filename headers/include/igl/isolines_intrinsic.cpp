...
template <typename DerivedF, typename DerivedS, typename Derivedvals,
          typename DerivediB, typename DerivediFI, typename DerivediE,
          typename DerivedI>
                             ...
                             Eigen::PlainObjectBase<DerivedI> &I)
{
...

    {
        ...
        for (int j = 0; j < vals.size(); j++) {
            isolines_intrinsic(F, S, uE, EMAP, uEC, uEE, vals(j), viB[j],
                               viFI[j], viE[j]);
...
template <typename DerivedF, typename DerivedS, typename DeriveduE,
          typename DerivedEMAP, typename DeriveduEC, typename DeriveduEE,
          typename DerivediB, typename DerivediFI, typename DerivediE>
void igl::isolines_intrinsic(const Eigen::MatrixBase<DerivedF> &F,
                             const Eigen::MatrixBase<DerivedS> &S,
                             ...
                             Eigen::PlainObjectBase<DerivediE> &iE)
{
    ...
    {
        const auto add_vertex_crossing = [&iB, &iFI](const int k, const int i,
                                                     const int j) {
            if (k >= iB.rows()) {
    ...
    };
    {
        ...
        for (int f = 0; f < F.rows(); f++) {
            ...
            if (j < 3) {
                ...
                // Could have 2 vertex crossings. We're only interested if
                // there're exactly two and if the other vertex is "above".
                int i = 0;
                for (i = 0; i < 3; i++) {
                    if (S(F(f, i)) == val) {
                        break;
...
template void igl::isolines_intrinsic<Eigen::Matrix<int, -1, -1, 0, -1, -1>,

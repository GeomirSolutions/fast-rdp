...
template <typename DerivedV, typename DerivedF> class PlanarizerShapeUp
{
  protected:
    ...
    // Q: lhs matrix
    // Ni: matrix that subtracts the mean of a face from the 4 vertices of a
    // face
    Eigen::SparseMatrix<typename DerivedV::Scalar> Q, Ni;
    Eigen::SimplicialLDLT<Eigen::SparseMatrix<typename DerivedV::Scalar>>
        solver;
...
inline igl::PlanarizerShapeUp<DerivedV, DerivedF>::PlanarizerShapeUp(
    ...
    const Eigen::MatrixBase<DerivedF> &F_, const int maxIter_,
...
inline void igl::PlanarizerShapeUp<DerivedV, DerivedF>::assembleP()
{
    ...
    for (int fi = 0; fi < numF; fi++) {
        ...
        Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 1> Vi =
            NSi * Vv;
        ...
        Eigen::EigenSolver<Eigen::Matrix<typename DerivedV::Scalar, 3, 3>> es(
            C);
        // the real() is for compilation purposes
        Eigen::Matrix<typename DerivedV::Scalar, 3, 1> lambda =
            es.eigenvalues().real();
        Eigen::Matrix<typename DerivedV::Scalar, 3, 3> U =
    ...
    Eigen::PlainObjectBase<DerivedV> &Vout)
{
    ...
    for (int iter = 0; iter < maxIter; ++iter) {
        ...
        typename DerivedV::Scalar nonPlanarity =
            planarity.cwiseAbs().maxCoeff();
        ...
        if (solver.info() != Eigen::Success) {
            std::cerr << "Linear solve failed - PlanarizerShapeUp.cpp"
                      << std::endl;
            assert(0);
        }
        for (int i = 0; i < numV; ++i)

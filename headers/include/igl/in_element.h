...
/// @param[out] I  #Q list of indices into Ele of first containing element (-1
/// means no
///     containing element)
template <typename DerivedV, typename DerivedQ, int DIM>
IGL_INLINE void in_element(const Eigen::MatrixBase<DerivedV> &V,
                           const Eigen::MatrixXi &Ele,
                           const Eigen::MatrixBase<DerivedQ> &Q,
                           const AABB<DerivedV, DIM> &aabb, Eigen::VectorXi &I);
/// \overload
///
/// @param[out] I  #Q by #Ele sparse matrix revealing whether each element
/// contains each
///     point: I(q,e) means point q is in element e
template <typename DerivedV, typename DerivedQ, int DIM, typename Scalar>
IGL_INLINE void
in_element(const Eigen::MatrixBase<DerivedV> &V, const Eigen::MatrixXi &Ele,
           const Eigen::MatrixBase<DerivedQ> &Q,
           const AABB<DerivedV, DIM> &aabb, Eigen::SparseMatrix<Scalar> &I);
}; // namespace igl

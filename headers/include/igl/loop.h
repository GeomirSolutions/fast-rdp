...
IGL_INLINE void loop(const int n_verts, const Eigen::MatrixBase<DerivedF> &F,
                     Eigen::SparseMatrix<SType> &S,
                     Eigen::PlainObjectBase<DerivedNF> &NF);
/// Given the triangle mesh [V, F], computes number_of_subdivs steps of loop
/// subdivision and outputs the new mesh [newV, newF]
///
...
/// @param[in] number_of_subdivs an integer that specifies how many subdivision
/// steps to do
template <typename DerivedV, typename DerivedF, typename DerivedNV,
          typename DerivedNF>
IGL_INLINE void
loop(const Eigen::MatrixBase<DerivedV> &V, const Eigen::MatrixBase<DerivedF> &F,
     Eigen::PlainObjectBase<DerivedNV> &NV,
     Eigen::PlainObjectBase<DerivedNF> &NF, const int number_of_subdivs = 1);
} // namespace igl

...
///
/// @param[in] d  #d by dim list of points along a curve to be fit with a cubic
/// bezier
...
IGL_INLINE void fit_cubic_bezier(const Eigen::MatrixXd &d, const double error,
...
/// @param[in] force_split  whether to force a split (i.e., force a recursive
/// call)
/// @param[in] cubics  running list of cubics so far
/// @param[out] cubics  running list of cubics so far (new cubics appended)
IGL_INLINE void
fit_cubic_bezier_substring(const Eigen::MatrixXd &d, const int first,
                           const int last, const Eigen::RowVectorXd &tHat1,
                           const Eigen::RowVectorXd &tHat2, const double error,

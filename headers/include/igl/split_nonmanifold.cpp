...
IGL_INLINE void igl::split_nonmanifold(const Eigen::MatrixBase<DerivedF> &F,
                                       ...
                                       Eigen::PlainObjectBase<DerivedSVI> &SVI)
{
    ...
    for (int i = 0; i < EI.rows(); i++) {
        for (int j = 0; j < 2; j++) {
            Aijv.emplace_back(EI(i, j), EJI_flip(i, j), true);
        }
    }
...
template <typename DerivedV, typename DerivedF, typename DerivedSV,
          typename DerivedSF, typename DerivedSVI>
IGL_INLINE void igl::split_nonmanifold(const Eigen::MatrixBase<DerivedV> &V,
...
template void igl::split_nonmanifold<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

...
IGL_INLINE void
per_corner_normals(const Eigen::MatrixBase<DerivedV> &V,
                   const Eigen::MatrixBase<DerivedF> &F,
                   const typename DerivedV::Scalar corner_threshold_degrees,
                   Eigen::PlainObjectBase<DerivedCN> &CN);
/// \overload
/// @param[in] VF  3*#F list  List of faces indice on each vertex, so that
/// VF(NI(i)+j) =
...
template <typename DerivedV, typename DerivedF, typename DerivedVF,
          typename DerivedNI, typename DerivedCN>
IGL_INLINE void
per_corner_normals(const Eigen::MatrixBase<DerivedV> &V,
                   const Eigen::MatrixBase<DerivedF> &F,
                   const typename DerivedV::Scalar corner_threshold_degrees,
...
/// @param[in] CC  3*#F+1 list of cumulative sizes so that CC(i*3+j+1) -
/// CC(i*3+j) is
///     the number of faces considered smoothly incident on corner at F(i,j)
///
/// \see smooth_corner_adjacency
template <typename DerivedV, typename DerivedF, typename DerivedCI,
          typename DerivedCC, typename DerivedCN>
IGL_INLINE void per_corner_normals(const Eigen::MatrixBase<DerivedV> &V,
                                   const Eigen::MatrixBase<DerivedF> &F,
...
IGL_INLINE void per_corner_normals(const Eigen::MatrixBase<DerivedNV> &NV,
                                   const Eigen::MatrixBase<DerivedNF> &NF,
                                   Eigen::PlainObjectBase<DerivedCN> &CN);
/// Per-corner normals for a polygon mesh
///
/// @param[in] V  #V by 3 list of mesh vertex positions
/// @param[in] I  #I vectorized list of polygon corner indices into rows of some
/// matrix V
/// @param[in] C  #polygons+1 list of cumulative polygon sizes so that
/// C(i+1)-C(i) = size of
///              the ith polygon, and so I(C(i)) through I(C(i+1)-1) are the
...
/// @param[out] VV  #I+#polygons by 3 list of auxiliary triangle mesh vertex
/// positions
...
template <typename DerivedV, typename DerivedI, typename DerivedC,
          typename DerivedN, typename DerivedVV, typename DerivedFF,
          typename DerivedJ, typename DerivedNN>
IGL_INLINE void per_corner_normals(
    const Eigen::MatrixBase<DerivedV> &V, const Eigen::MatrixBase<DerivedI> &I,
    const Eigen::MatrixBase<DerivedC> &C,
    const typename DerivedV::Scalar corner_threshold_degrees,
    Eigen::PlainObjectBase<DerivedN> &N, Eigen::PlainObjectBase<DerivedVV> &VV,

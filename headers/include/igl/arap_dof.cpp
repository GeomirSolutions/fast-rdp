...
{
    static void slice(const Mat &A, const Eigen::VectorXi &I,
                      const Eigen::VectorXi &J, Mat &B)
    {
        static_assert(std::is_base_of<Eigen::SparseMatrixBase<Mat>, Mat>::value,
                      "Mat must be sparse");
        igl::slice(A, I, J, B);
    }
};
                             ...
                             ArapDOFData<LbsMatrixType, SSCALAR> &data)
{
    ...
    // matrices that when multiplied on the right by column of the
    // transformation matrix entries (the degrees of freedom) L, we get a stack
    // of dim by 1 covariance matrix column, with a column in the stack for each
    // rotation *group*. The output is a list of matrices because we construct
    // each column in the stack of covariance matrices with an independent
    // matrix-vector multiplication.
    //
    // We want to build S which is a stack of dim by dim covariance matrices.
    // Thus S is dim*g by dim, where dim is the number of dimensions and g is
    // the number of groups. We can precompute dim matrices CSM_M such that
    // column i in S is computed as S(:,i) = CSM_M{i} * L, where L is a column
    // of the skinning transformation matrix values. To be clear, the covariance
    // matrix for group k is then given as the dim by dim matrix pulled from the
    // stack: S((k-1)*dim + 1:dim,:)
    ...
    for (int i = 0; i < data.dim; i++) {
        ...
        if constexpr (std::is_base_of<SparseMatrixBase<LbsMatrixType>,
                                      LbsMatrixType>::value) {
            slice(M, (span_n.array() + i * n).matrix().eval(), span_mlbs_cols,
                  M_i);
        } else {
            M_i = M((span_n.array() + i * n).eval(), span_mlbs_cols);
        }
#else
        constexpr bool LbsMatrixTypeIsSparse =
            std::is_base_of<SparseMatrixBase<LbsMatrixType>,
                            LbsMatrixType>::value;
        arap_dof_slice_helper<LbsMatrixType, LbsMatrixTypeIsSparse>::slice(
            M, (span_n.array() + i * n).matrix().eval(), span_mlbs_cols, M_i);
#endif
    ...
    for (int r = 0; r < numRows; r++) {
        for (int coord = 0; coord < dim + 1; coord++) {
            for (int b = 0; b < numBones; b++) {
                // this is just a test if we really have a multiple of 3x3
                // identity
                Eigen::Matrix3f blok;
                for (int v = 0; v < 3; v++) {
                    for (int w = 0; w < 3; w++) {
                        blok(v, w) = CSM_M_SSCALAR[v](
                            r, coord * (numBones * dim) + b + w * numBones);
                    }
                }
            ...
            for (int colCoord = 0; colCoord < dim; colCoord++) {
                for (int g = 0; g < numGroups; g++) {
                    ...
                    for (int r = 0; r < 3; r++) {
                        for (int c = 0; c < 3; c++) {
                            blok(r, c) = Solve1(
                                rowCoord * numBones * dim + r * numBones + b,
    ...
    ArapDOFData<LbsMatrixType, SSCALAR> &data)
{
    ...
    const int fsRows =
        data.m * data.dim * (data.dim + 1); // 12 * number_of_bones
    ...
    if (data.with_dynamics) {
        ...
        data.Pi_1 =
            M_Solve.block(0, 0, fsRows, fsRows).template cast<SSCALAR>();
    }
                     ...
                     Eigen::MatrixXd &L)
{
    ...
    const MatrixXS L_part3 =
        data.M_FullSolve.block(0, Rcol.rows() + B_eq_SSCALAR.rows(),
                               data.M_FullSolve.rows(),
                               B_eq_fix_SSCALAR.rows()) *
        B_eq_fix_SSCALAR;
...
#endif
    {
        ...
        // all this shuffling is retarded and not completely negligible
        // time-wise;
        // TODO: change fit_rotations_XXX so it returns R in the format ready
        // for CSolveBlock1 multiplication
        ...
        if (data.with_dynamics) {
            ...
            // -1.0 because we've moved these linear terms to the right hand
            // side
            // MatrixXS temp = -1.0 *
            ...
            MatrixXS temp =
                -1.0 * ((-1.0 / (data.h * data.h)) * data.L0.array() +
            ...
            MatrixXd temp2 = data.Mass_tilde * temp_d + temp_g +
                             data.fext.template cast<double>();
        ...
        max_diff =
            (L_SSCALAR - L_prev).eval().array().abs().matrix().maxCoeff();
#endif
        iters++;

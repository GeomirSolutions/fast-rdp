...
class MismatchCalculatorLine
{
  ...
  private:
    ...
    inline int mismatchByLine(const int f0, const int f1)
    {
        ...
        //    double a1 =
        //    atan2(dir1Rot.dot(B2.row(f0)),dir1Rot.dot(B1.row(f0)));
        //
  ...
  public:
    ...
    inline void
    calculateMismatchLine(Eigen::PlainObjectBase<DerivedO> &Handle_MMatch)
    {
...
template <typename DerivedV, typename DerivedF, typename DerivedO>
IGL_INLINE void
igl::line_field_mismatch(const Eigen::PlainObjectBase<DerivedV> &V,
                         const Eigen::PlainObjectBase<DerivedF> &F,
                         const Eigen::PlainObjectBase<DerivedV> &PD1,
                         const bool isCombed,
                         Eigen::PlainObjectBase<DerivedO> &mismatch)
{
    ...
    PD2_combed = igl::rotate_vectors(
        PD1_combed, Eigen::VectorXd::Constant(1, igl::PI / 2), B1, B2);
    igl::MismatchCalculatorLine<DerivedV, DerivedF, DerivedO> sf(

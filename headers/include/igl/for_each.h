...
/// @param[in] func  function handle with prototype "compatible with" `void
/// (Index i,
///     Index j, Scalar & v)`. Return values will be ignored.
///
/// \see std::for_each
template <typename AType, typename Func>
inline void for_each(const Eigen::SparseMatrix<AType> &A, const Func &func);
/// \overload
template <typename DerivedA, typename Func>
inline void for_each(const Eigen::DenseBase<DerivedA> &A, const Func &func);
} // namespace igl

// Implementation

template <typename AType, typename Func>
inline void igl::for_each(const Eigen::SparseMatrix<AType> &A, const Func &func)
{
    // Can **not** use parallel for because this must be _in order_
    // Iterate over outside
    for (int k = 0; k < A.outerSize(); ++k) {
        // Iterate over inside
        for (typename Eigen::SparseMatrix<AType>::InnerIterator it(A, k); it;
             ++it) {
...
template <typename DerivedA, typename Func>
inline void igl::for_each(const Eigen::DenseBase<DerivedA> &A, const Func &func)
{
    // Can **not** use parallel for because this must be _in order_
    if (A.IsRowMajor) {

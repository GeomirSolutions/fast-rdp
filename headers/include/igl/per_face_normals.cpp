...
template <typename DerivedV, typename DerivedF, typename DerivedN>
IGL_INLINE void
igl::per_face_normals_stable(const Eigen::MatrixBase<DerivedV> &V,
                             const Eigen::MatrixBase<DerivedF> &F,
                             Eigen::PlainObjectBase<DerivedN> &N)
{
    ...
    parallel_for(
        ...
        [&](const int f) {
            ...
            // careful sum
            for (int d = 0; d < 3; d++) {
                // This is a little _silly_ in terms of complexity, but its
                // recursive implementation is clean looking...
    ...
    J.resize(C(m));
    {
        ...
        for (Eigen::Index p = 0; p < m; p++) {
            ...
            for (Eigen::Index i = 0; i < np; i++) {
                FF.row(k) << I(C(p) + ((i + 0) % np)), I(C(p) + ((i + 1) % np)),
                    V.rows() + p;
                J(k) = p;
                k++;
                typedef Eigen::Matrix<Scalar, 1, 3> V3;
                N.row(p) +=
                    V3(VV.row(I(C(p) + ((i + 0) % np))) - VV.row(V.rows() + p))
                        .cross(V3(VV.row(I(C(p) + ((i + 1) % np))) -
                                  VV.row(V.rows() + p)));
            }
...
template void
igl::per_face_normals<Eigen::Matrix<float, -1, 3, 1, -1, 3>,
...
template void igl::per_face_normals<
...
template void igl::per_face_normals<Eigen::Matrix<float, -1, 3, 0, -1, 3>,
...
{
template <>
void per_face_normals<Eigen::Matrix<double, -1, 2, 0, -1, 2>,
...
template void igl::per_face_normals<Eigen::Matrix<float, -1, 3, 0, -1, 3>,
    ...
    Eigen::MatrixBase<Eigen::Matrix<int, -1, 3, 0, -1, 3>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<float, -1, 3, 0, -1, 3>> &);
template void igl::per_face_normals<Eigen::Matrix<float, -1, -1, 0, -1, -1>,
                                    Eigen::Matrix<int, -1, -1, 0, -1, -1>,
                                    Eigen::Matrix<float, -1, -1, 0, -1, -1>>(
...
template void
igl::per_face_normals<class Eigen::Matrix<double, -1, 3, 0, -1, 3>,
                      class Eigen::Matrix<int, -1, -1, 0, -1, -1>,
                      class Eigen::Matrix<double, -1, -1, 0, -1, -1>>(
    class Eigen::MatrixBase<class Eigen::Matrix<double, -1, 3, 0, -1, 3>> const
        &,
    ...
    class Eigen::PlainObjectBase<class Eigen::Matrix<double, -1, -1, 0, -1, -1>>
        &);
template void igl::per_face_normals<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                                    Eigen::Matrix<int, 2, 3, 0, 2, 3>,
                                    Eigen::Matrix<double, 2, 3, 0, 2, 3>>(
    Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, 2, 3, 0, 2, 3>> const &,

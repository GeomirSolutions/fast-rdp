...
template <typename DerivedV, typename DerivedF, typename DerivedVT,
          typename DerivedFT, typename Scalar, typename DerivedUE,
          typename DerivedUT, typename DerivedOT>
                                      ...
                                      Eigen::PlainObjectBase<DerivedOT> &OT)
{
    ...
    assert((!((BF && (BT != true)).any())) &&
           "Not dealing with boundaries of mesh that get 'stitched' in texture "
           "mesh");
    typedef Matrix<typename DerivedF::Scalar, Dynamic, 2> MatrixX2I;
    const MatrixX2I ET = (MatrixX2I(FT.rows() * 3, 2) << FT.col(1), FT.col(2),
                          FT.col(2), FT.col(0), FT.col(0), FT.col(1))
                             .finished();
    // "half"-edges with indices into 3D-mesh
    const MatrixX2I EF = (MatrixX2I(F.rows() * 3, 2) << F.col(1), F.col(2),
                          F.col(2), F.col(0), F.col(0), F.col(1))
                             .finished();
    ...
    Array<bool, Dynamic, 1> vBT =
        Map<Array<bool, Dynamic, 1>>(BT.data(), BT.size(), 1);
    Array<bool, Dynamic, 1> vBF =
        Map<Array<bool, Dynamic, 1>>(BF.data(), BF.size(), 1);
    const auto vBT_i = igl::find(vBT);
    MatrixX2I OF = EF(vBT_i, Eigen::all);
    OT = EF(vBT_i, Eigen::all);
    VectorXi OFMAP = EFMAP(vBT_i);
    // Two boundary edges on the texture-mapping are "equivalent" to each other
    // on the 3D-mesh if their 3D-mesh vertex indices match
    SparseMatrix<bool> OEQ;
    {
        SparseMatrix<bool> OEQR;
        sparse(igl::LinSpaced<VectorXi>(OT.rows(), 0, OT.rows() - 1), OFMAP,
               Array<bool, Dynamic, 1>::Ones(OT.rows(), 1), OT.rows(), m * 3,
               OEQR);
        OEQ = OEQR * OEQR.transpose();
        // Remove diagonal
        OEQ.prune([](const int r, const int c, const bool) -> bool {
            return r != c;
        });
    }
    // For each edge in OT, for each endpoint, how many _other_ texture-vertices
    // are images of all the 3d-mesh vertices in F who map from "corners" in
    // F/FT mapping to this endpoint.
    //
    // Adjacency matrix between 3d-vertices and texture-vertices
    SparseMatrix<bool> V2VT;
    sparse(F, FT, Array<bool, Dynamic, 3>::Ones(F.rows(), F.cols()), V.rows(),
           VT.rows(), V2VT);
    // For each 3d-vertex count how many different texture-coordinates its
    // getting from different incident corners
    VectorXi DV;
    count(V2VT, 2, DV);
    VectorXi M, I;
    max(V2VT, 1, M, I);
    ...
    SparseMatrix<bool> VTOT;
    {
        Eigen::MatrixXi I =
            igl::LinSpaced<VectorXi>(OT.rows(), 0, OT.rows() - 1)
                .replicate(1, 2);
        sparse(OT, I, Array<bool, Dynamic, 2>::Ones(OT.rows(), OT.cols()),
               VT.rows(), OT.rows(), VTOT);
...

    {
        ...
        //  There might be an ear on one copy, so mark vertices on other copies,
        //  too ears as they live on the 3D mesh
        Array<bool, Dynamic, 1> earT =
            Array<bool, Dynamic, 1>::Zero(VT.rows(), 1);
        for (int e = 0; e < ear.size(); e++)
            earT(FT(ear(e), ear_opp(e))) = 1;
        // std::cout<<"#earT: "<<earT.count()<<std::endl;
        //  Even if ear-vertices are marked as sharp if it changes, e.g., from
        //  convex to concave then it will _force_ a flip of the ear triangle.
        //  So, declare that neighbors of ears are also sharp.
        SparseMatrix<bool> A;
        adjacency_matrix(FT, A);
        earT = (earT || (A * earT.matrix()).array()).eval();
        // std::cout<<"#earT: "<<earT.count()<<std::endl;
        assert(earT.size() == SV.size());
        for (int e = 0; e < earT.size(); e++)
            if (earT(e))
                SV(e) = true;
        // std::cout<<"#SV: "<<SV.count()<<std::endl;
    }
    ...
    for (int c = 0; c < nc; c++) {
        ...
        case 1: {
            ...
            assert((vpath(0) != vpath(vpath.size() - 1) || !SVvpath.any()) &&
        ...
        case 2: {
            ...
            VectorXi Icc;
            {
                ...
                assert(II.size() == Ic.size() &&
                       (II.array() ==
                        igl::LinSpaced<VectorXi>(Ic.size(), 0, Ic.size() - 1)
                            .array())
                           .all());
            ...
            if (Ic.size() == 1) {
                ...
                assert(Icc.size() == 1);
                vUE.push_back({OT(Icc(0), flipped(0) ? 1 : 0),
                               OT(Icc(0), flipped(0) ? 0 : 1)});
            } else {
                MatrixX2I OTIc = OT(Ic, Eigen::all);
                edges_to_path(OTIc, vpath, epath, eend);
                // Flip endpoints if needed
                for (int e = 0; e < eend.size(); e++)
                    if (flipped(e))
                        eend(e) = 1 - eend(e);
                ...
                Matrix<Scalar, Dynamic, Dynamic> PI(vpath.size(),
                                                    VT.cols() * 2);
...
template void
igl::straighten_seams<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

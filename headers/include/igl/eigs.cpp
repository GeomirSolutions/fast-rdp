                          ...
                          Eigen::PlainObjectBase<DerivedS> &sS)
{
    ...
    typedef Eigen::Matrix<typename DerivedU::Scalar,
                          DerivedU::RowsAtCompileTime, 1>
        VectorXS;
    ...
    while (true) {
        ...
        if (i == 0 || (S.head(i).array() - sigma).abs().maxCoeff() > 1e-14 ||
            ...
            // std::cout<<"
            // "<<(S.head(i).array()-sigma).abs().maxCoeff()<<std::endl;
            // std::cout<<"
            // "<<(U.leftCols(i).transpose()*B*x).array().abs().transpose()<<std::endl;
...
igl::eigs<double, double, Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::SparseMatrix<double, 0, int> const &, const size_t, igl::EigsType,

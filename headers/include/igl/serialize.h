...
#define SERIALIZE_TYPE(Type, Params)                                           \
    ...
    {                                                                          \
    void _serialization(bool s, Type &obj, std::vector<char> &buffer)          \
    {                                                                          \
        Params                                                                 \
    }                                                                          \
    template <>                                                                \
...
template <typename T1, typename T2> struct is_stl_container<std::vector<T1, T2>>
{
    static const bool value = true;
};
...
template <typename T> struct is_stl_container<std::list<T>>
{
    static const bool value = true;
};

template <typename T> struct is_eigen_type
{
    static const bool value = false;
};
...
// std::list
template <typename T> inline size_t getByteSize(const std::list<T> &obj);
template <typename T>
inline void serialize(const std::list<T> &obj, std::vector<char> &buffer,
                      std::vector<char>::iterator &iter);
template <typename T>
...
{
    // space for numbers of rows,cols and data
    return 2 * sizeof(typename Eigen::Array<T, R, C, P, MR, MC>::Index) +
           sizeof(T) * obj.rows() * obj.cols();
}
                      ...
                      std::vector<char>::iterator &iter)
{
    ...
    for (int k = 0; k < obj.outerSize(); ++k) {
        for (typename Eigen::SparseMatrix<T, P, I>::InnerIterator it(obj, k);
             it; ++it) {
...
template <typename T>
inline void serialize(const std::weak_ptr<T> & /*obj*/,
                      std::vector<char> & /*buffer*/,
                      std::vector<char>::iterator & /*iter*/)
{
}
...
inline void serialize(const T &obj, std::vector<char> & /*buffer*/)
{
    std::cerr
        << typeid(obj).name()
        << " is not serializable: derive from igl::Serializable or specialize "
...
inline void deserialize(T &obj, const std::vector<char> & /*buffer*/)
{
    std::cerr << typeid(obj).name()
              << " is not deserializable: derive from igl::Serializable or "
                 "specialize the template function "
                 "igl::serialization::deserialize(T& obj, const "
                 "std::vector<char>& buffer)"
              << std::endl;
}
                ...
                std::map<std::uintptr_t, IndexedPointerBase *> &memoryMap)
{
    ...
    if (el.second && (iter == memoryMap.end() ||
        ...
        auto p = std::pair<std::uintptr_t, IndexedPointerBase *>(endAddress,
                                                                 endBasePtr);

...
IGL_INLINE bool igl::readOBJ(
    const std::string obj_file_name, std::vector<std::vector<Scalar>> &V,
    std::vector<std::vector<Scalar>> &TC, std::vector<std::vector<Scalar>> &N,
    std::vector<std::vector<Index>> &F, std::vector<std::vector<Index>> &FTC,
    std::vector<std::vector<Index>> &FN,
    std::vector<std::tuple<std::string, Index, Index>> &FM)
{
    // Open file, and check for error
...
template <typename Scalar, typename Index>
IGL_INLINE bool igl::readOBJ(
    FILE *obj_file, std::vector<std::vector<Scalar>> &V,
    std::vector<std::vector<Scalar>> &TC, std::vector<std::vector<Scalar>> &N,
    std::vector<std::vector<Index>> &F, std::vector<std::vector<Index>> &FTC,
    std::vector<std::vector<Index>> &FN,
    std::vector<std::tuple<std::string, Index, Index>> &FM)
...
#define MATERIAL_LINE_MAX 2048
    ...
    while (fgets(line, IGL_LINE_MAX, obj_file) != NULL) {
        ...
        if (sscanf(line, "%s", type) == 1) {
            ...
            if (type == v) {
                ...
                if (vertex.size() < 3) {
                    fprintf(stderr,
                            "Error: readOBJ() vertex on line %d should have at "
                            "least 3 coordinates",
                            line_no);
                    fclose(obj_file);
                    return false;
                ...
                if (count != 3) {
                    fprintf(stderr,
                            "Error: readOBJ() normal on line %d should have 3 "
                            "coordinates",
                            line_no);
                    fclose(obj_file);
                    return false;
                ...
                if (count != 2 && count != 3) {
                    fprintf(stderr,
                            "Error: readOBJ() texture coords on line %d should "
                            "have 2 "
                            "or 3 coordinates (%d)",
                            line_no, count);
                    fclose(obj_file);
                ...
                while (sscanf(l, "%s%n", word, &offset) == 1) {
                    ...
                    if (sscanf(word, "%ld/%ld/%ld", &i, &it, &in) == 3) {
                        ...
                        fprintf(stderr,
                                "Error: readOBJ() face on line %d has invalid "
                                "element format\n",
                                line_no);
                        fclose(obj_file);
                        return false;
                ...
                if ((f.size() > 0 && fn.size() == 0 && ftc.size() == 0) ||
                    ...
                    (f.size() > 0 && fn.size() == f.size() &&
                     ftc.size() == f.size())) {
                    // No matter what add each type to lists so that lists are
                    // the correct lengths
                    F.push_back(f);
                    FTC.push_back(ftc);
                    FN.push_back(fn);
                ...
                if (FMwasinit) {
                    FM.push_back(std::make_tuple(currentmaterialref,
                                                 previous_face_no,
                                                 current_face_no - 1));
    ...
    if (strcmp(currentmaterialref, "") != 0)
        FM.push_back(std::make_tuple(currentmaterialref, previous_face_no,
                                     current_face_no - 1));
    fclose(obj_file);
                             ...
                             Eigen::PlainObjectBase<DerivedFN> &FN)
{
    ...
    if (!success) {
        // readOBJ(str,vV,vTC,vN,vF,vFTC,vFN) should have already printed an
...
template bool igl::readOBJ<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 1, 0, -1, 1>> &);
template bool igl::readOBJ<double, int>(
    std::basic_string<char, std::char_traits<char>, std::allocator<char>>,
...
template bool igl::readOBJ<double, int>(
    ...
    std::vector<std::vector<int, std::allocator<int>>,
                std::allocator<std::vector<int, std::allocator<int>>>> &,
    std::vector<std::tuple<std::basic_string<char, std::char_traits<char>,
                                             std::allocator<char>>,
                           int, int>,
                std::allocator<
...
template bool igl::readOBJ<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &);
template bool igl::readOBJ<Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                           Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                           Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &);
template bool igl::readOBJ<Eigen::Matrix<float, -1, 3, 1, -1, 3>,

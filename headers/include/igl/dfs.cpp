...
IGL_INLINE void igl::dfs(const std::vector<std::vector<AType>> &A,
                         const size_t s, Eigen::PlainObjectBase<DerivedD> &D,
...
IGL_INLINE void igl::dfs(const std::vector<std::vector<AType>> &A,
                         ...
                         std::vector<PType> &P, std::vector<CType> &C)
{
    // number of nodes
    int N = s + 1;
    for (const auto &Ai : A)
    ...
    dfs_helper = [&D, &P, &C, &dfs_helper, &seen, &A](const size_t s,
                                                      const size_t p) {
        if (seen[s])
            return;
        seen[s] = true;
...
template void igl::dfs<int, Eigen::Matrix<int, -1, 1, 0, -1, 1>,
                       Eigen::Matrix<int, -1, 1, 0, -1, 1>,
                       Eigen::Matrix<int, -1, 1, 0, -1, 1>>(
    std::vector<std::vector<int, std::allocator<int>>,
                std::allocator<std::vector<int, std::allocator<int>>>> const &,
    const size_t, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 1, 0, -1, 1>> &,

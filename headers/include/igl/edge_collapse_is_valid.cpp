                                            ...
                                            const Eigen::MatrixXi &EI)
{
    ...
    // check if edge collapse is valid: intersection of vertex neighbors of s
    // and d should be exactly 2+(s,d) = 4
    // http://stackoverflow.com/a/27049418/148668
    {
        // all vertex neighbors around edge, including the two vertices of the
        // edge
...
IGL_INLINE bool igl::edge_collapse_is_valid(std::vector<int> &Nsv,
                                            std::vector<int> &Ndv)
{
    ...
    std::vector<int> Nint;
    std::set_intersection(Nsv.begin(), Nsv.end(), Ndv.begin(), Ndv.end(),
                          std::back_inserter(Nint));
    // check if edge collapse is valid: intersection of vertex neighbors of s
    // and d should be exactly 2+(s,d) = 4
    // http://stackoverflow.com/a/27049418/148668
    if (Nint.size() != 2) {

                                           ...
                                           Eigen::PlainObjectBase<DerivedJ> &J)
{
    ...
    simplify = [&I, &P, &stol, &simplify](const int ixs,
                                          const int ixe) -> void {
        assert(ixe > ixs);
        Scalar sdmax = 0;
        ...
        if ((ixe - ixs) > 1) {
            ...
            const auto &Pblock =
                P.block(ixs + 1, 0, ((ixe + 1) - ixs) - 2, P.cols());
            if (sdes <= EPS<Scalar>()) {
                sD = (Pblock.rowwise() - P.row(ixs)).rowwise().squaredNorm();
            } else {
                Eigen::Matrix<Scalar, Eigen::Dynamic, 1> T;
                project_to_line(Pblock, P.row(ixs).eval(), P.row(ixe).eval(), T,
                                sD);
            }
                                           ...
                                           Eigen::PlainObjectBase<DerivedQ> &Q)
{
    ...
    L(0) = 0;
    L.block(1, 0, n - 1, 1) =
        (P.bottomRows(n - 1) - P.topRows(n - 1)).rowwise().norm();
    // Give extra on end
    VectorXS T;
    cumsum(L, 1, T);
...
template void
igl::ramer_douglas_peucker<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
...
// generated by autoexplicit.sh
template void
igl::ramer_douglas_peucker<Eigen::Matrix<double, -1, 2, 0, -1, 2>,

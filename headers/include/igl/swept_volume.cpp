...
IGL_INLINE void igl::swept_volume(
    ...
    const size_t steps, const size_t grid_res, const size_t isolevel_grid,
    Eigen::MatrixXd &SV, Eigen::MatrixXi &SF)
{
    ...
    const auto &Vtransform = [&V, &transform](const size_t vi,
                                              const double t) -> RowVector3d {
    ...
    // compute values
    VectorXd S;
    swept_volume_signed_distance(V, F, transform, steps, GV, res, h, isolevel,
                                 S);
    S.array() -= isolevel;

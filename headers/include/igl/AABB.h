...
template <typename DerivedV, int DIM> class AABB
{
  public:
    ...
    /// Index of single primitive in this node if full leaf, otherwise -1 for
    /// non-leaf
    int m_primitive;
    // Scalar m_low_sqr_d;
    // int m_depth;
    /// @private
    AABB() : m_left(NULL), m_right(NULL), m_box(), m_primitive(-1)
    // m_low_sqr_d(std::numeric_limits<double>::infinity()),
    // m_depth(0)
    {
        static_assert(DerivedV::ColsAtCompileTime == DIM ||
                          DerivedV::ColsAtCompileTime == Eigen::Dynamic,
                      "DerivedV::ColsAtCompileTime == DIM || "
                      "DerivedV::ColsAtCompileTime == Eigen::Dynamic");
    }
    /// @private
    ...
    template <typename DerivedEle, typename Derivedbb_mins,
              typename Derivedbb_maxs, typename Derivedelements>
    IGL_INLINE void init(const Eigen::MatrixBase<DerivedV> &V,
    ...
    /// Ele's
    ///              barycenters would be sorted: SI(e,d) = i --> the dth
    ///              coordinate of the barycenter of the eth element would be
    ///              placed at position i in a sorted list.
    ...
    ///
    /// @param[in]  V  #V by dim list of mesh vertex positions. **Should be same
    /// as used to
    ///     construct mesh.**
    /// @param[in]  Ele  #Ele by dim+1 list of mesh indices into #V. **Should be
    /// same as used to
    ///     construct mesh.**
    ...
    IGL_INLINE std::vector<int> find(const Eigen::MatrixBase<DerivedV> &V,
                                     const Eigen::MatrixBase<DerivedEle> &Ele,
    ...
    ///
    /// @return Number of elements m then total tree size should be 2*h where h
    /// is the deepest depth 2^ceil(log(#Ele*2-1))
    ...
    /// positions
    /// @param[out]  elements  max_tree list of element or (not leaf id) indices
    /// into Ele
    ...
    IGL_INLINE void serialize(Eigen::PlainObjectBase<Derivedbb_mins> &bb_mins,
    ...
    /// @param[in] Ele  #Ele by dim list of simplex indices
    /// @param[in] p  dim-long query point
    /// @param[in] low_sqr_d  lower bound on squared distance, specified maximum
    /// squared
    ///              distance
    /// @param[in] up_sqr_d  current upper bounded on squared distance, current
    /// minimum
    ///              squared distance (only consider distances less than this),
    ///              see output.
    /// @param[out]  i  facet index corresponding to smallest distances
    /// @param[out]  c  closest point
    /// @return squared distance
    ...
    IGL_INLINE Scalar squared_distance(
        const Eigen::MatrixBase<DerivedV> &V,
        const Eigen::MatrixBase<DerivedEle> &Ele, const RowVectorDIMS &p,
        const Scalar low_sqr_d, const Scalar up_sqr_d, int &i,
    ...
    template <typename DerivedEle>
    IGL_INLINE Scalar
    squared_distance(const Eigen::MatrixBase<DerivedV> &V,
                     const Eigen::MatrixBase<DerivedEle> &Ele,
                     const RowVectorDIMS &p, const Scalar up_sqr_d, int &i,
                     Eigen::PlainObjectBase<RowVectorDIMS> &c) const;
    ...
    template <typename DerivedEle>
    IGL_INLINE bool intersect_ray(const Eigen::MatrixBase<DerivedV> &V,
                                  const Eigen::MatrixBase<DerivedEle> &Ele,
    ...
    IGL_INLINE bool intersect_ray(const Eigen::MatrixBase<DerivedV> &V,
                                  const Eigen::MatrixBase<DerivedEle> &Ele,
                                  const RowVectorDIMS &origin,
                                  const RowVectorDIMS &dir, const Scalar min_t,
                                  igl::Hit &hit) const;
  ...
  public:
    ...
    IGL_INLINE void squared_distance(const Eigen::MatrixBase<DerivedV> &V,
                                     const Eigen::MatrixBase<DerivedEle> &Ele,
    ...
    /// @param[in] V  #V by dim list of vertex positions
    /// @param[in] Ele  #Ele by dim list of simplex indices
    /// @param[in] other  AABB hierarchy of another set of primitives (must be
    /// points)
    /// @param[in] other_V  #other_V by dim list of query points
    /// @param[in] other_Ele  #other_Ele by ss list of simplex indices into
    /// other_V
    ...
    template <typename DerivedEle, typename Derivedother_V,
              typename Derivedother_Ele, typename DerivedsqrD,
              typename DerivedI, typename DerivedC>
    IGL_INLINE void
    squared_distance(const Eigen::MatrixBase<DerivedV> &V,
                     const Eigen::MatrixBase<DerivedEle> &Ele,
  ...
  private:
    ...
    template <typename DerivedEle>
    IGL_INLINE void
    leaf_squared_distance(const Eigen::MatrixBase<DerivedV> &V,
                          const Eigen::MatrixBase<DerivedEle> &Ele,
                          const RowVectorDIMS &p, const Scalar low_sqr_d,
                          Scalar &sqr_d, int &i,
                          Eigen::PlainObjectBase<RowVectorDIMS> &c) const;
    // Default low_sqr_d
    template <typename DerivedEle>
    ...
    leaf_squared_distance(const Eigen::MatrixBase<DerivedV> &V,
                          const Eigen::MatrixBase<DerivedEle> &Ele,
                          const RowVectorDIMS &p, Scalar &sqr_d, int &i,
                          Eigen::PlainObjectBase<RowVectorDIMS> &c) const;
    ...
    //   i  possibly updated index into Ele of closest point
    //   c  dim-long possibly updated closest point
    IGL_INLINE void set_min(const RowVectorDIMS &p,
                            const Scalar sqr_d_candidate, const int i_candidate,
                            const RowVectorDIMS &c_candidate, Scalar &sqr_d,
                            int &i,
                            Eigen::PlainObjectBase<RowVectorDIMS> &c) const;
    ...
    /// @return  true if any hits
    template <typename DerivedEle>
    IGL_INLINE bool intersect_ray_opt(const Eigen::MatrixBase<DerivedV> &V,
                                      const Eigen::MatrixBase<DerivedEle> &Ele,
                                      const RowVectorDIMS &origin,
                                      const RowVectorDIMS &dir,
    ...
    IGL_INLINE bool intersect_ray_opt(const Eigen::MatrixBase<DerivedV> &V,
                                      ...
                                      const RowVectorDIMS &inv_dir,
                                      const RowVectorDIMS &inv_dir_pad,
                                      const Scalar min_t, igl::Hit &hit) const;

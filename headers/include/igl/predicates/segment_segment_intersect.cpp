...
IGL_INLINE bool igl::predicates::segment_segment_intersect(
    const Eigen::MatrixBase<DerivedP> &a, const Eigen::MatrixBase<DerivedP> &b,
    const Eigen::MatrixBase<DerivedP> &c, const Eigen::MatrixBase<DerivedP> &d)
{
    ...
    auto on_segment = [](const Eigen::MatrixBase<DerivedP> &m,
                         const Eigen::MatrixBase<DerivedP> &n,
                         const Eigen::MatrixBase<DerivedP> &p) {
        return (
            (p(0) >= std::min(m(0), n(0))) && (p(0) <= std::max(m(0), n(0))) &&
            (p(1) >= std::min(m(1), n(1))) && (p(1) <= std::max(m(1), n(1))));
    };

    // colinear case
    if ((t1 == igl::predicates::Orientation::COLLINEAR &&
         on_segment(a, b, c)) ||
        (t2 == igl::predicates::Orientation::COLLINEAR &&
         on_segment(c, d, b)) ||
        (t3 == igl::predicates::Orientation::COLLINEAR &&
         on_segment(a, b, d)) ||
...
template bool igl::predicates::segment_segment_intersect<

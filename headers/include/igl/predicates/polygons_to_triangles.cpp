    ...
    Eigen::PlainObjectBase<DerivedJ> &J)
{
    ...
    J.resize(F.rows());
    {
        ...
        for (Index p = 0; p < C.size() - 1; p++) {
            ...
            if (np == 3) {
                ...
                case 2:
                    ...
                    break;
                case 3: {
                    Eigen::MatrixXd P = (pV.rowwise() - pV.colwise().mean())
                    ...
                    Eigen::Matrix3d C = es.eigenvectors().real();
                    {
                        Eigen::Vector3d _1;
                        Eigen::Vector3i I;
                        igl::sort(es.eigenvalues().real().eval(), 1, false, _1,
                                  I);
                ...
                default:
                    assert(false && "dim>3 not supported");
                }
                ...
                // compute signed area
                {
                    double area = 0;
                    for (Index c = 0; c < np; c++) {
                        area += S((c + 0) % np, 0) * S((c + 1) % np, 1) -
                                S((c + 1) % np, 0) * S((c + 0) % np, 1);
                    }
                    // prIndexf("area: %g\n",area);
                    if (area < 0) {
                        S.col(0) *= -1;
                ...
                // This is a really low quality triangulator and will contain
                // nearly degenerate elements which become degenerate or worse
                // when unprojected back to 3D. igl::predicates::ear_clipping
                // does not gracefully fail when the input is not simple.
                // Instead it (tends?) to output too few triangles.
        ...
        //        //        c/ | \b            c/f1 \b
                ...
                //        //igl::edge_lengths(pV,pF,pl);
                //        // recompute edge lengths of two faces. (extra work on
                //        untouched
                //        // edges)
                //        for(Index f : {f1,f2})
                //        {
                ...
                //    for(Index c =0;c<3;c++)
                //    {
                //      assert(pF(f,c) != pF(f,(c+1)%3) && "self loops should
                //      not exist");
                //    }
                //  }
                //}

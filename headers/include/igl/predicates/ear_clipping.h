...
/// @param[in] RT: n*1, preserved vertices (do not clip) marked as 1, otherwise
/// 0
/// @param[out] eF: clipped ears, in original index of P
/// @param[out] I : size #nP vector, maps index from nP to P, e.g. nP's ith
/// vertex is origianlly I(i) in P
///
...
template <typename DerivedP, typename DerivedRT, typename DerivedF,
          typename DerivedI>
IGL_INLINE void ear_clipping(const Eigen::MatrixBase<DerivedP> &P,
...
IGL_INLINE bool ear_clipping(const Eigen::MatrixBase<DerivedP> &P,
                             Eigen::PlainObjectBase<DerivedF> &eF);

...
#define IGL_PREDICATES_ASSERT_SCALAR(Vector)                                   \
    static_assert(std::is_same<typename Vector::Scalar, float>::value,         \
                  "Shewchuk's exact predicates only support float")
#else
#define IGL_PREDICATES_ASSERT_SCALAR(Vector)                                   \
    static_assert(std::is_same<typename Vector::Scalar, double>::value ||      \
...
IGL_INLINE Orientation orient2d(const Eigen::MatrixBase<Vector2D> &pa,
...
IGL_INLINE Orientation orient3d(const Eigen::MatrixBase<Vector3D> &pa,
...
template <typename Vector2D>
IGL_INLINE Orientation incircle(const Eigen::MatrixBase<Vector2D> &pa,
                                const Eigen::MatrixBase<Vector2D> &pb,
                                const Eigen::MatrixBase<Vector2D> &pc,
...
template <typename Vector3D>
IGL_INLINE Orientation insphere(const Eigen::MatrixBase<Vector3D> &pa,
                                const Eigen::MatrixBase<Vector3D> &pb,
                                const Eigen::MatrixBase<Vector3D> &pc,
                                const Eigen::MatrixBase<Vector3D> &pd,
...
#define IGL_ORIENT2D(Vector)                                                   \
    template igl::predicates::Orientation igl::predicates::orient2d<Vector>(   \
        const Eigen::MatrixBase<Vector> &, const Eigen::MatrixBase<Vector> &,  \
        const Eigen::MatrixBase<Vector> &)
#define IGL_INCIRCLE(Vector)                                                   \
    template igl::predicates::Orientation igl::predicates::incircle<Vector>(   \
        const Eigen::MatrixBase<Vector> &, const Eigen::MatrixBase<Vector> &,  \
        const Eigen::MatrixBase<Vector> &, const Eigen::MatrixBase<Vector> &)
#define IGL_ORIENT3D(Vector)                                                   \
    template igl::predicates::Orientation igl::predicates::orient3d<Vector>(   \
        const Eigen::MatrixBase<Vector> &, const Eigen::MatrixBase<Vector> &,  \
        const Eigen::MatrixBase<Vector> &, const Eigen::MatrixBase<Vector> &)
#define IGL_INSPHERE(Vector)                                                   \
    template igl::predicates::Orientation igl::predicates::insphere<Vector>(   \
        const Eigen::MatrixBase<Vector> &, const Eigen::MatrixBase<Vector> &,  \
        const Eigen::MatrixBase<Vector> &, const Eigen::MatrixBase<Vector> &,  \
        const Eigen::MatrixBase<Vector> &)

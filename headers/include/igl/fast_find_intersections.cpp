...
template <typename DerivedV1, typename DerivedF1, typename DerivedV2,
          typename DerivedF2, typename DerivedI, typename DerivedE>
IGL_INLINE void
igl::fast_find_intersections(const Eigen::MatrixBase<DerivedV1> &V1,
...
template <typename DerivedV1, typename DerivedF1, typename DerivedV2,
          typename DerivedF2, typename DerivedI, typename DerivedE>
IGL_INLINE void
igl::fast_find_intersections(const AABB<DerivedV1, 3> &tree,
                             ...
                             Eigen::PlainObjectBase<DerivedE> &edges)
{
    ...
    using VERTEX =
        Eigen::Matrix<typename DerivedE::Scalar, 1, 3, Eigen::RowMajor>;
    ...
    for (int i = 0; i < F2.rows(); ++i) {
        ...
        std::function<void(const AABBTree &, int)> check_intersect =
            ...
            if (t.m_primitive !=
                -1) // check for the actual intersection //t.is_leaf()
            {
                bool coplanar = false;
                VERTEX edge1, edge2;
                ...
                if (igl::tri_tri_intersection_test_3d(
                        ...
                        V1.row(F1(t.m_primitive, 0)),
                        V1.row(F1(t.m_primitive, 1)),
                        V1.row(F1(t.m_primitive, 2)), coplanar, edge1, edge2)) {
                    if (!coplanar) {
                ...
                if (t.m_box.intersects(tri_box)) { // need to check all branches
...
template void
igl::fast_find_intersections<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

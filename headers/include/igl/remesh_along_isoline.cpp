...
template <typename DerivedV, typename DerivedF, typename DerivedS,
          ...
          typename BCtype, typename DerivedSU, typename DerivedL>
IGL_INLINE void igl::remesh_along_isoline(
    const Eigen::MatrixBase<DerivedV> &V, const Eigen::MatrixBase<DerivedF> &F,
    const Eigen::MatrixBase<DerivedS> &S, const typename DerivedS::Scalar val,
    Eigen::PlainObjectBase<DerivedU> &U, Eigen::PlainObjectBase<DerivedG> &G,
    Eigen::PlainObjectBase<DerivedSU> &SU, Eigen::PlainObjectBase<DerivedJ> &J,
    Eigen::SparseMatrix<BCtype> &BC, Eigen::PlainObjectBase<DerivedL> &L)
{
    igl::remesh_along_isoline(V.rows(), F, S, val, G, SU, J, BC, L);
    U = BC * V;
}

template <typename DerivedF, typename DerivedS, typename DerivedG,
          typename DerivedJ, typename BCtype, typename DerivedSU,
          typename DerivedL>
IGL_INLINE void igl::remesh_along_isoline(
    const int num_vertices, const Eigen::MatrixBase<DerivedF> &F,
    const Eigen::MatrixBase<DerivedS> &S, const typename DerivedS::Scalar val,
    Eigen::PlainObjectBase<DerivedG> &G, Eigen::PlainObjectBase<DerivedSU> &SU,
    ...
    Eigen::PlainObjectBase<DerivedL> &L)
{
    ...
    for (int f = 0; f < F.rows(); f++) {
        ...
        case 0: {
            // Easy case
            std::vector<typename DerivedG::Scalar> row = {F(f, 0), F(f, 1),
                                                          F(f, 2)};
            vG.push_back(row);
            vJ.push_back(f);
        ...
        case 2: {
            ...
            for (int i = 0; i < 2; i++) {
                if ((edgeToBirthVert.find(F(f, P[i])) ==
                     edgeToBirthVert.end()) ||
                    (edgeToBirthVert.at(F(f, P[i]))
                         .find(F(f, (P[i] + 1) % 3)) ==
                     edgeToBirthVert.at(F(f, P[i])).end())) {
                    const double bci =
                        (isoval - S(F(f, (P[i] + 1) % 3))) /
        ...
        // v1--v12---v2
            typedef std::vector<typename DerivedG::Scalar> Row;
            {
                ...
                vG.push_back(row);
                vJ.push_back(f);
                vL.push_back(Psign[0] ? isoval_i : isoval_i + 1);
            }
            {
                Row row = {v12, v2, v01};
                vG.push_back(row);
                vJ.push_back(f);
                vL.push_back(Psign[1] ? isoval_i : isoval_i + 1);
            }
            {
                Row row = {v2, v0, v01};
                vG.push_back(row);
...
template void
igl::remesh_along_isoline<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

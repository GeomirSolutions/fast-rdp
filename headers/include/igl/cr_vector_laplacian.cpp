...
IGL_INLINE void igl::cr_vector_laplacian(const Eigen::MatrixBase<DerivedV> &V,
...
template <typename DerivedF, typename DerivedL_sq, typename DerivedE,
          typename DerivedOE, typename ScalarL>
IGL_INLINE void
igl::cr_vector_laplacian_intrinsic(const Eigen::MatrixBase<DerivedF> &F,
                                   ...
                                   Eigen::SparseMatrix<ScalarL> &L)
{
    ...
    assert(E.rows() == F.rows() && E.cols() == F.cols() &&
           oE.rows() == F.rows() && oE.cols() == F.cols() &&
           "Wrong dimension in edge vectors");
    assert(l_sq.rows() == F.rows() && l_sq.cols() == 3 &&
           "l_sq dimensions wrong");
    ...
    for (Eigen::Index f = 0; f < m; ++f) {
        for (int e = 0; e < 3; ++e) {
            ...
            tripletList.emplace_back(E(f, e) + nE, E(f, e) + nE,
                                     2. / dA(f) * eij);

            const ScalarL Dijki =
                o * pow(eij - ejk + eki, 2) / (2. * lens * dA(f));
...
template void
igl::cr_vector_laplacian<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
...
template void
igl::cr_vector_laplacian_intrinsic<Eigen::Matrix<int, -1, -1, 0, -1, -1>,

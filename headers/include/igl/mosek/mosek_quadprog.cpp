    ...
    std::vector<Scalar> &x)
{
    ...
    for (int j = 0; j < n; j++) {
        ...
        if (m > 0) {
            // Input column j of A
#if MSK_VERSION_MAJOR >= 7
            mosek_guarded(MSK_putacol(task, j, Acp[j + 1] - Acp[j],
                                      &Ari[Acp[j]], &Av[Acp[j]]));
#elif MSK_VERSION_MAJOR == 6
...
#if MSK_VERSION_MAJOR <= 8
        mosek_guarded(
            MSK_putbound(task, MSK_ACC_CON, i, MSK_BK_RA, lc[i], uc[i]));
#else
    ...
    for (std::map<MSKdparame, double>::iterator pit =
             mosek_data.douparam.begin();
                           ...
                           MosekData &mosek_data, Eigen::VectorXd &x)
{
    ...
    vector<Scalar> vx;
    bool ret = mosek_quadprog<Index, Scalar>(Q.rows(), vQi, vQj, vQv, vc, cf,
                                             nr, vAv, vAr, vAc, vlc, vuc, vlx,
                                             vux, mosek_data, vx);
    list_to_matrix(vx, x);
    return ret;
}

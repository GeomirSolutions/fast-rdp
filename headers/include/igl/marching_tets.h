...
/// isovalue.
///
/// @param[in] TV  #tet_vertices x 3 array -- The vertices of the tetrahedral
/// mesh
/// @param[in] TT  #tets x 4 array --  The indexes of each tet in the
/// tetrahedral mesh
/// @param[in] S  #tet_vertices x 1 array -- The values defined on each tet
/// vertex
/// @param[in] isovalue  scalar -- The isovalue of the level set we want to
/// compute
/// @param[out] SV  #SV x 3 array -- The vertices of the output level surface
/// mesh
/// @param[out] SF  #SF x 3 array -- The face indexes of the output level
/// surface mesh
/// @param[out] J   #SF list of indices into TT revealing which tet each face
/// comes from
...
template <typename DerivedTV, typename DerivedTT, typename DerivedS,
          ...
          typename BCType>
IGL_INLINE void marching_tets(const Eigen::MatrixBase<DerivedTV> &TV,
...
/// \overload
template <typename DerivedTV, typename DerivedTT, typename DerivedS,
          typename DerivedSV, typename DerivedSF, typename BCType>
IGL_INLINE void marching_tets(const Eigen::MatrixBase<DerivedTV> &TV,
                              const Eigen::MatrixBase<DerivedTT> &TT,
                              const Eigen::MatrixBase<DerivedS> &S,
                              const typename DerivedS::Scalar isovalue,
...
/// \overload
template <typename DerivedTV, typename DerivedTT, typename DerivedS,
          typename DerivedSV, typename DerivedSF>
IGL_INLINE void marching_tets(const Eigen::MatrixBase<DerivedTV> &TV,
                              const Eigen::MatrixBase<DerivedTT> &TT,
                              const Eigen::MatrixBase<DerivedS> &S,
                              const typename DerivedS::Scalar isovalue,

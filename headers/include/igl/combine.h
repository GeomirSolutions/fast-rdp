...
/// @param[in] FF  k-long list of lists of mesh face indices so that FF[i]
/// indexes
///     VV[i]
/// @param[out] V   VV[0].rows()+...+VV[k-1].rows() by VV[0].cols() list of mesh
///     vertex positions
/// @param[out] F   FF[0].rows()+...+FF[k-1].rows() by FF[0].cols() list of mesh
/// faces
///     indices into V
/// @param[out] Vsizes  k list so that Vsizes(i) is the #vertices in the ith
/// input
...
/// \endcode
///
template <typename DerivedVV, typename DerivedFF, typename DerivedV,
          typename DerivedF, typename DerivedVsizes, typename DerivedFsizes>
IGL_INLINE void combine(const std::vector<DerivedVV> &VV,

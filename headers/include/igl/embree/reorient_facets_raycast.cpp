    ...
    Eigen::PlainObjectBase<DerivedI> &I, Eigen::PlainObjectBase<DerivedC> &C)
{
    ...
    for (int c = 0; c < num_cc; ++c) {
        ...
        discrete_distribution<int> ddist(
            CF.size(), 0, CF.size(), [&](double i) {
                return CF_area[static_cast<int>(i)];
            }); // simple ctor of (Iter, Iter) not provided by the stupid
                // VC11/12
        for (int i = 0; i < num_rays_per_component[c]; ++i) {
            int f = CF[ddist(prng)]; // select face with probability
                                     // proportional to face area
            float s = rdist(prng); // random barycentric coordinate (reference:
                                   // Generating Random Points in Triangles
                                   // [Turk, Graphics Gems I 1990])
            float t = rdist(prng);
    ...
    for (int f = 0; f < m; ++f) {
        ...
        if (use_parity) {
            I(f) = C_vote_parity[c].first > C_vote_parity[c].second
                       ? 1
                       : 0; // Ideally, parity for the front/back side should be
                            // 1/0 (i.e., parity sum for all rays should be
                            // smaller on the front side)

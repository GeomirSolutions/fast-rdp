...
IGL_INLINE igl::embree::EmbreeRenderer::EmbreeRenderer()
    : scene(NULL), geomID(0), initialized(false),
...
IGL_INLINE igl::embree::EmbreeRenderer::EmbreeRenderer(const EmbreeRenderer &)
    : // To make -Weffc++ happy
      scene(NULL), geomID(0), initialized(false)
{
...
IGL_INLINE igl::embree::EmbreeRenderer &
igl::embree::EmbreeRenderer::operator=(const EmbreeRenderer &)
{
    assert(false && "Embree: Assigning an EmbreeRenderer is not allowed");
    return *this;
}

IGL_INLINE void igl::embree::EmbreeRenderer::init(const PointMatrixType &V,
...
IGL_INLINE void
igl::embree::EmbreeRenderer::init(const std::vector<const PointMatrixType *> &V,
                                  const std::vector<const FaceMatrixType *> &F,
                                  const std::vector<int> &masks, bool isStatic)
{
    if (initialized)
        deinit();
    ...
    RTCBuildQuality buildQuality =
        isStatic ? RTC_BUILD_QUALITY_HIGH : RTC_BUILD_QUALITY_MEDIUM;
    ...
    for (int g = 0; g < (int)V.size(); g++) {
        ...
        Eigen::Map<Eigen::Matrix<float, -1, 4, Eigen::RowMajor>> vertices(
            (float *)rtcSetNewGeometryBuffer(geom_0, RTC_BUFFER_TYPE_VERTEX, 0,
                                             RTC_FORMAT_FLOAT3,
                                             4 * sizeof(float), V[g]->rows()),
            V[g]->rows(), 4);
        vertices.block(0, 0, V[g]->rows(), 3) = V[g]->cast<float>();

        // fill triangle buffer
        Eigen::Map<Eigen::Matrix<unsigned int, -1, 3, Eigen::RowMajor>>
            triangles((unsigned int *)rtcSetNewGeometryBuffer(
                          geom_0, RTC_BUFFER_TYPE_INDEX, 0, RTC_FORMAT_UINT3,
                          3 * sizeof(unsigned int), F[g]->rows()),
                      F[g]->rows(), 3);
    ...
    if (rtcGetDeviceError(device) != RTC_ERROR_NONE)
        std::cerr << "Embree: An error occurred while initializing the "
                     "provided geometry!"
                  << endl;
#ifdef IGL_VERBOSE
    else
...
IGL_INLINE void igl::embree::EmbreeRenderer::deinit()
{
    if (scene) {
        ...
        if (rtcGetDeviceError(device) != RTC_ERROR_NONE) {
            std::cerr << "Embree: An error occurred while resetting!"
                      << std::endl;
        }
#ifdef IGL_VERBOSE
        else {
    ...
    Hit &hit, float tnear, float tfar, int mask) const
{
    ...
    // shot ray
    {
        ...
        ray.hit.Ng_x =
            -ray.hit.Ng_x; // EMBREE_FIXME: only correct for triangles,quads,
...
IGL_INLINE void igl::embree::EmbreeRenderer ::create_ray(
    RTCRayHit &ray, const Eigen::RowVector3f &origin,
...
IGL_INLINE void
igl::embree::EmbreeRenderer ::set_mesh(const Eigen::MatrixBase<DerivedV> &MV,
...
IGL_INLINE void igl::embree::EmbreeRenderer ::render_buffer(PixelMatrixType &R,
                                                            PixelMatrixType &G,
                                                            PixelMatrixType &B,
                                                            PixelMatrixType &A)
{
    ...
    if (orthographic) {
        ...
        igl::ortho(-h * width / height, h * width / height, -h, h, camera_dnear,
                   camera_dfar, proj);
    } else {
    ...
    for (int x = 0; x < (int)width; ++x) {
        for (int y = 0; y < (int)height; ++y) {
            ...
            auto clamp = [](float x) -> unsigned char {
                return (unsigned char)(x < 0 ? 0 : x > 1.0 ? 255 : x * 255);
            };
            ...
            if (this->intersect_ray(s, dir, hit)) {
                if (double_sided || dir.dot(hit.N) > 0.0f) {
                    ...
                    if (this->uniform_color) {
                        ...
                        // flat color per face
                        c = this->C.row(hit.id);
                    } else { // use barycentric coordinates to interpolate
                             // colour
...
IGL_INLINE void
igl::embree::EmbreeRenderer ::set_colors(const Eigen::MatrixBase<DerivedC> &C)
{
...
IGL_INLINE void
igl::embree::EmbreeRenderer ::set_data(const Eigen::MatrixBase<DerivedD> &D,
...
template <typename T>
IGL_INLINE void igl::embree::EmbreeRenderer::set_zoom(T zoom)
{
    this->camera_zoom = zoom;
}

template <typename Derivedtr>
IGL_INLINE void igl::embree::EmbreeRenderer::set_translation(
    const Eigen::MatrixBase<Derivedtr> &tr)
{
    this->camera_translation = tr.template cast<float>();
}

IGL_INLINE void igl::embree::EmbreeRenderer::set_face_based(bool _f)
{
    this->face_based = _f;
}

IGL_INLINE void igl::embree::EmbreeRenderer::set_orthographic(bool o)
{
    this->orthographic = o;
}

IGL_INLINE void igl::embree::EmbreeRenderer::set_double_sided(bool d)
{
    this->double_sided = d;
}
...
template void
igl::embree::EmbreeRenderer::set_rot<Eigen::Matrix<double, 3, 3, 0, 3, 3>>(
    Eigen::MatrixBase<Eigen::Matrix<double, 3, 3, 0, 3, 3>> const &);
template void
igl::embree::EmbreeRenderer::set_data<Eigen::Matrix<double, -1, 1, 0, -1, 1>>(
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> const &,
    igl::ColorMapType);
template void
igl::embree::EmbreeRenderer::set_mesh<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

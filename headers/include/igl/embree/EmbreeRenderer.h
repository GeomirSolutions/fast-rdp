...
class EmbreeRenderer
{
  ...
  public:
    ...
    void set_mesh(const Eigen::MatrixBase<DerivedV> &V,
                  const Eigen::MatrixBase<DerivedF> &F, bool is_static = true);
    ...
    void set_data(const Eigen::MatrixBase<DerivedD> &D, T caxis_min,
                  T caxis_max,
                  igl::ColorMapType cmap = igl::COLOR_MAP_TYPE_VIRIDIS);
    ...
    template <typename T> void set_zoom(T z);
    ...
    //     area outside of the visible object will have zero alpha component
    //     (transparant)
    //   R - red channel
    //   G - green channel
    //   B - blue channel
    //   A - alpha channel
    void render_buffer(PixelMatrixType &R, PixelMatrixType &G,
                       PixelMatrixType &B, PixelMatrixType &A);
    ...
    bool intersect_ray(const Eigen::RowVector3f &origin,
  ...
  private:
    ...
    void create_ray(RTCRayHit &ray, const Eigen::RowVector3f &origin,

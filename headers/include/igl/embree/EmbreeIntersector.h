...
class EmbreeIntersector
{
  ...
  public:
    ...
    void init(const PointMatrixType &V, const FaceMatrixType &F,
              bool isStatic = false);

    /// Initialize with a given mesh.
    ///
    /// @param[in] V  vector of #V by 3 list of vertex positions for each
    /// geometry
    /// @param[in] F  vector of #F by 3 list of Oriented triangles for each
    /// geometry
    ...
    /// ##### Side effects:
    ///   The first time this is ever called the embree engine is initialized.
    void init(const std::vector<const PointMatrixType *> &V,
              const std::vector<const FaceMatrixType *> &F,
              const std::vector<int> &masks, bool isStatic = false);
    ...
    /// @param[in] direction  3d (not necessarily normalized) direction vector
    /// of ray
    ...
    bool intersectRay(const Eigen::RowVector3f &origin,
    ...
    /// Given a ray find the first hit
    /// This is a conservative hit test where multiple rays within a small
    /// radius will be tested and only the closesest hit is returned.
    ///
    /// @param[in] origin     3d origin point of ray
    /// @param[in] direction  3d (not necessarily normalized) direction vector
    /// of ray
    ...
    /// @param[in] tfar       end of ray segment
    /// @param[in] masks      a 32 bit mask to identify active geometries.
    /// @param[in] geoId      id of geometry mask (default
    /// std::numeric_limits<float>::infinity() if no: no masking)
    /// @param[in] closestHit true for gets closest hit, false for furthest hit
    /// @param[out] hit        information about hit
    /// @return true if and only if there was a hit
    bool intersectBeam(const Eigen::RowVector3f &origin,
                       const Eigen::RowVector3f &direction, Hit &hit,
                       float tnear = 0,
                       float tfar = std::numeric_limits<float>::infinity(),
                       int mask = 0xFFFFFFFF, int geoId = -1,
                       bool closestHit = true, unsigned int samples = 4) const;
    ...
    bool intersectRay(const Eigen::RowVector3f &origin,
                      ...
                      std::vector<Hit> &hits, int &num_rays, float tnear = 0,
                      float tfar = std::numeric_limits<float>::infinity(),
                      int mask = 0xFFFFFFFF) const;
    ...
    /// @param[out] hit  information about hit
    /// @return true if and only if there was a hit
    bool intersectSegment(const Eigen::RowVector3f &a,
  ...
  private:
    ...
    void createRay(RTCRayHit &ray, const Eigen::RowVector3f &origin,

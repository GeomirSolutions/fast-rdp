...
IGL_INLINE igl::embree::EmbreeIntersector::EmbreeIntersector()
    : // scene(NULL),
      geomID(0), vertices(NULL), triangles(NULL), initialized(false),
      device(igl::embree::EmbreeDevice::get_device())
{
}

IGL_INLINE
igl::embree::EmbreeIntersector::EmbreeIntersector(const EmbreeIntersector &)
    : // To make -Weffc++ happy
      // scene(NULL),
      geomID(0), vertices(NULL), triangles(NULL), initialized(false)
{
...
IGL_INLINE void igl::embree::EmbreeIntersector::init(const PointMatrixType &V,
...
IGL_INLINE void igl::embree::EmbreeIntersector::init(
    ...
    const std::vector<const FaceMatrixType *> &F, const std::vector<int> &masks,
    bool isStatic)
{
    ...
    for (int g = 0; g < (int)V.size(); g++) {
        ...
        // fill vertex buffer
        vertices = (Vertex *)rtcSetNewGeometryBuffer(
        ...
        triangles = (Triangle *)rtcSetNewGeometryBuffer(
            geom_0, RTC_BUFFER_TYPE_INDEX, 0, RTC_FORMAT_UINT3, 3 * sizeof(int),
            F[g]->rows());
    ...
    unsigned int samples) const
{
    ...
    if (direction.cross(up).norm() < eps)
        up = Eigen::RowVector3f(1, 0, 0);
    ...
    int mask) const
{
    ...
    int last_id0 = -1;
    double self_hits = 0;
    // This epsilon is directly correleated to the number of missed hits,
    // smaller means more accurate and slower
    ...
    while (true) {
        ...
        num_rays++;
        {
            ...
            rtcIntersect1(scene, &context, &ray);
            ray.hit.Ng_x =
                -ray.hit.Ng_x; // EMBREE_FIXME: only correct for
...
#endif
                // Instead of moving origin, just change min_t. That way
                // calculations all use exactly same origin values
                min_t = ray.ray.tfar;
        ...
        if (hits.size() > 1000 && !large_hits_warned) {
            ...
            std::cout << "[ ";
            for (vector<Hit>::iterator hit = hits.begin(); hit != hits.end();
                 hit++) {
                std::cout << (hit->id + 1) << " ";
            }
...
IGL_INLINE void igl::embree::EmbreeIntersector ::createRay(

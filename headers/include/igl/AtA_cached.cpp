...
{
    // 1 Compute At (this could be avoided, but performance-wise it will not
    // make a difference)
    ...
    for (unsigned k = 0; k < A.outerSize(); ++k) {
        ...
        unsigned next_outer_index = (k + 1 == A.outerSize())
                                        ? A.nonZeros()
                                        : *(A.outerIndexPtr() + k + 1);
    ...
    for (unsigned k = 0; k < AtA.outerSize(); ++k) {
        ...
        unsigned next_outer_index = (k + 1 == AtA.outerSize())
                                        ? AtA.nonZeros()
                                        : *(AtA.outerIndexPtr() + k + 1);
        ...
        for (unsigned l = outer_index; l < next_outer_index; ++l) {
            ...
            while (i < Col_RowPtr[row].size() && j < Col_RowPtr[col].size()) {
                if (Col_RowPtr[row][i] == Col_RowPtr[col][j]) {
                    ...
                    ++i;
                    ++j;
                } else if (Col_RowPtr[row][i] > Col_RowPtr[col][j])
                    ++j;
                else
                    ++i;
            }
    ...
    data.I_outer.push_back(
        data.I_row.size()); // makes it more efficient to iterate later on
    ...
    for (unsigned i = 0; i < data.I_outer.size() - 1; ++i) {
        *(AtA.valuePtr() + i) = 0;
        for (unsigned j = data.I_outer[i]; j < data.I_outer[i + 1]; ++j)
            *(AtA.valuePtr() + i) += *(A.valuePtr() + data.I_row[j]) *
                                     data.W[data.I_w[j]] *
                                     *(A.valuePtr() + data.I_col[j]);
    }
}

#ifdef IGL_STATIC_LIBRARY
template void
igl::AtA_cached<double>(Eigen::SparseMatrix<double, 0, int> const &,
                        igl::AtA_cached_data const &,
                        Eigen::SparseMatrix<double, 0, int> &);
template void
igl::AtA_cached_precompute<double>(Eigen::SparseMatrix<double, 0, int> const &,
                                   igl::AtA_cached_data &,
                                   Eigen::SparseMatrix<double, 0, int> &);
#endif

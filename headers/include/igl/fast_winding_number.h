...
/// @param[in] point_indices  a vector of vectors, where the ith entry is a
/// vector of
///                  the indices into P that are the ith octree cell's points
/// @param[in] CH             #OctreeCells by 8, where the ith row is the
/// indices of
///                  the ith octree cell's children
/// @param[in] expansion_order    the order of the taylor expansion. We support
/// 0,1,2.
/// @param[out] CM  #OctreeCells by 3 list of each cell's center of mass
/// @param[out] R   #OctreeCells by 1 list of each cell's maximum distance of
/// any point
///       to the center of mass
/// @param[out] EC  #OctreeCells by #TaylorCoefficients list of expansion
/// coefficients.
...
/// @param[in] R   #OctreeCells by 1 list of each cell's maximum distance of any
/// point
///       to the center of mass
/// @param[in] EC  #OctreeCells by #TaylorCoefficients list of expansion
/// coefficients.
///        (Note that #TaylorCoefficients = ∑_{i=1}^{expansion_order} 3^i)
/// @param[in] Q  #Q by 3 list of query points for the winding number
/// @param[in] beta  This is a Barnes-Hut style accuracy term that separates
/// near feild
...
IGL_INLINE void
fast_winding_number(const Eigen::MatrixBase<DerivedP> &P,
...
template <typename DerivedV, typename DerivedF, typename DerivedQ,
          typename DerivedW>
IGL_INLINE void fast_winding_number(const Eigen::MatrixBase<DerivedV> &V,
...
IGL_INLINE void fast_winding_number(const FastWindingNumberBVH &fwn_bvh,
...
fast_winding_number(const FastWindingNumberBVH &fwn_bvh,
                    const float accuracy_scale,

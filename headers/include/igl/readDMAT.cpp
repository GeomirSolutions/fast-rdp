                              ...
                              Eigen::PlainObjectBase<DerivedW> &W)
{
    ...
    if (head_success != 0) {
        if (head_success == 1) {
            fprintf(stderr, "IOError: readDMAT() first row should be [num "
                            "cols] [num rows]...\n");
        }
        fclose(fp);
        return false;
...
template bool igl::readDMAT<Eigen::Matrix<double, -1, 3, 1, -1, 3>>(
...
template bool igl::readDMAT<Eigen::Matrix<float, 1, 3, 1, 1, 3>>(
    std::string, Eigen::PlainObjectBase<Eigen::Matrix<float, 1, 3, 1, 1, 3>> &);
template bool igl::readDMAT<Eigen::Matrix<double, 1, 1, 0, 1, 1>>(
    std::string,
    Eigen::PlainObjectBase<Eigen::Matrix<double, 1, 1, 0, 1, 1>> &);
#endif

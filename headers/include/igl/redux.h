...
/// @param[in] func  function handle with the prototype `X(Y a, I i, J j, Z b)`
/// where a
...
inline void redux(const Eigen::SparseMatrix<AType> &A, const int dim,
                  const Func &func, Eigen::PlainObjectBase<DerivedB> &B);
} // namespace igl
...
template <typename AType, typename Func, typename DerivedB>
inline void igl::redux(const Eigen::SparseMatrix<AType> &A, const int dim,
                       const Func &func, Eigen::PlainObjectBase<DerivedB> &B)
{
    ...
    const auto func_wrap = [&func, &B, &dim](const Index i, const Index j,
                                             const AType v) {
        if (dim == 1) {

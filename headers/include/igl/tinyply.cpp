...
namespace tinyply
{
template <typename T, typename T2> T2 endian_swap(const T & /*v*/) noexcept
{
    assert(false);
} //{ return v; }

template <>
uint16_t IGL_INLINE endian_swap<uint16_t, uint16_t>(const uint16_t &v) noexcept
{
    return (v << 8) | (v >> 8);
}
template <>
uint32_t IGL_INLINE endian_swap<uint32_t, uint32_t>(const uint32_t &v) noexcept
{
    return (v << 24) | ((v << 8) & 0x00ff0000) | ((v >> 8) & 0x0000ff00) |
           (v >> 24);
}
template <>
uint64_t IGL_INLINE endian_swap<uint64_t, uint64_t>(const uint64_t &v) noexcept
{
    return (
...
template <>
int16_t IGL_INLINE endian_swap<int16_t, int16_t>(const int16_t &v) noexcept
{
    uint16_t r = endian_swap<uint16_t, uint16_t>(*(uint16_t *)&v);
    return *(int16_t *)&r;
}
template <>
int32_t IGL_INLINE endian_swap<int32_t, int32_t>(const int32_t &v) noexcept
{
    uint32_t r = endian_swap<uint32_t, uint32_t>(*(uint32_t *)&v);
    return *(int32_t *)&r;
}
template <>
int64_t IGL_INLINE endian_swap<int64_t, int64_t>(const int64_t &v) noexcept
{
    uint64_t r = endian_swap<uint64_t, uint64_t>(*(uint64_t *)&v);
    return *(int64_t *)&r;
}
template <>
float IGL_INLINE endian_swap<uint32_t, float>(const uint32_t &v) noexcept
{
    union
    {
        float f;
...
template <>
double IGL_INLINE endian_swap<uint64_t, double>(const uint64_t &v) noexcept
{
    union
    {
        double d;
...
struct PlyFile::PlyFileImpl
{
    ...
    void add_properties_to_element(const std::string &elementKey,
                                   ...
                                   uint8_t *data, const Type listType,
    ...
    void write_property_ascii(Type t, std::ostream &os, uint8_t *src,
                              size_t &srcOffset);
    void write_property_binary(std::ostream &os, uint8_t *src,
                               size_t &srcOffset,
...
IGL_INLINE PlyElement::PlyElement(std::istream &is) { is >> name >> size; }
...
IGL_INLINE void endian_swap_buffer(uint8_t *data_ptr, const size_t num_bytes,
                                   const size_t stride)
{
...
IGL_INLINE int64_t find_element(const std::string &key,
...
IGL_INLINE int64_t find_property(const std::string &key,
...
// is the property.
IGL_INLINE std::vector<std::vector<PlyFile::PlyFileImpl::PropertyLookup>>
...
IGL_INLINE void PlyFile::PlyFileImpl::read_header_text(
    std::string line, std::vector<std::string> &place, int erase)
{
...
IGL_INLINE size_t PlyFile::PlyFileImpl::read_property_binary(
    const size_t &stride, void *dest, size_t &destOffset,
...
IGL_INLINE size_t PlyFile::PlyFileImpl::read_property_ascii(
    const Type &t, const size_t &stride, void *dest, size_t &destOffset,
    std::istream &is)
{
    destOffset += stride;
    switch (t) {
...
IGL_INLINE void PlyFile::PlyFileImpl::write_property_ascii(Type t,
                                                           std::ostream &os,
                                                           uint8_t *src,
                                                           size_t &srcOffset)
{
...
IGL_INLINE void
PlyFile::PlyFileImpl::write_property_binary(std::ostream &os, uint8_t *src,
                                            size_t &srcOffset,
                                            const size_t &stride) noexcept
{
...
IGL_INLINE void PlyFile::PlyFileImpl::read(std::istream &is)
{
    ...
    if (isBigEndian) {
        for (auto &b : buffers) {
            ...
            switch (b->t) {
            case Type::INT16:
                endian_swap_buffer<int16_t, int16_t>(data_ptr,
                                                     buffer_size_bytes, stride);
                break;
            case Type::UINT16:
                ...
                break;
            case Type::INT32:
                endian_swap_buffer<int32_t, int32_t>(data_ptr,
                                                     buffer_size_bytes, stride);
                break;
            case Type::UINT32:
            ...
            case Type::FLOAT32:
                ...
                break;
            case Type::FLOAT64:
                endian_swap_buffer<uint64_t, double>(data_ptr,
                                                     buffer_size_bytes, stride);
                break;
            default:
                break;
        ...
        for (size_t i = 0; i < e.size; ++i) {
            ...
            for (auto &p : e.properties) {
                ...
                if (p.isList) {
                    ...
                    write_property_binary(os, listSize, dummyCount,
                                          f.list_stride);
                    write_property_binary(os,
...
IGL_INLINE std::shared_ptr<PlyData>
    ...
    const uint32_t list_size_hint)
{
    ...
    if (elementIndex >= 0) {
        ...
        // check the number of unique PlyData shared pointers and allocate a
        // single buffer that will be used by each property key group. That way,
        // properties like, {"x", "y", "z"} will all be put into the same
        // buffer.
...
IGL_INLINE void PlyFile::PlyFileImpl::add_properties_to_element(
    const std::string &elementKey, const std::vector<std::string> propertyKeys,
...
IGL_INLINE void PlyFile::PlyFileImpl::parse_data(std::istream &is,
                                                 bool firstPass)
{
            ...
            for (auto &property : element.properties) {
                ...
                if (!lookup.skip) {
                    ...
                    if (firstPass) {
                        ...
                        // variable length lists. We add it here so our header
                        // data structure contains enough info to write it back
                        // out again (e.g. transcoding).
...
IGL_INLINE bool PlyFile::parse_header(std::istream &is)
{
    return impl->parse_header(is);
}
IGL_INLINE void PlyFile::read(std::istream &is) { return impl->read(is); }
IGL_INLINE void PlyFile::write(std::ostream &os, bool isBinary)
{
    return impl->write(os, isBinary);
}
IGL_INLINE std::vector<PlyElement> PlyFile::get_elements() const
{
    return impl->elements;
}
IGL_INLINE std::vector<std::string> &PlyFile::get_comments()
{
    return impl->comments;
}
IGL_INLINE std::vector<std::string> PlyFile::get_info() const
{
    return impl->objInfo;
}
IGL_INLINE bool PlyFile::is_binary_file() const { return impl->isBinary; }
IGL_INLINE std::shared_ptr<PlyData> PlyFile::request_properties_from_element(
    const std::string &elementKey, const std::vector<std::string> propertyKeys,
    const uint32_t list_size_hint)
{
...
IGL_INLINE void PlyFile::add_properties_to_element(
    const std::string &elementKey, const std::vector<std::string> propertyKeys,
    const Type type, const size_t count, uint8_t *data, const Type listType,
    const size_t listCount)
{

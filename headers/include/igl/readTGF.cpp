                             ...
                             std::vector<std::vector<int>> &PE)
{
    ...
    while (fgets(line, MAX_LINE_LENGTH, tgf_file) != NULL) {
        ...
        if (line[0] == '#') {
            ...
            if (count != 4) {
                fprintf(stderr,
                        "Error: readTGF.h: bad format in vertex line\n");
                fclose(tgf_file);
                return false;
            }
            ...
            if (is_PE == 1) {
                // PE should index P
                fprintf(stderr, "Warning: readTGF.h found pseudo edges but "
                                "does not support "
                                "them\n");
            }
            if (is_CE == 1) {
                // CE should index P
                fprintf(stderr, "Warning: readTGF.h found cage edges but does "
                                "not support them\n");
            }
        } else {
            // ignore faces

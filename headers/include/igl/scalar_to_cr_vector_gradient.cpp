...
IGL_INLINE void igl::scalar_to_cr_vector_gradient(
    const Eigen::MatrixBase<DerivedV> &V, const Eigen::MatrixBase<DerivedF> &F,
    const Eigen::MatrixBase<DerivedE> &E,
    const Eigen::MatrixBase<DerivedOE> &oE, Eigen::SparseMatrix<ScalarG> &G)
{
...
template <typename DerivedF, typename DerivedL_sq, typename DerivedE,
          typename DerivedOE, typename ScalarG>
IGL_INLINE void igl::scalar_to_cr_vector_gradient_intrinsic(
    ...
    for (Eigen::Index f = 0; f < m; ++f) {
        for (int e = 0; e < 3; ++e) {
            const typename DerivedF::Scalar i = F(f, (e + 1) % 3),
                                            j = F(f, (e + 2) % 3), k = F(f, e);
            const ScalarG o = oE(f, e), eij = l_sq(f, e),
                          ejk = l_sq(f, (e + 1) % 3),
                          eki = l_sq(f, (e + 2) %
            ...
            tripletList.emplace_back(E(f, e) + nE, i,
                                     -o * (eij + ejk - eki) / (12. * s_eij));
            tripletList.emplace_back(E(f, e), j, o * dA(f) / (6. * s_eij));
            tripletList.emplace_back(E(f, e) + nE, j,
                                     -o * (eij - ejk + eki) / (12. * s_eij));
...
template void igl::scalar_to_cr_vector_gradient_intrinsic<

...
#warning "ACTIVE_SET_CPP_DEBUG"
    ...
    assert((Aieq.size() == 0 && Bieq.size() == 0) ||
           Aieq.rows() == Bieq.rows());
...
#define FALSE 0
    ...
    while (true) {
        ...
        const int as_lx_count =
            std::count(as_lx.data(), as_lx.data() + n, TRUE);
        const int as_ux_count =
            std::count(as_ux.data(), as_ux.data() + n, TRUE);
        ...
        Eigen::Matrix<typename DerivedY::Scalar, Eigen::Dynamic, 1> as_ieq_list(
            as_ieq_count, 1);
        ...
        if (known_i.size() == A.rows()) {
...
#endif
            if (!min_quad_with_fixed_precompute(A, known_i, Aeq_i,
                                                params.Auu_pd, data)) {
                cerr << "Error: min_quad_with_fixed precomputation failed."
                     << endl;
                if (iter > 0 && Aeq_i.rows() > Aeq.rows()) {
                    cerr << "  *Are you sure rows of [Aeq;Aieq] are linearly "
                            "independent?*"
                         << endl;
                }
            ...
            cout << "  post" << endl;
#endif
            // Computing Lagrange multipliers needs to be adjusted slightly if A
            // is not symmetric
            assert(data.Auu_sym);
        }

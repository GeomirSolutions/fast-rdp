...
template <typename DerivedA, typename DerivedB, typename DerivedIA,
                              ...
                              Eigen::PlainObjectBase<DerivedLOCB> &LOCB)
{
    ...
    const VectorA vA(
        Eigen::Map<const VectorA>(DerivedA(A).data(), A.cols() * A.rows(), 1));
    const VectorB vB(
        Eigen::Map<const VectorB>(DerivedB(B).data(), B.cols() * B.rows(), 1));
    VectorA uA;
    VectorB uB;
    ...
    Eigen::Matrix<typename DerivedLOCB::Scalar, Eigen::Dynamic, 1> uLOCB =
        Eigen::Matrix<typename DerivedLOCB::Scalar, Eigen::Dynamic,
                      1>::Constant(sA.size(), 1, -1);
    {
        int bi = 0;
        // loop over sA
        bool past = false;
    ...
    Map<Matrix<typename DerivedIA::Scalar, Dynamic, 1>> vIA(
        IA.data(), IA.cols() * IA.rows(), 1);
    Map<Matrix<typename DerivedLOCB::Scalar, Dynamic, 1>> vLOCB(
        LOCB.data(), LOCB.cols() * LOCB.rows(), 1);
...
template void igl::ismember<Eigen::Matrix<int, -1, -1, 0, -1, -1>,

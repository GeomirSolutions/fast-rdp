...
template <typename Scalar> struct KelvinletParams
{
    ...
    KelvinletParams(const Scalar &epsilon, const int falloff,
                    const BrushType &type)
        : epsilon(epsilon), scale(falloff), brushType(type)
    {
        static constexpr std::array<Scalar, 3> brush_scaling_params{1.0f, 1.1f,
                                                                    1.21f};
        for (int i = 0; i < 3; i++) {
            ep[i] = epsilon * brush_scaling_params[i];
        }
        w[0] = 1;
        w[1] = -((ep[2] * ep[2] - ep[0] * ep[0]) /
                 (ep[2] * ep[2] - ep[1] * ep[1]));
        w[2] =
            (ep[1] * ep[1] - ep[0] * ep[0]) / (ep[2] * ep[2] - ep[1] * ep[1]);
    }
};
...
/// @param[in] params  parameters for the kelvinlet brush like brush radius,
/// scale etc

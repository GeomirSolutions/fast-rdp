                       ...
                       Eigen::PlainObjectBase<DerivedW> &W)
{
    ...
    std::vector<Eigen::Matrix<ChildrenType, 8, 1>,
                Eigen::aligned_allocator<Eigen::Matrix<ChildrenType, 8, 1>>>
        children;
    std::vector<Eigen::Matrix<CentersType, 1, 3>,
                Eigen::aligned_allocator<Eigen::Matrix<CentersType, 1, 3>>>
        centers;
    ...
    std::function<RowVector3CentersType(const RowVector3CentersType,
                                        const CentersType, const ChildrenType)>
        translate_center = [](const RowVector3CentersType &parent_center,
                              const CentersType h,
            ...
            if (child_index == 2 || child_index == 3 || child_index == 6 ||
                child_index == 7) {
                change_vector(1) = h;
            }
    ...
    const Vector8i zero_to_seven =
        (Vector8i() << 0, 1, 2, 3, 4, 5, 6, 7).finished();
    ...
    // list.
    helper = [&helper, &translate_center, &get_octant, &m, &zero_to_seven,
              &neg_ones, &P, &point_indices, &children, &centers, &widths,
              &MAX_DEPTH](const ChildrenType index, const int depth) -> void {
        if (point_indices.at(index).size() > 1 && depth < MAX_DEPTH) {
            ...
            for (int j = 0; j < point_indices.at(index).size(); j++) {
                ...
                point_indices.at(cell_of_curr_point)
                    .emplace_back(curr_point_index);
            }
...

    {
        ...
        for (IndexType i = 0; i < all.size(); i++)
            all[i] = i;
    ...
    RowVector3CentersType aabb_center =
        (backleftbottom + frontrighttop) / PointScalar(2.0);
...
template void igl::octree<Eigen::Matrix<double, -1, -1, 0, -1, -1>, int,

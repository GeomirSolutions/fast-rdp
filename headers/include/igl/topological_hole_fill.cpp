...
IGL_INLINE void
igl::topological_hole_fill(const Eigen::MatrixBase<DerivedF> &F,
                           ...
                           Eigen::PlainObjectBase<DerivedF_filled> &F_filled)
{
    ...
    for (int i = 0; i < num_holes; i++, new_vert_id++) {
        ...
        F_filled.row(new_face_id++) << holes[i][it], holes[i][back],
            new_vert_id;
        while (it != back) {
            F_filled.row(new_face_id++) << holes[i][(it + 1)], holes[i][(it)],
                new_vert_id;
            it++;
        }
    }
...
template void igl::topological_hole_fill<Eigen::Matrix<int, -1, -1, 0, -1, -1>,

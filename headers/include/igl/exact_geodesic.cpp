  ...
  public:
    ...
    template <class T> T *allocate(unsigned n)
    {
        ...
        if (wanted > m_num_bytes) {
            unsigned new_size =
                (unsigned)std::ceil(wanted / (double)sizeof(double));
  ...
  public:
    ...
    void geodesic(std::vector<SurfacePoint> &sources,
                  ...
                  std::vector<std::vector<SurfacePoint>>
                      &paths); // lazy people can find geodesic paths with one
                               // function call
    ...
    std::vector<SurfacePoint> *stop_points)
{
    ...
    while (!m_queue.empty()) {
        m_queue_max_size = std::max(static_cast<unsigned int>(m_queue.size()),
                                    m_queue_max_size);
             ...
             ++i) // two possible faces to propagate
        {
                                      ...
                                      // edges
            {
                if ((i == 0 &&
                    ...
                    (i == 1 &&
                     min_interval->direction() == Interval::FROM_FACE_1)) {
                    continue;
                }
            }
            ...
            if (num_propagated) {
                ...
                bool const invert =
                    next_edge->v0()->id() !=
                    edge->v0()->id(); // if the origins coinside, do not invert
                                      // intervals
            ...
            if (propagate_to_right) {
                ...
                if (num_propagated) {
                    bool const invert =
                        next_edge->v0()->id() !=
                        edge->v1()->id(); // if the origins coinside, do not
                                          // invert intervals
...
{
    assert((V.cols() == 3 || V.cols() == 2) && F.cols() == 3 &&
           "Only support 2D/3D triangle mesh");
...
template void igl::exact_geodesic<

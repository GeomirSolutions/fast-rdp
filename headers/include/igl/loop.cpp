...
IGL_INLINE void
igl::loop(const int n_verts, const Eigen::MatrixBase<DerivedF> &F,
          Eigen::SparseMatrix<SType> &S, Eigen::PlainObjectBase<DerivedNF> &NF)
{
    ...
    for (int i = 0; i < FF.rows(); ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            if (NIdoubles(i, j) == 0) {
                if (FF(i, j) == -1) {
                    // Boundary vertex
                    tripletList.emplace_back(NI(i, j) + n_odd, F(i, j),
                                             1. / 2.);
                    tripletList.emplace_back(NI(i, j) + n_odd,
                                             F(i, (j + 1) % 3), 1. / 2.);
                } else {
                    tripletList.emplace_back(NI(i, j) + n_odd, F(i, j),
                                             3. / 8.);
    ...
    for (int i = 0; i < F.rows(); ++i) {
        ...
        Eigen::Matrix<typename DerivedF::Scalar, 3, 1> f0(3), f1(3), f2(3),
            f3(3);
        f0 << VI(0), VI(3), VI(5);
        f1 << VI(1), VI(4), VI(3);
        f2 << VI(3), VI(4), VI(5);
...
template <typename DerivedV, typename DerivedF, typename DerivedNV,
          typename DerivedNF>
IGL_INLINE void igl::loop(const Eigen::MatrixBase<DerivedV> &V,
...
template void igl::loop<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &, int);
#endif

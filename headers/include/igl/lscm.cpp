...
IGL_INLINE bool igl::lscm(const Eigen::MatrixBase<DerivedV> &V,
                          ...
                          Eigen::SparseMatrix<QScalar> &Q)
{
    ...
    Eigen::Matrix<typename Derivedb::Scalar, Eigen::Dynamic, 1> b_flat(
        b.size() * bc.cols(), 1);
    Eigen::Matrix<typename Derivedbc::Scalar, Eigen::Dynamic, 1> bc_flat(
        bc.size(), 1);
    ...
    if (!igl::min_quad_with_fixed_precompute(Q, b_flat, SparseMatrix<QScalar>(),
                                             true, data)) {
        return false;
    }
...
template bool igl::lscm<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

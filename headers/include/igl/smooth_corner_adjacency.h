...
/// @param[in] corner_threshold_radians  dihedral angle considered non-smooth
/// (in
///     radians)
/// @param[out] CI  #CI list of face neighbors as indices into rows of F
/// @param[out] CC  3*#F+1 list of cumulative sizes so that CC(i*3+j+1) -
/// CC(i*3+j) is
///     the number of faces considered smoothly incident on corner at F(i,j)
void smooth_corner_adjacency(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                             const double corner_threshold_radians,
                             Eigen::VectorXi &CI, Eigen::VectorXi &CC);
...
/// CC(i*3+j) is
///     the number of faces considered smoothly incident on corner at F(i,j)
void smooth_corner_adjacency(const Eigen::MatrixXi &FV,
                             const Eigen::MatrixXi &FN, Eigen::VectorXi &CI,
                             Eigen::VectorXi &CC);
} // namespace igl

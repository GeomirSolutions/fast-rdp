                                ...
                                const std::string &filename)
{
    ...
    if (error != tinyxml2::XML_SUCCESS) {
        ...
        if (element == NULL) {
            std::cerr
                << "Name of object not found! Initialized with default value."
                << std::endl;
            obj = T();
        } else {
...
IGL_INLINE void deserialize(std::string &obj,
                            const tinyxml2::XMLDocument * /*doc*/,
                            const tinyxml2::XMLElement *element,
                            const std::string &name)
{
                          ...
                          const std::string &name)
{
    const std::function<std::string(const T &)> to_string =
        ...
        return STR(std::setprecision(std::numeric_limits<double>::digits10 + 2)
                   << v);
    };
...
{
    const std::function<void(const std::string &, T &)> &from_string =
        [](const std::string &s, T &v) { getAttribute(s.c_str(), v); };
...
template <typename T, int P, typename I>
IGL_INLINE void deserialize(Eigen::SparseMatrix<T, P, I> &obj,
                            const tinyxml2::XMLDocument * /*doc*/,
                            const tinyxml2::XMLElement *element,
                            const std::string &name)
{
...
std::string base64_decode(std::string const &encoded_string)
{
    ...
    while (in_len-- && (encoded_string[in_] != '=') &&
        ...
        if (i == 4) {
            for (i = 0; i < 4; i++)
                char_array_4[i] =
                    charLookup[char_array_4[i]]; // new fast lookup

...
IGL_INLINE void validate_arg_scalar(const int i, const int nrhs,
                                    const mxArray *prhs[], const char *name);
/// \overload
/// \breif Throw an error if arg i+1 is not logical
IGL_INLINE void validate_arg_logical(const int i, const int nrhs,
                                     const mxArray *prhs[], const char *name);
/// \overload
/// \breif Throw an error if arg i+1 is not char
IGL_INLINE void validate_arg_char(const int i, const int nrhs,
                                  const mxArray *prhs[], const char *name);
/// \overload
/// \breif Throw an error if arg i+1 is not double
IGL_INLINE void validate_arg_double(const int i, const int nrhs,
                                    const mxArray *prhs[], const char *name);
/// \overload
/// \breif Throw an error if arg i+1 is not a function handle
IGL_INLINE void validate_arg_function_handle(const int i, const int nrhs,
                                             const mxArray *prhs[],
                                             const char *name);
} // namespace matlab
} // namespace igl

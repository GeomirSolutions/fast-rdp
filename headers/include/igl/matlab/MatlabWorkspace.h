...
class MatlabWorkspace
{
  ...
  public:
    ...
    /// @tparam DerivedM  eigen matrix (e.g. MatrixXd)
    template <typename DerivedM>
    inline MatlabWorkspace &save_index(const Eigen::DenseBase<DerivedM> &M,
                                       const std::string &name);
    /// \overload
    /// @tparam MT  sparse matrix type (e.g. double)
    template <typename ScalarM>
                                   ...
                                   const std::string &name)
{
    ...
    for (int j = 0; j < M.outerSize(); j++) {
        jc[j] = k;
        // Iterate over inside
        for (typename Eigen::SparseMatrix<MT>::InnerIterator it(M, j); it;
             ++it) {
            pr[k] = it.value();
            ir[k] = it.row();
            k++;
...
template <typename DerivedM>
inline igl::matlab::MatlabWorkspace &
igl::matlab::MatlabWorkspace::save_index(const Eigen::DenseBase<DerivedM> &M,
                                         const std::string &name)
{
    DerivedM Mp1 = M;
    Mp1.array() += 1;
                                   ...
                                   Eigen::PlainObjectBase<DerivedM> &M)
{
    ...
    M = Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>>(
            mxGetPr(mx_data), M.rows(), M.cols())
            .cast<typename DerivedM::Scalar>();
    return true;
}
...
inline bool igl::matlab::MatlabWorkspace::find(const std::string &name, int &v)
{
    using namespace std;
    const int i = std::find(names.begin(), names.end(), name) - names.begin();
    if (i >= (int)names.size()) {

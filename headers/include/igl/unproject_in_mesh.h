...
IGL_INLINE int unproject_in_mesh(
    ...
    Eigen::PlainObjectBase<Derivedobj> &obj, std::vector<igl::Hit> &hits);
/// \overload
template <typename DerivedV, typename DerivedF, typename Derivedobj>
IGL_INLINE int
...
IGL_INLINE int unproject_in_mesh(
    const Eigen::Vector2f &pos, const Eigen::Matrix4f &model,
    const Eigen::Matrix4f &proj, const Eigen::Vector4f &viewport,
    const std::function<void(const Eigen::Vector3f &, const Eigen::Vector3f &,
                             std::vector<igl::Hit> &)> &shoot_ray,
    Eigen::PlainObjectBase<Derivedobj> &obj, std::vector<igl::Hit> &hits);
} // namespace igl

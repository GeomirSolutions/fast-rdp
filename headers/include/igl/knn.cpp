...
template <typename DerivedP, typename IndexType, typename DerivedCH,
          typename DerivedCN, typename DerivedW, typename DerivedI>
IGL_INLINE void knn(const Eigen::MatrixBase<DerivedP> &P, size_t k,
...
template <typename DerivedP, typename DerivedV, typename IndexType,
          ...
          typename DerivedI>
IGL_INLINE void
knn(const Eigen::MatrixBase<DerivedP> &P, const Eigen::MatrixBase<DerivedV> &V,
    size_t k, const std::vector<std::vector<IndexType>> &point_indices,
    const Eigen::MatrixBase<DerivedCH> &CH,
    const Eigen::MatrixBase<DerivedCN> &CN,
    const Eigen::MatrixBase<DerivedW> &W, Eigen::PlainObjectBase<DerivedI> &I)
{
    ...
    if (Vsize <= k) {
        ...
        for (size_t i = 0; i < Psize; ++i) {
            Eigen::Matrix<Scalar, Eigen::Dynamic, 1> D =
                (V.rowwise() - P.row(i)).rowwise().norm();
    ...
    auto distance_to_width_one_cube =
        [](const RowVector3PType &point) -> Scalar {
        return std::sqrt(
            std::pow<Scalar>(std::max<Scalar>(std::abs(point(0)) - 1, 0.0), 2) +
    ...
    igl::parallel_for(
        ...
        [&](size_t i) {
            ...
            auto cmp = [&point_of_interest, &V, &CN, &W, Vsize,
                        &distance_to_cube](int left, int right) {
                ...
                if (left < Vsize) { // left is a point index
                    ...
                    leftdistance =
                        distance_to_cube(point_of_interest,
                                         CN.row(left - Vsize), W(left - Vsize));
                }
            ...
            while (points_found < k) {
                ...
                queue.pop();
                if (curr_cell_or_point <
                    Vsize) { // current index is for is a point
...
template void igl::knn<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::MatrixBase<Eigen::Matrix<int, -1, 8, 0, -1, 8>> const &,
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 3, 0, -1, 3>> const &,
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &);
template void igl::knn<Eigen::Matrix<double, -1, -1, 0, -1, -1>, int,
                       Eigen::Matrix<int, -1, -1, 0, -1, -1>,
                       Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                       Eigen::Matrix<double, -1, 1, 0, -1, 1>,
                       Eigen::Matrix<int, -1, -1, 0, -1, -1>>(

...
/// @param[out] U  #U by #A list of indices into each attribute for each unique
/// mesh
///               vertex: U(v,a) is the attribute index of vertex v in attribute
///               a.
...
///   tsurf(G,TC(U(:,2),:));
///
template <typename DerivedA, typename DerivedU, typename DerivedG,
          typename DerivedJ>
IGL_INLINE void
unzip_corners(const std::vector<std::reference_wrapper<DerivedA>> &A,

    ...
    const Eigen::MatrixXi &F2E, Eigen::PlainObjectBase<DerivedK> &K)
{
    ...
    for (unsigned eid = 0; eid < numE; ++eid) {
        if (!isBorderEdge[eid]) {
            ...
            //      Eigen::Matrix<typename DerivedV::Scalar, 1, 3> N1 =
            //      FN.row(fid1);
            ...
            // Map the two triangles in a new space where the common edge is the
            // x axis and the N0 the z axis
            ...
            double alpha =
                -atan2(V1((fid1_vc + 2) % 3, 2), V1((fid1_vc + 2) % 3, 1));
            ...
            // k_ij is the angle between the triangle on the left and the one on
            // the right

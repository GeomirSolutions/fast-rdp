...
/// Compared to igl::sparse, this version is slower for the first time (since it
/// requires a precomputation), but faster to the subsequent evaluations.
///
...
IGL_INLINE void sparse_cached_precompute(const Eigen::MatrixBase<DerivedI> &I,
...
IGL_INLINE void
sparse_cached_precompute(const std::vector<Eigen::Triplet<Scalar>> &triplets,
...
template <typename Scalar>
IGL_INLINE void
sparse_cached(const std::vector<Eigen::Triplet<Scalar>> &triplets,
...
IGL_INLINE void sparse_cached(const Eigen::MatrixBase<DerivedV> &V,

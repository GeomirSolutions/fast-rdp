...
/// @param[out] TT   #F by #3 adjacent matrix, the element i,j is the id of the
/// triangle
///        adjacent to the j edge of triangle i
/// @param[out] TTi  #F by #3 adjacent matrix, the element i,j is the id of edge
/// of the
...
IGL_INLINE void
triangle_triangle_adjacency(const Eigen::MatrixBase<DerivedF> &F,
...
/// @param[in] F  #F by simplex_size list of mesh faces (must be triangles)
/// @param[in] TT   #F by #3 adjacent matrix, the element i,j is the id of the
/// triangle
///        adjacent to the j edge of triangle i
///
/// \fileinfo
...
IGL_INLINE void
triangle_triangle_adjacency_preprocess(const Eigen::MatrixBase<DerivedF> &F,
...
IGL_INLINE void
triangle_triangle_adjacency_extractTT(const Eigen::MatrixBase<DerivedF> &F,
                                      std::vector<std::vector<TTT_type>> &TTT,
                                      Eigen::PlainObjectBase<DerivedTT> &TT);
...
IGL_INLINE void
triangle_triangle_adjacency_extractTTi(const Eigen::MatrixBase<DerivedF> &F,
                                       std::vector<std::vector<TTT_type>> &TTT,
                                       Eigen::PlainObjectBase<DerivedTTi> &TTi);
/// Adjacency list version, which works with non-manifold meshes
///
/// @param[in] F  #F by 3 list of triangle indices
/// @param[out] TT  #F by 3 list of lists so that TT[i][c] --> {j,k,...} means
/// that
///   faces j and k etc. are edge-neighbors of face i on face i's edge
///   opposite corner c
/// @param[out] TTj  #F list of lists so that TTj[i][c] --> {j,k,...} means that
/// face
...
///     `oriented_facets`)
/// @param[in] EMAP #F*3 list of indices into uE, mapping each directed edge to
/// unique
///     undirected edge
...
IGL_INLINE void triangle_triangle_adjacency(
    ...
    std::vector<std::vector<std::vector<TTiIndex>>> &TTi);
/// \overload
/// @param[in] uEC  #uE+1 list of cumulative counts of directed edges sharing
/// each
///    unique edge so the uEC(i+1)-uEC(i) is the number of directed edges
///    sharing the ith unique edge.
/// @param[in] uEE  #E list of indices into E, so that the consecutive segment
/// of

...
IGL_INLINE void igl::curved_hessian_energy(const Eigen::MatrixBase<DerivedV> &V,
...
IGL_INLINE void igl::curved_hessian_energy(
    const Eigen::MatrixBase<DerivedV> &V, const Eigen::MatrixBase<DerivedF> &F,
    const Eigen::MatrixBase<DerivedE> &E,
    const Eigen::MatrixBase<DerivedOE> &oE, Eigen::SparseMatrix<ScalarQ> &Q)
{
...
template <typename DerivedF, typename DerivedL_sq, typename DerivedE,
          typename DerivedOE, typename ScalarQ>
IGL_INLINE void
igl::curved_hessian_energy_intrinsic(const Eigen::MatrixBase<DerivedF> &F,
                                     ...
                                     Eigen::SparseMatrix<ScalarQ> &Q)
{
    ...
    for (Eigen::Index k = 0; k < M.outerSize(); ++k) {
        for (typename Eigen::SparseMatrix<ScalarQ>::InnerIterator it(M, k); it;
             ++it) {
            if (it.value() > 0) {
...
template void
igl::curved_hessian_energy<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

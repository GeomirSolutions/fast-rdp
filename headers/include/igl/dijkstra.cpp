...
IGL_INLINE int igl::dijkstra(const IndexType &source,
                             ...
                             Eigen::PlainObjectBase<DerivedP> &previous)
{
    ...
    min_distance.setConstant(
        numV, 1, std::numeric_limits<typename DerivedD::Scalar>::max());
...
IGL_INLINE void igl::dijkstra(const IndexType &vertex,
...
template <typename IndexType, typename DerivedV, typename DerivedD,
          typename DerivedP>
IGL_INLINE int igl::dijkstra(const Eigen::MatrixBase<DerivedV> &V,
                             const std::vector<std::vector<IndexType>> &VV,
                             ...
                             Eigen::PlainObjectBase<DerivedP> &previous)
{
    ...
    while (!vertex_queue.empty()) {
        ...
        for (std::vector<int>::const_iterator neighbor_iter = neighbors.begin();
             neighbor_iter != neighbors.end(); neighbor_iter++) {
            IndexType v = *neighbor_iter;
            typename DerivedD::Scalar distance_through_u =
                dist + (V.row(u) - V.row(v)).norm();
            if (distance_through_u < min_distance[v]) {
                vertex_queue.erase(std::make_pair(min_distance[v], v));
...
template int igl::dijkstra<int, Eigen::Matrix<double, -1, 1, 0, -1, 1>,
...
template int igl::dijkstra<int, Eigen::Matrix<int, -1, 1, 0, -1, 1>,
                           Eigen::Matrix<int, -1, 1, 0, -1, 1>>(
    int const &, std::set<int, std::less<int>, std::allocator<int>> const &,
    std::vector<std::vector<int, std::allocator<int>>,
                std::allocator<std::vector<int, std::allocator<int>>>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 1, 0, -1, 1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 1, 0, -1, 1>> &);
template void igl::dijkstra<int, Eigen::Matrix<int, -1, 1, 0, -1, 1>>(
    int const &, Eigen::MatrixBase<Eigen::Matrix<int, -1, 1, 0, -1, 1>> const &,

...
IGL_INLINE bool igl::boundary_conditions(
    ...
    const Eigen::MatrixXi &CF, Eigen::VectorXi &b, Eigen::MatrixXd &bc)
{
    ...
    for (int e = 0; e < BE.rows(); e++) {
        ...
        for (int i = 0; i < V.rows(); i++) {
            ...
            if (t >= -FLOAT_EPS && t <= (1.0f + FLOAT_EPS) &&
                sqrd <= FLOAT_EPS) {
                bci.push_back(i);
    ...
    for (int f = 0; f < CF.rows(); f++) {
        ...
        for (int i = 0; i < V.rows(); i++) {
            ...
            if (dist <= 1.e-1f) {
                ...
                Vector3d vec_0 = v_1 - v_0, vec_1 = v_2 - v_0,
                         vec_2 = point - v_0;

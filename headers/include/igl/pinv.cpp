...
{
    Eigen::JacobiSVD<DerivedA> svd(A,
    ...
    if (tol < 0) {
        ...
        tol = (Scalar)(std::max(A.rows(), A.cols())) *
              (smax -
               std::nextafter(smax, std::numeric_limits<Scalar>::epsilon()));
    }
    const int rank = (S.array() > 0).count();
    X = (V.leftCols(rank).array().rowwise() *
         (1.0 / S.head(rank).array()).transpose())
            .matrix() *
...
template void igl::pinv<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

...
class Frame_field_deformer
{
  public:
    ...
    IGL_INLINE void extractBlock(Eigen::SparseMatrix<double> &mat, int r0,
                                 int c0, int r, int c,
    ...
    Eigen::MatrixXd
        RHS; // pre-computed part of known term in global optimization
    std::vector<Eigen::Matrix<double, 3, 3>>
        RW; // optimal rotation-warping matrices (parallel to m.F) --
            // INCORPORATES WW
...
IGL_INLINE void Frame_field_deformer::precompute_opt()
{
    ...
    nfree =
        V.rows() - fixed; // free vertices (at the beginning ov m.V) - global
...
IGL_INLINE void Frame_field_deformer::compute_optimal_positions()
{
    ...
    for (int i = 0; i < nfree; i++) {
        b.row(i) << 0.0, 0.0, 0.0;
        for (int k = 0; k < (int)VT[i].size();
             k++) // for all incident triangles
        {
            ...
            i1 = F(t, (vi + 1) % 3);
            i2 = F(t, (vi + 2) % 3);
            b.row(i) += (C(t, (vi + 2) % 3) * RW[t] *
                         (V.row(i1) - V.row(i)).transpose())
                            .transpose();
            b.row(i) += (C(t, (vi + 1) % 3) * RW[t] *
                         (V.row(i2) - V.row(i)).transpose())
                            .transpose();
        }
    }
    b /= 2.0;
...
IGL_INLINE void igl::frame_field_deformer(
    ...
    Eigen::MatrixXd &V_d, Eigen::MatrixXd &FF1_d, Eigen::MatrixXd &FF2_d,
    const int iterations, const double lambda, const bool perturb_initial_guess)
{
    using namespace Eigen;
    // Solvers

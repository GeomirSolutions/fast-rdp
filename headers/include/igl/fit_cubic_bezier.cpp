...
IGL_INLINE void igl::fit_cubic_bezier(const Eigen::MatrixXd &d,
                                      const double error,
                                      std::vector<Eigen::MatrixXd> &cubics)
{
    ...
    const static auto tangent = [](const Eigen::MatrixXd &d, const int i,
                                   const int dir) -> Eigen::RowVectorXd {
        int j = i;
        const int nPts = d.rows();
        Eigen::RowVectorXd t;
        while (true) {
    ...
    cubics.clear();
    fit_cubic_bezier_substring(d, 0, nPts - 1, tHat1, tHat2, error, closed,
                               cubics);
};

IGL_INLINE void
igl::fit_cubic_bezier_substring(const Eigen::MatrixXd &d, const int first,
                                const int last, const Eigen::RowVectorXd &tHat1,
                                const Eigen::RowVectorXd &tHat2,
                                const double error, const bool force_split,
                                std::vector<Eigen::MatrixXd> &cubics)
{
    // Helper functions
    // Evaluate a Bezier curve at a particular parameter value
    const static auto bezier_eval = [](const Eigen::MatrixXd &V,
                                       const double t) {
        Eigen::RowVectorXd P;
        bezier(V, t, P);
        return P;
    ...
    const static auto NewtonRaphsonRootFind = [](const Eigen::MatrixXd &Q,
        ...
        const double numerator =
            ((Q_u - P).array() * Q1_u.array()).array().sum();
        const double denominator =
            Q1_u.squaredNorm() +
            ((Q_u - P).array() * Q2_u.array()).array().sum();
        /* u = u - f(u)/f'(u) */
        return u - (numerator / denominator);
    };
    const static auto ComputeMaxError =
        [](const Eigen::MatrixXd &d, const int first, const int last,
           const Eigen::MatrixXd &bezCurve, const Eigen::VectorXd &u,
        ...
        //   if(run>half_total)
        //   {
        //     // When accumulated ½ the error --> more symmetric, but requires
        //     more
        //     // curves
        //     splitPoint = i;
        //     break;
    ...
    const static auto Straight =
        [](const Eigen::MatrixXd &d, const int first, const int last,
           const Eigen::RowVectorXd &tHat1, const Eigen::RowVectorXd &tHat2,
    ...
    const static auto GenerateBezier =
        [](const Eigen::MatrixXd &d, const int first, const int last,
           const Eigen::VectorXd &uPrime, const Eigen::RowVectorXd &tHat1,
           const Eigen::RowVectorXd &tHat2, Eigen::MatrixXd &bezCurve) {
            bezCurve.resize(4, d.cols());
            ...
            for (int i = 0; i < nPts; i++) {
                ...
                const Eigen::RowVectorXd tmp =
                    ...
                    (d.row(first) * B0(uPrime(i)) +
                     ...
                     d.row(last) * B2(uPrime(i)) + d.row(last) * B3(uPrime(i)));
                X(0) += A[i][0].dot(tmp);
                X(1) += A[i][1].dot(tmp);
            }
    ...
    // foobar
    fit_cubic_bezier_substring(d, first, splitPoint, tHat1, tHatCenter, error,
                               false, cubics);
    fit_cubic_bezier_substring(d, splitPoint, last, (-tHatCenter).eval(), tHat2,
                               error, false, cubics);
}

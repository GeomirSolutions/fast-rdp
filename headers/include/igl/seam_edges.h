...
/// @param[out] seams  Edges where the forwards and backwards directions have
/// different
...
///         F[ seams( i, 0 ), seams( i, 1 ) ], F[ seams( i, 0 ), (seams( i, 1 )
///         + 1) % 3 ]
///     and the other side is the edge:
///         F[ seams( i, 2 ), seams( i, 3 ) ], F[ seams( i, 2 ), (seams( i, 3 )
///         + 1) % 3 ]
/// @param[out] boundaries  Edges with only one incident triangle, as a
/// #boundaries-by-2
///     matrix of indices. Each row is organized as
///         [ face_index, face_vertex_index ]
///     such that the edge is:
///         F[ boundaries( i, 0 ), boundaries( i, 1 ) ], F[ boundaries( i, 0 ),
///         (boundaries( i, 1 ) + 1) % 3 ]
/// @param[out] foldovers  Edges where the two incident triangles fold over each
/// other
///     in UV-space, as a #foldovers-by-4 matrix of indices.
///     Each row is organized as [ forward_face_index,
///     forward_face_vertex_index, backwards_face_index,
///     backwards_face_vertex_index ] such that one side of the foldover is the
///     edge:
///       F[ foldovers( i, 0 ), foldovers( i, 1 ) ], F[ foldovers( i, 0 ),
///       (foldovers( i, 1 ) + 1) % 3 ]
///     and the other side is the edge:
///       F[ foldovers( i, 2 ), foldovers( i, 3 ) ], F[ foldovers( i, 2 ),
///       (foldovers( i, 3 ) + 1) % 3 ]
...
IGL_INLINE void
seam_edges(const Eigen::PlainObjectBase<DerivedV> &V,

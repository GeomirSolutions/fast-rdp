...
IGL_INLINE void
igl::lexicographic_triangulation(const Eigen::MatrixBase<DerivedP> &P,
                                 Orient2D orient2D,
                                 Eigen::PlainObjectBase<DerivedF> &F)
{
    ...
    for (size_t i = 2; i < num_pts; i++) {
        ...
        if (faces.size() == 0) {
            ...
            // Check if the current point is collinear with every points before
            // it.
            ...
            if (orientation != 0) {
                ...
                boundary.insert(boundary.end(), order.data(),
                                order.data() + i + 1);
                if (orientation < 0) {
                    boundary.reverse();
                }
...
template void igl::lexicographic_triangulation<

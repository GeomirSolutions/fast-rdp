...
IGL_INLINE void igl::bezier(const Eigen::MatrixBase<DerivedV> &V,
                            ...
                            Eigen::PlainObjectBase<DerivedP> &P)
{
    ...
    for (int i = 1; i <= degree; i++) {
        for (int j = 0; j <= degree - i; j++) {
            Vtemp.row(j) =
                ((1.0 - t) * Vtemp.row(j) + t * Vtemp.row(j + 1)).eval();
        }
    }
    P = Vtemp.row(0);
}
                            ...
                            Eigen::PlainObjectBase<DerivedP> &P)
{
    ...
    for (int i = 0; i < T.size(); i++) {
        Eigen::Matrix<typename DerivedV::Scalar, 1, DerivedV::ColsAtCompileTime>
            Pi;
        bezier(V, T(i), Pi);
        P.row(i) = Pi;
    }
...
IGL_INLINE void igl::bezier(const std::vector<VMat> &spline,
                            const Eigen::MatrixBase<DerivedT> &T,
                            Eigen::PlainObjectBase<DerivedP> &P)
{
    if (spline.size() == 0)
    ...
    for (int c = 0; c < spline.size(); c++) {
        assert(dim == spline[c].cols() &&
               "All curves must have same dimension");
        DerivedP Pc;
...
template void igl::bezier<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

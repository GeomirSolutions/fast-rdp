                              ...
                              Eigen::PlainObjectBase<DerivedNF> &NF)
{
    ...
    for (int i = 0; i < FF.rows(); ++i) {
        for (int j = 0; j < 3; ++j) {
            if (NIdoubles(i, j) == 0) {
                ...
                tripletList.emplace_back(NI(i, j) + n_odd, F(i, (j + 1) % 3),
                                         1. / 2.);
            }
        }
    }
    ...
    for (int i = 0; i < F.rows(); ++i) {
        ...
        Eigen::Matrix<typename DerivedNF::Scalar, 1, 3> f0(3), f1(3), f2(3),
            f3(3);
        f0 << VI(0), VI(3), VI(5);
        f1 << VI(1), VI(4), VI(3);
        f2 << VI(3), VI(4), VI(5);
...
template <typename DerivedV, typename DerivedF, typename DerivedNV,
          typename DerivedNF>
IGL_INLINE void igl::upsample(const Eigen::MatrixBase<DerivedV> &V,
                              const Eigen::MatrixBase<DerivedF> &F,
...
IGL_INLINE void igl::upsample(MatV &V, MatF &F, const int number_of_subdivs)
{
    const MatV V_copy = V;
    const MatF F_copy = F;
...
template void igl::upsample<Eigen::Matrix<int, -1, -1, 0, -1, -1>, double,
                            Eigen::Matrix<int, -1, -1, 0, -1, -1>>(

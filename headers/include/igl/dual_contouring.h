...
/// @param[in] constrained  whether to force dual vertices to lie strictly
/// inside
///     corresponding primal cell (prevents self-intersections at cost of
///     surface quality; marginally slower)
/// @param[in] triangles  whether to output four triangles instead of one quad
/// per
///     crossing edge (quad mesh usually looks fine)
/// @param[in] root_finding  whether to use root finding to identify crossing
/// point on
...
/// \overload
///
/// @param[in] Gf  nx*ny*nz list of function values so that Gf(k) = f(GV.row(k))
/// (only
///   needs to be accurate near f=0 and correct sign elsewhere)
/// @param[in] GV  nx*ny*nz list of grid positions so that the x,y,z grid
/// position is at
///     GV.row(x+nx*(y+z*ny))
...
///
/// @param[in] Gf  #GV list of corresponding f values. If using root finding
/// then only the

                              ...
                              Eigen::VectorXi &J, Eigen::VectorXi &I)
{
    ...
    // decimate will not work correctly on non-edge-manifold meshes. By
    // extension this includes meshes with non-manifold vertices on the boundary
    // since these will create a non-manifold edge when connected to infinity.
    {
    ...
    bool ret = decimate(VO, FO, shortest_edge_and_midpoint,
                        max_faces_stopping_condition(m, orig_m, max_m),
                        always_try, never_care, E, EMAP, EF, EI, U, G, J, I);
...
IGL_INLINE bool
igl::decimate(const Eigen::MatrixXd &OV, const Eigen::MatrixXi &OF,
              ...
              Eigen::VectorXi &I)
{
    ...
    decimate_post_collapse_callback never_care;
    decimate_trivial_callbacks(always_try, never_care);
    return igl::decimate(OV, OF, cost_and_placement, stopping_condition,
                         always_try, never_care, U, G, J, I);
}
              ...
              Eigen::VectorXi &I)
{
    ...
    return igl::decimate(OV, OF, cost_and_placement, stopping_condition,
                         pre_collapse, post_collapse, E, EMAP, EF, EI, U, G, J,
                         I);
}
...
igl::decimate(const Eigen::MatrixXd &OV, const Eigen::MatrixXi &OF,
              ...
              const decimate_pre_collapse_callback &pre_collapse,
              const decimate_post_collapse_callback &post_collapse,
              const Eigen::MatrixXi & /*OE*/, const Eigen::VectorXi & /*OEMAP*/,
              const Eigen::MatrixXi & /*OEF*/, const Eigen::MatrixXi & /*OEI*/,
              Eigen::MatrixXd &U, Eigen::MatrixXi &G, Eigen::VectorXi &J,
              Eigen::VectorXi &I)
{
    // Decimate 1
    ...
    while (true) {
        int e, e1, e2, f1, f2;
        if (collapse_edge(cost_and_placement, pre_collapse, post_collapse, V, F,
                          E, EMAP, EF, EI, Q, EQ, C, e, e1, e2, f1, f2)) {
            if (stopping_condition(V, F, E, EMAP, EF, EI, Q, EQ, C, e, e1, e2,
                                   f1, f2)) {
                clean_finish = true;
                break;
            }
            ...
            if (prev_e == e) {
                assert(false &&
                       "Edge collapse no progress... bad stopping condition?");
                break;
            }
            // Edge was not collapsed... must have been invalid. collapse_edge
            // should have updated its cost to inf... continue
        }
        prev_e = e;
    }

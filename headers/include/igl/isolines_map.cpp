...
IGL_INLINE void
igl::isolines_map(const Eigen::MatrixBase<DerivedCM> &CM,
...
{
    ICM.resize(CM.rows() * interval_thickness + (CM.rows() - 1) * iso_thickness,
               3);
    {
        int k = 0;
...
IGL_INLINE void igl::isolines_map(const Eigen::MatrixBase<DerivedCM> &CM,
                                  Eigen::PlainObjectBase<DerivedICM> &ICM)
{
    return isolines_map(
        CM, Eigen::Matrix<typename DerivedCM::Scalar, 1, 3>(0, 0, 0), 10, 1,
        ICM);
}

#ifdef IGL_STATIC_LIBRARY
// Explicit template instantiation
template void igl::isolines_map<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

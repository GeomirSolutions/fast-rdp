                                ...
                                Eigen::PlainObjectBase<DerivedB> &B)
{
    ...
    // (2.*(u - cu)/w - 1) * (M.block(3,0,1,3)*x + M(3,3)) - M.block(0,0,1,3)*x
    // - M(0,3) = 0 (2.*(u - cu)/w - 1) * (M.block(3,0,1,3)*x + M(3,3)) -
    // M.block(0,0,1,3)*x = M(0,3)
    // ((2.*(u - cu)/w - 1) * M.block(3,0,1,3) - M.block(0,0,1,3))*x = M(0,3) -
    // (2.*(u - cu)/w - 1)*M(3,3)
    Eigen::Matrix<Scalar, 4, 4> M = _M.template cast<Scalar>();
    A.resize(2, 3);
    A << ((2. * (u - cu) / w - 1.) * M.block(3, 0, 1, 3) - M.block(0, 0, 1, 3)),
        ((2. * (v - cv) / h - 1.) * M.block(3, 0, 1, 3) - M.block(1, 0, 1, 3));
    B.resize(2, 1);
    B << M(0, 3) - (2. * (u - cu) / w - 1.) * M(3, 3),
...
template void igl::projection_constraint<
    Eigen::Matrix<double, -1, 1, 0, 3, 1>, Eigen::Matrix<float, 4, 4, 0, 4, 4>,
    Eigen::Matrix<float, 4, 1, 0, 4, 1>, Eigen::Matrix<double, 2, 3, 0, 2, 3>,
    Eigen::Matrix<double, 2, 1, 0, 2, 1>>(
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 1, 0, 3, 1>> const &,
...
// generated by autoexplicit.sh
template void igl::projection_constraint<
    Eigen::Matrix<double, 2, 1, 0, 2, 1>, Eigen::Matrix<float, 4, 4, 0, 4, 4>,
    Eigen::Matrix<float, 4, 1, 0, 4, 1>, Eigen::Matrix<double, 2, 3, 0, 2, 3>,
    Eigen::Matrix<double, 2, 1, 0, 2, 1>>(

...
///
/// \note At least for Dense matrices this is replaced by `replicate` e.g., dst
/// = src.replicate(n,m);
/// http://forum.kde.org/viewtopic.php?f=74&t=90876#p173517
///
template <typename DerivedA, typename DerivedB>
IGL_INLINE void repmat(const Eigen::MatrixBase<DerivedA> &A, const int r,
                       const int c, Eigen::PlainObjectBase<DerivedB> &B);
/// \overload
template <typename T, int majorType>
IGL_INLINE void repmat(const Eigen::SparseMatrix<T, majorType> &A, const int r,
                       const int c, Eigen::SparseMatrix<T, majorType> &B);
} // namespace igl

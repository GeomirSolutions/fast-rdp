    ...
    const Eigen::Matrix<Scalar, n, 1> &ub)
{
    ...
    // iterations of igl::quadprog become very fast. Even if it ends up doing
    // many more iterations than igl::copyleft::quadprog it would be much faster
    // (in reality it doesn't do that many more iterations). It's a healthy
    // 10-100x faster than igl::copyleft::quadprog for specific cases of QPs.
    //
    // Unfortunately, that set is limited. igl::quadprog is really good at tiny
    // box-constrained QPs with a positive definite objective (like the kind
    // that show up in dual contouring). igl::copyleft::quadprog handles more
    // general problems (and also starts to beat igl::quadprog when the number
    // of variables gets over ~20). I tried extending igl::quadprog so that we
    // could use it for igl::copyleft::progressive_hulls and drop
    // igl::copyleft::quadprog but it was trickier than I thought. Something
    // like qpmad or the non GPL version of quadrog++ would be good future PR.
    //
    ...
    for (iter = 0; iter < dyn_n * dyn_n * dyn_n; iter++) {
        ...
        Eigen::Index best_add = -1;
        Scalar worst_offense = 0;
        bool add_lower;
        Eigen::Index best_remove = -1;
        Scalar worst_lambda = 0;
...
IGL_INLINE Eigen::Matrix<Scalar, n, 1>
igl::quadprog(const Eigen::Matrix<Scalar, n, n> &H,
              ...
              const Eigen::Matrix<Scalar, n, 1> &ub)
{
    ...
    return quadprog<Scalar, n, m>(H, f,
                                  Eigen::Matrix<Scalar, m, n>(0, H.cols()),
                                  Eigen::Matrix<Scalar, m, 1>(0, 1), lb, ub);
}

                                         ...
                                         Eigen::VectorXi &J)
{
    ...
    // direction doesn't change pointset, inf otherwise) and placement (in
    // lowest cost direction).
    const auto &perfect =
        [&N](const int e, const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
             const Eigen::MatrixXi &E, const Eigen::VectorXi &EMAP,
             const Eigen::MatrixXi &EF, const Eigen::MatrixXi &EI, double &cost,
             Eigen::RowVectorXd &p) {
            // Function for ocmputing cost (0 or inf) of collapsing edge by
            // placing vertex at `positive` end of edge.
            const auto &perfect_directed =
                [&N](const int e, const bool positive, const Eigen::MatrixXd &V,
                    ...
                    std::vector<int> faces =
                        igl::circulation(e, positive, EMAP, EF, EI);
                    cost = 0;
                    for (auto f : faces) {
    ...
    Eigen::VectorXi I;
    igl::decimate(OV, OF, perfect,
                  igl::infinite_cost_stopping_condition(perfect), V, F, J, I);
}

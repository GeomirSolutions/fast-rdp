...
IGL_INLINE double igl::mapping_energy_with_jacobians(
    const Eigen::MatrixXd &Ji, const Eigen::VectorXd &areas,
    ...
    if (Ji.cols() == 4) {
        ...
        for (int i = 0; i < Ji.rows(); i++) {
            ...
            case igl::MappingEnergyType::EXP_SYMMETRIC_DIRICHLET: {
                energy +=
                    areas(i) * exp(exp_factor * (pow(s1, 2) + pow(s1, -2) +
                                                 pow(s2, 2) + pow(s2, -2)));
                break;
            }
            ...
            case igl::MappingEnergyType::CONFORMAL: {
                energy +=
                    areas(i) * ((pow(s1, 2) + pow(s2, 2)) / (2 * s1 * s2));
                break;
            }
            case igl::MappingEnergyType::EXP_CONFORMAL: {
                energy +=
                    areas(i) * exp(exp_factor *
                                   ((pow(s1, 2) + pow(s2, 2)) / (2 * s1 * s2)));
                break;
            }
            default:
                assert(false);
        ...
        for (int i = 0; i < Ji.rows(); i++) {
            ...
            case igl::MappingEnergyType::SYMMETRIC_DIRICHLET: {
                energy += areas(i) * (pow(s1, 2) + pow(s1, -2) + pow(s2, 2) +
            ...
            case igl::MappingEnergyType::EXP_SYMMETRIC_DIRICHLET: {
                energy +=
                    areas(i) *
                    exp(exp_factor * (pow(s1, 2) + pow(s1, -2) + pow(s2, 2) +
                                      pow(s2, -2) + pow(s3, 2) + pow(s3, -2)));
                break;
            }
            case igl::MappingEnergyType::LOG_ARAP: {
                energy +=
                    areas(i) * (pow(log(s1), 2) + pow(log(std::abs(s2)), 2) +
            ...
            case igl::MappingEnergyType::EXP_CONFORMAL: {
                energy +=
                    areas(i) *
                    exp(exp_factor * (pow(s1, 2) + pow(s2, 2) + pow(s3, 2)) /
                        (3 * pow(s1 * s2 * s3, 2. / 3.)));
                break;
            }
            default:

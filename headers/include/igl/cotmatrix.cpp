                               ...
                               Eigen::SparseMatrix<Scalar> &L)
{
    ...
    if (simplex_size == 3) {
        // This is important! it could decrease the comptuation time by a factor
        // of 2 Laplacian for a closed 2d manifold mesh will have on average 7
        // entries per row
        L.reserve(10 * V.rows());
        edges.resize(3, 2);
        edges << 1, 2, 2, 0, 0, 1;
                               ...
                               Eigen::SparseMatrix<Scalar> &P)
{
    ...
    for (Index p = 0; p < C.size() - 1; p++) {
        ...
        // products below.
        Eigen::Matrix<Scalar, Eigen::Dynamic, 3> X =
            decltype(X)::Zero(np + 1, 3);
        for (Index i = 0; i < np; i++) {
            X.row(i).head(V.cols()) = V.row(I(C(p) + i));
        };
        // determine weights definig position of inserted vertex
        {
            ...
            for (Index k = 0; k < np; k++) {
                ...
                for (Index i = 0; i < np; i++) {
                    ...
                    for (Index j = 0; j < np; j++) {
                        A(i, j) += 2. * (X.row(j).cross(Xkp1mk))
                                            .dot(X.row(i).cross(Xkp1mk));
                    }
                }
            }
            ...
            b(np) = 1;
            const VectorXS w =
                Eigen::CompleteOrthogonalDecomposition<Eigen::MatrixXd>(A)
                    .solve(b);
            X.row(np) = w.transpose() * X.topRows(np);
            // scatter w into new row of P
            for (Index i = 0; i < np; i++) {
                Pijv.emplace_back(n + p, I(C(p) + i), w(i));
            }
        }
        ...
        const auto J = [&n, &np, &p, &I, &C](Index i) -> Index {
            return i == np ? n + p : I(C(p) + i);
        };
        // Should just build Mf as a vector...
        for (Index i = 0; i < np + 1; i++) {
            Mfijv.emplace_back(J(i), J(i), Mp(i));
        }
        // loop over faces
...
template void igl::cotmatrix<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::SparseMatrix<double, 0, int> &);
// generated by autoexplicit.sh
template void igl::cotmatrix<Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                             Eigen::Matrix<int, -1, -1, 0, -1, -1>, double>(

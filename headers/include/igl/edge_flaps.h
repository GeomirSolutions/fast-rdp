...
/// unique
///     unique edge in uE
/// @param[out] EF  #E by 2 list of edge flaps, EF(e,0)=f means e=(i-->j) is the
/// edge of
...
/// all(efEMAP(sub2ind(size(F),repmat(1:size(F,1),3,1)',repmat([1 2
/// 3],size(F,1),1))) == etFE(:,[2 3 1])) \endcode
IGL_INLINE void edge_flaps(const Eigen::MatrixXi &F, const Eigen::MatrixXi &uE,
                           const Eigen::VectorXi &EMAP, Eigen::MatrixXi &EF,
                           Eigen::MatrixXi &EI);
/// \overload
IGL_INLINE void edge_flaps(const Eigen::MatrixXi &F, Eigen::MatrixXi &uE,

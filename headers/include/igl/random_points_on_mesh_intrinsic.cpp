...
template <typename DeriveddblA, typename DerivedB, typename DerivedFI,
          typename URBG>
IGL_INLINE void igl::random_points_on_mesh_intrinsic(
    const int n, const Eigen::MatrixBase<DeriveddblA> &dblA,
    Eigen::PlainObjectBase<DerivedB> &B, Eigen::PlainObjectBase<DerivedFI> &FI,
    URBG &&urbg)
{
    ...
    const VectorXs R =
        (VectorXs::NullaryExpr(n, 1, [&]() { return dis(urbg); }).array() +
         1.) /
        2.;
    assert(R.minCoeff() >= 0);
    assert(R.maxCoeff() <= 1);
    ...
    FI = FI.array().min(
        static_cast<typename DerivedFI::Scalar>(dblA.rows() - 1));
    const VectorXs S =
        (VectorXs::NullaryExpr(n, 1, [&]() { return dis(urbg); }).array() +
         1.) /
        2.;
...
template <typename DeriveddblA, typename DerivedF, typename ScalarB,
          typename DerivedFI, typename URBG>
IGL_INLINE void igl::random_points_on_mesh_intrinsic(
    const int n, const Eigen::MatrixBase<DeriveddblA> &dblA,
    const int num_vertices, const Eigen::MatrixBase<DerivedF> &F,
    Eigen::SparseMatrix<ScalarB> &B, Eigen::PlainObjectBase<DerivedFI> &FI,
    URBG &&urbg)
{
    using namespace Eigen;
    using namespace std;
...
template void
igl::random_points_on_mesh_intrinsic<Eigen::Matrix<double, -1, 1, 0, -1, 1>,

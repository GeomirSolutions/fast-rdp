...
struct PlyProperty
{
    ...
    PlyProperty(Type list_type, Type prop_type, std::string &_name,
                size_t list_count)
        : name(_name), propertyType(prop_type), isList(true),
          listType(list_type), listCount(list_count)
    {
    }
    std::string name;
...
struct PlyFile
{
     ...
     * case of writing, get_comments() reference may also be used to add new
     * comments to the ply header.
     */

...
IGL_INLINE void
find(const Eigen::SparseMatrix<T> &X, Eigen::DenseBase<DerivedI> &I,
     Eigen::DenseBase<DerivedJ> &J, Eigen::DenseBase<DerivedV> &V);
/// \overload
template <typename T>
IGL_INLINE std::vector<Eigen::Triplet<T>> find(const Eigen::SparseMatrix<T> &X);
/// \overload
template <typename DerivedX, typename DerivedI, typename DerivedJ,
          typename DerivedV>
IGL_INLINE void
find(const Eigen::DenseBase<DerivedX> &X, Eigen::PlainObjectBase<DerivedI> &I,
     Eigen::PlainObjectBase<DerivedJ> &J, Eigen::PlainObjectBase<DerivedV> &V);
/// \overload
///
/// @param[out] I  nnz vector of indices into vectorization of X
template <typename DerivedX, typename DerivedI>
IGL_INLINE void find(const Eigen::DenseBase<DerivedX> &X,
...
find(const Eigen::Array<bool, RowsAtCompileTime, 1, 0, MaxRowsAtCompileTime, 1>
         &M);
} // namespace igl

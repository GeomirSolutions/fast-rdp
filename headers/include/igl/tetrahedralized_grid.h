...
/// @param[out] GT  (nx-1)*(ny-1)*(nz-1)*k by 4 list of tetrahedron indices into
/// rows of
...
IGL_INLINE void tetrahedralized_grid(const int nx, const int ny, const int nz,
...
IGL_INLINE void tetrahedralized_grid(const Eigen::MatrixBase<Derivedside> &side,
                                     const TetrahedralizedGripType type,
                                     Eigen::PlainObjectBase<DerivedGT> &GT);
} // namespace igl

...
/// @tparam  MatC  matrix type for output matrix (e.g. MatrixXd) needs to
/// support
///     resize
...
/// @param[out]  C  output matrix
///
/// \note If you're using Dense matrices you might be better off using the <<
/// operator
...
///
/// @param[in] dim  dimension along which to concatenate, 1 or 2
/// @param[in] A  std::vector of eigen matrices. Must have identical # cols if
/// dim == 1 or rows if dim == 2
/// @param[out] C  output matrix
template <typename T, typename DerivedC>
IGL_INLINE void cat(const int dim, const std::vector<T> &A,

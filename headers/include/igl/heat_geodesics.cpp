                               ...
                               const Scalar t, HeatGeodesicsData<Scalar> &data)
{
    ...
    if (data.use_intrinsic_delaunay) {
        ...
        igl::massmatrix_intrinsic(l_intrinsic, F_intrinsic,
                                  MASSMATRIX_TYPE_DEFAULT, M);
    ...
    igl::unique(O, data.b);
    {
        ...
        L *= -0.5;
        if (!igl::min_quad_with_fixed_precompute(L, Eigen::VectorXi(), Aeq,
                                                 true, data.Poisson)) {
            return false;
        }
    }
                          ...
                          Eigen::PlainObjectBase<DerivedD> &D)
{
    ...
    for (int i = 0; i < m; i++) {
        ...
        // calculation will suffer from underflow. Dividing by the max value is
        // more stable. (Eigen implements this as stableNorm or blueNorm).
        Scalar norm = 0;
        Scalar ma = 0;
        for (int d = 0; d < data.ng; d++) {
            ma = std::max(ma, std::fabs(grad_u(d * m + i)));
        }
        for (int d = 0; d < data.ng; d++) {
    ...
    igl::min_quad_with_fixed_solve(data.Poisson, (-div_X).eval(), DerivedD(),
                                   Beq, D);

...
template <typename DerivedV, typename DerivedT, typename SType,
          typename DerivedW>
IGL_INLINE bool
igl::biharmonic_coordinates(const Eigen::MatrixBase<DerivedV> &V,
                            ...
                            const int k, Eigen::PlainObjectBase<DerivedW> &W)
{
    ...
    SparseMatrix<Scalar> A;
    {
        ...
        // normalize
        M /=
            ((Matrix<Scalar, Dynamic, 1>)M.diagonal()).array().abs().maxCoeff();
        Minv = ((Matrix<Scalar, Dynamic, 1>)M.diagonal().array().inverse())
                   .asDiagonal();
#else
        ...
        Eigen::SparseMatrix<Scalar> Ad(E.rows(), V.rows());
        {
            ...
            for (int e = 0; e < E.rows(); e++) {
                for (int c = 0; c < E.cols(); c++) {
                    AIJV[e + c * E.rows()] =
                        Eigen::Triplet<Scalar>(e, E(e, c), 1);
                }
            }
    ...
    Matrix<Scalar, Dynamic, Dynamic> J =
        Matrix<Scalar, Dynamic, Dynamic>::Zero(mp + mr, mp + r * (dim + 1));
    ...
    return min_quad_with_fixed(
        A, Matrix<Scalar, Dynamic, 1>::Zero(A.rows()).eval(), b, J,
        SparseMatrix<Scalar>(), Matrix<Scalar, Dynamic, 1>(), true, W);
}

...
template <typename Scalar> class DualContouring
{
    // Types
  public:
  ...
  public:
    ...
    std::vector<Eigen::RowVector3i,
                Eigen::aligned_allocator<Eigen::RowVector3i>>
        vI;
  ...
  public:
    DualContouring(
        ...
        const bool _constrained = false, const bool _triangles = false,
        const bool _root_finding = true)
        : f(_f), f_grad(_f_grad), constrained(_constrained),
          triangles(_triangles), root_finding(_root_finding), n(0), C2V(0),
          vV(), vI(), vH(), vcount(), m(0)
    {
    }
    ...
    Eigen::RowVector3i inverse_primal(const RowVector3S &x) const
    {
        ...
        return ((x - min_corner).array() / step.array())
            .round()
            .template cast<int>();
    }
    // Inputs:
    ...
    bool single_edge(const int &x, const int &y, const int &z, const int &o,
                     const RowVector3S &e0, const Scalar &f0)
    {
    ...
    bool single_edge(const int &x, const int &y, const int &z, const int &o,
                     const RowVector3S &e0, const Scalar &f0,
                     const RowVector3S &e1, const Scalar &f1)
    {
        const Scalar isovalue = 0;
        if ((f0 > isovalue) == (f1 > isovalue)) {
            return false;
        }
        ...
        }
        {
            ...
            if (triangles) {
                if (m + 4 >= Q.rows()) {
                    Q.conservativeResize(2 * m + 4, Q.cols());
                }
                if (f0 > f1) {
                ...
                if (m + 1 >= Q.rows()) {
                    Q.conservativeResize(2 * m + 1, Q.cols());
                }
                if (f0 > f1) {
        ...
        igl::parallel_for(
            ...
            [&](const Eigen::Index v) {
                ...
                const RowVector3S p0 =
                    min_corner +
                    ((vI[v].template cast<Scalar>().array()) * step.array())
                        .matrix();
                const RowVector3S x =
                    constrained
                        ? igl::quadprog<Scalar, 3>(A, (p0 * A - b).transpose(),
                                                   Vector3S(0, 0, 0),
                                                   step.transpose())
                        : Eigen::LLT<Matrix3S>(A).solve(
                              -(p0 * A - b).transpose());
                V.row(v) = p0 + x;
            },
            1000ul);
    ...
    void dense(const RowVector3S &_min_corner, const RowVector3S &max_corner,
               const int nx, const int ny, const int nz)
    {
        min_corner = _min_corner;
        step = (max_corner - min_corner).array() /
               (RowVector3S(nx, ny, nz).array() - 1);
                ...
                for (int y = 0; y < ny; y++) {
                    for (int z = 0; z < nz; z++) {
                        const RowVector3S e0 =
                            primal(Eigen::RowVector3i(x, y, z));
                        const Scalar f0 = f(e0);
                        // we'll consider the edges going "back" from this
                        // vertex
                        for (int o = 0; o < 3; o++) {
    ...
    void dense(const Eigen::MatrixBase<DerivedGf> &Gf,
               const Eigen::MatrixBase<DerivedGV> &GV, const int nx,
               const int ny, const int nz)
    {
        ...
        const auto xyz2i = [&nx, &ny](const int &x, const int &y,
                                      const int &z) -> Eigen::Index {
            return x + nx * (y + ny * (z));
        };
    ...
    void sparse(const RowVector3S &_step,
                ...
                const Eigen::Matrix<int, Eigen::Dynamic, 2> &GI)
    {
        ...
        for (Eigen::Index i = 0; i < GI.rows(); i++) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                if (ic0(j) - ic1(j) == 1) {
                    assert(o == -1 &&
                           "Edges should differ in just one coordinate");
                    o = j;
...
IGL_INLINE void igl::dual_contouring(
    const std::function<typename DerivedV::Scalar(
        const Eigen::Matrix<typename DerivedV::Scalar, 1, 3> &)> &f,
    const std::function<Eigen::Matrix<typename DerivedV::Scalar, 1, 3>(
    ...
    const int nx, const int ny, const int nz, const bool constrained,
    const bool triangles, const bool root_finding,
    Eigen::PlainObjectBase<DerivedV> &V, Eigen::PlainObjectBase<DerivedQ> &Q)
{
...
template <typename DerivedGf, typename DerivedGV, typename DerivedV,
          typename DerivedQ>
IGL_INLINE void igl::dual_contouring(
    const std::function<typename DerivedV::Scalar(
        const Eigen::Matrix<typename DerivedV::Scalar, 1, 3> &)> &f,
    ...
    const Eigen::MatrixBase<DerivedGV> &GV, const int nx, const int ny,
    const int nz, const bool constrained, const bool triangles,
    const bool root_finding, Eigen::PlainObjectBase<DerivedV> &V,
...
template <typename DerivedGf, typename DerivedGV, typename DerivedGI,
          typename DerivedV, typename DerivedQ>
IGL_INLINE void igl::dual_contouring(
    const std::function<typename DerivedV::Scalar(
        const Eigen::Matrix<typename DerivedV::Scalar, 1, 3> &)> &f,
    const std::function<Eigen::Matrix<typename DerivedV::Scalar, 1, 3>(
    ...
    const Eigen::MatrixBase<DerivedGI> &GI, const bool constrained,
    const bool triangles, const bool root_finding,
    Eigen::PlainObjectBase<DerivedV> &V, Eigen::PlainObjectBase<DerivedQ> &Q)
{
    if (GI.rows() == 0) {
...
template void igl::dual_contouring<Eigen::Matrix<double, -1, 1, 0, -1, 1>,
                                   Eigen::Matrix<double, -1, 3, 1, -1, 3>,
                                   Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                                   Eigen::Matrix<int, -1, -1, 0, -1, -1>>(
    std::function<Eigen::Matrix<double, -1, -1, 0, -1, -1>::Scalar(
        Eigen::Matrix<Eigen::Matrix<double, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1,
                      1, 3> const &)> const &,
    std::function<Eigen::Matrix<
        Eigen::Matrix<double, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1, 1, 3>(
        Eigen::Matrix<Eigen::Matrix<double, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1,
                      1, 3> const &)> const &,
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 3, 1, -1, 3>> const &, int, int,
    int, bool, bool, bool,
...
template void igl::dual_contouring<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    std::function<Eigen::Matrix<
        ...
        Eigen::Matrix<Eigen::Matrix<double, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1,
                      1, 3> const &)> const &,
    Eigen::Matrix<Eigen::Matrix<double, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1, 1,
                  3> const &,
    Eigen::Matrix<Eigen::Matrix<double, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1, 1,
                  3> const &,
...
template void igl::dual_contouring<Eigen::Matrix<double, -1, 1, 0, -1, 1>,
    ...
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> const &, bool,
    bool, bool,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &);
template void igl::dual_contouring<Eigen::Matrix<double, -1, 1, 0, -1, 1>,
    ...
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, -1, 2, 0, -1, 2>> const &, bool, bool,
    bool, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &);
template void igl::dual_contouring<Eigen::Matrix<float, -1, 1, 0, -1, 1>,
                                   Eigen::Matrix<float, -1, 3, 1, -1, 3>,
                                   Eigen::Matrix<float, -1, -1, 0, -1, -1>,
                                   Eigen::Matrix<int, -1, -1, 0, -1, -1>>(
    std::function<Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar(
        Eigen::Matrix<Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1,
                      1, 3> const &)> const &,
    std::function<Eigen::Matrix<Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar,
                                1, 3, 1, 1, 3>(
        Eigen::Matrix<Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1,
                      1, 3> const &)> const &,
    Eigen::MatrixBase<Eigen::Matrix<float, -1, 1, 0, -1, 1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<float, -1, 3, 1, -1, 3>> const &, int, int,
    int, bool, bool, bool,
...
template void igl::dual_contouring<Eigen::Matrix<float, -1, 1, 0, -1, 1>,
    ...
    std::function<Eigen::Matrix<Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar,
        ...
        Eigen::Matrix<Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1,
                      1, 3> const &)> const &,
    Eigen::MatrixBase<Eigen::Matrix<float, -1, 1, 0, -1, 1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<float, -1, -1, 0, -1, -1>> const &, int,
    int, int, bool, bool, bool,
    Eigen::PlainObjectBase<Eigen::Matrix<float, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &);
template void igl::dual_contouring<Eigen::Matrix<float, -1, -1, 0, -1, -1>,
    ...
    std::function<Eigen::Matrix<Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar,
        ...
        Eigen::Matrix<Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1,
                      1, 3> const &)> const &,
    Eigen::Matrix<Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1, 1,
                  3> const &,
    Eigen::Matrix<Eigen::Matrix<float, -1, -1, 0, -1, -1>::Scalar, 1, 3, 1, 1,
                  3> const &,
    int, int, int, bool, bool, bool,

...
struct ShapeupData
{
    ...
    ShapeupData()
        : maxIterations(50), pTolerance(10e-6), shapeCoeff(1.0),
          closeCoeff(100.0), smoothCoeff(0.0)
    {
    }
};
...
using shapeup_projection_function =
    std::function<bool(const Eigen::PlainObjectBase<Eigen::MatrixXd> &,
                       const Eigen::PlainObjectBase<Eigen::VectorXi> &,
                       const Eigen::PlainObjectBase<Eigen::MatrixXi> &,
                       Eigen::PlainObjectBase<Eigen::MatrixXd> &)>;
...
///             where the first three parameters are as defined, and "projP" is
///             the output, as a #S by 3*max(SC) function in format xyzxyzxyz,
///             and where it returns the projected points corresponding to each
///             set in S in the same order.
///            NOTE: the input values in P0 don't need to correspond to
///            prescribed values in bc; the iterations will project them
///            automatically (by design).
/// @param[in] P0  #P by 3 initial solution (point positions)
/// @param[in] sudata  the ShapeUpData structure computed in

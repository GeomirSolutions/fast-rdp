...
template <typename Derivedl_in, typename DerivedF_in, typename Derivedl,
          typename DerivedF>
IGL_INLINE void igl::intrinsic_delaunay_triangulation(
    ...
    Eigen::PlainObjectBase<Derivedl> &l, Eigen::PlainObjectBase<DerivedF> &F)
{
    typedef Eigen::Matrix<typename DerivedF::Scalar, Eigen::Dynamic, 2>
        MatrixX2I;
    typedef Eigen::Matrix<typename DerivedF::Scalar, Eigen::Dynamic, 1>
        VectorXI;
    MatrixX2I E, uE;
    VectorXI EMAP;
    std::vector<std::vector<typename DerivedF::Scalar>> uE2E;
    return intrinsic_delaunay_triangulation(l_in, F_in, l, F, E, uE, EMAP,
                                            uE2E);
}

template <typename Derivedl_in, typename DerivedF_in, typename Derivedl,
          typename DerivedF, typename DerivedE, typename DeriveduE,
          typename DerivedEMAP, typename uE2EType>
IGL_INLINE void igl::intrinsic_delaunay_triangulation(
    const Eigen::MatrixBase<Derivedl_in> &l_in,
    const Eigen::MatrixBase<DerivedF_in> &F_in,
    Eigen::PlainObjectBase<Derivedl> &l, Eigen::PlainObjectBase<DerivedF> &F,
    Eigen::PlainObjectBase<DerivedE> &E, Eigen::PlainObjectBase<DeriveduE> &uE,
    ...
    std::vector<std::vector<uE2EType>> &uE2E)
{
    ...
    // I tried using a "delaunay_since = iter" flag to avoid duplicates, but
    // there was no speed up.
    ...
    while (!Q.empty()) {
        ...
        // Expensive sanity check
        {
            ...
            for (Index uei = 0; uei < uE2E.size(); uei++) {
                if (!inQ(uei) &&
                    !is_intrinsic_delaunay(l, uE2E, num_faces, uei)) {
                    std::cout << "  " << uei << " might never be fixed!"
                              << std::endl;
                }
            }
        }
        ...
        //        c/ | \b            c/f1 \b
                ...
                IGL_ASSERT(uE2E[uei].size() == 2 &&
                           "edge should have 2 incident faces");
                ...
                IGL_ASSERT(std::abs(l(f1, c1) - l(f2, c2)) <
                           igl::EPS<Scalar>());
                ...
                // tan((α+δ)/2)
                const Scalar tan_a_d_2 =
                    (tan_a_2 + tan_d_2) / (1.0 - tan_a_2 * tan_d_2);
                // cos(α+δ)
                const Scalar cos_a_d = (1.0 - tan_a_d_2 * tan_a_d_2) /
                                       (1.0 + tan_a_d_2 * tan_a_d_2);
                ...
                l(f2, 1) = d;
                l(f2, 2) = a;
                // Important to grab these indices _before_ calling flip_edges
                // (they will be correct after)
...
template void
igl::intrinsic_delaunay_triangulation<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
...
// generated by autoexplicit.sh
template void
igl::intrinsic_delaunay_triangulation<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                                      Eigen::Matrix<int, -1, -1, 0, -1, -1>,
                                      Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                                      Eigen::Matrix<int, -1, -1, 0, -1, -1>,
                                      Eigen::Matrix<int, -1, -1, 0, -1, -1>,

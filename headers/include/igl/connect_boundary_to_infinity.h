...
/// @param[out] FO  #F+#O by 3 list of face indices into [V;inf inf inf],
/// original F are
...
IGL_INLINE void
connect_boundary_to_infinity(const Eigen::MatrixBase<DerivedF> &F,
                             Eigen::PlainObjectBase<DerivedFO> &FO);
/// Connect all boundary edges to a fictitious point at infinity.
///
/// @param[in] F  #F by 3 list of face indices into some V
/// @param[in] inf_index  index of point at infinity (usually V.rows() or
/// F.maxCoeff())
/// @param[out] FO  #F+#O by 3 list of face indices into [V;inf inf inf],
/// original F are
///     guaranteed to come first. If (V,F) was a manifold mesh, now it is
...
/// @param[in] F  #F by 3 list of face indices into some V
/// @param[out] VO  #V+1 by 3 list of vertex positions, original V are
/// guaranteed to
///     come first. Last point is inf, inf, inf
/// @param[out] FO  #F+#O by 3 list of face indices into VO
///
template <typename DerivedV, typename DerivedF, typename DerivedVO,
          typename DerivedFO>
IGL_INLINE void
connect_boundary_to_infinity(const Eigen::MatrixBase<DerivedV> &V,

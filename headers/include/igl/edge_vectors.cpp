...
IGL_INLINE void igl::edge_vectors(const Eigen::MatrixBase<DerivedV> &V,
...
template <bool computePerpendicular, typename DerivedV, typename DerivedF,
          typename DerivedE, typename DerivedoE, typename DerivedvecParallel,
          typename DerivedvecPerpendicular>
IGL_INLINE void igl::edge_vectors(
    ...
    Eigen::PlainObjectBase<DerivedvecPerpendicular> &vecPerpendicular)
{
    ...
    for (Eigen::Index i = 0; i < E.rows(); ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            const typename DerivedF::Scalar vi = F(i, (j + 1) % 3),
                                            vj = F(i, (j + 2) % 3);
            vecParallel.row(e) = (V.row(vj) - V.row(vi)).normalized();
            if (computePerpendicular) { // This should ideally be an if
                                        // constexpr
...
template void igl::edge_vectors<true, Eigen::Matrix<double, -1, -1, 0, -1, -1>,

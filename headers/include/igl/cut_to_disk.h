...
/// mesh into a topological disk, without disconnecting any connected
/// components. Nothing else about the cuts (including number, total length, or
/// smoothness) is guaranteed to be optimal.
///
...
/// "Geometry Images."
///
/// @tparam Index  Integrable type large enough to represent the total number of
/// faces
///     and edges in the surface represented by F, and all entries of F.
/// @param[in] F  #F by 3 list of the faces (must be triangles)
/// @param[out] cuts  List of cuts. Each cut is a sequence of vertex indices
/// (where
...
IGL_INLINE void cut_to_disk(const Eigen::MatrixBase<DerivedF> &F,

                                   ...
                                   int &e1, int &e2, int &f1, int &f2)
{
    ...
    return collapse_edge(e, p, Nsv, Nsf, Ndv, Ndf, V, F, E, EMAP, EF, EI, e1,
                         e2, f1, f2);
}
        ...
        for (auto f : nV2Fd) {
            for (int v = 0; v < 3; v++) {
                if (F(f, v) == d) {
                    ...
                    // Skip if we just handled this edge (claim: this will be
                    // all except for the first non-trivial face)
                    if (e2 != p1) {
...
IGL_INLINE bool igl::collapse_edge(
    ...
    igl::min_heap<std::tuple<double, int, int>> &Q, Eigen::VectorXi &EQ,
    Eigen::MatrixXd &C)
{
    int e, e1, e2, f1, f2;
    ...
    return collapse_edge(cost_and_placement, always_try, never_care, V, F, E,
                         EMAP, EF, EI, Q, EQ, C, e, e1, e2, f1, f2);
}

IGL_INLINE bool igl::collapse_edge(
    const decimate_cost_and_placement_callback &cost_and_placement,
    const decimate_pre_collapse_callback &pre_collapse,
    const decimate_post_collapse_callback &post_collapse, Eigen::MatrixXd &V,
    Eigen::MatrixXi &F, Eigen::MatrixXi &E, Eigen::VectorXi &EMAP,
    Eigen::MatrixXi &EF, Eigen::MatrixXi &EI,
    igl::min_heap<std::tuple<double, int, int>> &Q, Eigen::VectorXi &EQ,
    Eigen::MatrixXd &C)
{
    int e, e1, e2, f1, f2;
    return collapse_edge(cost_and_placement, pre_collapse, post_collapse, V, F,
                         E, EMAP, EF, EI, Q, EQ, C, e, e1, e2, f1, f2);
}
    ...
    Eigen::MatrixXd &C, int &e, int &e1, int &e2, int &f1, int &f2)
{
    ...
    // If we just need original face neighbors of edge, could we gather that
    // more directly than gathering face neighbors of each vertex?
    std::vector<int> /*Nse,*/ Nsf, Nsv;
    circulation(e, true, F, EMAP, EF, EI, /*Nse,*/ Nsv, Nsf);
    std::vector<int> /*Nde,*/ Ndf, Ndv;
    circulation(e, false, F, EMAP, EF, EI, /*Nde,*/ Ndv, Ndf);
    ...
    if (pre_collapse(V, F, E, EMAP, EF, EI, Q, EQ, C, e)) {
        collapsed = collapse_edge(e, C.row(e), Nsv, Nsf, Ndv, Ndf, V, F, E,
                                  EMAP, EF, EI, e1, e2, f1, f2);
    } else {
        // Aborted by pre collapse callback
        collapsed = false;
    }
    post_collapse(V, F, E, EMAP, EF, EI, Q, EQ, C, e, e1, e2, f1, f2,
                  collapsed);
    if (collapsed) {
        // Erase the two, other collapsed edges by marking their timestamps as
        // -1
        EQ(e1) = -1;
        EQ(e2) = -1;
        ...
        // **not** have given this un-collapsable edge inf cost already)
        // Increment timestamp
        EQ(e)++;
        // Replace in queue

...
igl::turning_number(const Eigen::MatrixBase<DerivedV> &V)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        const Scalar angle =
            atan2(d1(0) * d2(1) - d1(1) * d2(0), d1(0) * d2(0) + d1(1) * d2(1));
        total_angle += angle;
    }
...
template double igl::turning_number<Eigen::Matrix<double, -1, 2, 0, -1, 2>>(
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 2, 0, -1, 2>> const &);
template double igl::turning_number<Eigen::Matrix<double, -1, -1, 0, -1, -1>>(

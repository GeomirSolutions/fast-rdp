...
igl::is_irregular_vertex(const Eigen::MatrixBase<DerivedF> &F)
{
    ...
    if (F.cols() == 3) {
        ...
        Eigen::Matrix<typename DerivedF::Scalar, Eigen::Dynamic, Eigen::Dynamic>
            T(2 * F.rows(), 3);
        T << F.col(0), F.col(1), F.col(2), F.col(0), F.col(2), F.col(3);
...
igl::is_irregular_vertex<Eigen::Matrix<int, -1, 3, 0, -1, 3>>(
    Eigen::MatrixBase<Eigen::Matrix<int, -1, 3, 0, -1, 3>> const &);
template std::vector<bool, std::allocator<bool>>
igl::is_irregular_vertex<Eigen::Matrix<int, -1, -1, 0, -1, -1>>(

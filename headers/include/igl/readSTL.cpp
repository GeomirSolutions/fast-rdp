                               ...
                               std::vector<std::array<TypeN, 3>> &N)
{
    ...
    auto parse_ascii_facet = [&parse_ascii_vertex,
                              &parse_ascii_normal](std::istream &fin) {
        constexpr size_t LINE_SIZE = 256;
                                ...
                                std::vector<std::array<TypeN, 3>> &N)
{
    ...
    for (size_t i = 0; i < num_faces; i++) {
        ...
        auto ny =
            static_cast<TypeN>(*reinterpret_cast<float *>(buf + FLOAT_SIZE));
        auto nz = static_cast<TypeN>(
            *reinterpret_cast<float *>(buf + FLOAT_SIZE * 2));
        ...
        auto v1y =
            static_cast<TypeV>(*reinterpret_cast<float *>(buf + FLOAT_SIZE));
        auto v1z = static_cast<TypeV>(
            *reinterpret_cast<float *>(buf + FLOAT_SIZE * 2));
        IGL_ASSERT(input.good());
...
template <typename DerivedV, typename DerivedF, typename DerivedN>
IGL_INLINE bool readSTL(FILE *fp, Eigen::PlainObjectBase<DerivedV> &V,
                        Eigen::PlainObjectBase<DerivedF> &F,
                        Eigen::PlainObjectBase<DerivedN> &N)
{
...
template bool igl::readSTL<Eigen::Matrix<double, -1, 3, 1, -1, 3>,
...
template bool igl::readSTL<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &);
template bool igl::readSTL<Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                           Eigen::Matrix<int, -1, -1, 0, -1, -1>,
                           Eigen::Matrix<double, -1, -1, 0, -1, -1>>(
...
template bool igl::readSTL<Eigen::Matrix<float, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &);
template bool igl::readSTL<Eigen::Matrix<float, -1, 3, 0, -1, 3>,
                           Eigen::Matrix<int, -1, 3, 0, -1, 3>,
                           Eigen::Matrix<double, -1, -1, 0, -1, -1>>(

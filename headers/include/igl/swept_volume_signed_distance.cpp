...
IGL_INLINE void igl::swept_volume_signed_distance(
    ...
    const size_t &steps, const Eigen::MatrixXd &GV,
    const Eigen::RowVector3i &res, const double h, const double isolevel,
    const Eigen::VectorXd &S0, Eigen::VectorXd &S)
{
    using namespace std;
    using namespace igl;
    ...
    Eigen::AlignedBox3d box(V.colwise().minCoeff().array() - extension,
    ...
    for (int ti = 0; ti < t.size(); ti++) {
        ...
        for (int g = 0; g < GV.rows(); g++) {
            ...
            const double min_sqrd = finite_iso
...
#ifndef NDEBUG
        // Check for nans
        std::for_each(S.data(), S.data() + S.size(),
                      [](const double s) { assert(s == s); });
#endif
    }
}
    ...
    Eigen::VectorXd &S)
{
    ...
    return swept_volume_signed_distance(V, F, transform, steps, GV, res, h,
                                        isolevel, S, S);
}

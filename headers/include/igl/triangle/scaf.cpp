                              ...
                              Eigen::SparseMatrix<double> &Dz)
{
    ...
    standard_grad << -sin60 / deno, sin60 / deno, 0, -cos60 / deno,
        -cos60 / deno, 1 / deno, 0, 0, 0;
...
IGL_INLINE void mesh_improve(igl::triangle::SCAFData &s)
{
    ...
    if (s.rect_frame_V.size() == 0) {
        Matrix2d ob; // = rect_corners;
        {
            ...
            ob.row(0) =
                uv_mid.array() + scaf_range * ((uv_min - uv_mid).array());
            ob.row(1) =
                uv_mid.array() + scaf_range * ((uv_max - uv_mid).array());
        }
        Vector2d rect_len;
        ...
        s.frame_ids = Eigen::VectorXi::LinSpaced(
            s.rect_frame_V.rows(), s.mv_num, s.mv_num + s.rect_frame_V.rows());
    }
                              ...
                              const Eigen::MatrixXd &uv_init)
{
    ...
    for (auto cur_bnd : all_bnds) {
        s.internal_bnd.conservativeResize(s.internal_bnd.size() +
                                          cur_bnd.size());
                              ...
                              igl::MappingEnergyType energy_type)
{
    ...
    return energy +
           igl::mapping_energy_with_jacobians(Ji, areas, energy_type, 0);
}
                         ...
                         Eigen::VectorXd &f_rhs)
{
    ...
    for (int i = 0; i < f_n; i++) {
        ...
        f_rhs(i + 0 * f_n) =
            sqrt_area * (W(i, 0) * Ri(i, 0) + W(i, 1) * Ri(i, 1));
        f_rhs(i + 1 * f_n) =
            sqrt_area * (W(i, 0) * Ri(i, 2) + W(i, 1) * Ri(i, 3));
        f_rhs(i + 2 * f_n) =
            sqrt_area * (W(i, 2) * Ri(i, 0) + W(i, 3) * Ri(i, 1));
        f_rhs(i + 3 * f_n) =
            sqrt_area * (W(i, 2) * Ri(i, 2) + W(i, 3) * Ri(i, 3));
    }
}

IGL_INLINE void get_complement(const Eigen::VectorXi &bnd_ids, int v_n,
                                            ...
                                            Eigen::VectorXd &rhs)
{
    ...
    if (bnd_n == 0) {
        ...
        for (int d = 0; d < dim; d++) {
            ...
            unknown_ids.block(d * (v_n - n_b), 0, v_n - n_b,
                              unknown_ids.cols()) =
                                           ...
                                           Eigen::VectorXd &rhs)
{
    ...
    build_scaffold_linear_system(s, L_s,
                                 rhs_s); // complete As, without proximal
...
IGL_INLINE double perform_iteration(SCAFData &s)
{
    ...
    std::function<double(Eigen::MatrixXd &)> whole_E =
        [&s](Eigen::MatrixXd &uv) { return compute_energy(s, uv, true); };
    ...
    double soft_p)
{
    ...
    if (!data.has_pre_calc) {
        ...
        igl::triangle::scaf::compute_scaffold_gradient_matrix(s, s.Dx_s,
                                                              s.Dy_s);
                                                     ...
                                                     int iter_num)
{
    ...
    for (int it = 0; it < iter_num; it++) {
        s.total_energy = igl::triangle::scaf::compute_energy(s, s.w_uv, true) /
                         s.mesh_measure;
        ...
        s.energy = igl::triangle::scaf::compute_energy(s, s.w_uv, false) /
                   s.mesh_measure;
    }

...
/// @param[out] WE  #WE by 2 list of constraint edge indices into WV (might be
/// smaller
///     than E because degenerate constraints have been removed)
/// @param[out] J  #V list of indices into WF/WE for each vertex in V
///
template <typename DerivedV, typename DerivedE, typename DerivedWV,
          typename DerivedWF, typename DerivedWE, typename DerivedJ>
IGL_INLINE void
cdt(const Eigen::MatrixBase<DerivedV> &V, const Eigen::MatrixBase<DerivedE> &E,
    const std::string &flags, Eigen::PlainObjectBase<DerivedWV> &WV,
    Eigen::PlainObjectBase<DerivedWF> &WF,
    Eigen::PlainObjectBase<DerivedWE> &WE, Eigen::PlainObjectBase<DerivedJ> &J);
} // namespace triangle
} // namespace igl

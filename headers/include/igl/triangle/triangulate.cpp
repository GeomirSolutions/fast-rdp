...
template <typename DerivedV, typename DerivedE, typename DerivedH,
          typename DerivedV2, typename DerivedF2>
IGL_INLINE void igl::triangle::triangulate(
    const Eigen::MatrixBase<DerivedV> &V, const Eigen::MatrixBase<DerivedE> &E,
    const Eigen::MatrixBase<DerivedH> &H, const std::string flags,
...
template <typename DerivedV, typename DerivedE, typename DerivedH,
          typename DerivedVM, typename DerivedEM, typename DerivedV2,
          typename DerivedF2, typename DerivedVM2, typename DerivedE2,
          typename DerivedEM2>
IGL_INLINE void igl::triangle::triangulate(
    const Eigen::MatrixBase<DerivedV> &V, const Eigen::MatrixBase<DerivedE> &E,
    const Eigen::MatrixBase<DerivedH> &H,
    const Eigen::MatrixBase<DerivedVM> &VM,
    const Eigen::MatrixBase<DerivedEM> &EM, const std::string flags,
    Eigen::PlainObjectBase<DerivedV2> &V2,
    Eigen::PlainObjectBase<DerivedF2> &F2,
    ...
    Eigen::PlainObjectBase<DerivedEM2> &EM2)
{
    ...
    for (unsigned i = 0; i < V.rows(); ++i)
        in.pointmarkerlist[i] = VM.size() ? VM(i) : 1;
    ...
    for (unsigned i = 0; i < E.rows(); ++i)
        in.segmentmarkerlist[i] = EM.size() ? EM(i) : 1;
    ...
    // Return the mesh
    V2 = MapXdr(out.pointlist, out.numberofpoints, 2)
             .cast<typename DerivedV2::Scalar>();
    F2 = MapXir(out.trianglelist, out.numberoftriangles, 3)
             .cast<typename DerivedF2::Scalar>();
    E2 = MapXir(out.segmentlist, out.numberofsegments, 2)
             .cast<typename DerivedE2::Scalar>();
    if (VM.size()) {
        VM2 = MapXir(out.pointmarkerlist, out.numberofpoints, 1)
                  .cast<typename DerivedVM2::Scalar>();
    }
    if (EM.size()) {
        EM2 = MapXir(out.segmentmarkerlist, out.numberofsegments, 1)
                  .cast<typename DerivedEM2::Scalar>();
    }
...
template void
igl::triangle::triangulate<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
...
template void
igl::triangle::triangulate<Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                           Eigen::Matrix<int, -1, -1, 0, -1, -1>,

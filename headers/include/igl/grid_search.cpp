...
template <typename Scalar, typename DerivedX, typename DerivedLB,
          typename DerivedUB, typename DerivedI>
                                   ...
                                   DerivedX &X)
{
    ...
    int calls = 0;
    looper = [&](const int d, DerivedX &Xrun) {
        assert(d < dim);
        Eigen::Matrix<Scalar, Eigen::Dynamic, 1> vals =
            Eigen::Matrix<Scalar, Eigen::Dynamic, 1>::LinSpaced(I(d), LB(d),
                                                                UB(d));
        for (int c = 0; c < I(d); c++) {
            Xrun(d) = vals(c);
            if (d + 1 < dim) {
...
template double igl::grid_search<double, Eigen::Matrix<double, 1, 3, 1, 1, 3>,
                                 Eigen::Matrix<double, 1, 3, 1, 1, 3>,
                                 Eigen::Matrix<double, 1, 3, 1, 1, 3>,
                                 Eigen::Matrix<int, 1, 3, 1, 1, 3>>(
    std::function<double(Eigen::Matrix<double, 1, 3, 1, 1, 3> &)>,
    Eigen::MatrixBase<Eigen::Matrix<double, 1, 3, 1, 1, 3>> const &,
    Eigen::MatrixBase<Eigen::Matrix<double, 1, 3, 1, 1, 3>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, 1, 3, 1, 1, 3>> const &,
    Eigen::Matrix<double, 1, 3, 1, 1, 3> &);
template float igl::grid_search<float, Eigen::Matrix<float, 1, -1, 1, 1, -1>,

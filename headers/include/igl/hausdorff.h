...
IGL_INLINE void hausdorff(const Eigen::MatrixBase<DerivedVA> &VA,
                          const Eigen::MatrixBase<DerivedFA> &FA,
                          const Eigen::MatrixBase<DerivedVB> &VB,
                          const Eigen::MatrixBase<DerivedFB> &FB, Scalar &d);
/// Compute lower and upper bounds (l,u) on the Hausdorff distance between a
/// triangle (V) and a pointset (e.g., mesh, triangle soup) given by a distance
/// function handle (dist_to_B).
///
/// @param[in] V #V by 3 list of corner positions so that V.row(i) is the
/// position of the
///     ith corner
/// @param[in] dist_to_B  function taking the x,y,z coordinate of a query
/// position and
...
IGL_INLINE void hausdorff(
    ...
    const std::function<Scalar(const Scalar &, const Scalar &, const Scalar &)>
        &dist_to_B,
    Scalar &l, Scalar &u);
} // namespace igl

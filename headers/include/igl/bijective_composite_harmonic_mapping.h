...
namespace igl
{
/// Compute a injective planar mapping of a triangulated polygon (V,F) subjected
/// to boundary conditions (b,bc). The mapping should be bijective in the sense
...
IGL_INLINE bool
bijective_composite_harmonic_mapping(const Eigen::MatrixBase<DerivedV> &V,
...
///  @param[in] num_inner_iters  number of iterations of harmonic solves to run
///  after
///     for each morph step (to try to push flips back in)
///  @param[in] test_for_flips  whether to check if flips occurred (and trigger
///  more

...
template <typename DerivedS, typename DerivedGV, typename DerivedV,
          typename DerivedF>
IGL_INLINE void igl::marching_cubes(const Eigen::MatrixBase<DerivedS> &S,
                                    ...
                                    Eigen::PlainObjectBase<DerivedF> &F)
{
    ...
    const unsigned ioffset[8] = {
        0, 1, 1 + nx, nx, nx * ny, 1 + nx * ny, 1 + nx + nx * ny, nx + nx * ny};
    ...
    const auto xyz2i = [&nx, &ny](const int &x, const int &y,
                                  const int &z) -> unsigned {
        return x + nx * (y + ny * (z));
    };
    const auto cube = [&GV, &S, &V, &n, &F, &m, &isovalue, &E2V, &xyz2i,
                       &ioffset](const int x, const int y, const int z) {
    ...
    // Like red-black Gauss Seidel. Probably each thread need's their own
    // E2V,V,F, and then merge at the end. Annoying part are the edges lying on
...
template void igl::marching_cubes<Eigen::Matrix<float, -1, 1, 0, -1, 1>,
    ...
    Eigen::Matrix<float, -1, 1, 0, -1, 1>::Scalar,

...
IGL_INLINE bool _tinyply_buffer_to_matrix(tinyply::PlyData &D,
...
IGL_INLINE bool tinyply_buffer_to_matrix(tinyply::PlyData &D,
                                         Eigen::PlainObjectBase<Derived> &M,
                                         size_t rows, size_t cols)
{
    switch (D.t) {
...
template <typename T, typename Derived>
IGL_INLINE bool
_tinyply_tristrips_to_trifaces(tinyply::PlyData &D,
                               ...
                               size_t el_len)
{
    ...
    // to make it more interesting, triangles in triangle strip can be separated
    // by negative index elements
    ...
    // 2. convert triangles to faces, skipping over the negative indeces,
    // indicating separate strips
    ...
    for (size_t i = 0; i < el; i++) {
        ...
        for (size_t j = 0; j < (el_len - 2); j++) {
            if (_map(i, j) >= 0 && _map(i, j + 1) >= 0 && _map(i, j + 2) >= 0) {
                // consequtive faces on the same strip have to be flip-flopped,
                // to preserve orientation
                M(k, 0) = static_cast<typename Derived::Scalar>(_map(i, j));
                M(k, 1) = static_cast<typename Derived::Scalar>(
                    _map(i, j + 1 + flip));
                M(k, 2) = static_cast<typename Derived::Scalar>(
                    _map(i, j + 1 + (flip ^ 1)));
                k++;
                flip ^= 1;
            } else {
...
template <typename Derived>
IGL_INLINE bool tinyply_tristrips_to_faces(tinyply::PlyData &D,
                                           Eigen::PlainObjectBase<Derived> &M,
                                           size_t el, size_t el_len)
{
    switch (D.t) {
    case tinyply::Type::INT8:
        return _tinyply_tristrips_to_trifaces<int8_t, Derived>(D, M, el,
                                                               el_len);
    case tinyply::Type::UINT8:
        return _tinyply_tristrips_to_trifaces<uint8_t, Derived>(D, M, el,
                                                                el_len);
    case tinyply::Type::INT16:
        return _tinyply_tristrips_to_trifaces<int16_t, Derived>(D, M, el,
                                                                el_len);
    case tinyply::Type::UINT16:
        return _tinyply_tristrips_to_trifaces<uint16_t, Derived>(D, M, el,
                                                                 el_len);
    case tinyply::Type::INT32:
        return _tinyply_tristrips_to_trifaces<int32_t, Derived>(D, M, el,
                                                                el_len);
    case tinyply::Type::UINT32:
        return _tinyply_tristrips_to_trifaces<uint32_t, Derived>(D, M, el,
                                                                 el_len);
    case tinyply::Type::FLOAT32:
        return _tinyply_tristrips_to_trifaces<float, Derived>(D, M, el, el_len);
    case tinyply::Type::FLOAT64:
        return _tinyply_tristrips_to_trifaces<double, Derived>(D, M, el,
                                                               el_len);
    default:
        return false;
    }
...
IGL_INLINE bool readPLY(
    FILE *fp, Eigen::PlainObjectBase<DerivedV> &V,
    ...
    std::vector<std::string> &comments)
{
    ...
    try {
        ...
        FileMemoryStream stream((char *)fileBufferBytes.data(),
                                fileBufferBytes.size());
        return readPLY(stream, V, F, E, N, UV, VD, Vheader, FD, Fheader, ED,
                       Eheader, comments);
                        ...
                        Eigen::PlainObjectBase<DerivedF> &F)
{
    ...
    return readPLY(fp, V, F, E, N, UV, VD, Vheader, FD, Fheader, ED, Eheader,
                   comments);
}
...
template <typename DerivedV, typename DerivedF, typename DerivedE,
          ...
          typename DerivedFD, typename DerivedED>
IGL_INLINE bool readPLY(
    std::istream &ply_stream, Eigen::PlainObjectBase<DerivedV> &V,
    Eigen::PlainObjectBase<DerivedF> &F, Eigen::PlainObjectBase<DerivedE> &E,
    Eigen::PlainObjectBase<DerivedN> &N, Eigen::PlainObjectBase<DerivedUV> &UV,
    ...
    std::vector<std::string> &comments)
{
    ...
    std::set<std::string> vertex_std{"x",         "y",         "z", "nx",
                                     "ny",        "nz",        "u", "v",
                                     "texture_u", "texture_v", "s", "t"};
    std::set<std::string> face_std{"vertex_index", "vertex_indices"};
    std::set<std::string> edge_std{"vertex1",
                                   "vertex2"}; // non-standard edge indexes
    ...
    if (!texcoords) {
        try {
            // s t were the names used by blender and the previous libigl PLY
            // reader.
            texcoords =
                file.request_properties_from_element("vertex", {"s", "t"});
        } catch (const std::exception &) {
        }
    }
    ...
    if (!faces) {
        try {
            // try using tristrips
            tristrips = file.request_properties_from_element(
    ...
    try {
        edges = file.request_properties_from_element("edge",
                                                     {"vertex1", "vertex2"});
    } catch (const std::exception &) {
    }
    ...
    if (!_vertex_header.empty())
        _vertex_data =
            file.request_properties_from_element("vertex", _vertex_header);
    ...
    if (!vertices ||
        !tinyply_buffer_to_matrix(*vertices, V, vertices->count, 3)) {
        // Don't do this because V might have non-trivial compile-time size
        // V.resize(0,0);
    }
    ...
    if (!texcoords ||
        !tinyply_buffer_to_matrix(*texcoords, UV, texcoords->count, 2)) {
        // Don't do this (see above) UV.resize(0,0);
    }

    // HACK: Unfortunately, tinyply doesn't store list size as a separate
    // variable
    if (!faces ||
            ...
            *faces, F, faces->count,
            faces->count == 0 ? 0
                              : faces->buffer.size_bytes() /
                                    (tinyply::PropertyTable[faces->t].stride *
                                     faces->count))) {
        ...
        if (tristrips) { // need to convert to faces
            ...
            size_t el_count = tristrips->buffer.size_bytes() /
                              (tinyply::PropertyTable[tristrips->t].stride *
                               tristrips->count);

            // all strips should have tristrips->count elements
            if (!tinyply_tristrips_to_faces(*tristrips, F, tristrips->count,
                                            el_count)) {
    ...
    if (_vertex_header.empty()) {
        ...
        tinyply_buffer_to_matrix(*_vertex_data, VD, vertices->count,
                                 _vertex_header.size());
    }
    ...
    if (_face_header.empty()) {
        ...
        tinyply_buffer_to_matrix(*_face_data, FD, faces->count,
                                 _face_header.size());
    }
    ...
    if (_edge_header.empty()) {
        ...
        tinyply_buffer_to_matrix(*_edge_data, ED, _edge_data->count,
                                 _edge_header.size());
    }
    return true;
}
...
template <typename DerivedV, typename DerivedF, typename DerivedE,
          ...
          typename DerivedFD, typename DerivedED>
IGL_INLINE bool readPLY(
    const std::string &ply_file, Eigen::PlainObjectBase<DerivedV> &V,
    Eigen::PlainObjectBase<DerivedF> &F, Eigen::PlainObjectBase<DerivedE> &E,
    Eigen::PlainObjectBase<DerivedN> &N, Eigen::PlainObjectBase<DerivedUV> &UV,
    ...
    std::vector<std::string> &comments)
{
    ...
    try {
        return readPLY(ply_stream, V, F, E, N, UV, VD, VDheader, FD, FDheader,
                       ED, EDheader, comments);
    } catch (const std::exception &e) {
...
template <typename DerivedV, typename DerivedF, typename DerivedE,
          typename DerivedN, typename DerivedUV, typename DerivedD>
IGL_INLINE bool readPLY(
    const std::string &filename, Eigen::PlainObjectBase<DerivedV> &V,
    Eigen::PlainObjectBase<DerivedF> &F, Eigen::PlainObjectBase<DerivedE> &E,
    Eigen::PlainObjectBase<DerivedN> &N, Eigen::PlainObjectBase<DerivedUV> &UV,
    Eigen::PlainObjectBase<DerivedD> &VD, std::vector<std::string> &Vheader)
{
    ...
    std::vector<std::string> comments;
    return readPLY(filename, V, F, E, N, UV, VD, Vheader, FD, Fheader, ED,
                   Eheader, comments);
}
...
template bool igl::readPLY<Eigen::Matrix<double, -1, 3, 1, -1, 3>,
                           Eigen::Matrix<int, -1, -1, 0, -1, -1>>(
    FILE *, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 1, -1, 3>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &);
template bool igl::readPLY<Eigen::Matrix<float, -1, 3, 1, -1, 3>,
                           Eigen::Matrix<int, -1, -1, 0, -1, -1>>(
    FILE *, Eigen::PlainObjectBase<Eigen::Matrix<float, -1, 3, 1, -1, 3>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &);
template bool igl::readPLY<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
...
template bool igl::readPLY<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                           Eigen::Matrix<int, -1, -1, 0, -1, -1>>(
    FILE *, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &);
template bool igl::readPLY<Eigen::Matrix<double, -1, -1, 1, -1, -1>,
                           Eigen::Matrix<int, -1, -1, 0, -1, -1>>(
...
template bool igl::readPLY<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> &);
template bool igl::readPLY<Eigen::Matrix<float, -1, 3, 0, -1, 3>,
                           Eigen::Matrix<int, -1, 3, 0, -1, 3>>(
...
template bool igl::readPLY<Eigen::Matrix<float, -1, 3, 1, -1, 3>,
    ...
    FILE *, Eigen::PlainObjectBase<Eigen::Matrix<float, -1, 3, 1, -1, 3>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 3, 1, -1, 3>> &);
template bool igl::readPLY<Eigen::Matrix<float, -1, -1, 0, -1, -1>,
                           Eigen::Matrix<int, -1, -1, 0, -1, -1>>(

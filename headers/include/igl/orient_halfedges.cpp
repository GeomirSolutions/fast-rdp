...
IGL_INLINE void igl::orient_halfedges(const Eigen::MatrixBase<DerivedF> &F,
                                      ...
                                      Eigen::PlainObjectBase<DerivedOE> &oE)
{
    ...
    for (Eigen::Index f = 0; f < m; ++f) {
        for (int e = 0; e < 3; ++e) {
            const Int ind = f + m * e;
            E(f, e) = IC(ind);
            assert((EE(E(f, e), 0) == allE(ind, 0) ||
                    EE(E(f, e), 0) == allE(ind, 1)) &&
...
template void igl::orient_halfedges<Eigen::Matrix<int, -1, -1, 0, -1, -1>,

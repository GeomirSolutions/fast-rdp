...
// helper function, appends contents of Eigen matrix to an std::vector, in
// RowMajor fashion
template <typename T, typename Derived>
void append_mat_to_vec(std::vector<T> &vec,
                       const Eigen::PlainObjectBase<Derived> &mat)
{
    ...
    Eigen::Map<Eigen::Matrix<typename Derived::Scalar, Eigen::Dynamic,
        ...
        _map_vec(reinterpret_cast<T *>(vec.data() + st), mat.rows(),
                 mat.cols());
    _map_vec = mat;
}
...
IGL_INLINE bool igl::writeMSH(const std::string &msh, const Eigen::MatrixXd &X,
                              ...
                              const std::vector<Eigen::MatrixXd> &TetF)
{
    ...
    try {
            ...
            for (int i = 0; i < EFields.size(); ++i) {
                ...
                if (TetF[i].rows() != Tet.rows())
                    throw std::invalid_argument(
                        "Tetrahedra field size mismatch");
            }
        }

        // this is not the most optimal , it would be faster to modify
        // RRMshSaver to work with Eiged data types
        ...
        _Tri_Tet_type.insert(_Tri_Tet_type.end(), Tet.rows(),
        ...
        msh_saver.save_mesh(_X, _Tri_Tet, _Tri_Tet_len, _Tri_Tet_type,
                            _Tri_Tet_tag);
        ...
        for (size_t i = 0; i < XFields.size(); ++i) {
            ...
            if (XF[i].cols() == 1)
                ...
                throw std::invalid_argument(
                    "unsupported vertex field dimensionality");
            }
        }
        ...
        for (size_t i = 0; i < EFields.size(); ++i) {
            ...
            if (TriF[i].cols() == 1)
                ...
                throw std::invalid_argument(
                    "unsupported node field dimensionality");
            }
        }

                                                   ...
                                                   Eigen::MatrixXd &WS)
{
    ...
    const auto &bary_dist = [](const Eigen::MatrixXd &W,
                               const Eigen::MatrixXi &F, const int face_A,
                               const Eigen::Vector3d &bary_A, const int face_B,
                               const Eigen::Vector3d &bary_B) -> double {
        return ((bary_A(0) * W.row(F(face_A, 0)) +
    ...
    if (F.cols() == 4) {
        ...
        // Faces from tets
        MatrixXi TF = cat(
            1, cat(1, cat(2, T0, cat(2, T1, T2)), cat(2, T0, cat(2, T2, T3))),
    ...
    // List of list of face indices, for each sample gives index to face it is
    // on
    vector<vector<int>> sample_faces;
    sample_faces.resize(k);
    // List of list of barycentric coordinates, for each sample gives b-coords
    // in face its on
    ...
    for (int i = 0; i < k; i++) {
        ...
        // We're right on a face vertex so barycentric coordinates are 0, but 1
        // at that vertex
    ...
    while (radius > min_radius) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            // We're going to try to place num_rand_samples_per_triangle samples
            // on each sample *after* this location
            ...
            for (int f = 0; f < (int)incident_F.size(); f++) {
                ...
                for (int s = 0; s < max_sample_attempts_per_triangle; s++) {
                    ...
                    double d =
                        bary_dist(W, F, face_i, bary, face_f, sample_bary);
...
#ifdef EXTREME_VERBOSE
                        verbose(
                            "Reached maximum sample attempts per triangle\n");
#endif
                    }
                }
#ifdef EXTREME_VERBOSE
        ...
        // corner
        // Put -1 in entries corresponding distance of a seed's random samples
        // to self Loop over seeds
        for (int i = 0; i < k; i++) {
            ...
            for (int s = 0; s < (int)sample_faces[i].size(); s++) {
                ...
                for (int j = 0; j < k; j++) {
                    ...
                    if (i == j) {
                        ...
                        Eigen::Vector3d seed_j_bary(
                            sample_barys[j][cur_maxmin[j]]);
                        d = bary_dist(W, F, sample_face, sample_bary,
                                      seed_j_face, seed_j_bary);
                    }
                    D[i](s, j) = d;
                }
                // Loop over corners
        ...
        while (true) {
            ...
            for (int i = 0; i < k; i++) {
                ...
                // find random sample with maximum minimum distance to other
                // seeds
    ...
    for (int i = 0; i < k; i++) {
        int face_i = sample_faces[i][cur_maxmin[i]];
        Eigen::Vector3d bary(sample_barys[i][cur_maxmin[i]]);
        WS.row(i) = bary(0) * W.row(F(face_i, 0)) +
                    bary(1) * W.row(F(face_i, 1)) +
                    bary(2) * W.row(F(face_i, 2));
    }
    verbose("Lap: %g\n", get_seconds() - start);

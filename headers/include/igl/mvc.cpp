                         ...
                         Eigen::MatrixXd &W)
{
    ...
    for (int i = 0; i < C1.rows(); ++i) {
        ...
        for (int j = 0; j < V1.rows(); ++j) {
            ...
            // distance from V to every C, where dist_C_V(i,j) is the distance
            // from domain vertex j to handle i
            ...
            if (dist_C_V(i, j) < EPS)
                ...
                // segments afterwards -- should only be on one segment
                // (otherwise must be on a corner and we already handled that)
                // domain vertex j is on the segment from i to i+1 if the
                // distances from vj to pi and pi+1 are about
                if (std::abs((dist_C_V(i, j) + dist_C_V_next) / edge_length -
                             1) < EPS)

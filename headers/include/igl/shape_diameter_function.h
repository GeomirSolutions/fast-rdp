...
///
/// @param[in] shoot_ray  function handle that outputs hits of a given ray
/// against a
///              mesh (embedded in function handles as captured variable/data)
/// @param[in] P  #P by 3 list of origin points
/// @param[in] N  #P by 3 list of origin normals
/// @param[out] S  #P list of shape diamater function values between bounding
/// box
...
IGL_INLINE void shape_diameter_function(const igl::AABB<DerivedV, DIM> &aabb,
...
template <typename DerivedV, typename DerivedF, typename DerivedP,
          typename DerivedN, typename DerivedS>
IGL_INLINE void shape_diameter_function(const Eigen::MatrixBase<DerivedV> &V,
                                        const Eigen::MatrixBase<DerivedF> &F,
                                        const Eigen::MatrixBase<DerivedP> &P,
                                        const Eigen::MatrixBase<DerivedN> &N,
                                        const int num_samples,
                                        Eigen::PlainObjectBase<DerivedS> &S);
/// @param[in] per_face  whether to compute per face (S is #F by 1) or per
/// vertex (S is
///     #V by 1)

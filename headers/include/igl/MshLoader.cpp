...
IGL_INLINE void igl::MshLoader::parse_nodes(std::ifstream &fin)
{
    ...
    if (m_binary) {
        ...
        for (size_t i = 0; i < num_nodes; i++) {
            ...
            memcpy(&m_nodes[node_idx * 3], data + i * stride + 4,
                   m_data_size * 3);
        }
        delete[] data;
    } else {
...
IGL_INLINE void igl::MshLoader::parse_node_field(std::ifstream &fin)
{
    ...
    if (m_binary) {
        ...
        for (size_t i = 0; i < num_entries; i++) {
            ...
            memcpy(&field[node_idx * num_components], &data[base_idx],
...
IGL_INLINE void
igl::MshLoader::parse_unknown_field(std::ifstream &fin,
...
IGL_INLINE void igl::MshLoader::index_structures(int tag_column)
{
    ...
    std::unique_copy(
        std::begin(m_structure_index), std::end(m_structure_index),

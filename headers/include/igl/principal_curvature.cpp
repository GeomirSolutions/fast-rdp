...
class CurvatureCalculator
{
  public:
    ...
    class Quadric
    {
      public:
        ...
        IGL_INLINE Quadric(double av, double bv, double cv, double dv,
                           double ev)
        {
            a() = av;
            b() = bv;
        ...
        IGL_INLINE static Quadric fit(const std::vector<Eigen::Vector3d> &VV)
        {
            assert(VV.size() >= 5);
            if (VV.size() < 5) {
                std::cerr << "IGL_ASSERT FAILED! fit function requires at "
                             "least 5 points: Only "
  ...
  public:
    ...
    // The i-th row contains the indices of the vertices that forms the i-th
    // face in ccw order
    ...
    int step;    /* If expStep==false, by how much rhe radius increases on every
                    step */
    ...
    IGL_INLINE void getProjPlane(int, const std::vector<int> &,
                                 Eigen::Vector3d &);
    IGL_INLINE void applyMontecarlo(const std::vector<int> &,
                                    std::vector<int> *);
                                          ...
                                          const Eigen::MatrixXi &F)
{
    ...
    igl::vertex_triangle_adjacency(V, F, vertex_to_faces,
                                ...
                                const std::vector<int> &vv, Quadric *q)
{
    ...
    if (points.size() < 5) {
        std::cerr << "IGL_ASSERT FAILED! fit function requires at least 5 "
                     "points: Only "
                                              ...
                                              std::vector<int> &vv)
{
    ...
    while (!queue.empty()) {
        ...
        if (toVisit < vertex_to_vertices.size()) {
            if (distance < (int)r) {
                for (unsigned int i = 0; i < vertex_to_vertices[toVisit].size();
                     ++i) {
                    int neighbor = vertex_to_vertices[toVisit][i];
                    if (!visited[neighbor]) {
                        queue.push_back(
                            std::pair<int, int>(neighbor, distance + 1));
                                               ...
                                               std::vector<int> &vv, int min)
{
            ...
            if (!visited[neighbor]) {
                ...
                if (distance < r)
                    ...
                    extra_candidates.push(
                        std::pair<int, double>(neighbor, distance));
                visited[neighbor] = true;
            }
        }
    }
    while (!extra_candidates.empty() && (int)vv.size() < min) {
        ...
        for (unsigned int i = 0; i < vertex_to_vertices[cand.first].size();
             ++i) {
                                                  ...
                                                  Eigen::Vector3d &ppn)
{
    ...
    ppn =
        chooseMax(Eigen::Vector3d(nx, ny, nz), Eigen::Vector3d(a, b, c), a * b);
    ppn.normalize();
}
...
IGL_INLINE void CurvatureCalculator::computeCurvature()
{
    ...
    for (size_t i = 0; i < vertices_count; ++i) {
        ...
        if (vv.size() < 6) {
            // std::cerr << "Could not compute curvature of radius " <<
            // scaledRadius << std::endl;
            continue;
        }
...
IGL_INLINE void CurvatureCalculator::printCurvature(const std::string &outpath)
{
    ...
    for (int i = 0; i < vertices_count; ++i) {
        of << curv[i][0] << " " << curv[i][1] << " " << curvDir[i][0][0] << " "
           << curvDir[i][0][1] << " " << curvDir[i][0][2] << " "
           << curvDir[i][1][0] << " " << curvDir[i][1][1] << " "
           << curvDir[i][1][2] << endl;
    }
    ...
    Eigen::PlainObjectBase<DerivedPV2> &PV2, unsigned radius, bool useKring)
{
    ...
    for (unsigned i = 0; i < V.rows(); ++i) {
        ...
        if (PD1.row(i) * PD2.row(i).transpose() > 10e-6) {
            std::cerr << "PRINCIPAL_CURVATURE: Something is wrong with vertex: "
                      << i << std::endl;
            PD1.row(i) *= 0;
            PD2.row(i) *= 0;
        }

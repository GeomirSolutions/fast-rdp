...
/// @file decimate.h
///
/// igl::decimate implements a customizable greedy edge collapser using a
/// priority-queue:
///
///      Q ← {}
///      for each edge e
...
///
///
/// There are four important callbacks that can be customized
/// (decimate_callback_types.h). Note that any of these functions can capture
/// user-defined book-keeping variables.
///
...
IGL_INLINE bool decimate(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                         ...
                         Eigen::MatrixXi &G, Eigen::VectorXi &J,
                         Eigen::VectorXi &I);
/// \overload
IGL_INLINE bool decimate(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                         const size_t max_m, Eigen::MatrixXd &U,
...
/// callbacks in a priority queue. Functions control the cost and placement of
/// each collapse the stopping criteria for queue processing and the callbacks
/// for pre and post collapse operations. See the first implementation in
/// decimate.cpp for an example of how to deal with open/non-manifold meshes and
/// how to adjust cost and placement functions accordingly.
///
/// See \fileinfo for more details.
///
/// @param[in] V  #V by dim list of vertex positions
/// @param[in] F  #F by 3 list of face indices into V.
/// @param[in] cost_and_placement  function computing cost of collapsing an edge
/// and 3d
///     position where it should be placed:
///     cost_and_placement(V,F,E,EMAP,EF,EI,cost,placement);
/// @param[in] stopping_condition  function returning whether to stop collapsing
/// edges
...
/// @param[in] pre_collapse  callback called with index of edge whose collapse
/// is about
///              to be attempted (see collapse_edge)
/// @param[in] post_collapse  callback called with index of edge whose collapse
/// was
...
/// unique
///              unique edge in E
/// @param[in] EF  #E by 2 list of edge flaps, EF(e,0)=f means e=(i-->j) is the
/// edge of
///              F(f,:) opposite the vth corner, where EI(e,0)=v. Similarly
///              EF(e,1) " e=(j->i)
...
IGL_INLINE bool
decimate(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,

...
IGL_INLINE void
igl::facet_adjacency_matrix(const Eigen::MatrixBase<DerivedF> &F,
                            Eigen::SparseMatrix<Atype> &A)
{
    ...
    for (Eigen::Index g = 0; g < A.outerSize(); g++) {
        for (typename Eigen::SparseMatrix<Atype>::InnerIterator it(A, g); it;
             ++it) {
            if (it.value())
                it.valueRef() = 1;
        }
...
template void
igl::facet_adjacency_matrix<Eigen::Matrix<int, -1, -1, 0, -1, -1>, bool>(
    Eigen::MatrixBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> const &,
    Eigen::SparseMatrix<bool, 0, int> &);
// generated by autoexplicit.sh
template void
igl::facet_adjacency_matrix<Eigen::Matrix<int, -1, -1, 0, -1, -1>, int>(
    Eigen::MatrixBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> const &,

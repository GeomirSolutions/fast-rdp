...
class WindingNumberTree
{
  public:
    ...
    static std::map<
        ...
        cached;
    // This is only need to fill in references, it should never actually be
    // touched and shouldn't cause race conditions. (This is a hack, but I think
    // it's "safe")
  ...
  protected:
    ...
    typedef Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic,
                          Eigen::Dynamic>
        MatrixXS;
    typedef Eigen::Matrix<typename DerivedF::Scalar, Eigen::Dynamic,
                          Eigen::Dynamic>
        MatrixXF;
  ...
  public:
    ...
    // For root
    inline WindingNumberTree(const Eigen::MatrixBase<DerivedV> &V,
    ...
    inline virtual void set_mesh(const Eigen::MatrixBase<DerivedV> &V,
                                 const Eigen::MatrixBase<DerivedF> &F);
    // Set method
  ...
  public:
    ...
    winding_number_boundary(const Point &p) const;
    //// Same as winding_number above, but if max_simple_abs_winding_number is
    //// less than some threshold min_max_w just return 0 (colloquially the
    ...
    inline virtual typename DerivedV::Scalar
    max_abs_winding_number(const Point &p) const;
    // Same as above, but stronger assumptions on (V,F). Assumes (V,F) is a
    // simple polyhedron
    inline virtual typename DerivedV::Scalar
    max_simple_abs_winding_number(const Point &p) const;
    ...
    // Inputs:
    //   p  query point
    //   that  WindingNumberTree containing mesh w.r.t. which we're computing
    //   w.n.
    // Returns cached winding number
    inline virtual typename DerivedV::Scalar
    cached_winding_number(const WindingNumberTree &that, const Point &p) const;
};
} // namespace igl
...
// template <typename Point, typename DerivedV, typename DerivedF>
// WindingNumberMethod WindingNumberTree<Point,DerivedV,DerivedF>::method =
// EXACT_WINDING_NUMBER_METHOD; template <typename Point, typename DerivedV,
// typename DerivedF> double
// WindingNumberTree<Point,DerivedV,DerivedF>::min_max_w = 0;
template <typename Point, typename DerivedV, typename DerivedF>
std::map<std::pair<const igl::WindingNumberTree<Point, DerivedV, DerivedF> *,
                   const igl::WindingNumberTree<Point, DerivedV, DerivedF> *>,
         typename DerivedV::Scalar>
...
inline igl::WindingNumberTree<Point, DerivedV, DerivedF>::WindingNumberTree()
    : method(EXACT_WINDING_NUMBER_METHOD), parent(NULL), V(dummyV), SV(), F(),
      cap(), radius(std::numeric_limits<typename DerivedV::Scalar>::infinity()),
      center(0, 0, 0)
{
}
...
inline void igl::WindingNumberTree<Point, DerivedV, DerivedF>::delete_children()
{
    using namespace std;
    // Delete children
    typename list<WindingNumberTree<Point, DerivedV, DerivedF> *>::iterator
        cit = children.begin();
...
template <typename Point, typename DerivedV, typename DerivedF>
inline void igl::WindingNumberTree<Point, DerivedV, DerivedF>::set_method(
    const WindingNumberMethod &m)
{
    this->method = m;
...
template <typename Point, typename DerivedV, typename DerivedF>
inline const DerivedV &
igl::WindingNumberTree<Point, DerivedV, DerivedF>::getV() const
{
    return V;
}

template <typename Point, typename DerivedV, typename DerivedF>
inline const typename igl::WindingNumberTree<Point, DerivedV,
                                             DerivedF>::MatrixXF &
...
template <typename Point, typename DerivedV, typename DerivedF>
inline bool igl::WindingNumberTree<Point, DerivedV, DerivedF>::inside(
    const Point & /*p*/) const
{
    return true;
}
...
template <typename Point, typename DerivedV, typename DerivedF>
inline typename DerivedV::Scalar
igl::WindingNumberTree<Point, DerivedV, DerivedF>::winding_number(
    const Point &p) const
{
    using namespace std;
    // cout<<"+"<<boundary.rows();
    ...
    if (inside(p)) {
        ...
        if (children.size() > 0) {
            ...
            for (typename list<WindingNumberTree<Point, DerivedV, DerivedF>
                                   *>::const_iterator cit = children.begin();
...
template <typename Point, typename DerivedV, typename DerivedF>
inline typename DerivedV::Scalar
igl::WindingNumberTree<Point, DerivedV, DerivedF>::winding_number_all(
    const Point &p) const
{
...
template <typename Point, typename DerivedV, typename DerivedF>
inline typename DerivedV::Scalar
igl::WindingNumberTree<Point, DerivedV, DerivedF>::winding_number_boundary(
    const Point &p) const
{
...
// template <typename Point, typename DerivedV, typename DerivedF>
// inline double
// igl::WindingNumberTree<Point,DerivedV,DerivedF>::winding_number_approx_simple(
//   const Point & p,
//   const double min_max_w)
//{
//   using namespace std;
...
//   {
//     cout<<"Skipped!
//     "<<max_simple_abs_winding_number(p)<<"<"<<min_max_w<<endl; return 0;
//   }
// }

template <typename Point, typename DerivedV, typename DerivedF>
inline void
igl::WindingNumberTree<Point, DerivedV, DerivedF>::print(const char *tab)
{
    using namespace std;
    // Print all facets
    cout << tab << "[" << endl << F << endl << "]";
    // Print children
    for (typename list<WindingNumberTree<Point, DerivedV, DerivedF> *>::iterator
             cit = children.begin();
         cit != children.end(); cit++) {
        cout << "," << endl;
...
template <typename Point, typename DerivedV, typename DerivedF>
inline typename DerivedV::Scalar
igl::WindingNumberTree<Point, DerivedV, DerivedF>::max_abs_winding_number(
    const Point & /*p*/) const
{
    ...
    const Point &p) const
{
    ...
    //
    //
    // a = angle formed by trapazoid formed by raising sides with lengths r and
    // R at respective centers.
    //
    ...
    if (is_far) {
        typename DerivedV::Scalar a = atan2(
            that.radius - this->radius, (that.center - this->center).norm());
        assert(a > 0);
        is_far = (a < PI / 8.0);
    }
    ...
    if (is_far) {
        // Not implemented yet
        pair<const WindingNumberTree *, const WindingNumberTree *> this_that(
            this, &that);
        ...
        // Not inside any children? This can totally happen because bounding
        // boxes are set to bound contained facets. So sibilings may overlap and
        // their union may not contain their parent (though, their union is
        // certainly a subset of their parent).
        assert(false);
    }
    return 0;
}

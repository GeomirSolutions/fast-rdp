...
/// @param[out] I  #I vectorized list of polygon corner indices into rows of
/// some matrix V
/// @param[out] C  #P+1 list of cumulative polygon sizes so that C(i+1)-C(i) =
/// size of
...
IGL_INLINE void polygon_corners(const std::vector<std::vector<PType>> &P,
...
/// polygon corners and sizes
///
/// @param[in] Q  #Q by k list of polygon indices (ith row is a k-gon, unless
/// Q(i,j) =
///     -1 then it's a j-gon)
template <typename DerivedQ, typename DerivedI, typename DerivedC>
IGL_INLINE void polygon_corners(const Eigen::MatrixBase<DerivedQ> &Q,
                                Eigen::PlainObjectBase<DerivedI> &I,
                                Eigen::PlainObjectBase<DerivedC> &C);
} // namespace igl

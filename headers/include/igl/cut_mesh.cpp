...
IGL_INLINE void igl::cut_mesh(const Eigen::MatrixBase<DerivedV> &V,
...
template <typename DerivedV, typename DerivedF, typename DerivedC,
          typename DerivedI>
IGL_INLINE void igl::cut_mesh(Eigen::PlainObjectBase<DerivedV> &V,
                              ...
                              Eigen::PlainObjectBase<DerivedI> &I)
{
    ...
    for (Index i = 0; i < F.rows(); i++) {
        for (Index k = 0; k < 3; k++) {
            ...
            Index v = F(i, (k + 1) % 3);
            if (FF(i, k) ==
                -1) { // add one extra occurance for boundary vertices
                eventual(u) += 1;
            } else if (C(i, k) == 1 &&
                       u < v) { // only compute every (undirected) edge ones
                eventual(u) += 1;
                eventual(v) += 1;
            }
        }
    ...
    for (Index f = 0; f < C.rows(); f++) {
        for (Index k = 0; k < 3; k++) {
            Index v0 = F(f, k);
            if (F(f, k) >= n_v)
                continue; // ignore new vertices
            if (C(f, k) == 1 && occurence(v0) != eventual(v0)) {
                igl::HalfEdgeIterator<DerivedF, DerivedF, DerivedF> he(
                    F, FF, FFi, f, k);

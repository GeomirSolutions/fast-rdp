...
template <typename DerivedV, typename DerivedF, typename DerivedSE,
          typename DerivedE, typename DeriveduE, typename DerivedEMAP,
          typename uE2Etype, typename sharptype>
IGL_INLINE void igl::sharp_edges(const Eigen::MatrixBase<DerivedV> &V,
                                 ...
                                 std::vector<sharptype> &sharp)
{
    ...
    for (int u = 0; u < uE2E.size(); u++) {
        ...
        //
        // if there are 3 faces (non-manifold) it appears to follow that the
        // edge must be sharp if angle<60. Could skip those (they're likely
        // small number anyway).
        for (int i = 0; i < uE2E[u].size(); i++)
            for (int j = i + 1; j < uE2E[u].size(); j++) {
                ...
                const RowVector3S ev =
                    (V.row(E(ei, 1)) - V.row(E(ei, 0))).normalized();
                const Scalar dij =
...
template void igl::sharp_edges<Eigen::Matrix<double, -1, -1, 0, -1, -1>,

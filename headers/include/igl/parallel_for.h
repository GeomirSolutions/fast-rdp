...
/// \endcode
///
/// @param[in] loop_size  number of iterations. I.e. for(int i =
/// 0;i<loop_size;i++) ...
/// @param[in] func  function handle taking iteration index as only argument to
/// compute
///     inner block of for loop I.e. for(int i ...){ func(i); }
/// @param[in] min_parallel  min size of loop_size such that parallel
/// (non-serial)
///     thread pooling should be attempted {0}
/// @return true iff thread pool was invoked
template <typename Index, typename FunctionType>
inline bool parallel_for(const Index loop_size, const FunctionType &func,
...
///     argument
/// @param[in] func  function handle taking iteration index i and thread id t as
/// only
///     arguments to compute inner block of for loop I.e.
///     for(int i ...){ func(i,t); }
/// @param[in] accum_func  function handle taking thread index as only argument,
/// to be
...
///     thread pooling should be attempted {0}
/// @return true iff thread pool was invoked
template <typename Index, typename PrepFunctionType, typename FunctionType,
          typename AccumFunctionType>
inline bool
parallel_for(const Index loop_size, const PrepFunctionType &prep_func,
             const FunctionType &func, const AccumFunctionType &accum_func,
             const size_t min_parallel = 0);
} // namespace igl
...
template <typename Index, typename FunctionType>
inline bool igl::parallel_for(const Index loop_size, const FunctionType &func,
...
template <typename Index, typename PreFunctionType, typename FunctionType,
          typename AccumFunctionType>
inline bool
igl::parallel_for(const Index loop_size, const PreFunctionType &prep_func,
                  const FunctionType &func, const AccumFunctionType &accum_func,
                  const size_t min_parallel)
{
    assert(loop_size >= 0);
    ...
    if (loop_size < min_parallel || nthreads <= 1) {
        ...
        Index slice = std::max(
            (Index)std::round((loop_size + 1) / static_cast<double>(nthreads)),
            (Index)1);

...
void igl::crouzeix_raviart_massmatrix(
    ...
    const Eigen::MatrixBase<DerivedEMAP> &EMAP, Eigen::SparseMatrix<MT> &M)
{
    ...
    for (int f = 0; f < m; f++) {
        for (int c = 0; c < ss; c++) {
            MIJV[f + m * c] = Triplet<MT>(
                EMAP(f + m * c, 0), EMAP(f + m * c, 0), TA(f) / (double)(ss));
        }
    }
...
template void
igl::crouzeix_raviart_massmatrix<double,

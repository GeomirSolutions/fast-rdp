...
template <typename Scalar, typename DerivedX, typename DerivedLB,
          typename DerivedUB>
IGL_INLINE Scalar igl::pso(const std::function<Scalar(DerivedX &)> f,
                           const Eigen::MatrixBase<DerivedLB> &LB,
                           const Eigen::MatrixBase<DerivedUB> &UB,
                           const int max_iters, const int population,
                           DerivedX &X)
{
...
template <typename Scalar, typename DerivedX, typename DerivedLB,
          typename DerivedUB, typename DerivedP>
IGL_INLINE Scalar igl::pso(const std::function<Scalar(DerivedX &)> f,
                           const Eigen::MatrixBase<DerivedLB> &LB,
                           const Eigen::MatrixBase<DerivedUB> &UB,
                           ...
                           DerivedX &X)
{
    ...
    typedef std::vector<DerivedX, Eigen::aligned_allocator<DerivedX>>
        VectorList;
...
//#define IGL_PSO_REFLECTION
                ...
                //        // TODO: I'm not sure this is the right thing to
                //        do/enough. The
                //        // velocities could be weird. Suppose the current
                //        "best" value is ε and
                //        // the value is -ε and the "periodic bounds" [0,2π].
                //        Moding will send
                //        // the value to 2π-ε but the "velocity" term will now
                //        be huge pointing
                //        // all the way from 2π-ε to ε.
                //        //
                //        // Q: Would it be enough to try (all combinations) of
                //        ±(UB-LB) before
                //        // computing velocities to "best"s? In the example
                //        above, instead of
                //        //
                ...
                //        //
                //        // Though, for multivariate b,p,v this would seem to
                //        explode
                //        // combinatorially.
                //        //
                //        // Maybe periodic things just shouldn't be bounded and
                //        we hope that the
                //        // forces toward the current minima "regularize" them
                //        away from insane
                //        // values.
                //        if(P(d))
                //        {
                //          position[p](d) =
                //          std::fmod(position[p](d)-LB(d),UB(d)-LB(d))+LB(d);
                //        }else
                //        {
                //          position[p](d) =
                //          std::max(LB(d),std::min(UB(d),position[p](d)));
                //        }
                position[p](d) =
                    std::max(LB(d), std::min(UB(d), position[p](d)));
#endif
...
template float igl::pso<float, Eigen::Matrix<float, 1, -1, 1, 1, -1>,
                        Eigen::Matrix<float, 1, -1, 1, 1, -1>,
                        Eigen::Matrix<float, 1, -1, 1, 1, -1>>(
    std::function<float(Eigen::Matrix<float, 1, -1, 1, 1, -1> &)>,
    Eigen::MatrixBase<Eigen::Matrix<float, 1, -1, 1, 1, -1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<float, 1, -1, 1, 1, -1>> const &, int, int,
    Eigen::Matrix<float, 1, -1, 1, 1, -1> &);
#endif

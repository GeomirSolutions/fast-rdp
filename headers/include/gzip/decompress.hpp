...
class Decompressor
{
  ...
  public:
    Decompressor(
        std::size_t max_bytes = 1000000000) // by default refuse operation if
...
#pragma GCC diagnostic pop
...
#ifdef DEBUG
        // Verify if size (long type) input will fit into unsigned int, type
        // used for zlib's avail_in
        ...
        do {
            ...
            if (resize_to > max_) {
                inflateEnd(&inflate_s);
                throw std::runtime_error(
                    "size of output string will use more memory then intended "
                    "when decompressing");
            }

        ...
        for (std::size_t i = 0; i < items.size(); ++i) {
            auto &bucket =
                result.buckets[hash(key(items[i]),
                                    static_cast<std::size_t>(result.seed)) %
                               M];
...
template <std::size_t M, class Hasher> struct pmh_tables : private Hasher
{
    ...
    constexpr pmh_tables(std::uint64_t first_seed,
                         carray<seed_or_index, M> first_table,
                         carray<std::size_t, M> second_table,
                         Hasher hash) noexcept
        : Hasher(hash), first_seed_(first_seed), first_table_(first_table),
    ...
    {
        auto const d =
            first_table_[hasher(key, static_cast<std::size_t>(first_seed_)) %
                         M];
        if (!d.is_seed()) {
                                              ...
                                              PRG prg)
{
    ...
    // one-past-the-end of 'items' to function as a sentinel value. Both to
    // avoid the need to apply the KeyEqual predicate and to be easily
    // convertible to end(). Unused entries in both hash tables (G and H) have

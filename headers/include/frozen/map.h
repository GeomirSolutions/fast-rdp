...
class map : private impl::CompareKey<Compare>
{
  ...
  public:
    ...
    constexpr map(container_type items, Compare const &compare)
        : impl::CompareKey<Compare>{compare},
          items_{bits::quicksort(bits::remove_cv_t<container_type>(items),
                                 value_comp())}
    {
    }
    ...
    {
        return static_cast<impl::CompareKey<Compare> const &>(*this);
    }
  ...
  private:
    ...
    template <class This, class KeyType>
    static inline constexpr auto lower_bound_impl(This &&self,
                                                  KeyType const &key)
        -> decltype(self.end())
    {
        return bits::lower_bound<N>(self.items_.begin(), key,
                                    self.value_comp());
    }

    template <class This, class KeyType>
    static inline constexpr auto upper_bound_impl(This &&self,
                                                  KeyType const &key)
    {
        auto lower = self.lower_bound(key);
        if (lower != self.end() && !self.value_comp()(key, *lower))

  ...
  public:
    constexpr boyer_moore_searcher(char const (&needle)[size + 1])
        : skip_table_{build_skip_table(needle)},
          suffix_table_{build_suffix_table(needle)}, needle_(needle)
    {
    }

    template <class RandomAccessIterator>
    constexpr std::pair<RandomAccessIterator, RandomAccessIterator>
    operator()(RandomAccessIterator first, RandomAccessIterator last) const
    {
        ...
        while (true) {
            ...
            std::ptrdiff_t jump =
                std::max(skip_table_[*iter], suffix_table_[j]);
            if (jump >= last - iter)
                return {last, last};
            iter += jump;
        }

...
template <typename Ptr = char const *> struct generic_string
{
    ...
    msize_t size() const noexcept
    {
        if (is_short()) {
            auto const pos =
                static_cast<char const *>(std::memchr(s_.s_, '\0', 15));
...
template <typename Ptr> struct basic_string : public generic_string<Ptr>
{
...
template <typename Ptr> struct basic_string_view : public generic_string<Ptr>
{
    using base = generic_string<Ptr>;
...
template <typename Ptr> struct is_string_helper : std::false_type
{
};

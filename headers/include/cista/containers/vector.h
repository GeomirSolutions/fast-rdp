...
struct basic_vector
{
    ...
    template <typename It> basic_vector(It begin_it, It end_it)
    {
        set(begin_it, end_it);
    }

    basic_vector(basic_vector &&arr) noexcept
        : el_(arr.el_), used_size_(arr.used_size_),
    ...
    template <typename It> void set(It begin_it, It end_it)
    {
        ...
        for (; copy_source != end_it; ++copy_source, ++copy_target) {
            new (copy_target)
                T{std::forward<decltype(*copy_source)>(*copy_source)};
        }
    ...
    void reserve(TemplateSizeType new_size)
    {
        ...
        auto mem_buf =
            static_cast<T *>(std::malloc(to_idx(num_bytes))); // NOLINT
        if (mem_buf == nullptr) {
            throw std::bad_alloc();
        }
...
template <typename T> using vector = basic_vector<T, ptr<T>>;

template <typename T> using indexed_vector = basic_vector<T, ptr<T>, true>;

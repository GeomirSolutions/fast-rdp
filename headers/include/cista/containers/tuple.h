...
template <typename T, typename... Ts> struct tuple_element<0, T, Ts...>
{
    using type = T;
};
...
template <typename... Ts> constexpr std::size_t get_total_size()
{
...
template <typename... Ts> struct tuple
{
    ...
    {
        (new (get_ptr<Is>())
             tuple_element_t<Is, Ts...>(std::move(get<Is>(from))),
         ...);
    }
    ...
    template <std::size_t... Is> constexpr void destruct(seq_t<Is...>)
    {
...
{
    return apply_impl(std::make_index_sequence<
                          tuple_size_v<std::remove_reference_t<Tuple>>>{},
                      std::forward<F>(f), std::forward<Tuple>(a),
                      std::forward<Tuple>(b));
}
...
{
    return (
        (get<I>(std::forward<Tuple>(a)) == get<I>(std::forward<Tuple>(b))) &&
        ...);
}
...
std::enable_if_t<is_tuple_v<decay_t<Tuple>>, bool> operator==(Tuple &&a,
                                                              Tuple &&b)
{
    return eq(std::make_index_sequence<
                  tuple_size_v<std::remove_reference_t<Tuple>>>{},
...
template <typename Tuple, std::size_t Index = 0U> bool lt(Tuple &&a, Tuple &&b)
{
    if constexpr (Index == tuple_size_v<Tuple>) {
        ...
        return lt<Tuple, Index + 1>(std::forward<Tuple>(a),
                                    std::forward<Tuple>(b));
    }
}

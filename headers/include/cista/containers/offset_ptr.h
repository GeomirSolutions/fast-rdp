...
template <typename T, typename Enable = void> struct offset_ptr
{
    ...
    template <typename Int> T *operator+(Int i) const noexcept
    {
        return get() + i;
    }
...
template <class T> struct is_pointer_helper<offset_ptr<T>> : std::true_type
{
};
...
template <class T> struct remove_pointer_helper
{
    using type = T;
};

template <class T> struct remove_pointer_helper<T *>
{
    using type = T;
};

template <class T> struct remove_pointer_helper<offset_ptr<T>>
{
    using type = T;
};

...
struct hash_storage
{
    ...
    using mapped_type = decay_t<decltype(std::declval<GetValue>().operator()(
        std::declval<T>()))>;
    ...
    template <typename Key> hash_t compute_hash(Key const &k)
    {
    ...
    struct bit_mask
    {
        ...
        size_type leading_zeros() const noexcept
        {
            ...
            constexpr int extra_bits =
                sizeof(group_t) * 8 - total_significant_bits;
    ...
    struct group
    {
        ...
        {
            return (trailing_zeros(((~ctrl_ & (ctrl_ >> 7U)) | GAPS) + 1U) +
                    7U) >>
                   3U;
        }
        group_t ctrl_;
    ...
    struct iterator
    {
        ...
        void skip_empty_or_deleted() noexcept
        {
            while (is_empty_or_deleted(*ctrl_)) {
                auto const shift =
                    group{ctrl_}.count_leading_empty_or_deleted();
                ctrl_ += shift;
                entry_ += shift;
            }
        }
    ...
    capacity_to_growth(size_type const capacity) noexcept
    {
    ...
    hash_storage(hash_storage &&other) noexcept
        : entries_{other.entries_}, ctrl_{other.ctrl_}, size_{other.size_},
          capacity_{other.capacity_}, growth_left_{other.growth_left_},
    ...
    template <typename Key> mapped_type &bracket_operator_impl(Key &&key)
    {
        ...
        if (res.second) {
            new (entries_ + res.first)
                T{static_cast<key_type>(key), mapped_type{}};
        }
    ...
    template <typename Key> mapped_type &at(Key &&key)
    {
    ...
    {
        return const_cast<hash_storage *>(this)->find_impl(
            std::forward<Key>(key));
    }
    ...
    // --- erase()
    template <typename Key> size_t erase_impl(Key &&key)
    {
    ...
    template <typename Key> size_t erase(Key &&key)
    {
    ...
    find_info find_first_non_full(size_type const hash) const noexcept
    {
        for (auto seq = probe_seq{h1(hash), capacity_}; true; seq.next()) {
            auto const mask =
                group{ctrl_ + seq.offset_}.match_empty_or_deleted();
            if (mask) {
    ...
    void initialize_entries()
    {
        ...
        ctrl_ = reinterpret_cast<ctrl_t *>(
            reinterpret_cast<uint8_t *>(ptr_cast(entries_)) +
            capacity_ * sizeof(T));
        reset_ctrl();
        reset_growth_left();
    }

...
template <std::size_t Size, typename Enable = void> struct bytes_to_integer_type
{
};
...
template <typename... T> constexpr std::size_t bytes() noexcept
{
...
template <typename... T> struct variant
{
        ...
        -> decltype(f((vs, std::declval<type_at_index_t<0U, T...>>())...))
    {
        ...
        if constexpr (B + 15 < sizeof...(T)) {
            return apply<F, B + 16U>(std::forward<F>(f),
                                     std::forward<Vs>(vs)...);
        }
    ...
    template <typename AsType> AsType &as() noexcept
    {
...
constexpr cista::type_at_index_t<I, Ts...> const &
get(cista::variant<Ts...> const &v) noexcept
{
    return v.template as<cista::type_at_index_t<I, Ts...>>();
}

template <std::size_t I, typename... Ts>
constexpr cista::type_at_index_t<I, Ts...> &
get(cista::variant<Ts...> &v) noexcept
{
    return v.template as<cista::type_at_index_t<I, Ts...>>();
}
...
template <std::size_t I, typename... Ts>
constexpr std::add_pointer_t<cista::type_at_index_t<I, Ts...> const>
get_if(cista::variant<Ts...> const &v) noexcept
{
...
template <std::size_t I, typename... Ts>
constexpr std::add_pointer_t<cista::type_at_index_t<I, Ts...>>
get_if(cista::variant<Ts...> &v) noexcept
{
    static_assert(I < sizeof...(Ts));
    return v.idx_ == I ? &v.template as<cista::type_at_index_t<I, Ts...>>()
                       : nullptr;

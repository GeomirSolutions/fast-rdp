...
struct dynamic_fws_multimap_base
{
    ...
    template <bool Const> struct bucket
    {
        ...
        {
            if (it < begin() || it >= end()) {
                throw std::out_of_range{"dynamic_fws_multimap::bucket::bucket_"
                                        "index() out of range"};
            }
  ...
  protected:
    ...
    void release_bucket(index_type &bucket)
    {
        if (bucket.capacity_ != 0) {
            ...
            free_buckets_[to_idx(order)].push_back(
                index_type{bucket}); // NOLINT
...
{
    template <typename T> using vec = vector_map<K, T>;

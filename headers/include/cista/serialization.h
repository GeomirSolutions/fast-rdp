...
template <typename Target, mode Mode> struct serialization_context
{
    ...
    {
        if (std::is_same_v<decay_t<remove_pointer_t<Ptr>>, void> &&
            add_pending) {
    ...
    std::optional<offset_t> resolve_vector_range_ptr(Ptr ptr)
    {
        if (vector_ranges_.empty()) {
            ...
            return pred->second.contains(pred->first, ptr)
                       ? std::make_optional(
                             pred->second.offset_of(pred->first, ptr))
                       : std::nullopt;
        }
    }
...
void serialize(Ctx &c, T const *origin, offset_t const pos)
{
    ...
    if constexpr (std::is_union_v<Type>) {
        ...
        serialize(c, static_cast<typename Type::value_type const *>(origin),
                  pos);
    } else if constexpr (!std::is_scalar_v<Type>) {
        static_assert(to_tuple_works_v<Type>,
               ...
               offset_t const pos)
{
    ...
    auto const start = origin->el_ == nullptr
                           ? NULLPTR_OFFSET
                           : c.write(origin->el_, serialized_size<T>(),
                                     std::alignment_of_v<T>);
               ...
               offset_t const pos)
{
    ...
    auto const ctrl_start =
        start == NULLPTR_OFFSET
            ...
            : start + static_cast<offset_t>(origin->capacity_ *
                                            serialized_size<T>());
...
void serialize(Target &t, T &value)
{
    ...
    if constexpr (is_mode_enabled(Mode, mode::WITH_INTEGRITY)) {
        auto const csum = c.checksum(integrity_offset +
                                     static_cast<offset_t>(sizeof(hash_t)));
        c.write(integrity_offset, convert_endian<Mode>(csum));
    }
}

template <mode const Mode = mode::NONE, typename T> byte_buf serialize(T &el)
{
    auto b = buf{};
    serialize<Mode>(b, el);
...
template <mode Mode> struct deserialization_context
{
    static constexpr auto const MODE = Mode;
    ...
    template <typename T> void convert_endian(T &el) const
    {
    ...
    template <typename Ptr> void deserialize_ptr(Ptr **ptr) const
    {
        auto const offset =
        ...
        *ptr = offset == NULLPTR_OFFSET
                   ? nullptr
                   : reinterpret_cast<Ptr *>(checked_addition(
                         reinterpret_cast<offset_t>(ptr), offset));
    }

    template <typename T> constexpr static size_t type_size() noexcept
    {
        using Type = decay_t<T>;
    ...
    void check_ptr(T *el, size_t const size = type_size<T>()) const
    {
        ...
        if constexpr (!std::is_same_v<T, void>) {
            verify((pos & static_cast<intptr_t>(
                              std::alignment_of<decay_t<T>>() - 1)) == 0U,
                   "ptr alignment");
        }
    }
...
struct deep_check_context : public deserialization_context<Mode>
{
    ...
    template <typename T> bool add_checked(T const *v) const
    {
        return checked_.emplace(type_hash<T>(), static_cast<void const *>(v))
            .second;
    }
...
template <typename Ctx, typename T> void check_state(Ctx const &c, T *el)
{
    using Type = decay_t<T>;
...
template <typename Ctx, typename T> void deserialize(Ctx const &c, T *el)
{
    c.check_ptr(el);
                 ...
                 basic_vector<T, Ptr, Indexed, TemplateSizeType> *el)
{
    ...
    c.require((el->size() == 0) == (el->el_ == nullptr),
              "vec size=0 <=> ptr=0");
}
                 ...
                 hash_storage<T, Ptr, GetKey, GetValue, Hash, Eq> *el)
{
    ...
    c.check_ptr(
        ...
        checked_addition(checked_multiplication(
                             el->capacity_,
                             static_cast<typename Type::size_type>(sizeof(T))),
        ...
        el->entries_ == nullptr ||
            reinterpret_cast<uint8_t const *>(ptr_cast(el->ctrl_)) ==
                reinterpret_cast<uint8_t const *>(ptr_cast(el->entries_)) +
                    checked_multiplication(static_cast<size_t>(el->capacity_),
                                           sizeof(T)),
        "hash storage: entries!=null -> ctrl = entries+capacity");
    c.require((el->entries_ == nullptr) ==
                  (el->capacity_ == 0U && el->size_ == 0U),
    ...
    c.require(
        std::all_of(ptr_cast(el->ctrl_),
                    ptr_cast(el->ctrl_) + el->capacity_ + 1U + Type::WIDTH,
                    [](typename Type::ctrl_t const ctrl) {
                        return Type::is_empty(ctrl) || Type::is_deleted(ctrl) ||
                               Type::is_full(ctrl) || ctrl == Type::ctrl_t::END;
                    }),
        ...
        std::accumulate(
            ...
            [&](std::tuple<st_t, st_t, st_t, st_t> const acc,
                ...
                return std::tuple{
                    ...
                    (Type::is_empty(ctrl) &&
                             el->was_never_full(
                                 static_cast<st_t>(&ctrl - el->ctrl_))
                         ? growth_left + 1
                         : growth_left)};
            });

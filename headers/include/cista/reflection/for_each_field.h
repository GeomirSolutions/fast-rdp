...
template <typename T, typename Fn> inline void for_each_ptr_field(T &t, Fn &&fn)
{
...
template <typename T, typename Fn> inline void for_each_field(T &t, Fn &&fn)
{
    if constexpr (std::is_pointer_v<T>) {
        if (t != nullptr) {
...
template <typename T, typename Fn> inline void for_each_field(Fn &&fn)
{
    T t{};

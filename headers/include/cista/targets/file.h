...
struct file
{
...
#endif
            ReadFile(f_, b.data() + from, static_cast<DWORD>(block_size),
                     nullptr, &overlapped);
        });
        ...
        chunk(block_size, size_ - static_cast<size_t>(start),
              [&](auto const from, auto const size) {
                  ...
                  overlapped.OffsetHigh =
                      static_cast<DWORD>((start + from) >> 32U);
#endif
                  DWORD bytes_read = {0};
                  verify(ReadFile(f_, buf, static_cast<DWORD>(size),
                   ...
                   std::size_t alignment)
    {
        ...
        if (alignment != 0 && alignment != 1) {
            ...
            curr_offset = aligned_ptr
                              ? reinterpret_cast<std::uintptr_t>(aligned_ptr)
                              : curr_offset;
        }
...
#endif
            DWORD bytes_written = {0};
            verify(WriteFile(f_, buf, num_padding_bytes, &bytes_written,
                             &overlapped),
                   "write padding error");
        ...
        chunk(block_size, size, [&](size_t const from, unsigned block_size) {
            ...
            DWORD bytes_written = {0};
            verify(WriteFile(
                       f_, reinterpret_cast<unsigned char const *>(ptr) + from,
...
struct file
{
    ...
    uint64_t checksum(offset_t const start = 0) const
    {
        constexpr auto const block_size =
            static_cast<size_t>(512 * 1024); // 512kB
        verify(size_ >= static_cast<size_t>(start), "invalid checksum offset");
        verify(!std::fseek(f_, static_cast<long>(start), SEEK_SET),
               "fseek error");
        auto c = BASE_HASH;
        char buf[block_size];
        chunk(block_size, size_ - static_cast<size_t>(start),

...
template <typename Buf = byte_buf> struct buf
{
    buf() = default;
    explicit buf(Buf &&buf) : buf_{std::forward<Buf>(buf)} {}

    uint8_t *addr(offset_t const offset) noexcept
    {
        return (&buf_[0]) + offset;
    }
    ...
    {
        verify(buf_.size() >= pos + serialized_size<T>(),
               "out of bounds write");
                   ...
                   std::size_t alignment = 0)
    {
        ...
        if (alignment != 0 && alignment != 1 && buf_.size() != 0) {
            ...
            auto const adjustment =
                static_cast<std::size_t>(new_offset - start);
            start += adjustment;
        }
...
template <typename Buf> buf(Buf &&) -> buf<Buf>;

...
template <typename T, typename = void> struct has_std_hash : std::false_type
{
};
...
template <typename T> constexpr bool has_hash_v = detail::has_hash<T>::value;
...
template <std::size_t N> struct is_char_array_helper<char[N]> : std::true_type
{
};
...
template <typename T> struct hashing
{
    ...
    template <typename T1> static constexpr hashing<T1> create() noexcept
    {
    ...
    constexpr hash_t operator()(T const &el, hash_t const seed = BASE_HASH)
    {
        ...
        if constexpr (has_hash_v<Type>) {
            ...
            auto h = seed;
            for_each_field(
                el, [&h](auto &&f) { h = hashing<decltype(f)>{}(f, h); });
            return h;
...
template <typename T1, typename T2> struct hashing<std::pair<T1, T2>>
{
...
template <typename... Args> struct hashing<std::tuple<Args...>>
{
...
template <> struct hashing<char const *>
{
...
template <typename... Args> hash_t build_hash(Args... args)
{
    hash_t h = BASE_HASH;
    ((h = hashing<decltype(args)>{}(args, h)), ...);
    return h;

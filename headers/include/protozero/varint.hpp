 ...
 */
template <typename T> inline int write_varint(T data, uint64_t value)
{
    int n = 1;
...
{
    while (value >= 0x80U) {
        buffer_customization<TBuffer>::push_back(buffer,
...
inline constexpr uint32_t encode_zigzag32(int32_t value) noexcept
{
    return (static_cast<uint32_t>(value) << 1U) ^
           static_cast<uint32_t>(
               -static_cast<int32_t>(static_cast<uint32_t>(value) >> 31U));
}
...
inline constexpr uint64_t encode_zigzag64(int64_t value) noexcept
{
    return (static_cast<uint64_t>(value) << 1U) ^
           static_cast<uint64_t>(
               -static_cast<int64_t>(static_cast<uint64_t>(value) >> 63U));
}

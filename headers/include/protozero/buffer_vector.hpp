...
/// @cond INTERNAL
template <> struct buffer_customization<std::vector<char>>
{
    ...
    static void append(std::vector<char> *buffer, const char *data,
                       std::size_t count)
    {
    ...
    static void erase_range(std::vector<char> *buffer, std::size_t from,
                            std::size_t to)
    {
        protozero_assert(from <= buffer->size());
        protozero_assert(to <= buffer->size());
        protozero_assert(from <= to);
        buffer->erase(std::next(buffer->begin(), from),
                      std::next(buffer->begin(), to));
    }

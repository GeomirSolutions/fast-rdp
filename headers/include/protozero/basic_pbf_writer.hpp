 ...
 */
template <typename TBuffer> class basic_pbf_writer
{
    ...
    void add_varint(uint64_t value)
    {
        protozero_assert(
            m_pos == 0 &&
            "you can't add fields to a parent basic_pbf_writer if there is an "
            "existing basic_pbf_writer for a submessage");
    ...
    {
        protozero_assert(((tag > 0 && tag < 19000) ||
    ...
    template <typename T> void add_fixed(T value)
    {
        protozero_assert(
            m_pos == 0 &&
            "you can't add fields to a parent basic_pbf_writer if there is an "
...
#endif
        buffer_customization<TBuffer>::append(
            m_data, reinterpret_cast<const char *>(&value), sizeof(T));
    }

    template <typename T, typename It>
    void add_packed_fixed(pbf_tag_type tag, It first, It last,
    ...
    void open_submessage(pbf_tag_type tag, std::size_t size)
    {
        ...
        if (size == 0) {
            ...
            buffer_customization<TBuffer>::append_zeros(
                m_data, std::size_t(reserve_bytes));
        } else {
    ...
    void commit_submessage()
    {
        ...
        protozero_assert(m_rollback_pos != size_is_known);
        protozero_assert(m_data);
        const auto length = pbf_length_type(
            buffer_customization<TBuffer>::size(m_data) - m_pos);

        protozero_assert(buffer_customization<TBuffer>::size(m_data) >=
                         m_pos - reserve_bytes);
        const auto n =
            add_varint_to_buffer(buffer_customization<TBuffer>::at_pos(
                                     m_data, m_pos - reserve_bytes),
                                 length);

        buffer_customization<TBuffer>::erase_range(
            m_data, m_pos - reserve_bytes + n, m_pos);
        m_pos = 0;
    }
  ...
  public:
     ...
     * append data.
     */
    explicit basic_pbf_writer(TBuffer &buffer) noexcept : m_data{&buffer} {}
     ...
     */
    basic_pbf_writer(basic_pbf_writer &parent_writer, pbf_tag_type tag,
                     std::size_t size = 0)
        : m_data{parent_writer.m_data}, m_parent_writer{&parent_writer}
    {
    ...
    basic_pbf_writer(basic_pbf_writer &&other) noexcept
        : m_data{other.m_data}, m_parent_writer{other.m_parent_writer},
          m_rollback_pos{other.m_rollback_pos}, m_pos{other.m_pos}
    {
     ...
     */
    bool valid() const noexcept { return m_data != nullptr; }
    ...
    void commit()
    {
        ...
        protozero_assert(m_pos == 0 &&
                         "you can't call commit() on a basic_pbf_writer that "
                         "has an open nested submessage");
        m_parent_writer->close_submessage();
    ...
    void rollback()
    {
        ...
        protozero_assert(m_pos == 0 &&
                         "you can't call rollback() on a basic_pbf_writer that "
                         "has an open nested submessage");
    ...
    void add_bytes_vectored(pbf_tag_type tag, Ts &&...values)
    {
        ...
        protozero_assert(sum_size <=
                         std::numeric_limits<pbf_length_type>::max());
        add_length_varint(tag, pbf_length_type(sum_size));
        buffer_customization<TBuffer>::reserve_additional(m_data, sum_size);
        (void)std::initializer_list<int>{
            (buffer_customization<TBuffer>::append(m_data, values.data(),
                                                   values.size()),
             0)...};
    }
    ...
    void add_packed_bool(pbf_tag_type tag, InputIterator first,
                         InputIterator last)
    {
     ...
     *
     * @tparam InputIterator A type satisfying the InputIterator concept.
     *         Dereferencing the iterator must yield a type assignable to
     * int32_t.
     * @param tag Tag (field number) of the field
     * @param first Iterator pointing to the beginning of the data
     * @param last Iterator pointing one past the end of data
     */
    template <typename InputIterator>
    void add_packed_enum(pbf_tag_type tag, InputIterator first,
                         InputIterator last)
    {
        add_packed_varint(tag, first, last);
    }
     ...
     */
    template <typename InputIterator>
    void add_packed_int32(pbf_tag_type tag, InputIterator first,
                          InputIterator last)
    {
        add_packed_varint(tag, first, last);
    }
     ...
     */
    template <typename InputIterator>
    void add_packed_sint32(pbf_tag_type tag, InputIterator first,
                           InputIterator last)
    {
     ...
     */
    template <typename InputIterator>
    void add_packed_uint32(pbf_tag_type tag, InputIterator first,
                           InputIterator last)
    {
        add_packed_varint(tag, first, last);
    }
     ...
     */
    template <typename InputIterator>
    void add_packed_int64(pbf_tag_type tag, InputIterator first,
                          InputIterator last)
    {
        add_packed_varint(tag, first, last);
    }
     ...
     */
    template <typename InputIterator>
    void add_packed_sint64(pbf_tag_type tag, InputIterator first,
                           InputIterator last)
    {
        add_packed_svarint(tag, first, last);
    }
     ...
     */
    template <typename InputIterator>
    void add_packed_uint64(pbf_tag_type tag, InputIterator first,
                           InputIterator last)
    {
        add_packed_varint(tag, first, last);
    }
     ...
     *
     * @tparam ValueType One of the following types: (u)int32/64_t, double,
     * float.
     * @tparam InputIterator A type satisfying the InputIterator concept.
     * @param tag Tag (field number) of the field
     * @param first Iterator pointing to the beginning of the data
    ...
    void add_packed_fixed(pbf_tag_type tag, InputIterator first,
                          InputIterator last)
    {
        ...
        add_packed_fixed<ValueType, InputIterator>(
            tag, first, last,
     ...
     */
    template <typename InputIterator>
    void add_packed_fixed32(pbf_tag_type tag, InputIterator first,
                            InputIterator last)
    {
        add_packed_fixed<uint32_t, InputIterator>(
            tag, first, last,
            typename std::iterator_traits<InputIterator>::iterator_category{});
    }
     ...
     */
    template <typename InputIterator>
    void add_packed_sfixed32(pbf_tag_type tag, InputIterator first,
                             InputIterator last)
    {
        add_packed_fixed<int32_t, InputIterator>(
            tag, first, last,
            typename std::iterator_traits<InputIterator>::iterator_category{});
    }
     ...
     */
    template <typename InputIterator>
    void add_packed_fixed64(pbf_tag_type tag, InputIterator first,
                            InputIterator last)
    {
        add_packed_fixed<uint64_t, InputIterator>(
            tag, first, last,
            typename std::iterator_traits<InputIterator>::iterator_category{});
    }
     ...
     */
    template <typename InputIterator>
    void add_packed_sfixed64(pbf_tag_type tag, InputIterator first,
                             InputIterator last)
    {
        add_packed_fixed<int64_t, InputIterator>(
            tag, first, last,
            typename std::iterator_traits<InputIterator>::iterator_category{});
    }
     ...
     */
    template <typename InputIterator>
    void add_packed_float(pbf_tag_type tag, InputIterator first,
                          InputIterator last)
    {
        add_packed_fixed<float, InputIterator>(
            tag, first, last,
            typename std::iterator_traits<InputIterator>::iterator_category{});
    }
     ...
     */
    template <typename InputIterator>
    void add_packed_double(pbf_tag_type tag, InputIterator first,
                           InputIterator last)
    {
        add_packed_fixed<double, InputIterator>(
            tag, first, last,
            typename std::iterator_traits<InputIterator>::iterator_category{});
    }
...
inline void swap(basic_pbf_writer<TBuffer> &lhs,
                 basic_pbf_writer<TBuffer> &rhs) noexcept
{
    lhs.swap(rhs);
}
...
template <typename TBuffer> class packed_field
{
  ...
  public:
    ...
    bool valid() const noexcept { return m_writer.valid(); }

    void commit() { m_writer.commit(); }

    void rollback() { m_writer.rollback(); }

    basic_pbf_writer<TBuffer> &writer() noexcept { return m_writer; }
...
class packed_field_fixed : public packed_field<TBuffer>
{
  ...
  public:
    packed_field_fixed() : packed_field<TBuffer>{} {}
    ...
    void add_element(T value) { this->writer().template add_fixed<T>(value); }
...
class packed_field_varint : public packed_field<TBuffer>
{
  ...
  public:
    packed_field_varint() : packed_field<TBuffer>{} {}
    ...
    void add_element(T value) { this->writer().add_varint(uint64_t(value)); }
...
class packed_field_svarint : public packed_field<TBuffer>
{
  ...
  public:
    packed_field_svarint() : packed_field<TBuffer>{} {}

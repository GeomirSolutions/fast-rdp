...
class fixed_size_buffer_adaptor
{
  ...
  public:
    ...
    explicit fixed_size_buffer_adaptor(T &container)
        : m_data(container.data()), m_capacity(container.size())
    {
    }
...
/// @cond INTERNAL
template <> struct buffer_customization<fixed_size_buffer_adaptor>
{
    ...
    static void append(fixed_size_buffer_adaptor *buffer, const char *data,
                       std::size_t count)
    {
        buffer->append(data, count);
    }

    static void append_zeros(fixed_size_buffer_adaptor *buffer,
                             std::size_t count)
    {
    ...
    static void reserve_additional(fixed_size_buffer_adaptor * /*buffer*/,
                                   std::size_t /*size*/)
    {
        /* nothing to be done for fixed-size buffers */
    }

    static void erase_range(fixed_size_buffer_adaptor *buffer, std::size_t from,
                            std::size_t to)
    {

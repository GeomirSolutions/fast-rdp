    ...
    cuda_kernel<<<B, E::nt, 0, p.stream()>>>(
        [=] __device__(auto tid, auto bid) {
            ...
            cuda_strided_iterate<E::nt, E::vt>(
                [=](auto, auto j) { c(*(first + tile.begin + j)); }, tid,
                tile.count());
        });
}
...
/** @private */
template <typename P, typename I, typename C>
void cuda_for_each_index_loop(P &&p, I first, I inc, unsigned count, C c)
{
    ...
    cuda_kernel<<<B, E::nt, 0, p.stream()>>>([=] __device__(auto tid,
        ...
        auto tile = cuda_get_tile(bid, E::nv, count);
        cuda_strided_iterate<E::nt, E::vt>(
            [=] __device__(auto, auto j) { c(first + inc * (tile.begin + j)); },
            tid, tile.count());
    });
}
...
/** @private */
template <typename C> __global__ void cuda_single_task(C callable)
{
    callable();
}

// Function: single_task
template <typename C> cudaTask cudaFlow::single_task(C c)
{
    return kernel(1, 1, 0, cuda_single_task<C>, c);
}

// Function: single_task
template <typename C> void cudaFlow::single_task(cudaTask task, C c)
{
...
// Function: single_task
template <typename C> cudaTask cudaFlowCapturer::single_task(C callable)
{
    return on([=](cudaStream_t stream) mutable {
        cudaDefaultExecutionPolicy p(stream);

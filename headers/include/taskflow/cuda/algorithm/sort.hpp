...
__device__ auto cuda_odd_even_sort(cudaKVArray<K, V, vt> x, C comp,
                                   int flags = 0)
{
    cuda_iterate<vt>([&](auto I) {
#pragma unroll
...
/** @private */
__device__ inline auto cuda_compute_merge_sort_frame(unsigned partition,
                                                     unsigned coop,
                                                     unsigned spacing)
{
                                                     ...
                                                     unsigned spacing)
{
    ...
    return cudaMergeRange{frame.a_begin, min(count, frame.a_end),
                          min(count, frame.b_begin), min(count, frame.b_end)};
}

/** @private */
__device__ inline auto
cuda_compute_merge_sort_range(unsigned count, unsigned partition, unsigned coop,
...
template <unsigned nt, unsigned vt, typename K, typename V> struct cudaBlockSort
{
    ...
    template <typename C>
    __device__ auto merge_pass(cudaKVArray<K, V, vt> x, unsigned tid,
                               unsigned count, unsigned pass, C comp,
                               Storage &storage) const
    {
        ...
        auto mp = cuda_merge_path<cudaMergeBoundType::LOWER>(storage.keys,
                                                             range, diag, comp);
    ...
    template <typename C>
    __device__ auto block_sort(cudaKVArray<K, V, vt> x, unsigned tid,
                               unsigned count, C comp, Storage &storage) const
    {
...
void cuda_merge_sort_partitions(P &&p, K keys, unsigned count, unsigned coop,
                                unsigned spacing, C comp, unsigned *buf)
{
    ...
    cuda_kernel<<<B, nt, 0, p.stream()>>>([=] __device__(auto tid, auto bid) {
        ...
        cuda_strided_iterate<nt, vt>(
            [=](auto, auto j) {
                ...
                auto range =
                    cuda_compute_merge_sort_range(count, index, coop, spacing);
                auto diag = min(spacing * index, count) - range.a_begin;
                buf[index] = cuda_merge_path<cudaMergeBoundType::LOWER>(
                    keys + range.a_begin, range.a_count(), keys + range.b_begin,
                    range.b_count(), diag, comp);
            },
            tid, range.count());
    });
}
...
void merge_sort_loop(P &&p, K_it keys_input, V_it vals_input, unsigned count,
                     C comp, void *buf)
{
    ...
    ////std::cout << "keys_output = " << keys_temp.size()*sizeof(K) <<
    /// std::endl;
    ...
    cuda_kernel<<<B, E::nt, 0, p.stream()>>>([=] __device__(auto tid,
        ...
        // Blocksort.
        auto sorted =
            sort_t().block_sort(unsorted, tid, tile.count(), comp, shared.sort);

        // Store the keys and values.
        cuda_reg_to_mem_thread<E::nt, E::vt>(sorted.keys, tid, tile.count(),
                                             keys_blocksort + tile.begin,
                                             shared.keys);

        if (has_values) {
            cuda_reg_to_mem_thread<E::nt, E::vt>(sorted.vals, tid, tile.count(),
    ...
    for (unsigned pass = 0; pass < R; ++pass) {
        ...
        cuda_merge_sort_partitions(p, keys_input, count, coop, E::nv, comp,
                                   mp_data);
        ...
        cuda_kernel<<<B, E::nt, 0, p.stream()>>>([=] __device__(auto tid,
            ...
            cuda_reg_to_mem_thread<E::nt>(merge.keys, tid, tile.count(),
                                          keys_output + tile.begin,
                                          shared.keys);
            ...
            if (has_values) {
                ...
                cuda_transfer_two_streams_strided<E::nt>(
                    ...
                    vals_input + range.b_begin, range.b_count(), indices, tid,
...
*/
template <typename P, typename K_it, typename V_it, typename C>
void cuda_sort_by_key(P &&p, K_it k_first, K_it k_last, V_it v_first, C comp,
                      void *buf)
{
...
// Function: sort_by_key
template <typename K_it, typename V_it, typename C>
void cudaFlow::sort_by_key(cudaTask task, K_it k_first, K_it k_last,
                           V_it v_first, C comp)
{
    capture(task, [=](cudaFlowCapturer &cap) {
        cap.make_optimizer<cudaLinearCapturing>();
...
cudaTask cudaFlowCapturer::sort(I first, I last, C comp)
{
    ...
    return on([=, buf = MoC{cudaDeviceVector<std::byte>(bufsz)}](
        ...
        cuda_sort(cudaDefaultExecutionPolicy{stream}, first, last, comp,
                  buf.get().data());
    });
}
...
// Function: sort_by_key
template <typename K_it, typename V_it, typename C>
cudaTask cudaFlowCapturer::sort_by_key(K_it k_first, K_it k_last, V_it v_first,
                                       C comp)
{
    ...
    return on([=, buf = MoC{cudaDeviceVector<std::byte>(bufsz)}](
                  cudaStream_t stream) mutable {
        cuda_sort_by_key(cudaDefaultExecutionPolicy{stream}, k_first, k_last,
                         v_first, comp, buf.get().data());
    });
}

// Function: sort_by_key
template <typename K_it, typename V_it, typename C>
void cudaFlowCapturer::sort_by_key(cudaTask task, K_it k_first, K_it k_last,
                                   V_it v_first, C comp)
{

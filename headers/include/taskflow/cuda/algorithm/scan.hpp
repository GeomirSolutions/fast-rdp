...
template <unsigned nt, typename T> struct cudaBlockScan
{
    ...
    __device__ cudaScanResult<T>
    operator()(unsigned tid, T x, storage_t &storage, unsigned count = nt,
               op_t op = op_t(), T init = T(),
    ...
    __device__ cudaScanResult<T, vt>
    operator()(unsigned tid, cudaArray<T, vt> x, storage_t &storage,
               T carry_in = T(), bool use_carry_in = false, unsigned count = nt,
               op_t op = op_t(), T init = T(),
               cudaScanType type = cudaScanType::EXCLUSIVE) const;
};
...
template <typename op_t>
__device__ cudaScanResult<T>
cudaBlockScan<nt, T>::operator()(unsigned tid, T x, storage_t &storage,
                                 unsigned count, op_t op, T init,
...
__device__ cudaScanResult<T, vt> cudaBlockScan<nt, T>::operator()(
    unsigned tid, cudaArray<T, vt> x, storage_t &storage, T carry_in,
    bool use_carry_in, unsigned count, op_t op, T init, cudaScanType type) const
{
    ...
    auto result = operator()(tid, x[vt - 1], storage, (count + vt - 1) / vt, op,
                             init, cudaScanType::EXCLUSIVE);
...
void cuda_single_pass_scan(P &&p, cudaScanType scan_type, I input,
...
)
{
    ...
    cuda_kernel<<<1, E::nt, 0, p.stream()>>>([=] __device__(auto tid,
                                                            auto bid) {
        ...
        for (unsigned cur = 0; cur < count; cur += E::nv) {
            ...
            auto x = cuda_mem_to_reg_thread<E::nt, E::vt>(
                input + cur, tid, count2, shared.values);

            auto result = scan_t()(tid, x, shared.scan, carry_in, cur > 0,
                                   count2, op, T(), scan_type);
...
*/
template <typename P, typename I, typename O, typename C>
void cuda_scan_loop(P &&p, cudaScanType scan_type, I input, unsigned count,
                    O output, C op,
                    // reduction_it reduction,
                    void *ptr)
{
    ...
    if (B > cudaScanRecursionThreshold) {
        ...
        cuda_kernel<<<B, E::nt, 0, p.stream()>>>([=] __device__(auto tid,
            ...
            auto x = cuda_mem_to_reg_strided<E::nt, E::vt>(input + tile.begin,
                                                           tid, tile.count());
            ...
            auto all_reduce =
                cudaBlockReduce<E::nt, T>()(tid, scalar, shm, tile.count(), op);
        ...
        cuda_scan_loop(p, cudaScanType::EXCLUSIVE, buffer, B, buffer, op,
                       buffer + B);

        // downsweep: perform an intra-tile scan and add the scan of the
        ...
        cuda_kernel<<<B, E::nt, 0, p.stream()>>>(
            [=] __device__(auto tid, auto bid) {
                ...
                auto y = scan_t()(tid, x, shared.scan, buffer[bid], bid > 0,
                                  tile.count(), op, T(), scan_type)
                             .scan;
...
*/
template <typename P, typename T> unsigned cuda_scan_buffer_size(unsigned count)
{
    using E = std::decay_t<P>;
    unsigned B = (count + E::nv - 1) / E::nv;
    unsigned n = 0;
    for (auto b = B; b > detail::cudaScanRecursionThreshold;
         b = (b + E::nv - 1) / E::nv) {
        n += b;
    }
    return n * sizeof(T);
...
void cuda_inclusive_scan(P &&p, I first, I last, O output, C op, void *buf)
{
    ...
    // launch the scan loop
    detail::cuda_scan_loop(p, detail::cudaScanType::INCLUSIVE, first, count,
                           output, op, buf);
}
...
*/
template <typename P, typename I, typename O, typename C, typename U>
void cuda_transform_inclusive_scan(P &&p, I first, I last, O output, C bop,
                                   U uop, void *buf)
{
    ...
    // launch the scan loop
    detail::cuda_scan_loop(p, detail::cudaScanType::INCLUSIVE,
...
void cuda_exclusive_scan(P &&p, I first, I last, O output, C op, void *buf)
{
    ...
    // launch the scan loop
    detail::cuda_scan_loop(p, detail::cudaScanType::EXCLUSIVE, first, count,
                           output, op, buf);
}
...
*/
template <typename P, typename I, typename O, typename C, typename U>
void cuda_transform_exclusive_scan(P &&p, I first, I last, O output, C bop,
                                   U uop, void *buf)
{
    ...
    // launch the scan loop
    detail::cuda_scan_loop(p, detail::cudaScanType::EXCLUSIVE,
                           cuda_make_load_iterator<T>([=] __device__(auto i) {
                               return uop(*(first + i));
                           }),
                           count, output, bop, buf);
...
cudaTask cudaFlow::transform_inclusive_scan(I first, I last, O output, B bop,
                                            U uop)
{
    return capture([=](cudaFlowCapturer &cap) {
        cap.make_optimizer<cudaLinearCapturing>();
...
// Function: transform_inclusive_scan
template <typename I, typename O, typename B, typename U>
void cudaFlow::transform_inclusive_scan(cudaTask task, I first, I last,
                                        O output, B bop, U uop)
{
    capture(task, [=](cudaFlowCapturer &cap) {
        cap.make_optimizer<cudaLinearCapturing>();
        cap.transform_inclusive_scan(first, last, output, bop, uop);
...
template <typename I, typename O, typename B, typename U>
cudaTask cudaFlow::transform_exclusive_scan(I first, I last, O output, B bop,
                                            U uop)
{
    return capture([=](cudaFlowCapturer &cap) {
        cap.make_optimizer<cudaLinearCapturing>();
...
// Function: transform_exclusive_scan
template <typename I, typename O, typename B, typename U>
void cudaFlow::transform_exclusive_scan(cudaTask task, I first, I last,
                                        O output, B bop, U uop)
{
    capture(task, [=](cudaFlowCapturer &cap) {
        cap.make_optimizer<cudaLinearCapturing>();
...
// Function: inclusive_scan
template <typename I, typename O, typename C>
void cudaFlowCapturer::inclusive_scan(cudaTask task, I first, I last, O output,
                                      C op)
{
...
// Function: exclusive_scan
template <typename I, typename O, typename C>
void cudaFlowCapturer::exclusive_scan(cudaTask task, I first, I last, O output,
                                      C op)
{
...
// Function: transform_inclusive_scan
template <typename I, typename O, typename B, typename U>
cudaTask cudaFlowCapturer::transform_inclusive_scan(I first, I last, O output,
                                                    B bop, U uop)
{
    ...
    return on([=, buf = MoC{cudaDeviceVector<std::byte>(bufsz)}](
                  cudaStream_t stream) mutable {
        cudaDefaultExecutionPolicy p(stream);
        cuda_transform_inclusive_scan(p, first, last, output, bop, uop,
                                      buf.get().data());
    });
}
...
// Function: transform_inclusive_scan
template <typename I, typename O, typename B, typename U>
void cudaFlowCapturer::transform_inclusive_scan(cudaTask task, I first, I last,
                                                O output, B bop, U uop)
{
...
// Function: transform_exclusive_scan
template <typename I, typename O, typename B, typename U>
cudaTask cudaFlowCapturer::transform_exclusive_scan(I first, I last, O output,
                                                    B bop, U uop)
{
    ...
    return on([=, buf = MoC{cudaDeviceVector<std::byte>(bufsz)}](
                  cudaStream_t stream) mutable {
        cudaDefaultExecutionPolicy p(stream);
        cuda_transform_exclusive_scan(p, first, last, output, bop, uop,
                                      buf.get().data());
    });
}
...
// Function: transform_exclusive_scan
template <typename I, typename O, typename B, typename U>
void cudaFlowCapturer::transform_exclusive_scan(cudaTask task, I first, I last,
                                                O output, B bop, U uop)
{

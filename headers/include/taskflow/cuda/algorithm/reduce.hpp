...
/** @private */
template <unsigned nt, typename T> struct cudaBlockReduce
{
    ...
    static_assert(nt && (0 == nt % CUDA_WARP_SIZE),
                  "cudaBlockReduce requires num threads to be a multiple of "
                  "warp_size (32)");
    ...
    __device__ T operator()(unsigned, T, Storage &, unsigned, op_t,
                            bool = true) const;
};
...
template <typename op_t>
__device__ T cudaBlockReduce<nt, T>::operator()(unsigned tid, T x,
                                                Storage &storage,
                                                ...
                                                bool ret) const
{
    ...
    if (tid < group_size) {
        ...
        cuda_strided_iterate<group_size, num_items>(
            [&](auto i, auto j) {
                if (i > 0) {
    ...
    cuda_kernel<<<B, E::nt, 0, p.stream()>>>(
        [=] __device__(auto tid, auto bid) {
            ...
            cuda_strided_iterate<E::nt, E::vt>(
                [&](auto i, auto) { s = i ? op(s, x[i]) : x[0]; }, tid,
                tile.count());
            // reduce to a scalar per block.
            s = cudaBlockReduce<E::nt, U>()(
                tid, s, shm, (tile.count() < E::nt ? tile.count() : E::nt), op,
                false);
            if (!tid) {
...
/** @private */
template <typename P, typename I, typename T, typename O>
void cuda_uninitialized_reduce_loop(P &&p, I input, unsigned count, T *res,
                                    O op, void *ptr)
{
...
/**
@brief performs asynchronous parallel reduction over a range of transformed
items without an initial value
...
*/
template <typename P, typename I, typename T, typename O, typename U>
void cuda_transform_uninitialized_reduce(P &&p, I first, I last, T *res, O bop,
                                         U uop, void *buf)
{
    ...
    //);
    detail::cuda_uninitialized_reduce_loop(
        p, cuda_make_load_iterator<T>([=] __device__(auto i) {
            return uop(*(first + i));
        }),
        count, res, bop, buf);
}
...
cudaTask cudaFlowCapturer::transform_reduce(I first, I last, T *result, C bop,
                                            U uop)
{
    ...
    return on([=, buf = MoC{cudaDeviceVector<std::byte>(bufsz)}](
                  cudaStream_t stream) mutable {
        cudaDefaultExecutionPolicy p(stream);
        cuda_transform_reduce(p, first, last, result, bop, uop,
                              buf.get().data());
    });
}
...
template <typename I, typename T, typename C, typename U>
cudaTask cudaFlowCapturer::transform_uninitialized_reduce(I first, I last,
                                                          T *result, C bop,
                                                          U uop)
{
    ...
    return on([=, buf = MoC{cudaDeviceVector<std::byte>(bufsz)}](
                  cudaStream_t stream) mutable {
        cudaDefaultExecutionPolicy p(stream);
        cuda_transform_uninitialized_reduce(p, first, last, result, bop, uop,
                                            buf.get().data());
    });
}
...
// Function: reduce
template <typename I, typename T, typename C>
void cudaFlowCapturer::reduce(cudaTask task, I first, I last, T *result, C c)
{
...
// Function: uninitialized_reduce
template <typename I, typename T, typename C>
void cudaFlowCapturer::uninitialized_reduce(cudaTask task, I first, I last,
                                            T *result, C c)
{
    // TODO
    auto bufsz = cuda_reduce_buffer_size<cudaDefaultExecutionPolicy, T>(
...
// Function: transform_reduce
template <typename I, typename T, typename C, typename U>
void cudaFlowCapturer::transform_reduce(cudaTask task, I first, I last,
...
// Function: transform_uninitialized_reduce
template <typename I, typename T, typename C, typename U>
void cudaFlowCapturer::transform_uninitialized_reduce(cudaTask task, I first,
                                                      I last, T *result, C bop,
                                                      U uop)
{
...
// Function: transform_uninitialized_reduce
template <typename I, typename T, typename B, typename U>
cudaTask cudaFlow::transform_uninitialized_reduce(I first, I last, T *result,
                                                  B bop, U uop)
{
    return capture([=](cudaFlowCapturer &cap) {
        cap.make_optimizer<cudaLinearCapturing>();
...
// Function: uninitialized_reduce
template <typename I, typename T, typename C>
void cudaFlow::uninitialized_reduce(cudaTask task, I first, I last, T *result,
                                    C op)
{
    capture(task, [=](cudaFlowCapturer &cap) {
        cap.make_optimizer<cudaLinearCapturing>();
...
// Function: transform_reduce
template <typename I, typename T, typename B, typename U>
void cudaFlow::transform_reduce(cudaTask task, I first, I last, T *result,
                                B bop, U uop)
{
    capture(task, [=](cudaFlowCapturer &cap) {
        cap.make_optimizer<cudaLinearCapturing>();
...
// Function: transform_uninitialized_reduce
template <typename I, typename T, typename B, typename U>
void cudaFlow::transform_uninitialized_reduce(cudaTask task, I first, I last,

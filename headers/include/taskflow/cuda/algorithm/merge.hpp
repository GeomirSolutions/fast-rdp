...
struct cudaMergeRange
{
    ...
    {
        return cudaMergeRange{a_begin + mp0, a_end, b_begin + diag - mp0,
                              b_end};
    }
    ...
    {
        return cudaMergeRange{a_begin + mp0, a_begin + mp1,
                                ...
                                unsigned diag, comp_t comp)
{
    ...
    while (beg < end) {
        ...
        bool pred = (cudaMergeBoundType::UPPER == bounds) ? comp(a_key, b_key)
                                                          : !comp(b_key, a_key);
                                ...
                                unsigned diag, comp_t comp)
{
    ...
    return cuda_merge_path<bounds>(keys + range.a_begin, range.a_count(),
                                   keys + range.b_begin, range.b_count(), diag,
                                   comp);
}

/** @private */
template <cudaMergeBoundType bounds, bool range_check, typename T,
          typename comp_t>
__device__ bool cuda_merge_predicate(T a_key, T b_key, cudaMergeRange range,
                                     comp_t comp)
{
...
cuda_compute_merge_range(unsigned a_count, unsigned b_count, unsigned partition,
                         unsigned spacing, unsigned mp0, unsigned mp1)
{
...
__device__ auto cuda_load_two_streams_reg(const T *a, unsigned a_count,
                                          const T *b, unsigned b_count,
                                          unsigned tid)
{
...
__device__ std::enable_if_t<!(std::is_pointer<a_it>::value &&
                              std::is_pointer<b_it>::value),
                            cudaArray<T, vt>>
load_two_streams_reg(a_it a, unsigned a_count, b_it b, unsigned b_count,
                     unsigned tid)
{
    b -= a_count;
    cudaArray<T, vt> x;
...
/** @private */
template <unsigned nt, unsigned vt, typename A, typename B, typename T,
          unsigned S>
__device__ void cuda_load_two_streams_shared(A a, unsigned a_count, B b,
                                             unsigned b_count, unsigned tid,
                                             T (&shared)[S], bool sync = true)
{
...
/** @private */
template <unsigned nt, unsigned vt, typename T>
__device__ auto cuda_gather_two_streams_strided(const T *a, unsigned a_count,
                                                const T *b, unsigned b_count,
                                                cudaArray<unsigned, vt> indices,
                                                unsigned tid)
{
...
__device__ std::enable_if_t<!(std::is_pointer<a_it>::value &&
                              std::is_pointer<b_it>::value),
                            cudaArray<T, vt>>
cuda_gather_two_streams_strided(a_it a, unsigned a_count, b_it b,
                                unsigned b_count,
                                cudaArray<unsigned, vt> indices, unsigned tid)
{
    ...
    cudaArray<unsigned, vt> indices, unsigned tid, c_it c)
{
    ...
    auto x = cuda_gather_two_streams_strided<nt, vt, T>(a, a_count, b, b_count,
                                                        indices, tid);
...
__device__ auto cuda_serial_merge(const T *keys_shared, cudaMergeRange range,
...
*/
template <cudaMergeBoundType bounds, unsigned nt, unsigned vt, typename a_it,
          typename b_it, typename T, typename comp_t, unsigned S>
__device__ auto block_merge_from_mem(a_it a, b_it b, cudaMergeRange range_mem,
                                     unsigned tid, comp_t comp,
                                     T (&keys_shared)[S])
{
    ...
    cuda_load_two_streams_shared<nt, vt>(
        a + range_mem.a_begin, range_mem.a_count(), b + range_mem.b_begin,
        range_mem.b_count(), tid, keys_shared, true);
...
/** @private */
template <cudaMergeBoundType bounds, typename P, typename a_keys_it,
          typename b_keys_it, typename comp_t>
void cuda_merge_path_partitions(P &&p, a_keys_it a, unsigned a_count,
                                b_keys_it b, unsigned b_count, unsigned spacing,
                                comp_t comp, unsigned *buf)
{
    ...
    cuda_kernel<<<B, nt, 0, p.stream()>>>([=] __device__(auto tid, auto bid) {
        ...
        cuda_strided_iterate<nt, vt>(
            [=](auto, auto j) {
                ...
                buf[index] =
                    cuda_merge_path<bounds>(a, a_count, b, b_count, diag, comp);
            },
            tid, range.count());
    });
}
...
/** @private */
template <typename P, typename a_keys_it, typename a_vals_it,
          typename b_keys_it, typename b_vals_it, typename c_keys_it,
          typename c_vals_it, typename comp_t>
void cuda_merge_loop(P &&p, a_keys_it a_keys, a_vals_it a_vals,
                     unsigned a_count, b_keys_it b_keys, b_vals_it b_vals,
                     unsigned b_count, c_keys_it c_keys, c_vals_it c_vals,
                     comp_t comp, void *ptr)
{
    ...
    cuda_kernel<<<B, E::nt, 0, p.stream()>>>([=] __device__(auto tid,
        ...
        auto mp1 = buf[bid + 1];
        auto range =
            cuda_compute_merge_range(a_count, b_count, bid, E::nv, mp0, mp1);

        auto merge =
            block_merge_from_mem<cudaMergeBoundType::LOWER, E::nt, E::vt>(
                a_keys, b_keys, range, tid, comp, shared.keys);

        auto dest_offset = E::nv * bid;
        cuda_reg_to_mem_thread<E::nt>(merge.keys, tid, range.total(),
                                      c_keys + dest_offset, shared.keys);

        if (has_values) {
            // Transpose the indices from thread order to strided order.
            auto indices = cuda_reg_thread_to_strided<E::nt>(merge.indices, tid,
                                                             shared.indices);

            // Gather the input values and merge into the output values.
            cuda_transfer_two_streams_strided<E::nt>(
                a_vals + range.a_begin, range.a_count(), b_vals + range.b_begin,
                range.b_count(), indices, tid, c_vals + dest_offset);
        }
    });
}
...
//   cudaDeviceVector<std::byte> temp(cuda_merge_buffer_size<P>(a_count,
//   b_count));
//
...
into a single range, <tt>[c_keys_first, c_keys_last + (a_keys_last -
a_keys_first) + (b_keys_last - b_keys_first))</tt> such that the resulting range
...
<tt>[c_vals_first, c_vals_first + (a_keys_last - a_keys_first) + (b_keys_last -
b_keys_first))</tt> such that the resulting range is in ascending order implied
by each input element's associated key.
...
template <typename P, typename a_keys_it, typename a_vals_it,
          typename b_keys_it, typename b_vals_it, typename c_keys_it,
          typename c_vals_it, typename C>
void cuda_merge_by_key(P &&p, a_keys_it a_keys_first, a_keys_it a_keys_last,
                       a_vals_it a_vals_first, b_keys_it b_keys_first,
                       b_keys_it b_keys_last, b_vals_it b_vals_first,
                       c_keys_it c_keys_first, c_vals_it c_vals_first, C comp,
                       void *buf)
{
    ...
    detail::cuda_merge_loop(p, a_keys_first, a_vals_first, a_count,
                            b_keys_first, b_vals_first, b_count, c_keys_first,
                            c_vals_first, comp, buf);
}
...
*/
template <typename P, typename a_keys_it, typename b_keys_it,
          typename c_keys_it, typename C>
void cuda_merge(P &&p, a_keys_it a_keys_first, a_keys_it a_keys_last,
                b_keys_it b_keys_first, b_keys_it b_keys_last,
                c_keys_it c_keys_first, C comp, void *buf)
{
    cuda_merge_by_key(p, a_keys_first, a_keys_last, (const cudaEmpty *)nullptr,
                      b_keys_first, b_keys_last, (const cudaEmpty *)nullptr,
                      c_keys_first, (cudaEmpty *)nullptr, comp, buf);
}
...
cudaTask cudaFlow::merge(A a_first, A a_last, B b_first, B b_last, C c_first,
                         Comp comp)
{
...
// Function: merge
template <typename A, typename B, typename C, typename Comp>
void cudaFlow::merge(cudaTask task, A a_first, A a_last, B b_first, B b_last,
                     C c_first, Comp comp)
{
...
template <typename a_keys_it, typename a_vals_it, typename b_keys_it,
          typename b_vals_it, typename c_keys_it, typename c_vals_it,
          typename C>
cudaTask cudaFlow::merge_by_key(a_keys_it a_keys_first, a_keys_it a_keys_last,
                                a_vals_it a_vals_first, b_keys_it b_keys_first,
                                b_keys_it b_keys_last, b_vals_it b_vals_first,
                                ...
                                C comp)
{
    return capture([=](cudaFlowCapturer &cap) {
        cap.make_optimizer<cudaLinearCapturing>();
        cap.merge_by_key(a_keys_first, a_keys_last, a_vals_first, b_keys_first,
                         b_keys_last, b_vals_first, c_keys_first, c_vals_first,
                         comp);
    });
}
...
template <typename a_keys_it, typename a_vals_it, typename b_keys_it,
          typename b_vals_it, typename c_keys_it, typename c_vals_it,
          typename C>
void cudaFlow::merge_by_key(cudaTask task, a_keys_it a_keys_first,
                            a_keys_it a_keys_last, a_vals_it a_vals_first,
                            b_keys_it b_keys_first, b_keys_it b_keys_last,
                            b_vals_it b_vals_first, c_keys_it c_keys_first,
                            c_vals_it c_vals_first, C comp)
{
    capture(task, [=](cudaFlowCapturer &cap) {
        cap.make_optimizer<cudaLinearCapturing>();
        cap.merge_by_key(a_keys_first, a_keys_last, a_vals_first, b_keys_first,
...
// Function: merge
template <typename A, typename B, typename C, typename Comp>
cudaTask cudaFlowCapturer::merge(A a_first, A a_last, B b_first, B b_last,
                                 C c_first, Comp comp)
{
    // TODO
    ...
    return on([=, buf = MoC{cudaDeviceVector<std::byte>(bufsz)}](
        ...
        cuda_merge(cudaDefaultExecutionPolicy{stream}, a_first, a_last, b_first,
                   b_last, c_first, comp, buf.get().data());
    });
}

// Procedure: merge (update)
template <typename A, typename B, typename C, typename Comp>
void cudaFlowCapturer::merge(cudaTask task, A a_first, A a_last, B b_first,
                             B b_last, C c_first, Comp comp)
{
    // TODO
    auto bufsz = cuda_merge_buffer_size<cudaDefaultExecutionPolicy>(
        std::distance(a_first, a_last), std::distance(b_first, b_last));
    ...
    c_keys_it c_keys_first, c_vals_it c_vals_first, C comp)
{
    ...
    return on([=, buf = MoC{cudaDeviceVector<std::byte>(bufsz)}](
                  cudaStream_t stream) mutable {
        cuda_merge_by_key(cudaDefaultExecutionPolicy{stream}, a_keys_first,
                          a_keys_last, a_vals_first, b_keys_first, b_keys_last,
                          b_vals_first, c_keys_first, c_vals_first, comp,
                          buf.get().data());
    });
}
...
void cudaFlowCapturer::merge_by_key(
    cudaTask task, a_keys_it a_keys_first, a_keys_it a_keys_last,
    a_vals_it a_vals_first, b_keys_it b_keys_first, b_keys_it b_keys_last,
    b_vals_it b_vals_first, c_keys_it c_keys_first, c_vals_it c_vals_first,
    C comp)
{

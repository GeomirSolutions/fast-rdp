...
class cudaFlowCapturer
{
    ...
    using Optimizer =
        std::variant<cudaRoundRobinCapturing, cudaSequentialCapturing,
                     cudaLinearCapturing>;
  ...
  public:
    ...
    */
    ...
    @tparam C callable type constructible with @c
    std::function<void(cudaStream_t)>
    ...
    */
    template <typename C,
              std::enable_if_t<std::is_invocable_r_v<void, C, cudaStream_t>,
                               void> * = nullptr>
    ...
    */
    ...
    /**
    @brief captures a kernel that applies a callable to each dereferenced
    ...
    */
    template <typename I, typename T, typename C, typename U>
    void transform_reduce(cudaTask task, I first, I last, T *result, C bop,
                          U uop);
    ...
    */
    template <typename I, typename T, typename C, typename U>
    cudaTask transform_uninitialized_reduce(I first, I last, T *result, C bop,
                                            U uop);

    /**
    @brief updates a capture task to a transform-reduce task of no initialized
    value
    ...
    */
    ...
    /**
    @brief offloads the captured %cudaFlow onto a GPU and repeatedly runs it
...
inline cudaFlowCapturer::cudaFlowCapturer(cudaGraph &g)
    : _handle{std::in_place_type_t<Proxy>{}}, _graph{g}
{
}

// constructs a cudaFlow capturer from a taskflow
inline cudaFlowCapturer::cudaFlowCapturer(cudaGraph &g, Executor &e)
    : _handle{std::in_place_type_t<Internal>{}, e}, _graph{g}
{
}
...
// Function: empty
inline bool cudaFlowCapturer::empty() const { return _graph.empty(); }
...
{
    auto node =
        _graph.emplace_back(_graph, std::in_place_type_t<cudaNode::Capture>{},
...
// Function: memcpy
inline cudaTask cudaFlowCapturer::memcpy(void *dst, const void *src,
                                         size_t count)
{
...
cudaTask cudaFlowCapturer::copy(T *tgt, const T *src, size_t num)
{
    return on([tgt, src, num](cudaStream_t stream) mutable {
        TF_CHECK_CUDA(cudaMemcpyAsync(tgt, src, sizeof(T) * num,
...
inline cudaTask cudaFlowCapturer::memset(void *ptr, int v, size_t n)
{
    return on([ptr, v, n](cudaStream_t stream) mutable {
        TF_CHECK_CUDA(cudaMemsetAsync(ptr, v, n, stream),
...
// Function: kernel
template <typename F, typename... ArgsT>
cudaTask cudaFlowCapturer::kernel(dim3 g, dim3 b, size_t s, F f,
                                  ArgsT &&...args)
{
...
{
    return std::visit([this](auto &&opt) { return opt._optimize(_graph); },
                      _optimizer);
}

// Procedure: offload_until
template <typename P> void cudaFlowCapturer::offload_until(P &&predicate)
{
...
// Function: memcpy
inline void cudaFlowCapturer::memcpy(cudaTask task, void *dst, const void *src,
                                     size_t count)
{
...
// Function: kernel
template <typename F, typename... ArgsT>
void cudaFlowCapturer::kernel(cudaTask task, dim3 g, dim3 b, size_t s, F f,
                              ArgsT &&...args)
{

...
class cudaStream
{
  ...
  public:
    ...
    */
    ~cudaStream() { cudaStreamDeleter{}(_stream); }
    ...
    */
    ...
    {
        TF_CHECK_CUDA(cudaStreamSynchronize(_stream),
    ...
    + @c cudaStreamCaptureModeThreadLocal: If the local thread has an ongoing
    capture sequence not initiated with @c cudaStreamCaptureModeRelaxed, it is
    prohibited from potentially unsafe API calls. Concurrent capture sequences
    in other threads are ignored.
    ...
    prohibited from API calls which necessarily conflict with stream capture,
    for example, attempting @c cudaEventQuery on an event that was last recorded
      inside a capture sequence.
    */
    void
    begin_capture(cudaStreamCaptureMode m = cudaStreamCaptureModeGlobal) const
    {
        TF_CHECK_CUDA(cudaStreamBeginCapture(_stream, m),
                      "failed to begin capture on stream ", _stream,
                      " with thread mode ", m);
    }
    ...
    cudaStream::begin_capture. If capture was invalidated, due to a violation of
    the rules of stream capture, then a NULL graph will be returned.
    */
    cudaGraph_t end_capture() const
    {
        cudaGraph_t native_g;
        TF_CHECK_CUDA(cudaStreamEndCapture(_stream, &native_g),
    ...
    */
    ...
    {
        TF_CHECK_CUDA(cudaEventRecord(event, _stream),
    ...
    */
    ...
    {
        TF_CHECK_CUDA(cudaStreamWaitEvent(_stream, event, 0),
                      "failed to wait for event ", event, " on stream ",
                      _stream);
    }
...
class cudaEvent
{
    ...
    struct cudaEventCreator
    {
        ...
        {
            cudaEvent_t event;
            TF_CHECK_CUDA(cudaEventCreate(&event),
        ...
        {
            cudaEvent_t event;
            TF_CHECK_CUDA(cudaEventCreateWithFlags(&event, flag),
  ...
  public:
    ...
    */
    ...
    @brief move constructor
    */
    cudaEvent(cudaEvent &&rhs) : _event{rhs._event} { rhs._event = nullptr; }

    /**
    @brief destructs the CUDA event
    */
    ~cudaEvent() { cudaEventDeleter{}(_event); }

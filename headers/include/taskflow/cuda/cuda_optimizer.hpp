...
cudaCapturingBase::_levelize(cudaGraph &graph)
{
    ...
    for (auto &u : graph._nodes) {
        ...
        //   assert(hu.level <
        //   std::get_if<cudaNode::Capture>(&s->_handle)->level);
        // }
    }
...
The algorithm is based on the following paper published in Euro-Par 2021:
  + Dian-Lun Lin and Tsung-Wei Huang, &quot;Efficient GPU Computation using
%Task Graph Parallelism,&quot; <i>European Conference on Parallel and
Distributed Computing (Euro-Par)</i>, 2021
...
(e.g., kernels and memory copies) with many of them being able to run in
parallel. You can configure the number of streams to the optimizer to adjust the
...
inline void
cudaRoundRobinCapturing::_reset(std::vector<std::vector<cudaNode *>> &graph)
{
    // level == global id
            ...
            for (auto &pn : node->_dependents) {
                ...
                if (psid == hn->idx) {
                    if (wait_node == nullptr ||
                        std::get_if<cudaNode::Capture>(&wait_node->_handle)
                                ->level < phn->level) {
                        wait_node = pn;
                    }
                } else if (psid != sid) {

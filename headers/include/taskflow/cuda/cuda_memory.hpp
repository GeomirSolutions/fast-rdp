...
inline size_t cuda_get_free_mem(int d)
{
    ...
    size_t free, total;
    TF_CHECK_CUDA(cudaMemGetInfo(&free, &total),
...
/**
@brief allocates memory on the given device for holding @c N elements of type @c
T

The function calls @c cudaMalloc to allocate <tt>N*sizeof(T)</tt> bytes of
memory on the given device @c d and returns a pointer to the starting address of
the device memory.
*/
template <typename T> T *cuda_malloc_device(size_t N, int d)
{
    cudaScopedDevice ctx(d);
    T *ptr{nullptr};
    TF_CHECK_CUDA(cudaMalloc(&ptr, N * sizeof(T)),
                  "failed to allocate memory (", N * sizeof(T),
                  "bytes) on device ", d)
    return ptr;
}
...
{
    T *ptr{nullptr};
    TF_CHECK_CUDA(cudaMallocManaged(&ptr, N * sizeof(T)),
...
*/
inline void cuda_memcpy_async(cudaStream_t stream, void *dst, const void *src,
                              size_t count)
{
    TF_CHECK_CUDA(cudaMemcpyAsync(dst, src, count, cudaMemcpyDefault, stream),
...
*/
inline void cuda_memset_async(cudaStream_t stream, void *devPtr, int value,
                              size_t count)
{
    TF_CHECK_CUDA(cudaMemsetAsync(devPtr, value, count, stream),
...
template <typename T> class cudaDeviceAllocator
{
  ...
  public:
    ...
    */
    ...
    /**
    @brief its member type @c U is the equivalent allocator type to allocate
    elements of type U
    */
    ...
    */
    ...
    @param n number of elements (each of size sizeof(value_type)) to be
    allocated
    ...
    */
    ...
    {
        void *ptr = NULL;
        TF_CHECK_CUDA(cudaMalloc(&ptr, n * sizeof(T)), "failed to allocate ", n,
                      " elements (", n * sizeof(T), "bytes)")
  ...
  public:
    ...
    */
    pointer allocate(size_type n, std::allocator<void>::const_pointer = 0)
    {
        void *ptr{nullptr};
        TF_CHECK_CUDA(cudaMallocManaged(&ptr, n * sizeof(T)),
                      "failed to allocate ", n, " elements (", n * sizeof(T),
                      "bytes)")
        return static_cast<pointer>(ptr);
    }
...
// template <typename T>
// using cudaDeviceVector = std::vector<NoInit<T>,
// cudaDeviceAllocator<NoInit<T>>>;
...
template <typename T> class cudaDeviceVector
{
  ...
  public:
    ...
    {
        if (N) {
            TF_CHECK_CUDA(cudaMalloc(&_data, N * sizeof(T)),
                          "failed to allocate device memory (", N * sizeof(T),
                          " bytes)");
        }
    }

    cudaDeviceVector(cudaDeviceVector &&rhs) : _data{rhs._data}, _N{rhs._N}
    {
        rhs._data = nullptr;
        rhs._N = 0;
    }

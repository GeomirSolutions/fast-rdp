...
cudaMemcpy3DParms cuda_get_copy_parms(T *tgt, const T *src, size_t num)
{
    ...
    p.srcPtr =
        ::make_cudaPitchedPtr(const_cast<T *>(src), num * sizeof(U), num, 1);
    p.dstArray = nullptr;
...
*/
inline cudaMemcpy3DParms cuda_get_memcpy_parms(void *tgt, const void *src,
                                               size_t bytes)
{
...
{
    size_t num_nodes;
    TF_CHECK_CUDA(cudaGraphGetRootNodes(graph, nullptr, &num_nodes),
...
{
    size_t num_nodes;
    TF_CHECK_CUDA(cudaGraphGetNodes(graph, nullptr, &num_nodes),
...
{
    size_t num_edges;
    TF_CHECK_CUDA(cudaGraphGetEdges(graph, nullptr, nullptr, &num_edges),
...
inline std::vector<cudaGraphNode_t> cuda_get_graph_nodes(cudaGraph_t graph)
{
    ...
    TF_CHECK_CUDA(cudaGraphGetNodes(graph, nodes.data(), &num_nodes),
                  "failed to get native graph nodes");
    return nodes;
}
...
inline std::vector<cudaGraphNode_t> cuda_get_graph_root_nodes(cudaGraph_t graph)
{
    ...
    std::vector<cudaGraphNode_t> nodes(num_nodes);
    TF_CHECK_CUDA(cudaGraphGetRootNodes(graph, nodes.data(), &num_nodes),
                  "failed to get native graph nodes");
    return nodes;
}
...
inline cudaGraphNodeType cuda_get_graph_node_type(cudaGraphNode_t node)
{
    ...
    TF_CHECK_CUDA(cudaGraphNodeGetType(node, &type),
    ...
    while (stack.empty() == false) {
        ...
        for (auto &node : nodes) {
            ...
            if (type == cudaGraphNodeTypeGraph) {
                ...
                os << 'p' << node << "["
                   << "shape=folder, style=filled, fontcolor=white, "
                      "fillcolor=purple, "
...
class cudaGraphNative
{
    ...
    struct cudaGraphNativeCreator
    {
        ...
        {
            cudaGraph_t g;
            TF_CHECK_CUDA(cudaGraphCreate(&g, 0),
  ...
  public:
    ...
    */
    ~cudaGraphNative() { cudaGraphNativeDeleter{}(_native); }
    ...
    */
    operator cudaGraph_t() const { return _native; }
...
class cudaGraphExec
{
  ...
  public:
    ...
    */
    ...
    @brief destructs the CUDA exec
    */
    ~cudaGraphExec() { cudaGraphExecDeleter{}(_exec); }
    ...
    */
    ...
    {
        cudaGraphExecDeleter{}(_exec);
        TF_CHECK_CUDA(cudaGraphInstantiate(&_exec, graph, nullptr, nullptr, 0),
    ...
    */
    ...
    {
        TF_CHECK_CUDA(cudaGraphLaunch(_exec, stream),
    ...
    */
    operator cudaGraphExec_t() const { return _exec; }
...
class cudaGraph : public CustomGraphBase
{
  ...
  public:
    ...
    template <typename... ArgsT> cudaNode *emplace_back(ArgsT &&...);

    bool empty() const;

    void clear();
    void dump(std::ostream &, const void *,
...
class cudaNode
{
    ...
    using handle_t =
        std::variant<Empty, Host, Memset, Memcpy, Kernel, Subflow, Capture>;
  ...
  public:
    ...
    template <typename... ArgsT> cudaNode(cudaGraph &, ArgsT &&...);
...
template <typename C> cudaNode::Host::Host(C &&c) : func{std::forward<C>(c)} {}
...
template <typename F> cudaNode::Kernel::Kernel(F &&f) : func{std::forward<F>(f)}
{
}

// Capture handle constructor
template <typename C>
cudaNode::Capture::Capture(C &&work) : work{std::forward<C>(work)}
{
}
...
cudaNode::cudaNode(cudaGraph &graph, ArgsT &&...args)
    : _graph{graph}, _handle{std::forward<ArgsT>(args)...}
{
}
...
inline void cudaNode::_precede(cudaNode *v)
{
    ...
    if (_handle.index() != cudaNode::CAPTURE) {
        TF_CHECK_CUDA(cudaGraphAddDependencies(_graph._native_handle,
                                               &_native_handle,
                                               &v->_native_handle, 1),
...
inline cudaGraph::cudaGraph(cudaGraph &&g)
    : _native_handle{g._native_handle}, _nodes{std::move(g._nodes)}
{
...
// Function: empty
inline bool cudaGraph::empty() const { return _nodes.empty(); }
...
template <typename... ArgsT> cudaNode *cudaGraph::emplace_back(ArgsT &&...args)
{
...
inline void cudaGraph::dump(std::ostream &os, const void *root,
        ...
        for (auto &node : graph->_nodes) {
            ...
            case cudaNode::SUBFLOW:
                stack.push(std::make_tuple(
                    &(std::get_if<cudaNode::Subflow>(&v->_handle)->graph), v,
                    l + 1));
                os << " style=\"filled\""

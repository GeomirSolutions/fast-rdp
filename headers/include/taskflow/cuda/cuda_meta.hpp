...
inline __device__ cudaRange cuda_get_tile(unsigned b, unsigned nv,
                                          unsigned count)
{
...
__device__ auto cuda_mem_to_reg_strided(I mem, unsigned tid, unsigned count)
{
    ...
    cudaArray<T, vt> x;
    cuda_strided_iterate<nt, vt, vt0>([&](auto i, auto j) { x[i] = mem[j]; },
                                      tid, count);
    return x;
}

template <unsigned nt, unsigned vt, unsigned vt0 = vt, typename T,
          typename it_t>
__device__ void cuda_reg_to_mem_strided(cudaArray<T, vt> x, unsigned tid,
                                        unsigned count, it_t mem)
{

    cuda_strided_iterate<nt, vt, vt0>([=](auto i, auto j) { mem[j] = x[i]; },
                                      tid, count);
}

template <unsigned nt, unsigned vt, unsigned vt0 = vt, typename I, typename O>
__device__ auto cuda_transform_mem_to_reg_strided(I mem, unsigned tid,
                                                  unsigned count, O op)
{
    using T =
        std::invoke_result_t<O, typename std::iterator_traits<I>::value_type>;
    cudaArray<T, vt> x;
...
__device__ void cuda_reg_to_shared_thread(cudaArray<T, vt> x, unsigned tid,
                                          T (&shared)[shared_size],
                                          bool sync = true)
{
...
template <unsigned nt, unsigned vt, typename T, unsigned shared_size>
__device__ auto cuda_shared_to_reg_thread(const T (&shared)[shared_size],
                                          unsigned tid, bool sync = true)
{
...
template <unsigned nt, unsigned vt, typename T, unsigned shared_size>
__device__ void cuda_reg_to_shared_strided(cudaArray<T, vt> x, unsigned tid,
                                           T (&shared)[shared_size],
                                           bool sync = true)
{
    ...
    cuda_strided_iterate<nt, vt>([&](auto i, auto j) { shared[j] = x[i]; },
                                 tid);
...
template <unsigned nt, unsigned vt, typename T, unsigned shared_size>
__device__ auto cuda_shared_to_reg_strided(const T (&shared)[shared_size],
                                           unsigned tid, bool sync = true)
{
    ...
    cudaArray<T, vt> x;
    cuda_strided_iterate<nt, vt>([&](auto i, auto j) { x[i] = shared[j]; },
                                 tid);
    if (sync)
        __syncthreads();
...
template <unsigned nt, unsigned vt, unsigned vt0 = vt, typename T,
          typename it_t, unsigned shared_size>
__device__ auto cuda_reg_to_mem_thread(cudaArray<T, vt> x, unsigned tid,
                                       unsigned count, it_t mem,
                                       T (&shared)[shared_size])
{
...
template <unsigned nt, unsigned vt, unsigned vt0 = vt, typename T,
          typename it_t, unsigned shared_size>
__device__ auto cuda_mem_to_reg_thread(it_t mem, unsigned tid, unsigned count,
                                       T (&shared)[shared_size])
{
...
__device__ auto cuda_shared_gather(const T (&data)[S],
...
template <unsigned nt, unsigned vt, typename T, unsigned S>
__device__ auto cuda_reg_thread_to_strided(cudaArray<T, vt> x, unsigned tid,
                                           T (&shared)[S])
{
    cuda_reg_to_shared_thread<nt>(x, tid, shared);
...
template <unsigned nt, unsigned vt, typename T, unsigned S>
__device__ auto cuda_reg_strided_to_thread(cudaArray<T, vt> x, unsigned tid,
                                           T (&shared)[S])
{

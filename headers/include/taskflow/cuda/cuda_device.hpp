...
{
    TF_CHECK_CUDA(cudaGetDeviceProperties(&p, i),
...
{
    int dim = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&dim, cudaDevAttrMaxGridDimX, d),
                  "failed to query the maximum x-dimension per grid on device ",
                  d)
    return dim;
}
...
{
    int dim = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&dim, cudaDevAttrMaxGridDimY, d),
                  "failed to query the maximum y-dimension per grid on device ",
                  d)
    return dim;
}
...
{
    int dim = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&dim, cudaDevAttrMaxGridDimZ, d),
                  "failed to query the maximum z-dimension per grid on device ",
                  d)
    return dim;
}
...
{
    int num = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&num, cudaDevAttrWarpSize, d),
...
{
    int num = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&num, cudaDevAttrUnifiedAddressing, d),
...
/**
@brief queries the latest CUDA version (1000 * major + 10 * minor) supported by
the driver
*/
...
{
    int num = 0;
    TF_CHECK_CUDA(cudaRuntimeGetVersion(&num),
...
inline cudaScopedDevice::~cudaScopedDevice()
{
    if (_p != -1) {
        cudaSetDevice(_p);
        // TF_CHECK_CUDA(cudaSetDevice(_p), "failed to scope back to device ",
        // _p);
    }
}

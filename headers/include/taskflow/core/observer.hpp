...
struct Segment
{
    ...
    template <typename Archiver> auto save(Archiver &ar) const
    {
        return ar(name, type, beg, end);
    }

    template <typename Archiver> auto load(Archiver &ar)
    {
        return ar(name, type, beg, end);
    }
    ...
    Segment(const std::string &n, TaskType t, observer_stamp_t b,
            observer_stamp_t e)
...
executor.make_observer<MyObserver>("MyObserver");
...
class ObserverInterface
{
  ...
  public:
    ...
    */
    ...
    /**
    @brief constructor-like method to call when the executor observer is fully
    created
...
// constructor
inline ChromeObserver::Segment::Segment(const std::string &n,
                                        observer_stamp_t b, observer_stamp_t e)
    : name{n}, beg{b}, end{e}
{
}
...
inline void ChromeObserver::on_exit(WorkerView wv, TaskView tv)
{
    ...
    _timeline.segments[w].emplace_back(tv.name(), beg,
                                       observer_stamp_t::clock::now());
}
    ...
    for (size_t w = first; w < _timeline.segments.size(); w++) {
        ...
        for (size_t i = 0; i < _timeline.segments[w].size(); i++) {
            ...
            os << "\"ph\":\"X\","
               << "\"pid\":1,"
               << "\"tid\":" << w << ',' << "\"ts\":"
               << duration_cast<microseconds>(_timeline.segments[w][i].beg -
                                              _timeline.origin)
                      .count()
               << ',' << "\"dur\":"
               << duration_cast<microseconds>(_timeline.segments[w][i].end -
...
@brief class to create an observer based on the built-in taskflow profiler
format
...
executor.make_observer<tf::TFProfObserver>();
...
inline void TFProfObserver::Summary::dump_tsum(std::ostream &os) const
{
    ...
    os << std::setw(type_w) << "-Task-" << std::setw(count_w + 2) << "Count"
       << std::setw(time_w + 2) << "Time (us)" << std::setw(avg_w + 2)
       << "Avg (us)" << std::setw(min_w + 2) << "Min (us)"
    ...
    for (size_t i = 0; i < TASK_TYPES.size(); i++) {
        ...
        os << std::setw(type_w) << to_string(TASK_TYPES[i])
           << std::setw(count_w + 2) << tsum[i].count << std::setw(time_w + 2)
           << tsum[i].total_span << std::setw(avg_w + 2)
           << std::to_string(tsum[i].avg_span()) << std::setw(min_w + 2)
           << tsum[i].min_span << std::setw(max_w + 2) << tsum[i].max_span
           << '\n';
    }
}
...
inline void TFProfObserver::Summary::dump_wsum(std::ostream &os) const
{
    ...
    // task summary
    size_t w_w{10}, t_w{10}, l_w{5}, c_w{5}, d_w{9}, avg_w{8}, min_w{8},
        max_w{8};
    ...
    os << std::setw(w_w) << "-Worker-" << std::setw(l_w + 2) << "Level"
       << std::setw(t_w) << "Task" << std::setw(c_w + 2) << "Count"
       << std::setw(d_w + 2) << "Time (us)" << std::setw(avg_w + 2)
       << "Avg (us)" << std::setw(min_w + 2) << "Min (us)"
       << std::setw(max_w + 2) << "Max (us)" << '\n';
    ...
    for (const auto &ws : wsum) {
        ...
        for (size_t i = 0; i < TASK_TYPES.size(); i++) {
            ...
            os << to_string(TASK_TYPES[i]) << std::setw(c_w + 2)
               << ws.tsum[i].count << std::setw(d_w + 2)
               << ws.tsum[i].total_span << std::setw(avg_w + 2)
               << std::to_string(ws.tsum[i].avg_span()) << std::setw(min_w + 2)
               << ws.tsum[i].min_span << std::setw(max_w + 2)
               << ws.tsum[i].max_span << '\n';
        }

        // per-worker summary
        os << std::setw(w_w + l_w + t_w + c_w + 4) << ws.count
           << std::setw(d_w + 2) << ws.total_span << std::setw(avg_w + 2)
           << std::to_string(ws.avg_span()) << std::setw(min_w + 2)
           << ws.min_span << std::setw(max_w + 2) << ws.max_span << '\n';
    ...
    for (size_t w = first; w < _timeline.segments.size(); w++) {
        for (size_t l = 0; l < _timeline.segments[w].size(); l++) {
            ...
            for (size_t i = 0; i < _timeline.segments[w][l].size(); ++i) {
                ...
                // span
                os << "{\"span\":["
                   << duration_cast<microseconds>(s.beg - _timeline.origin)
                          .count()
                   << ","
                   << duration_cast<microseconds>(s.end - _timeline.origin)
                          .count()
                   << "],";
...
end_of_summary:
    ...
    os << "==Observer " << _timeline.uid << ": " << num_workers()
       << " workers completed " << num_tasks() << " tasks in " << view
       << " us\n";

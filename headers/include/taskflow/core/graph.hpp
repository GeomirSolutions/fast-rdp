...
class CustomGraphBase
{
  ...
  public:
    virtual void dump(std::ostream &, const void *,
...
inline Runtime::Runtime(Executor &e, Worker &w, Node *p)
    : _executor{e}, _worker{w}, _parent{p}
{
}

// Function: executor
inline Executor &Runtime::executor() { return _executor; }
...
class Node
{
  ...
  public:
    ...
    constexpr static auto MULTI_CONDITION =
        get_index_v<MultiCondition, handle_t>;
...
Node::cudaFlow::cudaFlow(C &&c, G &&g)
    : work{std::forward<C>(c)}, graph{std::forward<G>(g)}
{
}
...
Node::Async::Async(C &&c, std::shared_ptr<AsyncTopology> tpg)
    : work{std::forward<C>(c)}, topology{std::move(tpg)}
{
}
...
// Constructor
template <typename C>
Node::SilentAsync::SilentAsync(C &&c) : work{std::forward<C>(c)}
{
}
...
inline Node::~Node()
{
    ...
    if (_handle.index() == DYNAMIC) {
        ...
        std::move(subgraph._nodes.begin(), subgraph._nodes.end(),
        ...
        while (i < nodes.size()) {
            ...
            if (nodes[i]->_handle.index() == DYNAMIC) {
                auto &sbg =
                    std::get_if<Dynamic>(&(nodes[i]->_handle))->subgraph;
                std::move(sbg._nodes.begin(), sbg._nodes.end(),
                          std::back_inserter(nodes));
                sbg._nodes.clear();
            }
...
inline size_t Node::num_successors() const { return _successors.size(); }

// Function: dependents
inline size_t Node::num_dependents() const { return _dependents.size(); }
...
inline bool Node::_is_cancelled() const
{
    if (_handle.index() == Node::ASYNC) {
        ...
        if (h->topology &&
            h->topology->_is_cancelled.load(std::memory_order_relaxed)) {
            return true;
        }
        // async tasks spawned from subflow does not have topology
    }
    return _topology &&
           _topology->_is_cancelled.load(std::memory_order_relaxed);
}
...
// Procedure: clear
inline void Graph::clear() { _clear(); }
...
{
    if (auto I = std::find(_nodes.begin(), _nodes.end(), node);
        I != _nodes.end()) {
        _nodes.erase(I);
        node_pool.recycle(node);
    }
...
// Function: size
inline size_t Graph::size() const { return _nodes.size(); }

// Function: empty
inline bool Graph::empty() const { return _nodes.empty(); }

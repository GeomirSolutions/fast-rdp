...
// not cheap, but they are executed only if the preceeding predicate check has
// failed.
//
// Algorihtm outline:
...
class Notifier
{
    ...
    void commit_wait(Waiter *w)
    {
        ...
        for (;;) {
            if (int64_t((state & kEpochMask) - epoch) < 0) {
                ...
                // it calls either cancel_wait or commit_wait, or is notified.
            ...
            newstate =
                static_cast<uint64_t>((newstate & ~kStackMask) |
                                      static_cast<uint64_t>(w - &_waiters[0]));
            if ((state & kStackMask) == kStackMask)
                w->next.store(nullptr, std::memory_order_relaxed);
            else
                w->next.store(&_waiters[state & kStackMask],
    ...
    void notify(bool all)
    {
        ...
        for (;;) {
            ...
            if (all) {
                ...
                newstate =
                    (state & kEpochMask) + (kEpochInc * waiters) + kStackMask;
            } else if (waiters) {
                ...
                // lock-free stack can't happen because a waiter is re-pushed
                // onto the stack only after it was in the pre-wait state which
                // inevitably leads to epoch increment.
  ...
  private:
    ...
    static const uint64_t kEpochMask = ((1ull << kEpochBits) - 1)
                                       << kEpochShift;

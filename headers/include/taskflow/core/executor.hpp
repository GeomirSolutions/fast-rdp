...
class Executor
{
  ...
  public:
    ...
    */
    ...
    This member function executes the given taskflow once and returns a
    tf::Future object that eventually holds the result of the execution.
    ...
    */
    ...
    This member function executes the given taskflow @c N times and returns a
    tf::Future object that eventually holds the result of the execution.

    @code{.cpp}
    tf::Future<void> future = executor.run_n(taskflow, 2);  // run taskflow 2
    times
    // do something else
    future.wait();
    @endcode
    ...
    */
    ...
    This member function executes a moved taskflow @c N times and returns a
    tf::Future object that eventually holds the result of the execution. The
    executor will take care of the lifetime of the moved taskflow.
    ...
    */
    ...
    This member function executes the given taskflow @c N times and invokes the
    given callable when the execution completes. This member function returns a
    tf::Future object that eventually holds the result of the execution.

    @code{.cpp}
    tf::Future<void> future = executor.run(
      taskflow, 2, [](){ std::cout << "done"; }  // runs taskflow 2 times and
    invoke
    ...
    */
    ...
    This member function executes a moved taskflow @c N times and invokes the
    given callable when the execution completes. This member function returns a
    tf::Future object that eventually holds the result of the execution.
    ...
    */
    ...
    for(size_t n=0; n<1000; n++) {
      ...
      taskflow.emplace([&executor, &tf=others[n]](){
        ...
        //executor.run(tf).wait();  <- blocking the worker without doing
    anything
    ...
    @attention
    You must call tf::Executor::run_and_wait from a worker of the calling
    executor or an exception will be thrown.
    */
    ...
    or an exception will be thrown.
    */
    template <typename P> void loop_until(P &&predicate);
    ...
    */
    ...
    /**
    @brief similar to tf::Executor::named_async but does not return a future
    object
    ...
    */
    ...
    Each executor manages a list of observers with shared ownership with
    callers. For each of these observers, the two member functions,
  ...
  private:
    ...
    template <typename P> void _loop_until(Worker &, P &&);
...
inline Executor::Executor(size_t N, std::shared_ptr<WorkerInterface> wix)
    : _MAX_STEALS{((N + 1) << 1)}, _threads{N}, _workers{N}, _notifier{N},
...
// Function: max_steals
inline size_t Executor::max_steals() const noexcept { return _MAX_STEALS; }

// Function: num_topologies
inline size_t Executor::num_topologies() const { return _num_topologies; }
        ...
        [p = make_moc(std::move(p)), f = std::forward<F>(f),
         args...](bool cancel) mutable {
            if constexpr (std::is_same_v<R, void>) {
                ...
                p.object.set_value(cancel ? std::nullopt
...
template <typename F, typename... ArgsT>
void Executor::named_silent_async(const std::string &name, F &&f,
                                  ArgsT &&...args)
{
...
inline void Executor::_spawn(size_t N)
{
    ...
    for (size_t id = 0; id < N; ++id) {
        ...
        _threads[id] = std::thread(
            [this](Worker &w, std::mutex &mutex, std::condition_variable &cond,
                   size_t &n) -> void {
                // assign the thread
            ...
            std::ref(_workers[id]), std::ref(mutex), std::ref(cond),
            std::ref(n));
...
exploit:
    ...
    while (!stop_predicate()) {
        ...
        explore:
            ...
            t = (w._id == w._vtm) ? _wsq.steal()
                                  : _workers[w._vtm]._wsq.steal();
...
std::shared_ptr<Observer> Executor::make_observer(ArgsT &&...args)
{
    ...
    static_assert(std::is_base_of_v<ObserverInterface, Observer>,
...
// Procedure: _schedule
inline void Executor::_schedule(Worker &worker,
...
inline void Executor::_invoke_condition_task(Worker &worker, Node *node,
                                             SmallVector<int> &conds)
{
    _observer_prologue(worker, node);
...
inline void Executor::_invoke_multi_condition_task(Worker &worker, Node *node,
                                                   SmallVector<int> &conds)
{
    _observer_prologue(worker, node);
...
{
    return run_until(
        std::move(f), [repeat]() mutable { return repeat-- == 0; },
        std::forward<C>(c));
}
...
tf::Future<void> Executor::run_until(Taskflow &f, P &&p, C &&c)
{
    ...
    auto t =
        std::make_shared<Topology>(f, std::forward<P>(p), std::forward<C>(c));
...
template <typename T> void Executor::run_and_wait(T &target)
{
...
// Function: loop_until
template <typename P> void Executor::loop_until(P &&predicate)
{
...
inline void Executor::_tear_down_topology(Worker &worker, Topology *tpg)
{
    ...
    // case 2: the final run of this topology
        ...
        if (std::unique_lock<std::mutex> lock(f._mutex);
            ...
            // (which means the following code should the moved before
            // set_value)
            if (s) {
                std::scoped_lock<std::mutex> lock(_taskflow_mutex);
                _taskflows.erase(*s);
...
{
    return _named_async(*_executor._this_worker(), name, std::forward<F>(f),
                        std::forward<ArgsT>(args)...);
}

// Function: _named_async
template <typename F, typename... ArgsT>
auto Subflow::_named_async(Worker &w, const std::string &name, F &&f,
                           ArgsT &&...args)
{
...
template <typename F, typename... ArgsT>
void Subflow::_named_silent_async(Worker &w, const std::string &name, F &&f,
                                  ArgsT &&...args)
{
...
// Function: silent_async
template <typename F, typename... ArgsT>
void Subflow::named_silent_async(const std::string &name, F &&f,
                                 ArgsT &&...args)
{
    _named_silent_async(*_executor._this_worker(), name, std::forward<F>(f),
                        std::forward<ArgsT>(args)...);
}
...
// Procedure: emplace
template <typename T> void Runtime::run_and_wait(T &&target)
{

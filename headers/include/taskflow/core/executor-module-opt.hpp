...
// Constructor
inline Executor::Executor(size_t N) : _workers{N}, _notifier{N}
{
...
inline void Executor::_tear_down_invoke(Worker &worker, Node *node)
{
    ...
    if (auto parent = node->_parent; parent == nullptr) {
        ...
        auto deferred =
            parent->_state.load(std::memory_order_relaxed) & Node::DEFERRED;
...
inline void Executor::_invoke_dynamic_task_external(Worker &w, Node *p,
                                                    Graph &g, bool detach)
{
...
inline void Executor::_invoke_dynamic_task_internal(Worker &w, Node *p,
                                                    Graph &g)
{

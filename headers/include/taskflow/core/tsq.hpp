...
*/
template <typename T,
          unsigned MAX_PRIORITY = static_cast<unsigned>(TaskPriority::MAX)>
class TaskQueue
{
...
// Function: pop
template <typename T, unsigned MAX_PRIORITY> T TaskQueue<T, MAX_PRIORITY>::pop()
{
    for (unsigned i = 0; i < MAX_PRIORITY; i++) {
        if (auto t = pop(i); t) {
            return t;
...
T TaskQueue<T, MAX_PRIORITY>::pop(unsigned p)
{
    ...
    if (t <= b) {
        ...
        if (t == b) {
            ...
            if (!_top[p].data.compare_exchange_strong(
...
{
    size_t s;
    unroll<0, MAX_PRIORITY, 1>(
        [&](auto i) { s = i ? capacity(i) + s : capacity(i); });
    return s;
}

...
Task FlowBuilder::reduce(B beg, E end, T &init, O bop)
{
    ...
    Task task =
        emplace([b = beg, e = end, &r = init, bop](Subflow &sf) mutable {
...
Task FlowBuilder::transform_reduce(B beg, E end, T &init, BOP bop, UOP uop)
{
    ...
    Task task =
        emplace([b = beg, e = end, &r = init, bop, uop](Subflow &sf) mutable {
            // fetch the iterator values
            B_t beg = b;
            E_t end = e;
            ...
            for (size_t w = 0; w < W; w++) {
                ...
                //   break;
                // }
                // sf._named_silent_async(sf._worker, "loop-"s +
                // std::to_string(w), loop);

...
Task FlowBuilder::transform(B first1, E last1, O d_first, C c)
{
    ...
    Task task = emplace([first1, last1, d_first, c](Subflow &sf) mutable {
        ...
        auto loop = [=, &next]() mutable {
            ...
            while (s0 < N) {
                ...
                if (r < p1) {
                    while (1) {
                        s0 = next.fetch_add(chunk_size,
                                            std::memory_order_relaxed);
                        if (s0 >= N) {
                            return;
                        }
                        size_t e0 =
                            (chunk_size <= (N - s0)) ? s0 + chunk_size : N;
                ...
                // coarse-grained
                    ...
                    if (next.compare_exchange_strong(
                            s0, e0, std::memory_order_relaxed,
                            std::memory_order_relaxed)) {
                        std::advance(beg, s0 - z);
                        std::advance(d_beg, s0 - z);
                        for (size_t x = s0; x < e0; x++) {
        ...
        for (size_t w = 0; w < W; w++) {
            ...
            if (r <= chunk_size || w == W - 1) {
                ...
                sf._named_silent_async(sf._worker, "loop-"s + std::to_string(w),
                                       loop);
            }
        }
...
Task FlowBuilder::transform(B1 first1, E1 last1, B2 first2, O d_first, C c)
{
    ...
    Task task =
        emplace([first1, last1, first2, d_first, c](Subflow &sf) mutable {
            // fetch the stateful values
            B1_t beg1 = first1;
            E1_t end1 = last1;
            B2_t beg2 = first2;
            ...
            for (size_t w = 0; w < W; w++) {
                ...
                if (r <= chunk_size || w == W - 1) {
                    ...
                    sf._named_silent_async(sf._worker,
                                           "loop-"s + std::to_string(w), loop);
                }
            }

...
template <typename C = std::function<void(tf::Pipeflow &)>> class Pipe
{
  ...
  public:
    ...
    */
    ...
    The callable must take a referenced tf::Pipeflow object in the first
    argument.
    ...
    */
    template <typename U> void callable(U &&callable)
    {
...
<i>task-parallel pipeline scheduling framework</i> using a module task in a
taskflow. Unlike the conventional pipeline programming frameworks (e.g., Intel
TBB), %Taskflow's pipeline algorithm does not provide any data abstraction,
...
Pipeline<Ps...>::Pipeline(size_t num_lines, Ps &&...ps)
    : _pipes{std::make_tuple(std::forward<Ps>(ps)...)}, _meta{PipeMeta{
                                                            ps.type()}...},
      _lines(num_lines), _tasks(num_lines + 1), _pipeflows(num_lines)
{
...
Pipeline<Ps...>::Pipeline(size_t num_lines, std::tuple<Ps...> &&ps)
    : _pipes{std::forward<std::tuple<Ps...>>(ps)},
      _meta{_gen_meta(std::forward<std::tuple<Ps...>>(ps),
...
template <typename... Ps>
template <size_t... I>
auto Pipeline<Ps...>::_gen_meta(std::tuple<Ps...> &&ps,
...
// Function: graph
template <typename... Ps> Graph &Pipeline<Ps...>::graph() { return _graph; }
...
template <typename... Ps> void Pipeline<Ps...>::reset()
{
    ...
    for (size_t l = 1; l < num_lines(); l++) {
        for (size_t f = 1; f < num_pipes(); f++) {
            _lines[l][f].join_counter.store(static_cast<size_t>(_meta[f].type),
    ...
    for (size_t l = 1; l < num_lines(); l++) {
        _lines[l][0].join_counter.store(static_cast<size_t>(_meta[0].type) - 1,
                                        std::memory_order_relaxed);
    }
}
...
void Pipeline<Ps...>::_on_pipe(Pipeflow &pf, Runtime &rt)
{
    visit_tuple(
        [&](auto &&pipe) {
            using callable_t =
                typename std::decay_t<decltype(pipe)>::callable_t;
            if constexpr (std::is_invocable_v<callable_t, Pipeflow &>) {
                pipe._callable(pf);
            } else if constexpr (std::is_invocable_v<callable_t, Pipeflow &,
                                                     Runtime &>) {
                pipe._callable(pf, rt);
            } else {
                static_assert(dependent_false_v<callable_t>,
        ...
        _tasks[l + 1] = fb.emplace([this, l](tf::Runtime &rt) mutable {
                          ...
                          pipeline:
                              ...
                              _lines[pf->_line][pf->_pipe].join_counter.store(
                                  static_cast<size_t>(_meta[pf->_pipe].type),
                                  std::memory_order_relaxed);

                              if (pf->_pipe == 0) {
                                  pf->_token = _num_tokens;
                                  if (pf->_stop = false, _on_pipe(*pf, rt);
                                      pf->_stop == true) {
                                      // here, the pipeline is not stopped yet
                                      // because other lines of tasks may still
                                      // be running their last stages
                                      return;
                                  }
                                  ++_num_tokens;
                              } else {
                              ...
                              // Notice that the shared variable f must not be
                              // changed after this point because it can result
                              // in data race due to the following condition:
                              //
                              // a -> b
                              // |    |
                              // v    v
                              // c -> d
                              //
                              // d will be spawned by either c or b, so if c
                              // changes f but b spawns d then data race on f
                              // will happen
...
parallel lines to schedule tokens through three serial pipes in a custom
storage, then resetting the pipeline to a new range of five serial pipes:
...
four parallel lines in a circular fashion, first going through three serial
pipes and then five serial pipes:
...
ScalablePipeline<P>::ScalablePipeline(size_t num_lines)
    : _tasks(num_lines + 1), _pipeflows(num_lines)
{
...
ScalablePipeline<P>::ScalablePipeline(ScalablePipeline &&rhs)
    : _graph{std::move(rhs._graph)}, _num_tokens{rhs._num_tokens},
      _pipes{std::move(rhs._pipes)}, _tasks{std::move(rhs._tasks)},
      _pipeflows{std::move(rhs._pipeflows)}, _lines{std::move(rhs._lines)}
{
...
// Function: graph
template <typename P> Graph &ScalablePipeline<P>::graph() { return _graph; }

// Function: _line
template <typename P>
typename ScalablePipeline<P>::Line &ScalablePipeline<P>::_line(size_t l,
                                                               size_t p)
{
...
template <typename P> void ScalablePipeline<P>::reset()
{
    ...
    for (size_t l = 1; l < num_lines(); l++) {
        for (size_t f = 1; f < num_pipes(); f++) {
            _line(l, f).join_counter.store(
                static_cast<size_t>(_pipes[f]->type()),
                std::memory_order_relaxed);
        }
    }
    ...
    for (size_t l = 1; l < num_lines(); l++) {
        _line(l, 0).join_counter.store(static_cast<size_t>(_pipes[0]->type()) -
                                           1,
                                       std::memory_order_relaxed);
    }
}
            ...
            fb.emplace([this, l](tf::Runtime &rt) mutable {
              ...
              pipeline:
                  ...
                  _line(pf->_line, pf->_pipe)
                      .join_counter.store(
                          static_cast<size_t>(_pipes[pf->_pipe]->type()),
                          std::memory_order_relaxed);
                  ...
                  if (pf->_pipe == 0) {
                      ...
                      if (pf->_stop = false, _on_pipe(*pf, rt);
                          ...
                          // lines of tasks may still be running their last
                          // stages
                          return;
                      }
                      ++_num_tokens;
                  ...
                  // ---- scheduling starts here ----
                  // Notice that the shared variable f must not be changed after
                  // this point because it can result in data race due to the
                  // following condition:
                  //
                  // a -> b
                  // |    |
                  ...
                  // c -> d
                  //
                  // d will be spawned by either c or b, so if c changes f but b
                  // spawns d then data race on f will happen
                  ...
                  // forward dependency
                  if (_line(pf->_line, n_f)
                          .join_counter.fetch_sub(
                              1, std::memory_order_acq_rel) == 1) {
                      retval[n++] = 0;
                  }

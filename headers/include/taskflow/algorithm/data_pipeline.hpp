...
a data pipe. The input and output types of a tf::DataPipe should be decayed
types (though the library will always decay them for you using `std::decay`) to
allow internal storage to work. The data will be passed by reference to your
callable, at which you can take it by copy or reference.
...
int{ if(pf.token() == 5) { pf.stop(); return 0;
      }
      else {
        return pf.token();
      }
    ...
    tf::make_data_pipe<std::string, void>(tf::PipeType::SERIAL, [](std::string&
...
The pipeline schedules five tokens over four parallel lines in a circular
fashion, as depicted below:
...
template <typename... Ps> class DataPipeline
{
  ...
  public:
    ...
    */
    ...
    Constructs a data-parallel pipeline of up to @c num_lines parallel lines to
    schedule tokens through the given linear chain of pipes. The first pipe must
    define a serial direction (tf::PipeType::SERIAL) or an exception will be
    thrown.
    */
    ...
    Constructs a data-parallel pipeline of up to @c num_lines parallel lines to
    schedule tokens through the given linear chain of pipes stored in a
    std::tuple. The first pipe must define a serial direction
    (tf::PipeType::SERIAL) or an exception will be thrown.
    */

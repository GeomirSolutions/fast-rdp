...
template <class T> using padded_t = typename padded<T>::type;
...
class Pipeflow
{
    ...
    template <typename... Ps> friend class DataPipeline_aligned;
...
template <typename Input, typename Output, typename C> class DataPipe
{
  ...
  public:
    ...
    */
    // using callable_t = std::conditional_t<enable_pf,
    // std::function<Output(Input, Pipeflow&)>, std::function<Output(Input)>>;
    using callable_t = C;
    using input_t = Input;
...
template <typename... Ps> class DataPipeline
{
  ...
  private:
    ...
    using variant_t = unique_variant_t<std::variant<std::conditional_t<
        std::is_void_v<typename Ps::output_t>, std::monostate,
...
template <typename... Ps>
template <size_t... I>
auto DataPipeline<Ps...>::_gen_meta(std::tuple<Ps...> &&ps,
...
// Function: graph
template <typename... Ps> Graph &DataPipeline<Ps...>::graph() { return _graph; }
...
void DataPipeline<Ps...>::_on_pipe(Pipeflow &pf, Runtime &rt)
{
    visit_tuple(
        [&](auto &&pipe) {
            ...
            using input_t =
                std::decay_t<typename std::decay_t<decltype(pipe)>::input_t>;
            using output_t =
                std::decay_t<typename std::decay_t<decltype(pipe)>::output_t>;
            ...
            if constexpr (std::is_invocable_v<callable_t, Pipeflow &>) {
                ...
                if constexpr (std::is_void_v<output_t>) {
                    ...
                    _buffer[pf._line] = pipe._callable(
                        std::get<input_t>(_buffer[pf._line]), pf);
                }
...
template <typename... Ps> class DataPipeline_aligned
{
      ...
      _buffer(num_lines)
{
    ...
    // std::cout << "sizeof(padded_t<variant_t>)=" <<
    // sizeof(padded_t<variant_t>) << std::endl; std::cout <<
    // "alignof(padded_t<variant_t>)=" << alignof(padded_t<variant_t>) <<
    // std::endl; std::cout << "alignof(_buffer)=" << alignof(_buffer) <<
    // std::endl;
...
// constructor
template <typename... Ps>
DataPipeline_aligned<Ps...>::DataPipeline_aligned(size_t num_lines,
                                                  std::tuple<Ps...> &&ps)
    : _pipes{std::forward<std::tuple<Ps...>>(ps)},
      _meta{_gen_meta(std::forward<std::tuple<Ps...>>(ps),
                      std::make_index_sequence<sizeof...(Ps)>{})},
...
template <typename... Ps>
template <size_t... I>
auto DataPipeline_aligned<Ps...>::_gen_meta(std::tuple<Ps...> &&ps,
                                            std::index_sequence<I...>)
{
    return std::array{PipeMeta{std::get<I>(ps).type()}...};
}
...
// Function: graph
template <typename... Ps> Graph &DataPipeline_aligned<Ps...>::graph()
{
    return _graph;
}

// Function: reset
template <typename... Ps> void DataPipeline_aligned<Ps...>::reset()
{
        ...
        [&](auto &&pipe) {
            ...
            if constexpr (std::is_invocable_v<callable_t, Pipeflow &>) {
                ...
                if constexpr (std::is_void_v<output_t>) {
                    ...
                    _buffer[pf._line].v = pipe._callable(
                        std::get<input_t>(_buffer[pf._line].v), pf);
                }
            } else if constexpr (std::is_invocable_v<callable_t, Pipeflow &,
                                                     Runtime &>) {
                pipe._callable(pf, rt);
...
// Procedure: _build
template <typename... Ps> void DataPipeline_aligned<Ps...>::_build()
{

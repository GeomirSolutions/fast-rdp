...
template <unsigned nt, typename T> struct syclBlockReduce
{
    ...
    static_assert(nt && (0 == nt % SYCL_WARP_SIZE),
                  "syclBlockReduce requires num threads to be a multiple of "
                  "warp_size (32)");

    using shm_t = sycl::accessor<T, 1, sycl::access::mode::read_write,
                                 sycl::access::target::local>;

    template <typename op_t>
    T operator()(sycl::nd_item<1> &, T, const shm_t &, unsigned, op_t,
                 bool = true) const;
};
...
template <typename op_t>
T syclBlockReduce<nt, T>::operator()(sycl::nd_item<1> &item, T x,
                                     const shm_t &shm, unsigned count, op_t op,
                                     bool ret) const
{
...
sycl::event sycl_reduce_loop(P &&p, I input, unsigned count, T *res, O op,
                             bool incl, void *ptr, std::vector<sycl::event> evs)
{
    ...
    auto e = p.queue().submit([=, evs = std::move(evs)](sycl::handler &h) {
        ...
        h.parallel_for(
            ...
            [=](sycl::nd_item<1> item) {
                ...
                s = R()(item, s, shm,
                        (tile.count() < E::nt ? tile.count() : E::nt), op,
                        false);
                if (!tid) {
...
void sycl_reduce(P &&p, I first, I last, T *res, O op)
{
    ...
    auto tmp =
        sycl::malloc_device(sycl_reduce_buffer_size<P, T>(count), p.queue());
 ...
 */
template <typename P, typename I, typename T, typename O>
sycl::event sycl_reduce_async(P &&p, I first, I last, T *res, O op, void *buf,
                              std::vector<sycl::event> dep)
{
    ...
    // reduction loop
    return detail::sycl_reduce_loop(p, first, count, res, op, true, buf,
                                    std::move(dep));
}
...
*/
template <typename P, typename I, typename T, typename O>
sycl::event sycl_uninitialized_reduce_async(P &&p, I first, I last, T *res,
                                            O op, void *buf,
                                            std::vector<sycl::event> dep)
{
    ...
    // reduction loop
    return detail::sycl_reduce_loop(p, first, count, res, op, false, buf,
                                    std::move(dep));
}
...
syclTask syclFlow::reduce(I first, I last, T *res, C &&op)
{
    ...
    // return on(_reduce_cgh<I, T, C, false>(first, last, res,
    // std::forward<C>(op)));
    ...
    return on([=, buf = MoC{syclScopedDeviceMemory<std::byte>(bufsz, _queue)}](
        ...
        return sycl_reduce_async(p, first, last, res, op, buf.get().data(),
                                 std::move(events));
    });
}
...
{
    // return on(_reduce_cgh<I, T, C, true>(first, last, res,
    // std::forward<C>(op)));

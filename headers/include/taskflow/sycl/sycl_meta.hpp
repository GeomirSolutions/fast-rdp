...
auto sycl_mem_to_reg_strided(I mem, unsigned tid, unsigned count)
{
    ...
    syclArray<T, vt> x;
    sycl_strided_iterate<nt, vt, vt0>([&](auto i, auto j) { x[i] = mem[j]; },
                                      tid, count);
    return x;
}
...
template <unsigned nt, unsigned vt, unsigned vt0 = vt, typename T,
          typename it_t>
void sycl_reg_to_mem_strided(syclArray<T, vt> x, unsigned tid, unsigned count,
                             it_t mem)
{

    sycl_strided_iterate<nt, vt, vt0>([=](auto i, auto j) { mem[j] = x[i]; },
                                      tid, count);
}

template <unsigned nt, unsigned vt, unsigned vt0 = vt, typename I, typename O>
auto sycl_transform_mem_to_reg_strided(I mem, unsigned tid, unsigned count,
                                       O op)
{
    using T =
...
//   syclArray<T, vt> x, unsigned tid, T (&shared)[shared_size], bool sync =
//   true
//) {
//
...
//
//   syclArray<T, vt> x;
//   sycl_strided_iterate<nt, vt>([&](auto i, auto j) { x[i] = shared[j]; },
//   tid); if(sync) __syncthreads();
//
//   return x;
// }
//
...
template <typename T> class syclScopedDeviceMemory
{
  ...
  public:
    ...
    syclScopedDeviceMemory(size_t N, sycl::queue &queue) : _queue{queue}, _N{N}
    {
        if (N) {

...
class syclFlow
{
  ...
  public:
    ...
    */
    template <typename F,
              std::enable_if_t<std::is_invocable_r_v<void, F, sycl::handler &>,
                               void> * = nullptr>
    syclTask on(F &&func);
    ...
    */
    template <typename... ArgsT> syclTask parallel_for(ArgsT &&...args);
    ...
    kernel thread.
    */
    template <typename F> syclTask single_task(F &&func);
...
inline syclFlow::syclFlow(sycl::queue &queue)
    : _queue{queue}, _handle{std::in_place_type_t<External>{}},
...
inline syclFlow::syclFlow(Executor &e, syclGraph &g, sycl::queue &queue)
    : _queue{queue}, _handle{std::in_place_type_t<Internal>{}, e}, _graph{g}
{
}

// Function: empty
inline bool syclFlow::empty() const { return _graph._nodes.empty(); }

// Function: num_tasks
inline size_t syclFlow::num_tasks() const { return _graph._nodes.size(); }
...
// Procedure: clear
inline void syclFlow::clear() { _graph.clear(); }
...
{
    return on(
        [=](sycl::handler &h) { h.memcpy(target, source, count * sizeof(T)); });
}

// Function: on
template <
    typename F,
    std::enable_if_t<std::is_invocable_r_v<void, F, sycl::handler &>, void> *>
syclTask syclFlow::on(F &&f)
{
    auto node = _graph.emplace_back(
        _graph, std::in_place_type_t<syclNode::CGH>{}, std::forward<F>(f));
    return syclTask(node);
}

// Function: single_task
template <typename F> syclTask syclFlow::single_task(F &&func)
{
    return on(
        [f = std::forward<F>(func)](sycl::handler &h) { h.single_task(f); });
}

// Function: parallel_for
template <typename... ArgsT> syclTask syclFlow::parallel_for(ArgsT &&...args)
{
    return on([args...](sycl::handler &h) { h.parallel_for(args...); });
}

// Procedure: offload_until
template <typename P> void syclFlow::offload_until(P &&predicate)
{
...
// Function: memcpy
inline void syclFlow::memcpy(syclTask task, void *tgt, const void *src,
                             size_t bytes)
{
...
// Function: single_task
template <typename F> void syclFlow::single_task(syclTask task, F &&func)
{
    on(task,
       [f = std::forward<F>(func)](sycl::handler &h) { h.single_task(f); });
}
...
Task FlowBuilder::emplace_on(C &&callable, Q &&q)
{
    auto n = _graph._emplace_back(
        ...
        [c = std::forward<C>(callable),
         queue = std::forward<Q>(q)](Executor &e, Node *p) mutable {

...
template <typename Key> class compare_key
{
  public:
    const Key key;
...
template <typename Key, typename Value> class element
{
  public:
    using key_type = Key;
...
template <typename Element, std::size_t N> class map
{
  private:
  ...
  public:
    ...
    MAPBOX_ETERNAL_CONSTEXPR const mapped_type &
    ...
    MAPBOX_ETERNAL_CONSTEXPR std::size_t
...
map(const std::pair<const Key, const Value> (&items)[N]) noexcept
{
...
template <typename Key, typename Value, std::size_t N>
static constexpr auto
hash_map(const std::pair<const Key, const Value> (&items)[N]) noexcept
{
...
constexpr std::size_t hash_offset = std::conditional_t<
    sizeof(std::size_t) < 8, std::integral_constant<uint32_t, 0x811C9DC5>,
...
str_hash(const char *str, const std::size_t value = hash_offset) noexcept
{
    return *str ? str_hash(str + 1, (value ^ static_cast<std::size_t>(*str)) *
                                        hash_prime)
                : value;
}
...
class string
{
  ...
  public:
    ...
    MAPBOX_ETERNAL_CONSTEXPR string &
    operator=(const string &) noexcept = default;
...
template <> struct hash<::mapbox::eternal::string>
{

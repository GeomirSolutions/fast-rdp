...
struct deduced_result_type<F(Args...), decltype((void)std::declval<F>()(
                                           std::declval<Args>()...))>
{
        ...
        if (v0.template is<T>()) {
            if (v1.template is<T>()) {
                ...
                return binary_dispatcher_rhs<R, T, Types...>::apply(
                    std::forward<V>(v0), std::forward<V>(v1),
                    std::forward<F>(f));
            }
...
template <typename... Types> class variant
{
  ...
  private:
    ...
    using unwrap_first_type =
        typename detail::unwrapper<first_type>::value_type;
    using data_type =
  ...
  public:
    VARIANT_INLINE variant<Types...> &operator=(variant<Types...> &&other)
        // note we check for nothrow-constructible, not nothrow-assignable,
        // since move_assign uses move-construction via placement new.
    ...
    auto VARIANT_INLINE match(Fs &&...fs) && -> decltype(variant::visit(
        std::move(*this),
        ::mapbox::util::make_visitor(std::forward<Fs>(fs)...)))
    {
        return variant::visit(std::move(*this), ::mapbox::util::make_visitor(
                                                    std::forward<Fs>(fs)...));
    }
...
auto VARIANT_INLINE apply_visitor(F &&f, V &&v0, V &&v1)
    -> decltype(v0.binary_visit(std::forward<V>(v0), std::forward<V>(v1),
                                std::forward<F>(f)))
{
    return v0.binary_visit(std::forward<V>(v0), std::forward<V>(v1),
                           std::forward<F>(f));
}
...
template <typename... Types> struct hash<::mapbox::util::variant<Types...>>
{
    ...
    {
        return ::mapbox::util::apply_visitor(::mapbox::util::detail::hasher{},
                                             v);
    }
};

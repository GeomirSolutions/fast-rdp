...
class Cluster
{
  public:
    ...
    Cluster(const point<double> &pos_, const std::uint32_t num_points_,
    ...
    Cluster(const point<double> &pos_, const std::uint32_t num_points_,
            const std::uint32_t id_, const property_map &properties_)
        : pos(pos_), num_points(num_points_), id(id_)
    {
    ...
    mapbox::feature::feature<double> toGeoJSON() const
    {
        ...
        const double y =
            360.0 * std::atan(std::exp((180.0 - pos.y * 360.0) * M_PI / 180)) /
                M_PI -
            90.0;
...
template <> struct nth<0, Cluster>
{
    inline static double get(const Cluster &c) { return c.pos.x; };
};
template <> struct nth<1, Cluster>
{
    inline static double get(const Cluster &c) { return c.pos.y; };
};
...
class Timer
{
  public:
    ...
    Timer() { started = std::chrono::high_resolution_clock::now(); }
...
struct Options
{
    ...
    std::uint16_t extent =
        512; // tile extent (radius is calculated relative to it)
    std::size_t minPoints = 2; // minimum points to form a cluster
    bool generateId = false;   // whether to generate numeric ids for input
                               // features (in vector tiles)
...
class Supercluster
{
  ...
  public:
    ...
    TileFeatures getTile(const std::uint8_t z, const std::uint32_t x_,
                         const std::uint32_t y) const
    {
        ...
        const auto visitor = [&, this](const auto &id) {
            ...
            const TilePoint point(
                ::round(this->options.extent * (c.pos.x * z2 - x)),
            ...
            if (c.num_points == 1) {
                ...
                auto featureId =
                    options.generateId
                        ...
                        : original_feature.id;
                result.emplace_back(point, original_feature.properties,
                                    std::move(featureId));
            } else {
                result.emplace_back(
                              ...
                              const std::uint32_t offset = 0) const
    {
        ...
        eachLeaf(cluster_id, limit_, offset, skipped, [&, this](const auto &c) {
  ...
  private:
    struct Zoom
    {
            ...
            for (const auto &f : features_) {
                if (options_.reduce) {
                    ...
                    clusters.emplace_back(
                        project(f.geometry.get<GeoJSONPoint>()), 1, i++,
                        clusterProperties);
                } else {
                    clusters.emplace_back(
                        project(f.geometry.get<GeoJSONPoint>()), 1, i++);
             ...
             const Options &options_)
        {
            ...
            // Since point index is encoded in the upper 27 bits, clamp the
            // count of clusters
            const auto previous_clusters_size = std::min(
                previous.clusters.size(),
                static_cast<std::vector<Cluster>::size_type>(0x7ffffff));
            ...
            for (std::size_t i = 0; i < previous_clusters_size; i++) {
                ...
                previous.tree.within(
                    p.pos.x, p.pos.y, r, [&](const auto &neighbor_id) {
                ...
                auto clusterProperties =
                    p.properties ? *p.properties : property_map{};
                if (num_points >=
                    options_.minPoints) { // enough points to form a cluster
                    point<double> weight = p.pos * double(num_points_origin);
                    std::uint32_t id =
                        static_cast<std::uint32_t>((i << 5) + (zoom + 1));
                    ...
                    previous.tree.within(
                        p.pos.x, p.pos.y, r, [&](const auto &neighbor_id) {
                            ...
                            // accumulate coordinates for calculating weighted
                            // center
                            weight += b.pos * double(b.num_points);

                            if (options_.reduce && b.properties) {
                                // apply reduce function to update
                                // clusterProperites
                                options_.reduce(clusterProperties,
                                                *b.properties);
                            }
                        });
                    p.parent_id = id;
                    clusters.emplace_back(weight / double(num_points),
                                          num_points, id, clusterProperties);
                } else {
                    ...
                    if (num_points > 1) {
                        previous.tree.within(
                            p.pos.x, p.pos.y, r, [&](const auto &neighbor_id) {
                                assert(neighbor_id < cluster_size);
                                auto &b = previous.clusters[neighbor_id];
                                // filter out neighbors that are already
                                // processed
                                if (b.visited) {
                                    return;
                                }
    ...
    void eachChild(const std::uint32_t cluster_id,
                   const TVisitor &visitor) const
    {
        ...
        const double r = options.radius / (double(options.extent) *
                                           std::pow(2, origin_zoom - 1));
    ...
    template <typename TVisitor>
    void eachLeaf(const std::uint32_t cluster_id, std::uint32_t &limit,
                  const std::uint32_t offset, std::uint32_t &skipped,
                  const TVisitor &visitor) const
    {
        ...
        eachChild(cluster_id, [&, this](const auto &cluster_leaf) {
            ...
            if (cluster_leaf.num_points > 1) {
                if (skipped + cluster_leaf.num_points <= offset) {
                    ...
                    // enter the cluster
                    this->eachLeaf(cluster_leaf.id, limit, offset, skipped,
                                   visitor);
                    // exit the cluster
                }

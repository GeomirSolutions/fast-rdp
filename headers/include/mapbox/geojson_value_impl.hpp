...
{
    return numVal.match(
        [](double n) { return n; }, [](uint64_t n) { return n; },
        [](int64_t n) { return n; },
        [](const auto &) -> double {
...
template <> point convert<point>(const value &val)
{
...
template <typename Container> Container convert(const value &val)
{
    assert(val.is<value::array_type>());
    if (!val.is<value::array_type>()) {
        throw error("coordinates must be of an Array type");
...
template <> geometry convert<geometry>(const value &val)
{
...
template <> feature convert<feature>(const value &val)
{
    auto *valueObject = val.getObject();
    if (!valueObject) {
        throw error("GeoJSON must be an object");
...
template <> geojson convert<geojson>(const value &val)
{
    auto *valueObject = val.getObject();
    if (!valueObject) {
        throw error("GeoJSON must be an object");
    ...
    return val.match(
        ...
        [](const value::object_type &jsonObject) {
            return convert<geojson>(
                static_cast<const mapbox::geojson::value &>(jsonObject));
        },
        [](const auto &) -> geojson {
            throw error("Invalid GeoJSON value was provided.");
        });
}

value convert(const point &p) { return value::array_type{p.x, p.y, p.z}; }

template <typename Cont> value convert(const Cont &points)
{
...
value convert(const geometry &geom)
{
    return geom.match(
        ...
        [](const point &p) {
            return value::object_type{{"type", "Point"},
                                      {"coordinates", convert(p)}};
        },
        [](const multi_point &mp) {
            return value::object_type{{"type", "MultiPoint"},
                                      {"coordinates", convert(mp)}};
        },
        [](const line_string &ls) {
            return value::object_type{{"type", "LineString"},
                                      {"coordinates", convert(ls)}};
        },
        ...
        [](const polygon &pol) {
            return value::object_type{{"type", "Polygon"},
                                      {"coordinates", convert(pol)}};
        },
...
value convert(const feature &f)
{
    ...
    if (!f.id.is<mapbox::geojson::null_value_t>()) {
        value id =
            f.id.match([](uint64_t n) -> value { return n; },
                       [](int64_t n) -> value { return n; },
                       [](double n) -> value { return n; },
                       [](std::string s) -> value { return s; },
                       [](const auto &) -> value {
                           throw error("Unknown type for a Feature 'id'");
                       });

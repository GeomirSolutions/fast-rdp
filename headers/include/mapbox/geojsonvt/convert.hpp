...
struct project
{
    ...
    vt_empty operator()(const geometry::empty &empty) { return empty; }
    ...
    vt_point operator()(const geometry::point<double> &p)
    {
        ...
        const double y = std::max(
            std::min(0.5 - 0.25 * std::log((1 + sine) / (1 - sine)) / M_PI,
                     1.0),
            0.0);
        return {x, y, 0.0};
    }
    ...
    template <class T> auto operator()(const T &vector)
    {
                           ...
                           const double tolerance, bool generateId)
{
    ...
    for (const auto &feature : features) {
        ...
        projected.emplace_back(geometry::geometry<double>::visit(
                                   feature.geometry, project{tolerance}),

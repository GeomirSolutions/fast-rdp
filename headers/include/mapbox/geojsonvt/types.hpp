...
struct vt_point : mapbox::geometry::point<double>
{
    ...
    vt_point(double x_, double y_, double z_)
        : mapbox::geometry::point<double>(x_, y_), z(z_)
    {
    }
...
template <uint8_t I, typename T> inline double get(const T &);

template <> inline double get<0>(const vt_point &p) { return p.x; }
template <> inline double get<1>(const vt_point &p) { return p.y; }
template <> inline double get<0>(const mapbox::geometry::point<double> &p)
{
    return p.x;
}
template <> inline double get<1>(const mapbox::geometry::point<double> &p)
{
    return p.y;
}
...
template <>
inline double calc_progress<0>(const vt_point &a, const vt_point &b,
                               const double x)
{
...
template <>
inline double calc_progress<1>(const vt_point &a, const vt_point &b,
                               const double y)
{
...
template <>
inline vt_point intersect<0>(const vt_point &a, const vt_point &b,
...
template <>
inline vt_point intersect<1>(const vt_point &a, const vt_point &b,
...
struct vt_line_string : std::vector<vt_point>
{
    ...
    double segStart = 0.0;
    double segEnd = 0.0; // segStart and segEnd are distance along a line in
                         // tile units, when lineMetrics = true
};
...
using vt_geometry =
    mapbox::util::variant<vt_empty, vt_point, vt_line_string, vt_polygon,
                          vt_multi_point, vt_multi_line_string,
                          vt_multi_polygon, vt_geometry_collection>;
...
template <> struct vt_geometry_type<geometry::empty>
{
    using type = vt_empty;
};
template <> struct vt_geometry_type<geometry::point<double>>
{
    using type = vt_point;
};
template <> struct vt_geometry_type<geometry::line_string<double>>
{
    using type = vt_line_string;
};
template <> struct vt_geometry_type<geometry::polygon<double>>
{
    using type = vt_polygon;
};
template <> struct vt_geometry_type<geometry::multi_point<double>>
{
    using type = vt_multi_point;
};
template <> struct vt_geometry_type<geometry::multi_line_string<double>>
{
    using type = vt_multi_line_string;
};
template <> struct vt_geometry_type<geometry::multi_polygon<double>>
{
    using type = vt_multi_polygon;
};
template <> struct vt_geometry_type<geometry::geometry<double>>
{
    using type = vt_geometry;
};
template <> struct vt_geometry_type<geometry::geometry_collection<double>>
{
...
struct vt_feature
{
    ...
    vt_feature(const vt_geometry &geom,
               std::shared_ptr<const property_map> props, const identifier &id_)
    ...
    vt_feature(const vt_geometry &geom, const property_map &props,
        ...
        : geometry(geom), properties(std::make_shared<property_map>(props)),
          id(id_)
    {
        processGeometry();
    }

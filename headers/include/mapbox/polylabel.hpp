...
template <class T>
auto pointToPolygonDist(const geometry::point<T> &point,
                        const geometry::polygon<T> &polygon)
{
    ...
    for (const auto &ring : polygon) {
        for (std::size_t i = 0, len = ring.size(), j = len - 1; i < len;
             j = i++) {
            const auto &a = ring[i];
            const auto &b = ring[j];
...
{
    Cell(const geometry::point<T> &c_, T h_,
         const geometry::polygon<T> &polygon)
        : c(c_), h(h_), d(pointToPolygonDist(c, polygon)),
          max(d + h * std::sqrt(2))
    {
    }
...
template <class T> Cell<T> getCentroidCell(const geometry::polygon<T> &polygon)
{
    T area = 0;
...
geometry::point<T> polylabel(const geometry::polygon<T> &polygon,
                             T precision = 1, bool debug = false)
{
    ...
    const geometry::point<T> size{envelope.max.x - envelope.min.x,
    ...
    using Queue = std::priority_queue<Cell<T>, std::vector<Cell<T>>,
                                      decltype(compareMax)>;
    ...
    while (!cellQueue.empty()) {
        ...
        if (cell.d > bestCell.d) {
            bestCell = cell;
            if (debug)
                std::cout << "found best " << ::round(1e4 * cell.d) / 1e4
                          << " after " << numProbes << " probes" << std::endl;
        }

...
template <class T> struct feature
{
    ...
    feature(geometry_type &&geom_, property_map &&prop_, identifier &&id_)
        : geometry(std::move(geom_)), properties(std::move(prop_)),
          id(std::move(id_))
    {
    }
    feature(geometry_type const &geom_, property_map const &prop_,
            identifier const &id_, property_map const &custom_prop_)
        : geometry(geom_), properties(prop_), id(id_),
          custom_properties(custom_prop_)
    {
    }
    feature(geometry_type &&geom_, property_map &&prop_, identifier &&id_,
            property_map &&custom_prop_)
        : geometry(std::move(geom_)), properties(std::move(prop_)),
          id(std::move(id_)), custom_properties(std::move(custom_prop_))
    {
    }
    feature(geometry_type &&geom_, property_map &&prop_,
            property_map &&custom_prop_)
...
constexpr bool operator==(feature<T> const &lhs, feature<T> const &rhs)
{
    return lhs.id == rhs.id && lhs.geometry == rhs.geometry &&
           ...
           lhs.custom_properties == rhs.custom_properties;
}

...
template <typename T> double area(mapbox::geometry::linear_ring<T> const &poly)
{
...
template <typename T> inline T wround(double value)
{
    return static_cast<T>(::llround(value));
}

template <> inline std::int64_t wround<std::int64_t>(double value)
{

...
template <typename T> struct local_minimum
{
    bound<T> left_bound;
    bound<T> right_bound;
    T y;
    ...
    local_minimum(bound<T> &&left_bound_, bound<T> &&right_bound_, T y_,
        ...
        : left_bound(std::move(left_bound_)),
          right_bound(std::move(right_bound_)), y(y_),
          minimum_has_horizontal(has_horz_)
    {
    }
};

template <typename T> using local_minimum_list = std::deque<local_minimum<T>>;

template <typename T>
using local_minimum_itr = typename local_minimum_list<T>::iterator;

template <typename T> using local_minimum_ptr = local_minimum<T> *;
...
template <typename T> struct local_minimum_sorter
{
...
operator<<(std::basic_ostream<charT, traits> &out, const local_minimum<T> &lm)
{

...
template <typename T> using active_bound_list = std::vector<bound_ptr<T>>;
...
template <typename T> std::string output_edges(active_bound_list<T> const &bnds)
{
    ...
    for (auto const &bnd : bnds) {
        ...
        out << "[[" << bnd->current_edge->bot.x << ","
            << bnd->current_edge->bot.y << "],[";
...
template <typename T> struct bound_insert_location
{
        ...
        if (values_are_equal(bound2.current_x, bound1.current_x)) {
            if (bound2.current_edge->top.y > bound1.current_edge->top.y) {
                return less_than(
                    ...
                    get_current_x(*(bound1.current_edge),
                                  bound2.current_edge->top.y));
            } else {
                return greater_than(
                    static_cast<double>(bound1.current_edge->top.x),
                    get_current_x(*(bound2.current_edge),
                                  bound1.current_edge->top.y));
            }
        } else {
...
insert_bound_into_ABL(bound<T> &left, bound<T> &right,
...
template <typename T> inline bool is_maxima(bound<T> const &bnd, T y)
{
...
template <typename T> inline bool is_intermediate(bound<T> const &bnd, T y)
{
    return bnd.next_edge != bnd.edges.end() && bnd.current_edge->top.y == y;
}
                       ...
                       fill_type subject_fill_type, fill_type clip_fill_type)
{
    ...
    if (rev_bnd_itr == active_bounds.rend()) {
        ...
        if ((*rev_bnd_itr)->winding_count * (*rev_bnd_itr)->winding_delta < 0) {
            ...
            if (std::abs(static_cast<int>((*rev_bnd_itr)->winding_count)) > 1) {
                ...
                if ((*rev_bnd_itr)->winding_delta * (*bnd_itr)->winding_delta <
                    0) {
                    (*bnd_itr)->winding_count = (*rev_bnd_itr)->winding_count;
                } else {
                    // otherwise continue to 'decrease' WC ...
                    (*bnd_itr)->winding_count = (*rev_bnd_itr)->winding_count +
                                                (*bnd_itr)->winding_delta;
                }
            } else {
    ...
    if (is_even_odd_alt_fill_type(*(*bnd_itr), subject_fill_type,
        ...
        while (bnd_itr_forward != bnd_itr) {
            (*bnd_itr)->winding_count2 =
                ((*bnd_itr)->winding_count2 == 0 ? 1 : 0);
            ++bnd_itr_forward;
        }
    } else {
        // nonZero, Positive or Negative filling ...

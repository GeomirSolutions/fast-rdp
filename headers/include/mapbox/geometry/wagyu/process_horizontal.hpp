    ...
    fill_type clip_fill_type)
{
    ...
    while (bnd != active_bounds.end()) {
        ...
        // polygons) wherever hot pixels touch these horizontal edges. This
        // helps
        ...
        if ((*horz_bound)->ring) {
            add_point_to_ring(*(*horz_bound),
                              mapbox::geometry::point<T>(
                                  wround<T>((*bnd)->current_x), scanline_y),
                              rings);
        }
        ...
        intersect_bounds(*(*horz_bound), *(*bnd),
                         mapbox::geometry::point<T>(
                             wround<T>((*bnd)->current_x), scanline_y),
                         cliptype, subject_fill_type, clip_fill_type, rings,
                         active_bounds);
...
process_horizontal(T scanline_y, active_bound_list_itr<T> &horz_bound,
                   active_bound_list<T> &active_bounds, ring_manager<T> &rings,
                   scanbeam_list<T> &scanbeam, clip_type cliptype,
                   fill_type subject_fill_type, fill_type clip_fill_type)
{
    if ((*horz_bound)->current_edge->bot.x <
        (*horz_bound)->current_edge->top.x) {
...
void process_horizontals(T scanline_y, active_bound_list<T> &active_bounds,
                         ...
                         fill_type clip_fill_type)
{
    for (auto bnd_itr = active_bounds.begin();
         bnd_itr != active_bounds.end();) {
        if (*bnd_itr != nullptr && current_edge_is_horizontal<T>(bnd_itr)) {
            bnd_itr = process_horizontal(scanline_y, bnd_itr, active_bounds,

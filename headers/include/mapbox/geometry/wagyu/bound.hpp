...
template <typename T> struct bound
{
    ...
    bound() noexcept
        : edges(), current_edge(edges.end()), next_edge(edges.end()),
          last_point({0, 0}), ring(nullptr), maximum_bound(nullptr),
          current_x(0.0), pos(0), winding_count(0), winding_count2(0),
          winding_delta(0), poly_type(polygon_type_subject), side(edge_left)
    {
    }
...
operator<<(std::basic_ostream<charT, traits> &out, const bound<T> &bnd)
{
    out << "    Bound: " << &bnd << std::endl;
    out << "        current_x: " << bnd.current_x << std::endl;
    out << "        last_point: " << bnd.last_point.x << ", "
        << bnd.last_point.y << std::endl;

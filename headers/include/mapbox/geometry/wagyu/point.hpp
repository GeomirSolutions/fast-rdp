...
template <typename T> using point_ptr = point<T> *;

template <typename T> using const_point_ptr = point<T> *const;

template <typename T> struct ring;

template <typename T> using ring_ptr = ring<T> *;

template <typename T> using const_ring_ptr = ring<T> *const;
...
template <typename T> struct point
{
    ...
    point(ring_ptr<T> ring_, mapbox::geometry::point<T> const &pt,
        ...
        : ring(ring_), x(pt.x), y(pt.y), next(before_this_point),
...
template <typename T> using point_vector = std::vector<point_ptr<T>>;

template <typename T>
using point_vector_itr = typename point_vector<T>::iterator;

template <typename T> bool operator==(point<T> const &lhs, point<T> const &rhs)
{
...
operator<<(std::basic_ostream<charT, traits> &out, const point<T> &p)
{

...
template <typename T> struct hp_intersection_swap
{
                                 ...
                                 ring_manager<T> &manager)
{
    ...
    if (current_edge->bot.x < current_edge->top.x) {
        ...
        while (bnd_next != active_bounds.end() &&
            ...
            if (*bnd_next != nullptr &&
                (*bnd_next)->current_edge->top.y != top_y &&
        ...
        if (bnd_curr != active_bounds.begin()) {
            ...
            while (bnd_curr != active_bounds.begin() &&
                ...
                if (*bnd_prev != nullptr &&
                    (*bnd_prev)->current_edge->top.y != top_y &&
                    (*bnd_prev)->current_edge->bot.y != top_y) {
                    mapbox::geometry::point<T> pt(
                        wround<T>((*bnd_prev)->current_x), top_y);
                    add_to_hot_pixels(pt, manager);
                }
    ...
    ring_manager<T> &manager, scanbeam_list<T> &scanbeam)
{
    while (lm != minima_sorted.end() && (*lm)->y == top_y) {
        ...
        left_bound.current_x =
            static_cast<double>(left_bound.current_edge->bot.x);
        ...
        if (!current_edge_is_horizontal<T>(lb_abl_itr)) {
            insert_sorted_scanbeam(scanbeam,
                                   (*lb_abl_itr)->current_edge->top.y);
        }
        auto rb_abl_itr = std::next(lb_abl_itr);
        if (!current_edge_is_horizontal<T>(rb_abl_itr)) {
            insert_sorted_scanbeam(scanbeam,
                                   (*rb_abl_itr)->current_edge->top.y);
        }
        ++lm;
    }
}

...
template <typename T> using bound_ptr = bound<T> *;
...
template <typename T> struct edge
{
    ...
    edge(mapbox::geometry::point<T2> const &current,
        ...
        : bot(static_cast<T>(current.x), static_cast<T>(current.y)),
          top(static_cast<T>(current.x), static_cast<T>(current.y)), dx(0.0)
    {
...
template <typename T> using edge_ptr = edge<T> *;

template <typename T> using edge_list = std::vector<edge<T>>;

template <typename T> using edge_list_itr = typename edge_list<T>::iterator;

template <typename T> bool slopes_equal(edge<T> const &e1, edge<T> const &e2)
{
...
template <typename T> inline bool is_horizontal(edge<T> const &e)
{
...
operator<<(std::basic_ostream<charT, traits> &out, const edge<T> &e)
{
    out << "    Edge: " << std::endl;
...
template <class charT, class traits, typename T>
inline std::basic_ostream<charT, traits> &
operator<<(std::basic_ostream<charT, traits> &out, edge_list<T> const &edges)
{
    out << "[";
    bool first = true;

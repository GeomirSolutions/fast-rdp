...
do_maxima(active_bound_list_itr<T> &bnd, active_bound_list_itr<T> &bndMaxPair,
          clip_type cliptype, fill_type subject_fill_type,
          fill_type clip_fill_type, ring_manager<T> &manager,
          active_bound_list<T> &active_bounds)
{
    ...
    while (bnd_next != active_bounds.end() && bnd_next != bndMaxPair) {
        ...
        skipped = true;
        intersect_bounds(*(*bnd), *(*bnd_next), (*bnd)->current_edge->top,
                         cliptype, subject_fill_type, clip_fill_type, manager,
                         active_bounds);
    ...
    if ((*bnd)->ring && (*bndMaxPair)->ring) {
        add_local_maximum_point(*(*bnd), *(*bndMaxPair),
                                (*bnd)->current_edge->top, manager,
                                active_bounds);
    ...
    clip_type cliptype, fill_type subject_fill_type, fill_type clip_fill_type)
{
    ...
    for (auto bnd = active_bounds.begin(); bnd != active_bounds.end();) {
        ...
        // 1. Process maxima, treating them as if they are "bent" horizontal
        // edges, but exclude maxima with horizontal edges.
        ...
        if (is_intermediate(bnd, top_y) && next_edge_is_horizontal<T>(bnd)) {
            if ((*bnd)->ring) {
                insert_hot_pixels_in_path(*(*bnd), (*bnd)->current_edge->top,
                                          manager, false);
            }
